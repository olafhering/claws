...
static void reply_cb(GtkAction *action, gpointer data);
...
MessageView *messageview_create(MainWindow *mainwin)
{
	...
	gtk_box_pack_start(GTK_BOX(vbox), GTK_WIDGET_PTR(headerview), FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox), GTK_WIDGET_PTR(noticeview), FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(vbox), GTK_WIDGET_PTR(mimeview), TRUE, TRUE, 0);
	...
	messageview->msginfo_update_callback_id = hooks_register_hook(MSGINFO_UPDATE_HOOKLIST, messageview_update_msg, (gpointer)messageview);
...
static void messageview_add_toolbar(MessageView *msgview, GtkWidget *window)
{
	...
	action_group = cm_menu_create_action_group_full(msgview->ui_manager, "Menu", msgview_entries, G_N_ELEMENTS(msgview_entries), (gpointer)msgview);
	gtk_action_group_add_toggle_actions(action_group, msgview_toggle_entries, G_N_ELEMENTS(msgview_toggle_entries), (gpointer)msgview);
	gtk_action_group_add_radio_actions(action_group, msgview_radio_enc_entries, G_N_ELEMENTS(msgview_radio_enc_entries), C_AUTO, G_CALLBACK(set_charset_cb), (gpointer)msgview);
	gtk_action_group_add_radio_actions(action_group, msgview_radio_dec_entries, G_N_ELEMENTS(msgview_radio_dec_entries), C_AUTO, G_CALLBACK(set_decode_cb), (gpointer)msgview);
...
/* Help menu */
	...
	cm_toggle_menu_set_active_full(msgview->ui_manager, "Menu/View/AllHeaders", prefs_common.show_all_headers);
	...
	msgview->toolbar = toolbar_create(TOOLBAR_MSGVIEW, handlebox, (gpointer)msgview);
#ifndef GENERIC_UMPC
	statusbar = gtk_statusbar_new();
	gtk_widget_show(statusbar);
	gtk_box_pack_end(GTK_BOX(vbox), statusbar, FALSE, FALSE, 0);
	msgview->statusbar = statusbar;
	msgview->statusbar_cid = gtk_statusbar_get_context_id(GTK_STATUSBAR(statusbar), "Message View");
#else
...
static MessageView *messageview_create_with_new_window_visible(MainWindow *mainwin, gboolean show)
{
	...
	gtk_widget_set_size_request(window, prefs_common.msgwin_width, prefs_common.msgwin_height);
#ifdef G_OS_WIN32
	...
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(messageview_size_allocate_cb), msgview);
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(messageview_delete_cb), msgview);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(key_pressed), msgview);
	...
	toolbar_set_style(msgview->toolbar->toolbar, msgview->handlebox, prefs_common.toolbar_style);
...
static void notification_convert_header(gchar **dest, const gchar *src_, gint header_len)
{
	char *src;
...
static gint disposition_notification_send(MsgInfo *msginfo)
{
	...
	if (!msginfo->extradata)
		return -1;
	if (!msginfo->extradata->returnreceiptto && !msginfo->extradata->dispositionnotificationto)
		return -1;
	...
	if (ok != 0) {
		AlertValue val;
		gchar *message;
		message = g_markup_printf_escaped(_("The notification address to which the return receipt is\n" "to be sent does not correspond to the return path:\n" "Notification address: %s\n" "Return path: %s\n" "It is advised to not send the return receipt."), to, buf);
		val = alertpanel_full(_("Warning"), message, _("_Don't Send"), _("_Send"), NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
		g_free(message);
	...
	g_snprintf(tmp, sizeof(tmp), "%s%ctmpmsg%p", get_rc_dir(), G_DIR_SEPARATOR, msginfo);
	...
	/* write queue headers */
	ok = fprintf(fp, "AF:\n" "NF:0\n" "PS:10\n" "SRH:1\n" "SFN:\n" "DSR:\n" "MID:\n" "CFG:\n" "PT:0\n" "S:%s\n" "RQ:\n" "SSV:%s\n" "SSH:\n" "R:<%s>\n", account->address, account->smtp_server ? account->smtp_server : "", addrp);
	...
	ok = fprintf(fp, "MIME-Version: 1.0\n"
		     ...
		     "  boundary=\"%s\"\n" "\n" "--%s\n" "Content-Type: text/plain; charset=UTF-8\n" "Content-Transfer-Encoding: quoted-printable\n" "\n" "The message sent on: %s\n" "To: %s\n" "With subject: \"%s\"\n" "has been displayed at %s.\n" "\n" "There is no guarantee that the message has been read or understood.\n" "\n" "--%s\n" "Content-Type: message/disposition-notification\n" "\n" "Reporting-UA: %s\n" "Original-Recipient: rfc822;%s\n" "Final-Recipient: rfc822;%s\n" "Original-Message-ID: <%s>\n" "Disposition: manual-action/MDN-sent-manually; displayed\n" "\n" "--%s\n" "Content-Type: application/octet-stream\n" "\n" "Reporting-UA: %s\n" "Original-Recipient: rfc822;%s\n" "Final-Recipient: rfc822;%s\n" "Original-Message-ID: <%s>\n" "Disposition: manual-action/MDN-sent-manually; displayed\n" "\n" "--%s--\n", boundary, boundary, msginfo->date, orig_to ? orig_to : "No To:", enc_sub ? enc_sub : "No subject", date, boundary, PROG_VERSION, orig_to ? orig_to : "No To:", account->address, msginfo->msgid ? msginfo->msgid : "NO MESSAGE ID", boundary, PROG_VERSION, orig_to ? orig_to : "No To:", account->address, msginfo->msgid ? msginfo->msgid : "NO MESSAGE ID", boundary);
...
static MimeInfo *find_encrypted_part(MimeInfo *rootinfo)
{
	...
	g_node_traverse(rootinfo->node, G_IN_ORDER, G_TRAVERSE_ALL, -1, find_encrypted_func, &encinfo);
...
static MimeInfo *find_broken_part(MimeInfo *rootinfo)
{
	...
	g_node_traverse(rootinfo->node, G_IN_ORDER, G_TRAVERSE_ALL, -1, find_broken_func, &brokeninfo);
...
static void messageview_register_nav(MessageView *messageview)
{
	...
	if (pos != -1) {
		...
		while (messageview->trail_pos > prefs_common.nav_history_length) {
			...
			messageview->trail = g_list_delete_link(messageview->trail, messageview->trail);
...
{
	return messageview != NULL && messageview->trail != NULL && messageview->trail_pos > 0;
}
	...
	if (mimeinfo->type == MIMETYPE_TEXT) {
		if (!strcasecmp(mimeinfo->subtype, "calendar")
			...
			return TRUE;
			   ...
			   && mimeinfo->disposition != DISPOSITIONTYPE_ATTACHMENT && ((msginfo->folder && msginfo->folder->prefs->promote_html_part == HTML_PROMOTE_ALWAYS)
...
static void messageview_find_part_depth_first(MimeInfoSearch *context, MimeMediaType type, const gchar *subtype)
{
	...
	if (mimeinfo->type == type && !strcasecmp(mimeinfo->subtype, subtype)) {
		...
		if (!strcasecmp(mimeinfo->subtype, "alternative")
			...
			while (context->found && context->found != context->parent) {
				if (context->found->type == type && !strcasecmp(context->found->subtype, subtype))
					break;
		...
		if (!context->found && (!strcasecmp(mimeinfo->subtype, "related")
...
gint messageview_show(MessageView *messageview, MsgInfo *msginfo, gboolean all_headers)
{
	...
	if (messageview->mimeview->textview && messageview->mimeview->textview->loading) {
		messageview->mimeview->textview->stop_loading = TRUE;
		return 0;
	}

	if (messageview->toolbar)
		toolbar_set_learn_button(messageview->toolbar, MSG_IS_SPAM(msginfo->flags) ? LEARN_HAM : LEARN_SPAM);
	else
		toolbar_set_learn_button(messageview->mainwin->toolbar, MSG_IS_SPAM(msginfo->flags) ? LEARN_HAM : LEARN_SPAM);

	if (messageview->toolbar) {
		if (messageview->toolbar->learn_spam_btn) {
			gboolean can_learn = FALSE;
			if (procmsg_spam_can_learn() && (msginfo->folder && msginfo->folder->folder->klass->type != F_UNKNOWN && msginfo->folder->folder->klass->type != F_NEWS))
				can_learn = TRUE;

			gtk_widget_set_sensitive(messageview->toolbar->learn_spam_btn, can_learn);
		}
	}
	...
	if (msginfo->size > 1024 * 1024)
		statusbar_print_all(_("Fetching message (%s)..."), to_human_readable(msginfo->size));
	...
	if (!folder_has_parent_of_type(msginfo->folder, F_QUEUE) && !folder_has_parent_of_type(msginfo->folder, F_DRAFT))
	...
	while ((encinfo = find_encrypted_part(mimeinfo)) != NULL) {
		...
		if (privacy_mimeinfo_decrypt(encinfo) < 0) {
			text = g_strdup_printf(_("Couldn't decrypt: %s"), privacy_get_error());
	...
	if (messageview->msginfo != msginfo) {
		proc_msginfo_release(messageview->msginfo);
		messageview->msginfo = NULL;
		messageview_set_menu_sensitive(messageview);
		messageview->msginfo = procmsg_msginfo_get_full_info_from_file(msginfo, file);
	...
	if (messageview->window) {
		gtk_window_set_title(GTK_WINDOW(messageview->window), _("Claws Mail - Message View"));
		GTK_EVENTS_FLUSH();
	}
	...
	if (messageview->window && msginfo->subject) {
		...
		if (!g_utf8_validate(subject, -1, NULL)) {
			...
			conv_localetodisp(subject, strlen(msginfo->subject) * 2 + 1, msginfo->subject);
		}
		if (g_utf8_validate(subject, -1, NULL))
			gtk_window_set_title(GTK_WINDOW(messageview->window), subject);
		g_free(subject);
	}
	...
	if (messageview->msginfo && messageview->msginfo->extradata && messageview->msginfo->extradata->partial_recv && !noticeview_is_visible(messageview->noticeview))
		partial_recv_show(messageview->noticeview, messageview->msginfo);
	else if (messageview->msginfo && messageview->msginfo->extradata && (messageview->msginfo->extradata->dispositionnotificationto || messageview->msginfo->extradata->returnreceiptto) && !MSG_IS_RETRCPT_SENT(messageview->msginfo->flags) && !prefs_common.never_send_retrcpt && !noticeview_is_visible(messageview->noticeview))
	...
	if (find_broken_part(mimeinfo) != NULL) {
		...
		if (!noticeview_is_visible(messageview->noticeview)) {
			noticeview_set_text(messageview->noticeview, _("Message doesn't conform to MIME standard. " "It may render wrongly."));
			gtk_widget_hide(messageview->noticeview->button);
			gtk_widget_hide(messageview->noticeview->button2);
		} else {
			gchar *full = g_strconcat(gtk_label_get_text(GTK_LABEL(messageview->noticeview->text)),
						  "\n",
						  _("Message doesn't conform to MIME standard. " "It may render wrongly."), NULL);
	...
	if (!all_headers && mimeinfo && (mimeinfo->type != MIMETYPE_TEXT || strcasecmp(mimeinfo->subtype, "plain"))
	    && (mimeinfo->type != MIMETYPE_MULTIPART || strcasecmp(mimeinfo->subtype, "signed"))) {
		if (strcasecmp(mimeinfo->subtype, "html")) {
			...
			if (context.found && ((msginfo->folder && msginfo->folder->prefs->promote_html_part == HTML_PROMOTE_ALWAYS) || ((msginfo->folder && msginfo->folder->prefs->promote_html_part == HTML_PROMOTE_DEFAULT) && prefs_common.promote_html_part))) { /* html found */
	...
	if (!all_headers && mimeinfo && mimeinfo->type == MIMETYPE_MULTIPART && mimeview_has_viewer_for_content_type(messageview->mimeview, "text/calendar")) {
		/* look for a calendar part or it looks really strange */
		while (mimeinfo) {
			if (mimeinfo->type == MIMETYPE_TEXT && !strcasecmp(mimeinfo->subtype, "calendar")) {
				mimeview_select_mimepart_icon(messageview->mimeview, mimeinfo);
				goto done;
			}
			mimeinfo = procmime_mimeinfo_next(mimeinfo);
...
void messageview_destroy(MessageView *messageview)
{
	...
	if (!messageview->deferred_destroy)
		hooks_unregister_hook(MSGINFO_UPDATE_HOOKLIST, messageview->msginfo_update_callback_id);
...
void messageview_delete(MessageView *msgview)
{
	...
	if (msginfo && msgview->msginfo && msginfo->msgnum == msgview->msginfo->msgnum && msginfo->folder == msgview->msginfo->folder) {
...
static void messageview_update(MessageView *msgview, MsgInfo *old_msginfo)
{
	...
	if (summaryview->selected) {
		...
		messageview_show(msgview, msginfo, msgview->all_headers);
	}
}
...
gboolean messageview_search_string_backward(MessageView *messageview, const gchar *str, gboolean case_sens)
{
	...
	text = messageview_get_current_textview(messageview);
	if (text)
		return textview_search_string_backward(text, str, case_sens);
	return FALSE;
}
...
static gboolean key_pressed(GtkWidget *widget, GdkEventKey *event, MessageView *messageview)
{
	...
	if (event && (event->keyval == GDK_KEY_KP_Enter || event->keyval == GDK_KEY_Return) && messageview->window) {
...
void messageview_show_partial_display(MessageView *messageview, MsgInfo *msginfo, size_t length)
{
...
static void return_receipt_show(NoticeView *noticeview, MsgInfo *msginfo)
{
	...
	if (msginfo->folder && (folder_has_parent_of_type(msginfo->folder, F_QUEUE)
	...
	if (from_me) {
		...
		if (MSG_IS_RETRCPT_GOT(msginfo->flags)) {
			noticeview_set_text(noticeview, _("You got a return receipt for this message: " "it has been displayed by the recipient."));
		} else {
...
static void return_receipt_send_clicked(NoticeView *noticeview, void *user_data)
{
	...
	proc_msginfo_release(tmpmsginfo);
	g_free(file);
}
...
static void partial_recv_show(NoticeView *noticeview, MsgInfo *msginfo)
{
	...
	if (!partial_msg_in_uidl_list(msginfo)) {
		text = g_strdup_printf(_("This message has been partially " "retrieved,\nand has been deleted from the " "server."));
	} else {
		switch (msginfo->planned_download) {
		case POP3_PARTIAL_DLOAD_UNKN:
			text = g_strdup_printf(_("This message has been " "partially retrieved;\nit is %s."), to_human_readable((goffset) (msginfo->total_size)));
		...
		case POP3_PARTIAL_DLOAD_DLOAD:
			text = g_strdup_printf(_("This message has been " "partially retrieved;\nit is %s and " "will be downloaded."), to_human_readable((goffset) (msginfo->total_size)));
			button1 = _("Unmark");
		...
		case POP3_PARTIAL_DLOAD_DELE:
			text = g_strdup_printf(_("This message has been " "partially retrieved;\nit is %s and " "will be deleted."), to_human_readable((goffset) (msginfo->total_size)));
			button1 = _("Mark for download");
			button1_cb = partial_recv_dload_clicked;
			button2 = _("Unmark");
...
static PrefsAccount *select_account_from_list(GList *ac_list, gboolean has_accounts)
{
	...
	optmenu = gtkut_account_menu_new(ac_list, G_CALLBACK(select_account_cb), &account_id);
	if (!optmenu)
		return NULL;
	...
	if (!has_accounts) {
		gchar *tr;
		gchar *text;
		tr = g_strdup(C_("'%s' stands for 'To' then 'Cc'", "This message is asking for a return receipt notification\n" "but according to its '%s' and '%s' headers it was not\n" "officially addressed to you.\n" "It is advised to not send the return receipt."));
		text = g_strdup_printf(tr, prefs_common_translated_header_name("To"), prefs_common_translated_header_name("Cc"));
		val = alertpanel_with_widget(_("Return Receipt Notification"), text, _("_Cancel"), _("_Send Notification"), NULL, ALERTFOCUS_FIRST, FALSE, optmenu);
		g_free(tr);
		g_free(text);
	} else
		val = alertpanel_with_widget(_("Return Receipt Notification"), _("More than one of your accounts uses the " "address that this message was sent to.\n" "Please choose which account you want to " "use for sending the receipt notification:"), _("_Cancel"), _("_Send Notification"), NULL, ALERTFOCUS_FIRST, FALSE, optmenu);
...
{
	MainWindow *mainwin;
	if (!mimeview || !mimeview->textview || !mimeview->textview->text)
		alertpanel_warning(_("Cannot print: the message doesn't " "contain text."));
	else {
		...
		if (mimeview->type == MIMEVIEW_VIEWER) {
			...
			if (viewer && viewer->print) {
				...
				if (mimepart == NULL || (mimepart->type != MIMETYPE_TEXT && mimepart->type != MIMETYPE_MESSAGE)) {
					alertpanel_warning(_("Cannot print: the message doesn't " "contain text."));
					return;
				}
		...
		printing_print(GTK_TEXT_VIEW(mimeview->textview->text), mainwin ? GTK_WINDOW(mainwin->window) : NULL, sel_start, sel_end, (mimeview->textview->image ? GTK_IMAGE(mimeview->textview->image)
																	   : NULL));
	}
}
...
void messageview_print(MsgInfo *msginfo, gboolean all_headers, gint sel_start, gint sel_end, gint partnum)
{
	...
	MessageView *tmpview = messageview_create_with_new_window_visible(mainwindow_get_mainwindow(), FALSE);
	...
	if (font_desc) {
		gtk_widget_modify_font(tmpview->mimeview->textview->text, font_desc);
		pango_font_description_free(font_desc);
	}

	tmpview->all_headers = all_headers;
	if (msginfo && messageview_show(tmpview, msginfo, tmpview->all_headers) >= 0) {
		print_mimeview(tmpview->mimeview, sel_start, sel_end, partnum);
	}
...
{
	MessageView *messageview = (MessageView *)data;
	printing_page_setup(messageview ? GTK_WINDOW(messageview->window) : NULL);
}
...
static void print_cb(GtkAction *action, gpointer data)
{
	...
	textview_get_selection_offsets(messageview->mimeview->textview, &sel_start, &sel_end);
	messageview_print(messageview->msginfo, messageview->all_headers, sel_start, sel_end, partnum);
}
...
static void prev_history_cb(GtkAction *action, gpointer data)
{
	...
	if (info) {
		messageview->updating = TRUE;
		messageview_show(messageview, info, messageview->all_headers);
		messageview->updating = FALSE;
...
static void show_all_header_cb(GtkToggleAction *action, gpointer data)
{
	...
	messageview->all_headers = prefs_common.show_all_headers = gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action));
	if (!msginfo)
		return;
	messageview->msginfo = NULL;
	messageview_show(messageview, msginfo, messageview->all_headers);
...
static void compose_cb(GtkAction *action, gpointer data)
{
	...
	if (item) {
		...
		if (ac && ac->protocol == A_NNTP && FOLDER_TYPE(item->folder) == F_NEWS) {
...
static void create_filter_cb(GtkAction *gaction, gpointer data)
{
	...
	item = messageview->msginfo->folder;
	summary_msginfo_filter_open(item, messageview->msginfo, (PrefsFilterType) action, 0);
}
...
static void create_processing_cb(GtkAction *gaction, gpointer data)
{
	...
	item = messageview->msginfo->folder;
	summary_msginfo_filter_open(item, messageview->msginfo, (PrefsFilterType) action, 1);
}
...
static gboolean messageview_update_msg(gpointer source, gpointer data)
{
	...
	if ((msginfo_update->flags & MSGINFO_UPDATE_DELETED) || MSG_IS_DELETED(old_msginfo->flags)) {
		if (messageview->new_window) {
			if (old_msginfo->folder && old_msginfo->folder->total_msgs == 0) {
				messageview_clear(messageview);
				textview_show_info(messageview->mimeview->textview, _("\n  There are no messages in this folder"));
				return FALSE;
			}

			if (!OPEN_SELECTED_ON_DELETEMOVE && !OPEN_SELECTED_ON_PREVNEXT) {
				messageview_clear(messageview);
				textview_show_info(messageview->mimeview->textview, MSG_IS_DELETED(old_msginfo->flags) ? _("\n  Message has been deleted") : _("\n  Message has been deleted or moved to another folder"));
			} else
...
void messageview_set_menu_sensitive(MessageView *messageview)
{
	...
	cm_menu_set_sensitive_full(messageview->ui_manager, "Menu/Message/CheckSignature", !!messageview->mimeview->siginfo);
}
...
void messageview_learn(MessageView *msgview, gboolean is_spam)
{
	...
	if (msgview->toolbar)
		toolbar_set_learn_button(msgview->toolbar, MSG_IS_SPAM(msgview->msginfo->flags) ? LEARN_HAM : LEARN_SPAM);
	else
		toolbar_set_learn_button(msgview->mainwin->toolbar, MSG_IS_SPAM(msgview->msginfo->flags) ? LEARN_HAM : LEARN_SPAM);
}
...
void messageview_list_urls(MessageView *msgview)
{
	...
	uri_hashtable = g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify) g_free, NULL);

	for (; cur; cur = cur->next) {
		ClickableText *uri = (ClickableText *)cur->data;
		if (uri->uri && (!g_ascii_strncasecmp(uri->uri, "ftp.", 4) || !g_ascii_strncasecmp(uri->uri, "ftp:", 4) || !g_ascii_strncasecmp(uri->uri, "ftps:", 5) || !g_ascii_strncasecmp(uri->uri, "sftp:", 5) || !g_ascii_strncasecmp(uri->uri, "www.", 4) || !g_ascii_strncasecmp(uri->uri, "webcal:", 7) || !g_ascii_strncasecmp(uri->uri, "webcals:", 8) || !g_ascii_strncasecmp(uri->uri, "http:", 5) || !g_ascii_strncasecmp(uri->uri, "https:", 6))) {
			...
			g_hash_table_insert(uri_hashtable, tmp, GUINT_TO_POINTER(g_str_hash(tmp)));
		}
	}
