...
static void printing_textview_cb_draw_page(GtkPrintOperation *, GtkPrintContext *, gint, gpointer);
static gboolean cb_preview(GtkPrintOperation *, GtkPrintOperationPreview *, GtkPrintContext *, GtkWindow *, gpointer);
...
static void cb_preview_ready(GtkPrintOperationPreview *, GtkPrintContext *, gpointer);
static gboolean cb_preview_expose(GtkWidget *, GdkEventExpose *, gpointer);
static void cb_preview_got_page_size(GtkPrintOperationPreview *, GtkPrintContext *, GtkPageSetup *, gpointer);
...
static void cb_preview_request_page_setup(GtkPrintOperation *, GtkPrintContext *, gint, GtkPageSetup *, gpointer);
...
void printing_print_full(GtkWindow *parent, PrintRenderer *renderer, gpointer renderer_data, gint sel_start, gint sel_end, GtkImage *avatar)
{
	GtkPrintOperation *op;
	...
	res = gtk_print_operation_run(op, GTK_PRINT_OPERATION_ACTION_PRINT_DIALOG, parent, NULL);
...
void printing_page_setup(GtkWindow *parent)
{
	...
	prefs_common.print_paper_type = g_strdup(gtk_paper_size_get_name(gtk_page_setup_get_paper_size(page_setup)));
...
static gboolean cb_preview(GtkPrintOperation *operation, GtkPrintOperationPreview *preview, GtkPrintContext *context, GtkWindow *parent, gpointer data)
{
	PrintData *print_data;
	cairo_t *cr;
	...
	gtk_widget_set_size_request(dialog, prefs_common.print_previewwin_width, prefs_common.print_previewwin_height);
...
#define TOOLBAR_ITEM(item,text,tooltip,cb,cbdata) {								\
	...
	gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(sw), da);
	...
	/* signals */
	g_signal_connect(dialog, "key_press_event", G_CALLBACK(cb_preview_close), preview_data);
	g_signal_connect(dialog, "size_allocate", G_CALLBACK(cb_preview_size_allocate), NULL);
	g_signal_connect(dialog, "destroy", G_CALLBACK(cb_preview_destroy), preview_data);
	g_signal_connect(preview, "ready", G_CALLBACK(cb_preview_ready), preview_data);
	g_signal_connect(preview, "got-page-size", G_CALLBACK(cb_preview_got_page_size), preview_data);

	g_signal_connect(operation, "request-page-setup", G_CALLBACK(cb_preview_request_page_setup), preview_data);
...
static gboolean cb_preview_close(GtkWidget *widget, GdkEventAny *event, gpointer data)
{
...
static void cb_preview_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
...
static void cb_preview_ready(GtkPrintOperationPreview *preview, GtkPrintContext *context, gpointer data)
{
	PreviewData *preview_data;
	gint iPage;
	preview_data = (PreviewData *)data;
	...
	for (iPage = 0; iPage < (preview_data->print_data->npages); iPage++) {
		if (gtk_print_operation_preview_is_selected(preview_data->preview, iPage)) {
			preview_data->pages_to_print = g_list_prepend(preview_data->pages_to_print, GINT_TO_POINTER(iPage));
	...
	g_signal_connect(preview_data->area, "expose_event", G_CALLBACK(cb_preview_expose), preview_data);

	gtk_widget_queue_draw(preview_data->area);
}

static void cb_preview_got_page_size(GtkPrintOperationPreview *preview, GtkPrintContext *context, GtkPageSetup *page_setup, gpointer data)
{
	PreviewData *preview_data;
	...
	gtk_widget_set_size_request(GTK_WIDGET(preview_data->area), paper_width, paper_height);
}

static gboolean cb_preview_expose(GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
	...
	cairo_rectangle(cr, PREVIEW_SHADOW_OFFSET, PREVIEW_SHADOW_OFFSET, preview_data->page_width + PREVIEW_SHADOW_OFFSET, preview_data->page_height + PREVIEW_SHADOW_OFFSET);
	cairo_fill(cr);

	/* paper */
	cairo_set_source_rgb(cr, 1., 1., 1.);
	cairo_rectangle(cr, 0, 0, preview_data->page_width, preview_data->page_height);
	cairo_fill(cr);
	...
	if (preview_data->current_page) {
		...
		g_free(str);
		gtk_print_operation_preview_render_page(preview_data->preview, GPOINTER_TO_INT(preview_data->current_page->data));
...
static void cb_preview_zoom_fit(GtkButton *button, gpointer data)
{
	...
	zoom_w = ((gdouble)allocation.width) / ((gdouble)preview_data->page_width / preview_data->print_data->zoom + PREVIEW_SHADOW_OFFSET);
	zoom_h = ((gdouble)allocation.height) / ((gdouble)preview_data->page_height / preview_data->print_data->zoom + PREVIEW_SHADOW_OFFSET);
...
static void cb_preview_request_page_setup(GtkPrintOperation *op, GtkPrintContext *context, gint page_nr, GtkPageSetup *setup, gpointer data)
{
	GtkPaperSize *paper_size;
	...
	paper_size = gtk_paper_size_new_custom("preview paper", "preview_paper", width * preview_data->print_data->zoom, height * preview_data->print_data->zoom, GTK_UNIT_INCH);
	gtk_page_setup_set_paper_size(setup, paper_size);
	gtk_paper_size_free(paper_size);

	gtk_page_setup_set_top_margin(setup, top_margin * preview_data->print_data->zoom, GTK_UNIT_INCH);
	gtk_page_setup_set_bottom_margin(setup, bottom_margin * preview_data->print_data->zoom, GTK_UNIT_INCH);
	gtk_page_setup_set_left_margin(setup, left_margin * preview_data->print_data->zoom, GTK_UNIT_INCH);
	gtk_page_setup_set_right_margin(setup, right_margin * preview_data->print_data->zoom, GTK_UNIT_INCH);
}

static void printing_textview_cb_begin_print(GtkPrintOperation *op, GtkPrintContext *context, gpointer user_data)
{
	double width, height;
	int num_lines;
	double page_height;
	...
	if (prefs_common.use_different_print_font)
		...
		desc = pango_font_description_copy(pango_context_get_font_description(print_data->pango_context));
	...
	if (has_headers) {
		...
		pango_layout_index_to_line_x(print_data->layout, header_end_pos, 1, &num_header_lines, &dummy);
...
static cairo_surface_t *pixbuf_to_surface(GdkPixbuf *pixbuf)
{
	...
	guchar *pixels = g_malloc(4 * gdk_pixbuf_get_width(pixbuf) * gdk_pixbuf_get_height(pixbuf));
	...
	surface = cairo_image_surface_create_for_data(pixels, format, width, height, 4 * width);
	cairo_surface_set_user_data(surface, &key, pixels, (cairo_destroy_func_t) g_free);
...
static void printing_textview_cb_draw_page(GtkPrintOperation *op, GtkPrintContext *context, int page_nr, gpointer user_data)
{
	cairo_t *cr;
	PrintData *print_data;
	int start, end, ii;
	...
	do {
		...
		if (ii >= start) {
			...
			if (ii == 0 && print_data->ypos_line >= 0) {
				cairo_surface_t *surface;
				surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, gtk_print_context_get_width(context) / print_data->zoom, gtk_print_context_get_height(context) / print_data->zoom);
				cairo_set_line_width(cr, .5);
				cairo_set_source_surface(cr, surface, ((double)logical_rect.x) / PANGO_SCALE, ((double)baseline) / PANGO_SCALE - start_pos);
				cairo_move_to(cr, ((double)logical_rect.x) / PANGO_SCALE, (double)print_data->ypos_line / PANGO_SCALE);
			...
			cairo_move_to(cr, ((double)logical_rect.x) / PANGO_SCALE, ((double)baseline) / PANGO_SCALE - start_pos);
	...
	*has_headers = FALSE;
	...
	do {
		...
		if (prefs_common.print_imgs && (image = gtk_text_iter_get_pixbuf(&iter)) != NULL) {
			...
			attr = pango_attr_shape_new_with_data(&rect, &rect, scaled, NULL, NULL);
		...
		if (gtk_text_iter_ends_tag(&iter, NULL)) {
			...
			for (tag_walk = tags; tag_walk != NULL; tag_walk = tag_walk->next) {
				...
				g_object_get(G_OBJECT(tag), "background-set", &bg_set, "foreground-set", &fg_set, "underline-set", &under_set, "strikethrough-set", &strike_set, "weight-set", &weight_set, NULL);
