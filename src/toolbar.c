...
static void activate_compose_button(Toolbar *toolbar, ToolbarStyle style, ComposeButtonType type);

/* toolbar callbacks */
static void toolbar_reply(gpointer data, guint action);

static void toolbar_learn(gpointer data, guint action);

static void toolbar_delete_dup(gpointer data, guint action);
...
static void toolbar_parse_item(XMLFile *file, ToolbarType source, gboolean *rewrite)
{
	...
	if (item->index != -1) {
		if (!toolbar_is_duplicate(item->index, source)) {
			toolbar_config[source].item_list = g_slist_append(toolbar_config[source].item_list, item);
		} else {
...
const gchar *toolbar_get_short_text(int action)
{
	...
	case A_UNREAD:
		return _("Unread");
	case A_RUN_PROCESSING:
		return _("Run proc. rules");
...
gint toolbar_get_icon(int action)
{
	...
	case A_DELETE_DUP:
		return STOCK_PIXMAP_DELETE_DUP;
	case A_EXECUTE:
	...
	case A_IGNORE_THREAD:
		return STOCK_PIXMAP_MARK_IGNORETHREAD;
	case A_WATCH_THREAD:
		return STOCK_PIXMAP_MARK_WATCHTHREAD;
	case A_MARK:
		return STOCK_PIXMAP_MARK_MARK;
	case A_UNMARK:
		return STOCK_PIXMAP_MARK_UNMARK;
	case A_LOCK:
		return STOCK_PIXMAP_MARK_LOCKED;
	case A_UNLOCK:
		return STOCK_PIXMAP_MARK_UNLOCKED;
	case A_ALL_READ:
		return STOCK_PIXMAP_MARK_ALLREAD;
	case A_ALL_UNREAD:
		return STOCK_PIXMAP_MARK_ALLUNREAD;
	case A_READ:
		return STOCK_PIXMAP_MARK_READ;
	case A_UNREAD:
		return STOCK_PIXMAP_MARK_UNREAD;
	case A_RUN_PROCESSING:
		return STOCK_PIXMAP_DIR_OPEN;

	case A_PRINT:
		return STOCK_PIXMAP_PRINTER_BTN;
	case A_LEARN_SPAM:
	...
	case A_SEND_LATER:
		return STOCK_PIXMAP_MAIL_SEND_QUEUE;
	case A_DRAFT:
		return STOCK_PIXMAP_MAIL_DRAFT;
	case A_INSERT:
...
#endif
	case A_PRIVACY_SIGN:
		return STOCK_PIXMAP_MAIL_PRIVACY_SIGNED;
	case A_PRIVACY_ENCRYPT:
		return STOCK_PIXMAP_MAIL_PRIVACY_ENCRYPTED;
	...
	for (i = 0; default_toolbar[i].action != N_ACTION_VAL; i++) {
		...
		if (toolbar_item->index != -1) {
			if (!toolbar_is_duplicate(toolbar_item->index, toolbar_type)) {
				toolbar_config[toolbar_type].item_list = g_slist_append(toolbar_config[toolbar_type].item_list, toolbar_item);
			} else {
	...
	if (pfile) {
		...
		for (cur = toolbar_config[source].item_list; cur != NULL; cur = cur->next) {
			...
			if (toolbar_item->index != A_SEPARATOR) {
				if (fprintf(fp, "\t<%s %s=\"%s\" %s=\"", TOOLBAR_TAG_ITEM, TOOLBAR_ICON_FILE, toolbar_item->file, TOOLBAR_ICON_TEXT) < 0)
					goto fail;
				if (xml_file_put_escape_str(fp, toolbar_item->text) < 0)
					goto fail;
				if (fprintf(fp, "\" %s=\"%s\"/>\n", TOOLBAR_ICON_ACTION, toolbar_ret_text_from_val(toolbar_item->index)) < 0)
					goto fail;
			} else {
...
void toolbar_clear_list(ToolbarType source)
{
	while (toolbar_config[source].item_list != NULL) {
		...
		toolbar_config[source].item_list = g_slist_remove(toolbar_config[source].item_list, item);
...
void toolbar_set_list_item(ToolbarItem *t_item, ToolbarType source)
{
	...
	toolbar_config[source].item_list = g_slist_append(toolbar_config[source].item_list, toolbar_item);
}

static void toolbar_action_execute(GtkWidget *widget, GSList *action_list, gpointer data, gint source)
{
	GSList *cur;
	gint i = 0;
...
static void activate_compose_button(Toolbar *toolbar, ToolbarStyle style, ComposeButtonType type)
{
	...
	if (type == COMPOSEBUTTON_NEWS) {
		gtk_tool_button_set_icon_widget(GTK_TOOL_BUTTON(toolbar->compose_mail_btn), toolbar->compose_news_icon);
#ifndef GENERIC_UMPC
		...
		gtk_tool_button_set_icon_widget(GTK_TOOL_BUTTON(toolbar->compose_mail_btn), toolbar->compose_mail_icon);
#ifndef GENERIC_UMPC
...
void toolbar_set_compose_button(Toolbar *toolbar, ComposeButtonType compose_btn_type)
{
	if (toolbar->compose_btn_type != compose_btn_type)
		activate_compose_button(toolbar, prefs_common.toolbar_style, compose_btn_type);
}

static void activate_learn_button(Toolbar *toolbar, ToolbarStyle style, LearnButtonType type)
{
	if ((!toolbar->learn_spam_btn))
		return;

	if (type == LEARN_SPAM) {
		gtk_tool_button_set_icon_widget(GTK_TOOL_BUTTON(toolbar->learn_spam_btn), toolbar->learn_spam_icon);
		gtk_tool_button_set_label(GTK_TOOL_BUTTON(toolbar->learn_spam_btn), _("Spam"));
#ifndef GENERIC_UMPC
		CLAWS_SET_TOOL_ITEM_TIP(GTK_TOOL_ITEM(toolbar->learn_spam_btn), _("Learn spam"));
#endif
		gtk_widget_show(toolbar->learn_spam_icon);
	} else {
		gtk_tool_button_set_icon_widget(GTK_TOOL_BUTTON(toolbar->learn_spam_btn), toolbar->learn_ham_icon);
		gtk_tool_button_set_label(GTK_TOOL_BUTTON(toolbar->learn_spam_btn), _("Ham"));
#ifndef GENERIC_UMPC
...
void toolbar_set_learn_button(Toolbar *toolbar, LearnButtonType learn_btn_type)
{
	if (toolbar->learn_btn_type != learn_btn_type)
		activate_learn_button(toolbar, prefs_common.toolbar_style, learn_btn_type);
}
...
void toolbar_set_style(GtkWidget *toolbar_wid, GtkWidget *handlebox_wid, guint action)
{
	...
	case TOOLBAR_NONE:
		...
		break;
	case TOOLBAR_ICON:
		gtk_toolbar_set_style(GTK_TOOLBAR(toolbar_wid), GTK_TOOLBAR_ICONS);
		break;
	case TOOLBAR_TEXT:
		gtk_toolbar_set_style(GTK_TOOLBAR(toolbar_wid), GTK_TOOLBAR_TEXT);
		break;
	case TOOLBAR_BOTH:
		gtk_toolbar_set_style(GTK_TOOLBAR(toolbar_wid), GTK_TOOLBAR_BOTH);
		break;
	case TOOLBAR_BOTH_HORIZ:
		gtk_toolbar_set_style(GTK_TOOLBAR(toolbar_wid), GTK_TOOLBAR_BOTH_HORIZ);
		break;
	default:
		return;
	}
...
/* Toolbar handling */
static void toolbar_inc_cb(GtkWidget *widget, gpointer data)
{
...
static void toolbar_inc_all_cb(GtkWidget *widget, gpointer data)
{
	ToolbarItem *toolbar_item = (ToolbarItem *)data;
	MainWindow *mainwin;
...
static void toolbar_exec_cb(GtkWidget *widget, gpointer data)
{
...
static void toolbar_reply_cb(GtkWidget *widget, gpointer data)
{
	toolbar_reply(data, (prefs_common.reply_with_quote ? COMPOSE_REPLY_WITH_QUOTE : COMPOSE_REPLY_WITHOUT_QUOTE));
}
...
static void toolbar_reply_to_all_cb(GtkWidget *widget, gpointer data)
{
	toolbar_reply(data, (prefs_common.reply_with_quote ? COMPOSE_REPLY_TO_ALL_WITH_QUOTE : COMPOSE_REPLY_TO_ALL_WITHOUT_QUOTE));
}
...
static void toolbar_reply_to_list_cb(GtkWidget *widget, gpointer data)
{
	toolbar_reply(data, (prefs_common.reply_with_quote ? COMPOSE_REPLY_TO_LIST_WITH_QUOTE : COMPOSE_REPLY_TO_LIST_WITHOUT_QUOTE));
}
...
static void toolbar_reply_to_sender_cb(GtkWidget *widget, gpointer data)
{
	toolbar_reply(data, (prefs_common.reply_with_quote ? COMPOSE_REPLY_TO_SENDER_WITH_QUOTE : COMPOSE_REPLY_TO_SENDER_WITHOUT_QUOTE));
}
...
static void toolbar_buttons_cb(GtkWidget *widget, ToolbarItem *item)
{
	gint num_items;
	gint i;
	struct {
...
Toolbar *toolbar_create(ToolbarType type, GtkWidget *container, gpointer data)
{
	...
	for (cur = toolbar_list; cur != NULL; cur = cur->next) {
		...
		toolbar_data->item_list = g_slist_append(toolbar_data->item_list, toolbar_item);
		...
		case A_GO_FOLDERS:
			...
			break;
		case A_RECEIVE_ALL:
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Receive Mail from all Accounts"), _("Receive Mail from selected Account"));
		...
		case A_COMPOSE_EMAIL:
#ifndef GENERIC_UMPC
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Compose Email"), _("Compose with selected Account"));
...
#else
			TOOLBAR_ITEM(item, icon_wid, toolbar_item->text, _("Compose Email"));
			toolbar_data->compose_mail_btn = item;
			toolbar_data->compose_mail_icon = icon_wid;
			...
			break;
		case A_LEARN_SPAM:
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Spam"), _("Learn as..."));
		...
		case A_DELETE_DUP:
#ifndef GENERIC_UMPC
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Delete duplicates"), _("Delete duplicates options"));
		...
		case A_REPLY_MESSAGE:
#ifndef GENERIC_UMPC
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Reply to Message"), _("Reply to Message options"));
...
#else
			TOOLBAR_ITEM(item, icon_wid, toolbar_item->text, _("Reply to Message"));
			toolbar_data->reply_btn = item;
#endif
			break;
		case A_REPLY_SENDER:
#ifndef GENERIC_UMPC
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Reply to Sender"), _("Reply to Sender options"));
...
#else
			TOOLBAR_ITEM(item, icon_wid, toolbar_item->text, _("Reply to Sender"));
			toolbar_data->replysender_btn = item;
#endif
			break;
		case A_REPLY_ALL:
#ifndef GENERIC_UMPC
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Reply to All"), _("Reply to All options"));
...
#else
			TOOLBAR_ITEM(item, icon_wid, toolbar_item->text, _("Reply to All"));
			toolbar_data->replyall_btn = item;
#endif
			break;
		case A_REPLY_ML:
#ifndef GENERIC_UMPC
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Reply to Mailing-list"), _("Reply to Mailing-list options"));
...
#else
			TOOLBAR_ITEM(item, icon_wid, toolbar_item->text, _("Reply to Mailing-list"));
			toolbar_data->replylist_btn = item;
#endif
			break;
		case A_FORWARD:
#ifndef GENERIC_UMPC
			TOOLBAR_MENUITEM(item, icon_wid, toolbar_item->text, _("Forward Message"), _("Forward Message options"));
...
#else
			TOOLBAR_ITEM(item, icon_wid, toolbar_item->text, _("Forward Message"));
			toolbar_data->fwd_btn = item;
#endif
			break;
		case A_TRASH:
		...
		case A_PRIVACY_SIGN:
			...
			g_signal_connect(G_OBJECT(item), "toggled", G_CALLBACK(toolbar_privacy_sign_toggled_cb), NULL);
		...
		case A_PRIVACY_ENCRYPT:
			...
			g_signal_connect(G_OBJECT(item), "toggled", G_CALLBACK(toolbar_privacy_encrypt_toggled_cb), NULL);
			/* Call the "toggled" handler to set correct tooltip. */
		...
		case A_CLAWS_ACTIONS:
			...
			toolbar_data->action_list = g_slist_append(toolbar_data->action_list, action_item);
			break;
		case A_CANCEL_INC:
		...
		case A_CLAWS_PLUGINS:
			...
			break;
		default:
			TOOLBAR_ITEM(item, icon_wid, toolbar_item->text, toolbar_ret_descr_from_val(toolbar_item->index));
...
#endif
		activate_compose_button(toolbar_data, prefs_common.toolbar_style, toolbar_data->compose_btn_type);
	}
	if (type != TOOLBAR_COMPOSE)
		activate_learn_button(toolbar_data, prefs_common.toolbar_style, LEARN_SPAM);
...
void toolbar_update(ToolbarType type, gpointer data)
{
	...
	gtk_container_remove(GTK_CONTAINER(handlebox), GTK_WIDGET(toolbar_data->toolbar));
...
do { \
	...
	if (toolbar->get_btn)
		SET_WIDGET_COND(toolbar->get_btn, M_HAVE_ACCOUNT, M_UNLOCKED, M_HAVE_RETRIEVABLE_ACCOUNT);

	if (toolbar->getall_btn) {
		SET_WIDGET_COND(toolbar->getall_btn, M_HAVE_ACCOUNT, M_UNLOCKED, M_HAVE_ANY_RETRIEVABLE_ACCOUNT);
	}
	if (toolbar->send_btn) {
		SET_WIDGET_COND(toolbar->send_btn, M_HAVE_QUEUED_MAILS);
	}
	if (toolbar->compose_mail_btn) {
		SET_WIDGET_COND(toolbar->compose_mail_btn, M_HAVE_ACCOUNT);
	}
	if (toolbar->close_window_btn) {
		SET_WIDGET_COND(toolbar->close_window_btn, M_UNLOCKED);
	}
	if (toolbar->open_mail_btn) {
		SET_WIDGET_COND(toolbar->open_mail_btn, M_TARGET_EXIST, M_SUMMARY_ISLIST);
	}
	if (toolbar->reply_btn) {
		SET_WIDGET_COND(toolbar->reply_btn, M_HAVE_ACCOUNT, M_TARGET_EXIST, M_SUMMARY_ISLIST);
	}
	if (toolbar->replyall_btn) {
		SET_WIDGET_COND(toolbar->replyall_btn, M_HAVE_ACCOUNT, M_TARGET_EXIST, M_SUMMARY_ISLIST);
	}
	if (toolbar->replylist_btn) {
		SET_WIDGET_COND(toolbar->replylist_btn, M_HAVE_ACCOUNT, M_TARGET_EXIST, M_SUMMARY_ISLIST);
	}
	if (toolbar->replysender_btn) {
		SET_WIDGET_COND(toolbar->replysender_btn, M_HAVE_ACCOUNT, M_TARGET_EXIST, M_SUMMARY_ISLIST);
	}
	if (toolbar->fwd_btn) {
		SET_WIDGET_COND(toolbar->fwd_btn, M_HAVE_ACCOUNT, M_TARGET_EXIST, M_SUMMARY_ISLIST);
	}
	...
	if (toolbar->trash_btn)
		SET_WIDGET_COND(toolbar->trash_btn, M_TARGET_EXIST, M_ALLOW_DELETE, M_NOT_NEWS);

	if (toolbar->delete_btn)
		SET_WIDGET_COND(toolbar->delete_btn, M_TARGET_EXIST, M_ALLOW_DELETE);

	if (toolbar->delete_dup_btn)
		SET_WIDGET_COND(toolbar->delete_dup_btn, M_MSG_EXIST, M_ALLOW_DELETE, M_SUMMARY_ISLIST);

	if (toolbar->exec_btn)
		SET_WIDGET_COND(toolbar->exec_btn, M_DELAY_EXEC);

	if (toolbar->learn_spam_btn)
		SET_WIDGET_COND(toolbar->learn_spam_btn, M_TARGET_EXIST, M_CAN_LEARN_SPAM, M_SUMMARY_ISLIST);

	if (toolbar->cancel_inc_btn)
		SET_WIDGET_COND(toolbar->cancel_inc_btn, M_INC_ACTIVE);

	if (toolbar->cancel_send_btn)
		SET_WIDGET_COND(toolbar->cancel_send_btn, M_SEND_ACTIVE);
	...
	if (toolbar->cancel_all_btn)
		SET_WIDGET_COND(toolbar->cancel_all_btn, M_INC_ACTIVE, M_SEND_ACTIVE);
	...
	activate_compose_button(toolbar, prefs_common.toolbar_style, toolbar->compose_btn_type);
...
void send_queue_cb(gpointer data, guint action, GtkWidget *widget)
{
	...
	for (list = folder_get_list(); list != NULL; list = list->next) {
		...
		if (folder->queue) {
			if (procmsg_send_queue(folder->queue, prefs_common.savemsg, &errstr) < 0)
				got_error = TRUE;
		}
	}
	if (got_error) {
...
void compose_news_cb(gpointer data, guint action, GtkWidget *widget)
{
	...
	if (mainwin->summaryview->folder_item) {
		...
		if (ac && ac->protocol == A_NNTP) {
			compose_new_with_folderitem(ac, mainwin->summaryview->folder_item, NULL);
			return;
		}
	}
