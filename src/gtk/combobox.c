...
static gboolean _select_by_text_func(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, ComboboxSelCtx *ctx)
{
	GtkComboBox *combobox = ctx->combobox;
...
void combobox_set_popdown_strings(GtkComboBoxText *combobox, GList *list)
{
	...
	for (cur = list; cur != NULL; cur = g_list_next(cur))
		gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(combobox), (const gchar *)cur->data);
}
...
static void store_set_sensitive(GtkTreeModel *model, GtkTreeIter *iter, const gboolean sensitive)
{
	if (GTK_IS_LIST_STORE(model)) {
		gtk_list_store_set(GTK_LIST_STORE(model), iter, COMBOBOX_SENS, sensitive, -1);
	} else {
		gtk_tree_store_set(GTK_TREE_STORE(model), iter, COMBOBOX_SENS, sensitive, -1);
	}
}
