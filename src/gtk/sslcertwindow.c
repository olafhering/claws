...
static GtkWidget *cert_presenter(SSLCertificate *cert)
{
	...
	n = BUFFSIZE;
	if (gnutls_x509_crt_get_issuer_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_COMMON_NAME, 0, 0, issuer_commonname, &n))
		strncpy(issuer_commonname, _("<not in certificate>"), BUFFSIZE - 1);
	n = BUFFSIZE;

	if (gnutls_x509_crt_get_issuer_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_LOCALITY_NAME, 0, 0, issuer_location, &n)) {
		if (gnutls_x509_crt_get_issuer_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_COUNTRY_NAME, 0, 0, issuer_location, &n)) {
			strncpy(issuer_location, _("<not in certificate>"), BUFFSIZE - 1);
		}
	} else {
		tmp = g_malloc(BUFFSIZE);
		if (gnutls_x509_crt_get_issuer_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_COUNTRY_NAME, 0, 0, tmp, &n) == 0) {
	...
	n = BUFFSIZE;
	if (gnutls_x509_crt_get_issuer_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_ORGANIZATION_NAME, 0, 0, issuer_organization, &n))
		strncpy(issuer_organization, _("<not in certificate>"), BUFFSIZE - 1);

	n = BUFFSIZE;
	if (gnutls_x509_crt_get_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_COMMON_NAME, 0, 0, subject_commonname, &n))
		strncpy(subject_commonname, _("<not in certificate>"), BUFFSIZE - 1);
	n = BUFFSIZE;

	if (gnutls_x509_crt_get_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_LOCALITY_NAME, 0, 0, subject_location, &n)) {
		if (gnutls_x509_crt_get_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_COUNTRY_NAME, 0, 0, subject_location, &n)) {
			strncpy(subject_location, _("<not in certificate>"), BUFFSIZE - 1);
		}
	} else {
		tmp = g_malloc(BUFFSIZE);
		if (gnutls_x509_crt_get_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_COUNTRY_NAME, 0, 0, tmp, &n) == 0) {
	...
	n = BUFFSIZE;
	if (gnutls_x509_crt_get_dn_by_oid(cert->x509_cert, GNUTLS_OID_X520_ORGANIZATION_NAME, 0, 0, subject_organization, &n))
	...
	fingerprint = g_strdup_printf("SHA1: %s\nSHA256: %s", sha1_fingerprint, sha256_fingerprint);
...
void sslcertwindow_show_cert(SSLCertificate *cert)
{
	...
	alertpanel_full(buf, NULL, GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, FALSE, cert_widget, ALERT_NOTICE);
	g_free(buf);
}
...
static gboolean sslcertwindow_ask_new_cert(SSLCertificate *cert)
{
	...
	val = alertpanel_full(title, NULL, _("_Cancel connection"), _("_Accept and save"), NULL, ALERTFOCUS_FIRST, FALSE, vbox, ALERT_QUESTION);
...
static gboolean sslcertwindow_ask_expired_cert(SSLCertificate *cert)
{
	...
	val = alertpanel_full(title, NULL, _("_Cancel connection"), _("_Accept"), NULL, ALERTFOCUS_FIRST, FALSE, vbox, ALERT_QUESTION);
...
static gboolean sslcertwindow_ask_changed_cert(SSLCertificate *old_cert, SSLCertificate *new_cert)
{
	...
	val = alertpanel_full(title, NULL, _("_Cancel connection"), _("_Accept and save"), NULL, ALERTFOCUS_FIRST, FALSE, vbox2, ALERT_WARNING);
