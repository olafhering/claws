...
/* Callbacks */
static gboolean key_press_cb(GtkWidget *text_view, GdkEventKey *event, GtkAspell *gtkaspell);
static void entry_insert_cb(GtkTextBuffer *textbuf, GtkTextIter *iter, gchar *newtext, gint len, GtkAspell *gtkaspell);
static void entry_delete_cb(GtkTextBuffer *textbuf, GtkTextIter *startiter, GtkTextIter *enditer, GtkAspell *gtkaspell);
/*static gint button_press_intercept_cb		(GtkTextView	*gtktext,
						 GdkEvent	*e, 
						 GtkAspell	*gtkaspell);
*/
static void button_press_intercept_cb(GtkTextView *gtktext, GtkMenu *menu, GtkAspell *gtkaspell);
...
/* Checker actions */
static gboolean check_at(GtkAspell *gtkaspell, int from_pos);
...
static gboolean aspell_key_pressed(GtkWidget *widget, GdkEventKey *event, GtkAspell *gtkaspell);
static void change_dict_cb(GtkWidget *w, GtkAspell *gtkaspell);
static void switch_to_alternate_cb(GtkWidget *w, gpointer data);
...
static gboolean get_word_from_pos(GtkAspell *gtkaspell, gint pos, char *buf, gint buflen, gint *pstart, gint *pend);
static void allocate_color(GtkAspell *gtkaspell, gint rgbvalue);
static void change_color(GtkAspell *gtkaspell, gint start, gint end, gchar *newtext, GdkColor *color);
...
static void gtkaspell_checkers_error_message(gchar *message)
{
	...
	if (gtkaspellcheckers->error_message) {
		tmp = g_strdup_printf("%s\n%s", gtkaspellcheckers->error_message, message);
		g_free(message);
		g_free(gtkaspellcheckers->error_message);
...
GtkAspell *gtkaspell_new(const gchar *dictionary, const gchar *alt_dictionary, const gchar *encoding, /* unused */
			 gint misspelled_color, gboolean check_while_typing, gboolean recheck_when_changing_dict, gboolean use_alternate, gboolean use_both_dicts, GtkTextView *gtktext, GtkWindow *parent_win, void (dict_changed_cb) (void *data), void (*spell_menu_cb)(void *data), void *data)
{
	Dictionary *dict;
	GtkAspell *gtkaspell;
	...
	if (!gtkaspeller) {
		gtkaspell_checkers_error_message(g_strdup_printf(_("Couldn't initialize %s speller."), dictionary));
		return NULL;
	}
	...
	g_signal_connect(G_OBJECT(gtktext), "key_press_event", G_CALLBACK(key_press_cb), gtkaspell);
	g_signal_connect_after(G_OBJECT(buffer), "insert-text", G_CALLBACK(entry_insert_cb), gtkaspell);
	g_signal_connect_after(G_OBJECT(buffer), "delete-range", G_CALLBACK(entry_delete_cb), gtkaspell);
	/*g_signal_connect(G_OBJECT(gtktext), "button-press-event",
	   G_CALLBACK(button_press_intercept_cb),
	   gtkaspell); */
	g_signal_connect(G_OBJECT(gtktext), "populate-popup", G_CALLBACK(button_press_intercept_cb), gtkaspell);
...
void gtkaspell_delete(GtkAspell *gtkaspell)
{
	...
	g_signal_handlers_disconnect_by_func(G_OBJECT(gtktext), G_CALLBACK(key_press_cb), gtkaspell);
	g_signal_handlers_disconnect_by_func(G_OBJECT(gtktext), G_CALLBACK(entry_insert_cb), gtkaspell);
	g_signal_handlers_disconnect_by_func(G_OBJECT(gtktext), G_CALLBACK(entry_delete_cb), gtkaspell);
	g_signal_handlers_disconnect_by_func(G_OBJECT(gtktext), G_CALLBACK(button_press_intercept_cb), gtkaspell);
...
{
	if (!gtkaspell || !gtkaspell->dict_changed_cb || !gtkaspell->menu_changed_data)
		return;
...
static void entry_insert_cb(GtkTextBuffer *textbuf, GtkTextIter *iter, gchar *newtext, gint len, GtkAspell *gtkaspell)
{
	...
	if (iswordsep(g_utf8_get_char(newtext))) {
		 ...
		 * common case).
		 */
		if (pos < gtk_text_buffer_get_char_count(textbuf) && !iswordsep(get_text_index_whar(gtkaspell, pos))) {
			check_at(gtkaspell, pos - 1);
		}
	}
}
...
void gtkaspell_make_context_menu(GtkMenu *menu, GtkAspell *gtkaspell)
{
	...
	if (gtkaspell->misspelled && misspelled_suggest(gtkaspell, gtkaspell->theword)) {
	...
	g_signal_connect(G_OBJECT(menu), "deactivate", G_CALLBACK(destroy_menu), gtkaspell);
	if (suggest)
		g_signal_connect(G_OBJECT(menu), "key_press_event", G_CALLBACK(aspell_key_pressed), gtkaspell);
}
...
static GtkAspeller *gtkaspeller_new(Dictionary *dictionary)
{
	...
	if ((gtkaspeller = gtkaspeller_real_new(dict)) != NULL) {
		gtkaspellcheckers->checkers = g_slist_append(gtkaspellcheckers->checkers, gtkaspeller);
...
static GtkAspeller *gtkaspeller_real_new(Dictionary *dict)
{
	...
	if (!broker) {
		gtkaspell_checkers_error_message(g_strdup(_("Couldn't initialize Enchant broker.")));
		g_free(gtkaspeller);
		return NULL;
	}
	if ((speller = set_dictionary(broker, dict)) == NULL) {
		gtkaspell_checkers_error_message(g_strdup_printf(_("Couldn't initialize %s dictionary:"), dict->fullname));
		gtkaspell_checkers_error_message(g_strdup(enchant_broker_get_error(broker)));
		g_free(gtkaspeller);
		return NULL;
	}
...
int gtkaspell_misspelled_test(GtkAspell *gtkaspell, char *word)
{
	...
	return (result && strcasecmp(word, "sylpheed") && strcasecmp(word, "claws-mail"));
}
...
static gboolean get_word_from_pos(GtkAspell *gtkaspell, gint pos, char *buf, gint buflen, gint *pstart, gint *pend)
{
	...
	for (start = pos; start >= 0; --start) {
		...
		if (c == (gunichar)'\'') {
			if (start > 0) {
				if (g_unichar_isspace(get_text_index_whar(gtkaspell, start - 1))
				    || g_unichar_ispunct(get_text_index_whar(gtkaspell, start - 1))
				    || g_unichar_isdigit(get_text_index_whar(gtkaspell, start - 1))) {
					/* start_quote = TRUE; */
					break;
				}
			} else {
	...
	for (end = pos; end < get_textview_buffer_charcount(gtktext); end++) {
		...
		if (c == (gunichar)'\'') {
			if (end < get_textview_buffer_charcount(gtktext)) {
				if (g_unichar_isspace(get_text_index_whar(gtkaspell, end + 1))
				    || g_unichar_ispunct(get_text_index_whar(gtkaspell, end + 1))
				    || g_unichar_isdigit(get_text_index_whar(gtkaspell, end + 1))) {
					/* end_quote = TRUE; */
					break;
				}
			} else {
...
static gboolean check_at(GtkAspell *gtkaspell, gint from_pos)
{
	...
	if (!get_word_from_pos(gtkaspell, from_pos, buf, sizeof(buf), &start, &end))
		return FALSE;
...
static gboolean find_misspelled_cb(gpointer data, gboolean forward)
{
	...
	while (iswordsep(get_text_index_whar(gtkaspell, pos)) && pos > minpos && pos <= maxpos)
		pos += direc;
	while (!(misspelled = check_at(gtkaspell, pos)) && pos > minpos && pos <= maxpos) {
...
gboolean gtkaspell_check_next_prev(GtkAspell *gtkaspell, gboolean forward)
{
	...
	if (misspelled) {
		...
		if (gtkaspell == (GtkAspell *)gtkaspell->ctx.data) {
			...
			gtk_text_view_scroll_to_mark(gtkaspell->gtktext, gtk_text_buffer_get_insert(gtk_text_view_get_buffer(gtkaspell->gtktext)), 0.0, TRUE, 0.0, 0.5);
		...
		g_slist_free(list);
		gtk_menu_popup(GTK_MENU(menu), NULL, NULL, gtkaspell->ctx.set_menu_pos, gtkaspell->ctx.data, 0, GDK_CURRENT_TIME);
		g_signal_connect(G_OBJECT(menu), "deactivate", G_CALLBACK(destroy_menu), gtkaspell);
		g_signal_connect(G_OBJECT(menu), "key_press_event", G_CALLBACK(aspell_key_pressed), gtkaspell);
...
{
	gtkaspell->continue_check = NULL;
	gtkaspell->end_check_pos = get_textview_buffer_charcount(gtkaspell->gtktext);
	gtkaspell_context_set(gtkaspell);
...
void gtkaspell_highlight_all(GtkAspell *gtkaspell)
{
	...
	while (pos < len) {
		while (pos < len && iswordsep(get_text_index_whar(gtkaspell, pos)))
			pos++;
		while (pos < len && !iswordsep(get_text_index_whar(gtkaspell, pos)))
			pos++;
		if (pos > 0)
...
static void replace_with_supplied_word_cb(GtkWidget *w, GtkAspell *gtkaspell)
{
	...
	newword = gtk_editable_get_chars(GTK_EDITABLE(gtkaspell->replace_entry), 0, -1);

	if (strcmp(newword, gtkaspell->theword)) {
		gtkaspell->ctx.replace_word(gtkaspell->ctx.data, newword);

		if ((e->type == GDK_KEY_PRESS && ((GdkEventKey *)e)->state & GDK_CONTROL_MASK)) {
			enchant_dict_store_replacement(gtkaspell->gtkaspeller->speller, gtkaspell->theword, strlen(gtkaspell->theword), newword, strlen(newword));
		}
		gtkaspell->replace_entry = NULL;
	}
...
static void replace_word_cb(GtkWidget *w, gpointer data)
{
	...
	if ((e->type == GDK_KEY_PRESS && ((GdkEventKey *)e)->state & GDK_CONTROL_MASK) || (e->type == GDK_BUTTON_RELEASE && ((GdkEventButton *)e)->state & GDK_CONTROL_MASK)) {
		enchant_dict_store_replacement(gtkaspell->gtkaspeller->speller, gtkaspell->theword, strlen(gtkaspell->theword), newword, strlen(newword));
	}
...
void gtkaspell_block_check(GtkAspell *gtkaspell)
{
	...
	gtktext = gtkaspell->gtktext;
	g_signal_handlers_block_by_func(G_OBJECT(gtktext), G_CALLBACK(key_press_cb), gtkaspell);
	g_signal_handlers_block_by_func(G_OBJECT(gtktext), G_CALLBACK(entry_insert_cb), gtkaspell);
	g_signal_handlers_block_by_func(G_OBJECT(gtktext), G_CALLBACK(entry_delete_cb), gtkaspell);
}
...
void gtkaspell_unblock_check(GtkAspell *gtkaspell)
{
	...
	gtktext = gtkaspell->gtktext;
	g_signal_handlers_unblock_by_func(G_OBJECT(gtktext), G_CALLBACK(key_press_cb), gtkaspell);
	g_signal_handlers_unblock_by_func(G_OBJECT(gtktext), G_CALLBACK(entry_insert_cb), gtkaspell);
	g_signal_handlers_unblock_by_func(G_OBJECT(gtktext), G_CALLBACK(entry_delete_cb), gtkaspell);
}
...
static void replace_real_word(GtkAspell *gtkaspell, const gchar *newword)
{
	...
	gtk_text_buffer_get_iter_at_offset(textbuf, &startiter, gtkaspell->start_pos);
	gtk_text_buffer_get_iter_at_offset(textbuf, &enditer, gtkaspell->end_pos);
	g_signal_emit_by_name(G_OBJECT(textbuf), "delete-range", &startiter, &enditer, gtkaspell);
	g_signal_emit_by_name(G_OBJECT(textbuf), "insert-text", &startiter, newword, newlen, gtkaspell);
	...
	if (origpos - gtkaspell->start_pos < oldlen && origpos - gtkaspell->start_pos >= 0) {
...
static gboolean replace_key_pressed(GtkWidget *widget, GdkEventKey *event, GtkAspell *gtkaspell)
{
...
static void replace_with_create_dialog_cb(GtkWidget *w, gpointer data)
{
	...
	format = g_strconcat("<span weight=\"bold\" size=\"larger\">", _("Replace \"%s\" with: "), "</span>", NULL);
	...
	g_signal_connect(G_OBJECT(dialog), "key_press_event", G_CALLBACK(replace_key_pressed), gtkaspell);
	gtk_box_pack_start(GTK_BOX(vbox), entry, FALSE, FALSE, 0);
	g_free(utf8buf);

	label = gtk_label_new(_("Holding down Control key while pressing " "Enter\nwill learn from mistake.\n"));
	gtk_misc_set_alignment(GTK_MISC(label), 0, 0.5);
	gtk_label_set_justify(GTK_LABEL(label), GTK_JUSTIFY_LEFT);
	gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);
	...
	g_signal_connect(G_OBJECT(ok_button), "clicked", G_CALLBACK(replace_with_supplied_word_cb), gtkaspell);
	g_signal_connect_swapped(G_OBJECT(ok_button), "clicked", G_CALLBACK(gtk_widget_destroy), G_OBJECT(dialog));

	g_signal_connect_swapped(G_OBJECT(cancel_button), "clicked", G_CALLBACK(gtk_widget_destroy), G_OBJECT(dialog));
...
static void gtkaspell_uncheck_all(GtkAspell *gtkaspell)
{
	...
	gtk_text_buffer_get_iter_at_offset(buffer, &enditer, get_textview_buffer_charcount(gtktext) - 1);
	gtk_text_buffer_remove_tag_by_name(buffer, "misspelled", &startiter, &enditer);
}
...
static void list_dict_cb(const char *const lang_tag, const char *const provider_name, const char *const provider_desc, const char *const provider_file, void *data)
{
	...
	if (g_slist_find_custom(*list, dict, (GCompareFunc) compare_dict) == NULL) {
...
static GSList *gtkaspell_get_dictionary_list(gint refresh)
{
	...
	if (list == NULL) {
		...
		debug_print("Aspell: error when searching for dictionaries: " "No dictionary found.\n");
...
GtkTreeModel *gtkaspell_dictionary_store_new_with_refresh(gboolean refresh)
{
	...
	store = gtk_list_store_new(SET_GTKASPELL_SIZE, G_TYPE_STRING, G_TYPE_STRING, -1);

	for (tmp = dict_list; tmp != NULL; tmp = g_slist_next(tmp)) {
		dict = (Dictionary *)tmp->data;

		gtk_list_store_append(store, &iter);
		gtk_list_store_set(store, &iter, SET_GTKASPELL_NAME, dict->dictname, SET_GTKASPELL_FULLNAME, dict->fullname, -1);
	}
...
{
	return gtkaspell_dictionary_store_new_with_refresh(TRUE);
}
...
GtkWidget *gtkaspell_dictionary_combo_new(const gboolean refresh)
{
	...
	combo = gtk_combo_box_new_with_model(gtkaspell_dictionary_store_new_with_refresh(refresh));
	gtk_combo_box_set_active(GTK_COMBO_BOX(combo), 0);
	gtk_widget_show(combo);

	renderer = gtk_cell_renderer_text_new();
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(combo), renderer, TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo), renderer, "text", SET_GTKASPELL_NAME, NULL);
...
gchar *gtkaspell_get_dictionary_menu_active_item(GtkComboBox *combo)
{
	...
	gtk_tree_model_get(model, &iter, SET_GTKASPELL_FULLNAME, &dict_fullname, -1);

	return dict_fullname;
}

gint gtkaspell_set_dictionary_menu_active_item(GtkComboBox *combo, const gchar *dictionary)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	...
	do {
		gtk_tree_model_get(model, &iter, SET_GTKASPELL_FULLNAME, &dict_name, -1);
...
static void destroy_menu(GtkWidget *widget, gpointer user_data)
{
	...
	if (gtkaspell->accel_group) {
		gtk_window_remove_accel_group(GTK_WINDOW(gtkaspell->parent_window), gtkaspell->accel_group);
...
static gboolean aspell_key_pressed(GtkWidget *widget, GdkEventKey *event, GtkAspell *gtkaspell)
{
	if (event && (isascii(event->keyval) || event->keyval == GDK_KEY_Return)) {
		gtk_accel_groups_activate(G_OBJECT(gtkaspell->parent_window), event->keyval, event->state);
...
static GtkWidget *make_dictionary_list_submenu(GtkAspell *gtkaspell)
{
	...
	for (tmp = gtkaspellcheckers->dictionary_list; tmp != NULL; tmp = g_slist_next(tmp)) {
		if (count == MENUCOUNT) {

			moremenu = gtk_menu_new();
			item = gtk_menu_item_new_with_label(_("More..."));
			gtk_menu_item_set_submenu(GTK_MENU_ITEM(item), moremenu);
		...
		g_object_set_data(G_OBJECT(item), "dict_name", dict->dictname);
		if (g_strcmp0(dict->fullname, gtkaspell->gtkaspeller->dictionary->fullname))
			gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(item), FALSE);
		else {
			gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(item), TRUE);
			gtk_widget_set_sensitive(GTK_WIDGET(item), FALSE);
		}
		g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(change_dict_cb), gtkaspell);
		gtk_menu_shell_append(GTK_MENU_SHELL(curmenu), item);
...
static GSList *make_sug_menu(GtkAspell *gtkaspell)
{
	...
	caption = g_strdup_printf(_("\"%s\" unknown in dictionary '%s'"), utf8buf, gtkaspell->gtkaspeller->dictionary->dictname);
	...
	gtk_widget_show(item);
	list = g_slist_append(list, item);
	g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(add_word_to_session_cb), gtkaspell);
	gtk_widget_add_accelerator(item, "activate", accel, GDK_KEY_space, GDK_CONTROL_MASK, GTK_ACCEL_LOCKED | GTK_ACCEL_VISIBLE);

	item = gtk_menu_item_new_with_label(_("Add to personal dictionary"));
	gtk_widget_show(item);
	list = g_slist_append(list, item);
	g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(add_word_to_personal_cb), gtkaspell);
	gtk_widget_add_accelerator(item, "activate", accel, GDK_KEY_Return, GDK_CONTROL_MASK, GTK_ACCEL_LOCKED | GTK_ACCEL_VISIBLE);

	item = gtk_menu_item_new_with_label(_("Replace with..."));
	gtk_widget_show(item);
	list = g_slist_append(list, item);
	g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(replace_with_create_dialog_cb), gtkaspell);
	gtk_widget_add_accelerator(item, "activate", accel, GDK_KEY_R, 0, GTK_ACCEL_LOCKED | GTK_ACCEL_VISIBLE);
	gtk_widget_add_accelerator(item, "activate", accel, GDK_KEY_R, GDK_CONTROL_MASK, GTK_ACCEL_LOCKED);

	if (gtkaspell->use_alternate && gtkaspell->alternate_speller) {
		caption = g_strdup_printf(_("Check with %s"), gtkaspell->alternate_speller->dictionary->dictname);
		item = gtk_menu_item_new_with_label(caption);
		g_free(caption);
		gtk_widget_show(item);
		list = g_slist_append(list, item);
		g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(check_with_alternate_cb), gtkaspell);
		gtk_widget_add_accelerator(item, "activate", accel, GDK_KEY_X, 0, GTK_ACCEL_LOCKED | GTK_ACCEL_VISIBLE);
		gtk_widget_add_accelerator(item, "activate", accel, GDK_KEY_X, GDK_CONTROL_MASK, GTK_ACCEL_LOCKED);
	}
	...
	if (l == NULL) {
		...
		do {
			...
			g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(replace_word_cb), gtkaspell);

			if (curmenu == NULL && count < MENUCOUNT) {
				gtk_widget_add_accelerator(item, "activate", accel, GDK_KEY_A + count, 0, GTK_ACCEL_LOCKED | GTK_ACCEL_VISIBLE);
				gtk_widget_add_accelerator(item, "activate", accel, GDK_KEY_A + count, GDK_CONTROL_MASK, GTK_ACCEL_LOCKED);
			}
	...
	gtk_window_add_accel_group(GTK_WINDOW(gtkaspell->parent_window), accel);
...
static GSList *populate_submenu(GtkAspell *gtkaspell)
{
	...
	dictname = g_strdup_printf(_("Dictionary: %s"), gtkaspeller->dictionary->dictname);
	...
	if (gtkaspell->use_alternate && gtkaspell->alternate_speller) {
		dictname = g_strdup_printf(_("Use alternate (%s)"), gtkaspell->alternate_speller->dictionary->dictname);
		item = gtk_menu_item_new_with_label(dictname);
		g_free(dictname);
		g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(switch_to_alternate_cb), gtkaspell);
		gtk_widget_show(item);
		list = g_slist_append(list, item);
	}
	...
	g_signal_connect(G_OBJECT(both_dicts_item), "activate", G_CALLBACK(set_use_both_cb), gtkaspell);
	...
	if (gtkaspell->check_while_typing)
		...
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(item), FALSE);
	g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(toggle_check_while_typing_cb), gtkaspell);
	gtk_widget_show(item);
	list = g_slist_append(list, item);
...
static void set_menu_pos(GtkMenu *menu, gint *x, gint *y, gboolean *push_in, gpointer data)
{
	...
	gtk_text_buffer_get_iter_at_mark(textbuf, &iter, gtk_text_buffer_get_insert(textbuf));
	gtk_text_view_get_iter_location(gtkaspell->gtktext, &iter, &rect);
	gtk_text_view_buffer_to_window_coords(text, GTK_TEXT_WINDOW_TEXT, rect.x, rect.y, &rect.x, &rect.y);

	gdk_window_get_origin(gtk_widget_get_window(GTK_WIDGET(gtkaspell->gtktext)), &xx, &yy);
...
*/
gboolean gtkaspell_change_dict(GtkAspell *gtkaspell, const gchar *dictionary, gboolean always_set_alt_dict)
{
	Dictionary *dict;
	GtkAspeller *gtkaspeller;
...
static void allocate_color(GtkAspell *gtkaspell, gint rgbvalue)
{
	...
	if (rgbvalue != 0)
		gtk_text_buffer_create_tag(buffer, "misspelled", "foreground-gdk", color, NULL);
	else
		gtk_text_buffer_create_tag(buffer, "misspelled", "underline", PANGO_UNDERLINE_ERROR, NULL);
...
static void change_color(GtkAspell *gtkaspell, gint start, gint end, gchar *newtext, GdkColor *color)
{
	...
	if (color)
		gtk_text_buffer_apply_tag_by_name(buffer, "misspelled", &startiter, &enditer);
	else {
...
void gtkaspell_free_suggestions_list(GtkAspell *gtkaspell)
{
	...
	for (list = gtkaspell->suggestions_list; list != NULL; list = list->next)
		g_free(list->data);
...
{
	if (gtkaspell && gtkaspell->gtkaspeller && gtkaspell->gtkaspeller->dictionary)
