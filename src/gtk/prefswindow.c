...
static void prefs_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation, gpointer *user_data);
...
static gboolean prefswindow_row_selected(GtkTreeSelection *selector, GtkTreeModel *model, GtkTreePath *path, gboolean currently_selected, gpointer data);
...
static gboolean prefswindow_key_pressed(GtkWidget *widget, GdkEventKey *event, PrefsWindow *data)
{
	...
	if (event) {
		...
		case GDK_KEY_KP_Enter:
			focused_child = gtkut_get_focused_child(GTK_CONTAINER(data->notebook));
...
static gboolean find_node_by_name(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, FindNodeByName *data)
{
	gchar *name;
...
static gint prefswindow_tree_sort_by_weight(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b, gpointer context)
{
	gfloat f1, f2;
	gint i1, i2;
	...
	gtk_tree_model_get(model, a, PREFS_PAGE_INDEX, &i1, PREFS_PAGE_WEIGHT, &f1, -1);
	gtk_tree_model_get(model, b, PREFS_PAGE_INDEX, &i2, PREFS_PAGE_WEIGHT, &f2, -1);
...
static void prefswindow_build_page(PrefsWindow *prefswindow, PrefsPage *page)
{
	...
	if (!page->page_open) {
		...
		gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(scrolledwin), page->widget);
...
static void prefswindow_build_tree(GtkWidget *tree_view, GSList *prefs_pages, PrefsWindow *prefswindow, gboolean preload_pages)
{
	...
	for (cur = prefs_pages, index = 0; cur != NULL; cur = g_slist_next(cur), index++) {
		...
		for (i = 0; page->path[i] != NULL; i++) {
			...
			gtk_tree_model_foreach(GTK_TREE_MODEL(store), (GtkTreeModelForeachFunc) find_node_by_name, &find_name);
			if (find_name.found && (i == 0 || page->path[i] != page->path[i - 1])) {
				node = find_name.node;
				gtk_tree_model_get(GTK_TREE_MODEL(store), &node, PREFS_PAGE_DATA, &prefs_node, -1);
			} else {
				GAuto *autoptr;
				...
				gtk_tree_store_set(store, &child, PREFS_PAGE_TITLE, page->path[i], PREFS_PAGE_DATA, prefs_node, PREFS_PAGE_DATA_AUTO_FREE, autoptr, PREFS_PAGE_INDEX, index, PREFS_PAGE_WEIGHT, 0.0f, -1);
		...
		do {
			...
			gtk_tree_model_get(GTK_TREE_MODEL(store), &node, PREFS_PAGE_WEIGHT, &f, -1);
			if (page->weight > f) {
				f = page->weight;
				gtk_tree_store_set(store, &node, PREFS_PAGE_WEIGHT, f, -1);
			}
			child = node;
		} while (gtk_tree_model_iter_parent(GTK_TREE_MODEL(store), &node, &child));
	}

	gtk_tree_view_expand_all(GTK_TREE_VIEW(tree_view));

	/* set sort func & sort */
	gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(store), PREFS_PAGE_WEIGHT, prefswindow_tree_sort_by_weight, NULL, NULL);
	gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(store), PREFS_PAGE_WEIGHT, GTK_SORT_DESCENDING);
...
void prefswindow_open_full(const gchar *title, GSList *prefs_pages, gpointer data, GDestroyNotify func, gint *save_width, gint *save_height, gboolean preload_pages, PrefsOpenCallbackFunc open_cb, PrefsApplyCallbackFunc apply_cb, PrefsCloseCallbackFunc close_cb)
{
	...
	gtk_window_set_transient_for(GTK_WINDOW(prefswindow->window), GTK_WINDOW(mainwindow_get_mainwindow()->window));
	...
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(prefswindow->scrolledwindow1), GTK_SHADOW_IN);
	...
	gtk_container_add(GTK_CONTAINER(prefswindow->scrolledwindow1), prefswindow->tree_view);
	...
	gtk_table_attach(GTK_TABLE(prefswindow->table2), prefswindow->notebook, 0, 1, 1, 2, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 0, 4);
...
#ifndef GENERIC_UMPC
	gtkut_stock_button_set_create(&prefswindow->confirm_area, &prefswindow->apply_btn, GTK_STOCK_APPLY, &prefswindow->cancel_btn, GTK_STOCK_CANCEL, &prefswindow->ok_btn, GTK_STOCK_OK);
#else
	gtkut_stock_button_set_create(&prefswindow->confirm_area, &prefswindow->apply_btn, GTK_STOCK_APPLY, &prefswindow->ok_btn, GTK_STOCK_CLOSE, NULL, NULL);
#endif
...
#ifndef GENERIC_UMPC
	g_signal_connect(G_OBJECT(prefswindow->ok_btn), "clicked", G_CALLBACK(ok_button_clicked), prefswindow);
	g_signal_connect(G_OBJECT(prefswindow->cancel_btn), "clicked", G_CALLBACK(cancel_button_clicked), prefswindow);
	g_signal_connect(G_OBJECT(prefswindow->apply_btn), "clicked", G_CALLBACK(apply_button_clicked), prefswindow);
#else
	g_signal_connect(G_OBJECT(prefswindow->ok_btn), "clicked", G_CALLBACK(ok_button_clicked), prefswindow);
	g_signal_connect(G_OBJECT(prefswindow->apply_btn), "clicked", G_CALLBACK(apply_button_clicked), prefswindow);
#endif

	g_signal_connect(G_OBJECT(prefswindow->window), "delete_event", G_CALLBACK(window_closed), prefswindow);
	g_signal_connect(G_OBJECT(prefswindow->window), "key_press_event", G_CALLBACK(prefswindow_key_pressed), &(prefswindow->window));

	/* connect to callback only if we have non-NULL pointers to store size to */
	if (prefswindow->save_width && prefswindow->save_height) {
		g_signal_connect(G_OBJECT(prefswindow->window), "size_allocate", G_CALLBACK(prefs_size_allocate_cb), prefswindow);
	}
	...
	gtk_window_set_geometry_hints(GTK_WINDOW(prefswindow->window), NULL, &geometry, GDK_HINT_MIN_SIZE);
	if (prefswindow->save_width && prefswindow->save_height) {
		gtk_widget_set_size_request(prefswindow->window, *(prefswindow->save_width), *(prefswindow->save_height));
	}
#ifdef GENERIC_UMPC
	prefs_show_sections(prefswindow);
#endif
	gtk_widget_show(prefswindow->window);
	adj = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(prefswindow->scrolledwindow1));
	gtk_adjustment_set_value(adj, gtk_adjustment_get_lower(adj));
	gtk_adjustment_changed(adj);
}

void prefswindow_open(const gchar *title, GSList *prefs_pages, gpointer data, gint *save_width, gint *save_height, PrefsOpenCallbackFunc open_cb, PrefsApplyCallbackFunc apply_cb, PrefsCloseCallbackFunc close_cb)
{
	prefswindow_open_full(title, prefs_pages, data, NULL, save_width, save_height, FALSE, open_cb, apply_cb, close_cb);
}
...
static GtkTreeStore *prefswindow_create_data_store(void)
{
	return gtk_tree_store_new(N_PREFS_PAGE_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER, G_TYPE_AUTO_POINTER, G_TYPE_FLOAT, G_TYPE_INT, -1);
}
...
static GtkWidget *prefswindow_tree_view_create(PrefsWindow *prefswindow)
{
	...
	gtk_tree_selection_set_select_function(selector, prefswindow_row_selected, prefswindow, NULL);
...
static void prefs_filtering_create_tree_view_columns(GtkWidget *tree_view)
{
	...
	column = gtk_tree_view_column_new_with_attributes(_("Page Index"), renderer, "text", PREFS_PAGE_TITLE, NULL);
...
static gboolean prefswindow_row_selected(GtkTreeSelection *selector, GtkTreeModel *model, GtkTreePath *path, gboolean currently_selected, gpointer data)
{
	...
	if (page == NULL) {
		...
		pagenum = gtk_notebook_page_num(GTK_NOTEBOOK(prefswindow->notebook), prefswindow->empty_page);
		gtk_notebook_set_current_page(GTK_NOTEBOOK(prefswindow->notebook), pagenum);
		return TRUE;
	}
	...
	pagenum = gtk_notebook_page_num(GTK_NOTEBOOK(prefswindow->notebook), page->widget);
	gtk_notebook_set_current_page(GTK_NOTEBOOK(prefswindow->notebook), pagenum);

	adj = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(page->widget));
	lower = gtk_adjustment_get_lower(adj);
	gtk_adjustment_set_value(adj, lower);
	gtk_adjustment_changed(adj);
	adj = gtk_scrolled_window_get_hadjustment(GTK_SCROLLED_WINDOW(page->widget));
	gtk_adjustment_set_value(adj, lower);
	gtk_adjustment_changed(adj);
