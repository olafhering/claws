...
{
	return input_dialog_combo_remember(title, message, default_string, list, FALSE);
}
...
gchar *input_dialog_with_checkbtn(const gchar *title, const gchar *message, const gchar *default_string, const gchar *checkbtn_label, gboolean *checkbtn_state)
{
	...
	return input_dialog_open(title, message, checkbtn_label, default_string, prefs_common.inherit_folder_props, checkbtn_state);
}
...
gchar *input_dialog_query_password(const gchar *server, const gchar *user)
{
	...
	if (server && user)
		message = g_strdup_printf(_("Input password for %s on %s:"), user, server);
	else if (server)
		message = g_strdup_printf(_("Input password for %s:"), server);
	else if (user)
		message = g_strdup_printf(_("Input password for %s:"), user);
	else
...
gchar *input_dialog_query_password_keep(const gchar *server, const gchar *user, gchar **keep)
{
	...
	if (keep) {
		if (*keep != NULL) {
			...
			pass = input_dialog_with_invisible_checkbtn(_("Input password"), message, NULL, _("Remember password for this session"), &state);
			if (state) {
				*keep = g_strdup(pass);
...
static void input_dialog_create(gboolean is_password)
{
	...
	/* for title label */
	icon_q = gtk_image_new_from_stock(GTK_STOCK_DIALOG_QUESTION, GTK_ICON_SIZE_DIALOG);
	gtk_misc_set_alignment(GTK_MISC(icon_q), 0.5, 0.0);
	gtk_box_pack_start(GTK_BOX(hbox), icon_q, FALSE, FALSE, 0);
	icon_p = gtk_image_new_from_stock(GTK_STOCK_DIALOG_AUTHENTICATION, GTK_ICON_SIZE_DIALOG);
	...
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((combo)))), "activate", G_CALLBACK(combo_activated), NULL);
...
static gchar *input_dialog_open(const gchar *title, const gchar *message, const gchar *checkbtn_label, const gchar *default_string, gboolean default_checkbtn_state, gboolean *remember)
{
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(remember_checkbtn), default_checkbtn_state);
	if (remember)
		gtk_widget_show(remember_checkbtn);
	else
		gtk_widget_hide(remember_checkbtn);
