...
typedef gint (*GtkCMCListCompareFunc)(GtkCMCList *clist, gconstpointer ptr1, gconstpointer ptr2);
...
struct _GtkCMCListClass {
	...
	void (*set_scroll_adjustments)(GtkCMCList *clist, GtkAdjustment *hadjustment, GtkAdjustment *vadjustment);
	void (*refresh)(GtkCMCList *clist);
	void (*select_row)(GtkCMCList *clist, gint row, gint column, GdkEvent *event);
	void (*unselect_row)(GtkCMCList *clist, gint row, gint column, GdkEvent *event);
	void (*row_move)(GtkCMCList *clist, gint source_row, gint dest_row);
	void (*click_column)(GtkCMCList *clist, gint column);
	void (*resize_column)(GtkCMCList *clist, gint column, gint width);
	...
	void (*extend_selection)(GtkCMCList *clist, GtkScrollType scroll_type, gfloat position, gboolean auto_start_selection);
	void (*scroll_horizontal)(GtkCMCList *clist, GtkScrollType scroll_type, gfloat position);
	void (*scroll_vertical)(GtkCMCList *clist, GtkScrollType scroll_type, gfloat position);
	void (*toggle_add_mode)(GtkCMCList *clist);
	void (*abort_column_resize)(GtkCMCList *clist);
	void (*resync_selection)(GtkCMCList *clist, GdkEvent *event);
	GList *(*selection_find)(GtkCMCList *clist, gint row_number, GList *row_list_element);
	void (*draw_row)(GtkCMCList *clist, GdkRectangle * area, gint row, GtkCMCListRow * clist_row);
	void (*clear)(GtkCMCList *clist);
	void (*fake_unselect_all)(GtkCMCList *clist, gint row);
	void (*sort_list)(GtkCMCList *clist);
	gint (*insert_row)(GtkCMCList *clist, gint row, gchar *text[]);
	void (*remove_row)(GtkCMCList *clist, gint row);
	void (*set_cell_contents)(GtkCMCList *clist, GtkCMCListRow * clist_row, gint column, GtkCMCellType type, const gchar *text, guint8 spacing, GdkPixbuf *pixbuf);
	void (*cell_size_request)(GtkCMCList *clist, GtkCMCListRow * clist_row, gint column, GtkRequisition *requisition);
...
GtkWidget *gtk_cmclist_new_with_titles(gint columns, gchar *titles[]);

/* set adjustments of clist */
void gtk_cmclist_set_hadjustment(GtkCMCList *clist, GtkAdjustment *adjustment);
void gtk_cmclist_set_vadjustment(GtkCMCList *clist, GtkAdjustment *adjustment);
...
void gtk_cmclist_set_shadow_type(GtkCMCList *clist, GtkShadowType type);

/* set the clist's selection mode */
void gtk_cmclist_set_selection_mode(GtkCMCList *clist, GtkSelectionMode mode);

/* enable clists reorder ability */
void gtk_cmclist_set_reorderable(GtkCMCList *clist, gboolean reorderable);
void gtk_cmclist_set_use_drag_icons(GtkCMCList *clist, gboolean use_icons);
void gtk_cmclist_set_button_actions(GtkCMCList *clist, guint button, guint8 button_actions);
 ...
 * a title
 */
void gtk_cmclist_column_title_active(GtkCMCList *clist, gint column);
void gtk_cmclist_column_title_passive(GtkCMCList *clist, gint column);
...
void gtk_cmclist_set_column_title(GtkCMCList *clist, gint column, const gchar *title);

/* returns the title of column. Returns NULL if title is not set */
gchar *gtk_cmclist_get_column_title(GtkCMCList *clist, gint column);

/* set a widget instead of a title for the column title button */
void gtk_cmclist_set_column_widget(GtkCMCList *clist, gint column, GtkWidget *widget);

/* returns the column widget */
GtkWidget *gtk_cmclist_get_column_widget(GtkCMCList *clist, gint column);

/* set the justification on a column */
void gtk_cmclist_set_column_justification(GtkCMCList *clist, gint column, GtkJustification justification);

/* set visibility of a column */
void gtk_cmclist_set_column_visibility(GtkCMCList *clist, gint column, gboolean visible);

/* enable/disable column resize operations by mouse */
void gtk_cmclist_set_column_resizeable(GtkCMCList *clist, gint column, gboolean resizeable);

/* resize column automatically to its optimal width */
void gtk_cmclist_set_column_auto_resize(GtkCMCList *clist, gint column, gboolean auto_resize);

gint gtk_cmclist_columns_autosize(GtkCMCList *clist);

/* return the optimal column width, i.e. maximum of all cell widths */
gint gtk_cmclist_optimal_column_width(GtkCMCList *clist, gint column);
 ...
 */
void gtk_cmclist_set_column_width(GtkCMCList *clist, gint column, gint width);

/* set column minimum/maximum width. min/max_width < 0 => no restriction */
void gtk_cmclist_set_column_min_width(GtkCMCList *clist, gint column, gint min_width);
void gtk_cmclist_set_column_max_width(GtkCMCList *clist, gint column, gint max_width);

/* change the height of the rows, the default (height=0) is
 * the hight of the current font.
 */
void gtk_cmclist_set_row_height(GtkCMCList *clist, guint height);
 ...
 */
void gtk_cmclist_moveto(GtkCMCList *clist, gint row, gint column, gfloat row_align, gfloat col_align);

/* returns whether the row is visible */
GtkVisibility gtk_cmclist_row_is_visible(GtkCMCList *clist, gint row);

/* returns whether the row is above or below current viewport */
gboolean gtk_cmclist_row_is_above_viewport(GtkCMCList *clist, gint row);
gboolean gtk_cmclist_row_is_below_viewport(GtkCMCList *clist, gint row);

/* returns the cell type */
GtkCMCellType gtk_cmclist_get_cell_type(GtkCMCList *clist, gint row, gint column);

/* sets a given cell's text, replacing its current contents */
void gtk_cmclist_set_text(GtkCMCList *clist, gint row, gint column, const gchar *text);

/* for the "get" functions, any of the return pointer can be
 * NULL if you are not interested
 */
gint gtk_cmclist_get_text(GtkCMCList *clist, gint row, gint column, gchar **text);

/* sets a given cell's pixbuf, replacing its current contents */
void gtk_cmclist_set_pixbuf(GtkCMCList *clist, gint row, gint column, GdkPixbuf *pixbuf);

gint gtk_cmclist_get_pixbuf(GtkCMCList *clist, gint row, gint column, GdkPixbuf **pixbuf);

/* sets a given cell's pixbuf and text, replacing its current contents */
void gtk_cmclist_set_pixtext(GtkCMCList *clist, gint row, gint column, const gchar *text, guint8 spacing, GdkPixbuf *pixbuf);

gint gtk_cmclist_get_pixtext(GtkCMCList *clist, gint row, gint column, gchar **text, guint8 * spacing, GdkPixbuf **pixbuf);

/* sets the foreground color of a row, the color must already
 * be allocated
 */
void gtk_cmclist_set_foreground(GtkCMCList *clist, gint row, const GdkColor *color);

/* sets the background color of a row, the color must already
 * be allocated
 */
void gtk_cmclist_set_background(GtkCMCList *clist, gint row, const GdkColor *color);

/* set / get cell styles */
void gtk_cmclist_set_cell_style(GtkCMCList *clist, gint row, gint column, GtkStyle *style);

GtkStyle *gtk_cmclist_get_cell_style(GtkCMCList *clist, gint row, gint column);

void gtk_cmclist_set_row_style(GtkCMCList *clist, gint row, GtkStyle *style);

GtkStyle *gtk_cmclist_get_row_style(GtkCMCList *clist, gint row);
 ...
 */
void gtk_cmclist_set_shift(GtkCMCList *clist, gint row, gint column, gint vertical, gint horizontal);

/* set/get selectable flag of a single row */
void gtk_cmclist_set_selectable(GtkCMCList *clist, gint row, gboolean selectable);
gboolean gtk_cmclist_get_selectable(GtkCMCList *clist, gint row);

/* prepend/append returns the index of the row you just added,
 * making it easier to append and modify a row
 */
gint gtk_cmclist_prepend(GtkCMCList *clist, gchar *text[]);
gint gtk_cmclist_append(GtkCMCList *clist, gchar *text[]);

/* inserts a row at index row and returns the row where it was
 * actually inserted (may be different from "row" in auto_sort mode)
 */
gint gtk_cmclist_insert(GtkCMCList *clist, gint row, gchar *text[]);

/* removes row at index row */
void gtk_cmclist_remove(GtkCMCList *clist, gint row);

/* sets a arbitrary data pointer for a given row */
void gtk_cmclist_set_row_data(GtkCMCList *clist, gint row, gpointer data);

/* sets a data pointer for a given row with destroy notification */
void gtk_cmclist_set_row_data_full(GtkCMCList *clist, gint row, gpointer data, GDestroyNotify destroy);

/* returns the data set for a row */
gpointer gtk_cmclist_get_row_data(GtkCMCList *clist, gint row);

/* givin a data pointer, find the first (and hopefully only!)
 * row that points to that data, or -1 if none do
 */
gint gtk_cmclist_find_row_from_data(GtkCMCList *clist, gpointer data);

/* force selection of a row */
void gtk_cmclist_select_row(GtkCMCList *clist, gint row, gint column);

/* force unselection of a row */
void gtk_cmclist_unselect_row(GtkCMCList *clist, gint row, gint column);
 ...
 */
gint gtk_cmclist_get_selection_info(GtkCMCList *clist, gint x, gint y, gint *row, gint *column);
...
void gtk_cmclist_swap_rows(GtkCMCList *clist, gint row1, gint row2);

/* move row from source_row position to dest_row position */
void gtk_cmclist_row_move(GtkCMCList *clist, gint source_row, gint dest_row);

/* sets a compare function different to the default */
void gtk_cmclist_set_compare_func(GtkCMCList *clist, GtkCMCListCompareFunc cmp_func);

/* the column to sort by */
void gtk_cmclist_set_sort_column(GtkCMCList *clist, gint column);

/* how to sort : ascending or descending */
void gtk_cmclist_set_sort_type(GtkCMCList *clist, GtkSortType sort_type);

/* sort the list with the current compare function */
void gtk_cmclist_sort(GtkCMCList *clist);

/* Automatically sort upon insertion */
void gtk_cmclist_set_auto_sort(GtkCMCList *clist, gboolean auto_sort);

/* Private function for clist, ctree */

PangoLayout *_gtk_cmclist_create_cell_layout(GtkCMCList *clist, GtkCMCListRow * clist_row, gint column);
