...
static void gtk_shruler_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void gtk_shruler_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
...
static void gtk_shruler_size_allocate(GtkWidget *widget, GtkAllocation *allocation);
static void gtk_shruler_size_request(GtkWidget *widget, GtkRequisition *requisition);
static void gtk_shruler_style_set(GtkWidget *widget, GtkStyle *prev_style);
static gboolean gtk_shruler_motion_notify(GtkWidget *widget, GdkEventMotion *event);
static gboolean gtk_shruler_expose(GtkWidget *widget, GdkEventExpose *event);

static void gtk_shruler_draw_ticks(GtkSHRuler *ruler);
static void gtk_shruler_make_pixmap(GtkSHRuler *ruler);

static PangoLayout *gtk_shruler_get_layout(GtkWidget *widget, const gchar *text);

#if !GLIB_CHECK_VERSION(2, 58, 0)
G_DEFINE_TYPE(GtkSHRuler, gtk_shruler, GTK_TYPE_WIDGET)
#else
G_DEFINE_TYPE_WITH_CODE(GtkSHRuler, gtk_shruler, GTK_TYPE_WIDGET, G_ADD_PRIVATE(GtkSHRuler))
#endif
...
static void gtk_shruler_class_init(GtkSHRulerClass *klass)
{
	...
	g_object_class_install_property(object_class, PROP_ORIENTATION, g_param_spec_enum("orientation", "Orientation", "The orientation of the ruler", GTK_TYPE_ORIENTATION, GTK_ORIENTATION_HORIZONTAL, GTK_PARAM_READWRITE));

	g_object_class_install_property(object_class, PROP_LOWER, gtk_param_spec_unit("unit", "Unit", "Unit of ruler", TRUE, TRUE, CM_UNIT_PIXEL, GTK_PARAM_READWRITE));

	g_object_class_install_property(object_class, PROP_LOWER, g_param_spec_double("lower", "Lower", "Lower limit of ruler", -G_MAXDOUBLE, G_MAXDOUBLE, 0.0, GTK_PARAM_READWRITE));

	g_object_class_install_property(object_class, PROP_UPPER, g_param_spec_double("upper", "Upper", "Upper limit of ruler", -G_MAXDOUBLE, G_MAXDOUBLE, 0.0, GTK_PARAM_READWRITE));

	g_object_class_install_property(object_class, PROP_POSITION, g_param_spec_double("position", "Position", "Position of mark on the ruler", -G_MAXDOUBLE, G_MAXDOUBLE, 0.0, GTK_PARAM_READWRITE));

	g_object_class_install_property(object_class, PROP_MAX_SIZE, g_param_spec_double("max-size", "Max Size", "Maximum size of the ruler", -G_MAXDOUBLE, G_MAXDOUBLE, 0.0, GTK_PARAM_READWRITE));

	gtk_widget_class_install_style_property(widget_class, g_param_spec_double("font-scale", NULL, NULL, 0.0, G_MAXDOUBLE, DEFAULT_RULER_FONT_SCALE, GTK_PARAM_READABLE));
}

static void gtk_shruler_init(GtkSHRuler *ruler)
{
...
static void gtk_shruler_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_LOWER:
		gtk_shruler_set_range(ruler, g_value_get_double(value), priv->upper, priv->max_size);
		break;
	case PROP_UPPER:
		gtk_shruler_set_range(ruler, priv->lower, g_value_get_double(value), priv->max_size);
		break;
	...
	case PROP_MAX_SIZE:
		gtk_shruler_set_range(ruler, priv->lower, priv->upper, g_value_get_double(value));
		break;
...
{
	return g_object_new(GTK_TYPE_SHRULER, "orientation", orientation, NULL);
}

static void gtk_shruler_update_position(GtkSHRuler *ruler, gdouble x, gdouble y)
{
	GtkSHRulerPrivate *priv = GTK_SHRULER_GET_PRIVATE(ruler);
	...
	if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
		gtk_shruler_set_position(ruler, lower + (upper - lower) * x / allocation.width);
	} else {
		gtk_shruler_set_position(ruler, lower + (upper - lower) * y / allocation.height);
	}
}
 ...
 * Since: GTK 2.8
 */
void gtk_shruler_set_unit(GtkSHRuler *ruler, GtkCMUnit unit)
{
 ...
 * Since: GTK 2.8
 */
void gtk_shruler_set_position(GtkSHRuler *ruler, gdouble position)
{
	GtkSHRulerPrivate *priv;
 ...
 * Since: GTK 2.8
 */
void gtk_shruler_set_range(GtkSHRuler *ruler, gdouble lower, gdouble upper, gdouble max_size)
{
	GtkSHRulerPrivate *priv;
 ...
 * Since: GTK 2.8
 **/
void gtk_shruler_get_range(GtkSHRuler *ruler, gdouble *lower, gdouble *upper, gdouble *max_size)
{
	GtkSHRulerPrivate *priv;
...
static void gtk_shruler_realize(GtkWidget *widget)
{
	...
	attributes.event_mask = (gtk_widget_get_events(widget) | GDK_EXPOSURE_MASK | GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK);

	attributes_mask = GDK_WA_X | GDK_WA_Y;

	priv->input_window = gdk_window_new(gtk_widget_get_window(widget), &attributes, attributes_mask);
...
static void gtk_shruler_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
	...
	if (gtk_widget_get_realized(widget)) {
		gdk_window_move_resize(priv->input_window, allocation->x, allocation->y, allocation->width, allocation->height);
...
static void gtk_shruler_style_set(GtkWidget *widget, GtkStyle *prev_style)
{
	...
	gtk_widget_style_get(widget, "font-scale", &priv->font_scale, NULL);
...
static void gtk_shruler_draw_ticks(GtkSHRuler *ruler)
{
	...
	for (i = G_N_ELEMENTS(ruler_metric.subdivide) - 1; i >= 0; i--) {
		...
		if (unit == CM_UNIT_PIXEL && scale == 1 && i == 1)
			...
			subd_incr = ((gdouble)ruler_metric.ruler_scale[scale] / (gdouble)ruler_metric.subdivide[i]);
		...
		for (cur = start; cur <= end; cur += subd_incr) {
			...
			if (i == 0 && ((int)cur) % 10 == 0) {
				...
				if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
					...
					cairo_move_to(cr, pos + 2, ythickness + PANGO_PIXELS(logical_rect.y - digit_offset));
					...
					for (j = 0; j < (int)strlen(unit_str); j++) {
						...
						cairo_move_to(cr, xthickness + 1, pos + digit_height * j + 2 + PANGO_PIXELS(logical_rect.y - digit_offset));
						pango_cairo_show_layout(cr, layout);
					}
				}
			}
...
static void gtk_shruler_make_pixmap(GtkSHRuler *ruler)
{
	...
	priv->backing_store = gdk_window_create_similar_surface(gtk_widget_get_window(widget), CAIRO_CONTENT_COLOR, allocation.width, allocation.height);
}

static PangoLayout *gtk_shruler_create_layout(GtkWidget *widget, const gchar *text)
{
	GtkSHRulerPrivate *priv = GTK_SHRULER_GET_PRIVATE(widget);
	PangoLayout *layout;
	PangoAttrList *attrs;
