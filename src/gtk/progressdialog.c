...
static gint progress_dialog_insert_account(ProgressDialog *progress, gint row, const gchar *account, const gchar *status, GdkPixbuf *image);
...
ProgressDialog *progress_dialog_create(void)
{
	...
	gtk_container_set_border_width(GTK_CONTAINER(gtk_dialog_get_action_area(GTK_DIALOG(dialog))), 0);
	...
	store = gtk_list_store_new(N_PROGRESS_COLUMNS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER);
	...
	column = gtk_tree_view_column_new_with_attributes(NULL, renderer, "pixbuf", PROGRESS_IMAGE, NULL);
	...
	column = gtk_tree_view_column_new_with_attributes(_("Account"), renderer, "text", PROGRESS_ACCOUNT, NULL);
	...
	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(_("Status"), renderer, "text", PROGRESS_STATE, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_FIXED);
	gtk_tree_view_append_column(GTK_TREE_VIEW(treeview), column);

	/* the WM close and hide buttons are handled internally and not published to caller */
	g_signal_connect(G_OBJECT(dialog), "delete_event", G_CALLBACK(progress_dialog_delete_event_btn_cb), NULL);
	g_signal_connect(G_OBJECT(hide_btn), "clicked", G_CALLBACK(progress_dialog_hide_btn_cb), dialog);
...
{
	return progress_dialog_insert_account(progress, row, NULL, status, NULL);
}
...
{
	return progress_dialog_insert_account(progress, row, account_name, status, image);
}
...
static gint progress_dialog_insert_account(ProgressDialog *progress, gint row, const gchar *account, const gchar *status, GdkPixbuf *image)
{
	...
	if (row < 0 || !gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(store), &iter, NULL, row)) {
		result = gtk_tree_model_iter_n_children(GTK_TREE_MODEL(store), NULL);
	...
	if (account)
		gtk_list_store_set(store, &iter, PROGRESS_ACCOUNT, account, -1);
	if (status)
		gtk_list_store_set(store, &iter, PROGRESS_STATE, status, -1);
	if (image)
		gtk_list_store_set(store, &iter, PROGRESS_IMAGE, image, -1);
