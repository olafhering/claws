...
static gint gtk_vscrollbutton_button_release(GtkWidget *widget, GdkEventButton *event, GtkVScrollbutton *scrollbutton);

static void gtk_vscrollbutton_set_adjustment(GtkVScrollbutton *scrollbutton, GtkAdjustment *adjustment);

static gint gtk_vscrollbutton_button_press(GtkWidget *widget, GdkEventButton *event, GtkVScrollbutton *scrollbutton);
...
static void gtk_vscrollbutton_set_sensitivity(GtkAdjustment *adjustment, GtkVScrollbutton *scrollbutton);
...
static gboolean vscroll_leave_notify(GtkWidget *widget, GdkEventCrossing *event, gpointer data)
{
	gdk_window_set_cursor(gtk_widget_get_window(widget), NULL);
	return FALSE;
}

static gboolean vscroll_enter_notify(GtkWidget *widget, GdkEventCrossing *event, gpointer data)
{
	gdk_window_set_cursor(gtk_widget_get_window(widget), hand_cursor);
	return FALSE;
}
...
static void gtk_vscrollbutton_init(GtkVScrollbutton *scrollbutton)
{
	...
	g_signal_connect(G_OBJECT(scrollbutton->upbutton), "motion-notify-event", G_CALLBACK(vscroll_visi_notify), NULL);
	g_signal_connect(G_OBJECT(scrollbutton->upbutton), "leave-notify-event", G_CALLBACK(vscroll_leave_notify), NULL);
	g_signal_connect(G_OBJECT(scrollbutton->upbutton), "enter-notify-event", G_CALLBACK(vscroll_enter_notify), NULL);
	g_signal_connect(G_OBJECT(scrollbutton->downbutton), "motion-notify-event", G_CALLBACK(vscroll_visi_notify), NULL);
	g_signal_connect(G_OBJECT(scrollbutton->downbutton), "leave-notify-event", G_CALLBACK(vscroll_leave_notify), NULL);
	g_signal_connect(G_OBJECT(scrollbutton->downbutton), "enter-notify-event", G_CALLBACK(vscroll_enter_notify), NULL);

	g_signal_connect(G_OBJECT(scrollbutton->upbutton), "button_press_event", G_CALLBACK(gtk_vscrollbutton_button_press), scrollbutton);
	g_signal_connect(G_OBJECT(scrollbutton->downbutton), "button_press_event", G_CALLBACK(gtk_vscrollbutton_button_press), scrollbutton);
	g_signal_connect(G_OBJECT(scrollbutton->upbutton), "button_release_event", G_CALLBACK(gtk_vscrollbutton_button_release), scrollbutton);
	g_signal_connect(G_OBJECT(scrollbutton->downbutton), "button_release_event", G_CALLBACK(gtk_vscrollbutton_button_release), scrollbutton);
	gtk_box_pack_start(GTK_BOX(&scrollbutton->vbox), scrollbutton->upbutton, TRUE, TRUE, 0);
	gtk_box_pack_end(GTK_BOX(&scrollbutton->vbox), scrollbutton->downbutton, TRUE, TRUE, 0);
...
GtkWidget *gtk_vscrollbutton_new(GtkAdjustment *adjustment)
{
	...
	vscrollbutton = g_object_new(gtk_vscrollbutton_get_type(), NULL);
	gtk_vscrollbutton_set_adjustment(GTK_VSCROLLBUTTON(vscrollbutton), adjustment);
	g_signal_connect(G_OBJECT(GTK_VSCROLLBUTTON(vscrollbutton)->adjustment), "value_changed", G_CALLBACK(gtk_vscrollbutton_set_sensitivity), vscrollbutton);
	g_signal_connect(G_OBJECT(GTK_VSCROLLBUTTON(vscrollbutton)->adjustment), "changed", G_CALLBACK(gtk_vscrollbutton_set_sensitivity), vscrollbutton);
	return vscrollbutton;
}

void gtk_vscrollbutton_set_adjustment(GtkVScrollbutton *scrollbutton, GtkAdjustment *adjustment)
{
...
gboolean gtk_vscrollbutton_scroll(GtkVScrollbutton *scrollbutton)
{
	...
	if (new_value != value) {
		...
		g_signal_emit_by_name(G_OBJECT(scrollbutton->adjustment), "value_changed");
...
static gboolean gtk_vscrollbutton_timer_1st_time(GtkVScrollbutton *scrollbutton)
{
	...
	if (scrollbutton->timer) {
		...
		scrollbutton->timer = g_timeout_add(SCROLL_LATER_DELAY, (GSourceFunc)
						    gtk_real_vscrollbutton_timer, scrollbutton);
	}
...
static void gtk_vscrollbutton_add_timer(GtkVScrollbutton *scrollbutton)
{
	...
	if (!scrollbutton->timer) {
		...
		scrollbutton->timer = g_timeout_add(SCROLL_INITIAL_DELAY, (GSourceFunc)
						    gtk_vscrollbutton_timer_1st_time, scrollbutton);
	}
}
...
static gboolean gtk_real_vscrollbutton_timer(GtkVScrollbutton *scrollbutton)
{
	...
	if (!scrollbutton->timer) {
		...
		if (scrollbutton->need_timer)
			scrollbutton->timer = g_timeout_add(SCROLL_TIMER_LENGTH, (GSourceFunc) gtk_real_vscrollbutton_timer, (gpointer)scrollbutton);
		else {
			GDK_THREADS_LEAVE();
			return FALSE;
		}
...
{
	gfloat value;
	if (!gtk_widget_get_realized(GTK_WIDGET(scrollbutton)))
		return;
	if (scrollbutton->button != 0)
		return;	/* not while something is pressed */

	value = gtk_adjustment_get_value(adjustment);
	gtk_widget_set_sensitive(scrollbutton->upbutton, (value > gtk_adjustment_get_lower(adjustment)));
	gtk_widget_set_sensitive(scrollbutton->downbutton, (value < (gtk_adjustment_get_upper(adjustment) - gtk_adjustment_get_page_size(adjustment))));
}
