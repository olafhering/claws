...
static void gtk_cmoption_menu_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void gtk_cmoption_menu_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
static void gtk_cmoption_menu_size_request(GtkWidget *widget, GtkRequisition *requisition);
static void gtk_cmoption_menu_size_allocate(GtkWidget *widget, GtkAllocation *allocation);
static void gtk_cmoption_menu_paint(GtkWidget *widget, GdkRectangle * area);
static gint gtk_cmoption_menu_expose(GtkWidget *widget, GdkEventExpose *event);
static gint gtk_cmoption_menu_button_press(GtkWidget *widget, GdkEventButton *event);
static gint gtk_cmoption_menu_key_press(GtkWidget *widget, GdkEventKey *event);
static void gtk_cmoption_menu_selection_done(GtkMenuShell *menu_shell, GtkCMOptionMenu * option_menu);
...
static void gtk_cmoption_menu_position(GtkMenu *menu, gint *x, gint *y, gint *scroll_offet, gpointer user_data);
static void gtk_cmoption_menu_show_all(GtkWidget *widget);
static void gtk_cmoption_menu_hide_all(GtkWidget *widget);
static gboolean gtk_cmoption_menu_mnemonic_activate(GtkWidget *widget, gboolean group_cycling);
static GType gtk_cmoption_menu_child_type(GtkContainer *container);
static gint gtk_cmoption_menu_scroll_event(GtkWidget *widget, GdkEventScroll *event);
...
static void gtk_cmoption_menu_class_init(GtkCMOptionMenuClass *class)
{
	...
	signals[CHANGED] = g_signal_new(("changed"), G_OBJECT_CLASS_TYPE(class), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GtkCMOptionMenuClass, changed), NULL, NULL, claws_marshal_VOID__VOID, G_TYPE_NONE, 0);
	...
	g_object_class_install_property(gobject_class, PROP_MENU, g_param_spec_object("menu", ("Menu"), ("The menu of options"), GTK_TYPE_MENU, G_PARAM_READWRITE));

	gtk_widget_class_install_style_property(widget_class, g_param_spec_boxed("indicator-size", ("Indicator Size"), ("Size of dropdown indicator"), GTK_TYPE_REQUISITION, G_PARAM_READABLE));
	gtk_widget_class_install_style_property(widget_class, g_param_spec_boxed("indicator-spacing", ("Indicator Spacing"), ("Spacing around indicator"), GTK_TYPE_BORDER, G_PARAM_READABLE));
}
...
static void gtk_cmoption_menu_init(GtkCMOptionMenu *option_menu)
{
...
static void gtk_cmoption_menu_detacher(GtkWidget *widget, GtkMenu *menu)
{
	...
	g_signal_handlers_disconnect_by_func(option_menu->menu, gtk_cmoption_menu_selection_done, option_menu);
	g_signal_handlers_disconnect_by_func(option_menu->menu, gtk_cmoption_menu_calc_size, option_menu);
...
void gtk_cmoption_menu_set_menu(GtkCMOptionMenu *option_menu, GtkWidget *menu)
{
	...
	if (option_menu->menu != menu) {
		...
		gtk_menu_attach_to_widget(GTK_MENU(menu), GTK_WIDGET(option_menu), gtk_cmoption_menu_detacher);

		gtk_cmoption_menu_calc_size(option_menu);

		g_signal_connect_after(option_menu->menu, "selection-done", G_CALLBACK(gtk_cmoption_menu_selection_done), option_menu);
		g_signal_connect_swapped(option_menu->menu, "size_request", G_CALLBACK(gtk_cmoption_menu_calc_size), option_menu);
...
void gtk_cmoption_menu_remove_menu(GtkCMOptionMenu *option_menu)
{
	...
	if (option_menu->menu) {
		if (option_menu->menu_item == gtk_menu_get_active(GTK_MENU(option_menu->menu)))
...
void gtk_cmoption_menu_set_history(GtkCMOptionMenu *option_menu, guint index)
{
	GtkWidget *menu_item;
...
gint gtk_cmoption_menu_get_history(GtkCMOptionMenu *option_menu)
{
	...
	if (option_menu->menu) {
		...
		if (active_widget) {
			list = gtk_container_get_children(GTK_CONTAINER(GTK_MENU_SHELL(option_menu->menu)));
...
static void gtk_cmoption_menu_get_props(GtkCMOptionMenu *option_menu, GtkCMOptionMenuProps *props)
{
	GtkRequisition *indicator_size;
	GtkBorder *indicator_spacing;

	gtk_widget_style_get(GTK_WIDGET(option_menu), "indicator-size", &indicator_size, "indicator-spacing", &indicator_spacing, "interior-focus", &props->interior_focus, "focus-line-width", &props->focus_width, "focus-padding", &props->focus_pad, NULL);
...
static void gtk_cmoption_menu_size_request(GtkWidget *widget, GtkRequisition *requisition)
{
	...
	requisition->width = ((border_width + style->xthickness + props.focus_pad) * 2 + MAX(child_requisition.width, option_menu->width) + props.indicator_size.width + props.indicator_spacing.left + props.indicator_spacing.right + CHILD_LEFT_SPACING + CHILD_RIGHT_SPACING + props.focus_width * 2);
	requisition->height = ((border_width + style->ythickness + props.focus_pad) * 2 + MAX(child_requisition.height, option_menu->height) + CHILD_TOP_SPACING + CHILD_BOTTOM_SPACING + props.focus_width * 2);

	tmp = (requisition->height - MAX(child_requisition.height, option_menu->height) + props.indicator_size.height + props.indicator_spacing.top + props.indicator_spacing.bottom);
...
static void gtk_cmoption_menu_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
	...
	if (gtk_widget_get_realized(widget))
		gdk_window_move_resize(gtk_button_get_event_window(button), allocation->x + border_width, allocation->y + border_width, allocation->width - border_width * 2, allocation->height - border_width * 2);
	...
	if (child && gtk_widget_get_visible(child)) {
		...
		child_allocation.width = MAX(1, allocation->width - (border_width + xthickness + props.focus_width + props.focus_pad) * 2 - props.indicator_size.width - props.indicator_spacing.left - props.indicator_spacing.right - CHILD_LEFT_SPACING - CHILD_RIGHT_SPACING);
		child_allocation.height = MAX(1, allocation->height - (border_width + ythickness + props.focus_width + props.focus_pad) * 2 - CHILD_TOP_SPACING - CHILD_BOTTOM_SPACING);
...
static void gtk_cmoption_menu_paint(GtkWidget *widget, GdkRectangle *area)
{
	...
	if (gtk_widget_is_drawable(widget)) {
		...
		gtk_paint_box(style, window, gtk_widget_get_state(widget), GTK_SHADOW_OUT, area, widget, "optionmenu", button_area.x, button_area.y, button_area.width, button_area.height);

		if (gtk_widget_get_direction(GTK_WIDGET(widget)) == GTK_TEXT_DIR_RTL)
			tab_x = button_area.x + props.indicator_spacing.right + style->xthickness;
		else
			tab_x = button_area.x + button_area.width - props.indicator_size.width - props.indicator_spacing.right - style->xthickness;

		gtk_paint_tab(style, window, gtk_widget_get_state(widget), GTK_SHADOW_OUT, area, widget, "optionmenutab", tab_x, button_area.y + (button_area.height - props.indicator_size.height) / 2, props.indicator_size.width, props.indicator_size.height);
		...
		if (gtk_widget_has_focus(widget)) {
			if (props.interior_focus) {
				...
				button_area.width -= 2 * (style->xthickness + props.focus_pad) + props.indicator_spacing.left + props.indicator_spacing.right + props.indicator_size.width;
				button_area.height -= 2 * (style->ythickness + props.focus_pad);
				if (gtk_widget_get_direction(GTK_WIDGET(widget)) == GTK_TEXT_DIR_RTL)
					button_area.x += props.indicator_spacing.left + props.indicator_spacing.right + props.indicator_size.width;
			} else {
				button_area.x -= props.focus_width + props.focus_pad;
				button_area.y -= props.focus_width + props.focus_pad;
				button_area.width += 2 * (props.focus_width + props.focus_pad);
			...
			gtk_paint_focus(style, window, gtk_widget_get_state(widget), area, widget, "button", button_area.x, button_area.y, button_area.width, button_area.height);
		}
	}
}
...
static gint gtk_cmoption_menu_expose(GtkWidget *widget, GdkEventExpose *event)
{
	...
	if (gtk_widget_is_drawable(widget)) {
		...
		if (GTK_WIDGET_NO_WINDOW(child) && gtk_widget_intersect(child, &event->area, &child_event.area))
...
static gint gtk_cmoption_menu_button_press(GtkWidget *widget, GdkEventButton *event)
{
	...
	if ((event->type == GDK_BUTTON_PRESS) && (event->button == 1)) {
		gtk_cmoption_menu_remove_contents(option_menu);
		gtk_menu_popup(GTK_MENU(option_menu->menu), NULL, NULL, gtk_cmoption_menu_position, option_menu, event->button, event->time);
		menu_item = gtk_menu_get_active(GTK_MENU(option_menu->menu));
		if (menu_item)
...
static gint gtk_cmoption_menu_key_press(GtkWidget *widget, GdkEventKey *event)
{
	...
	case GDK_KEY_space:
		gtk_cmoption_menu_remove_contents(option_menu);
		gtk_menu_popup(GTK_MENU(option_menu->menu), NULL, NULL, gtk_cmoption_menu_position, option_menu, 0, event->time);
		menu_item = gtk_menu_get_active(GTK_MENU(option_menu->menu));
		if (menu_item)
			gtk_menu_shell_select_item(GTK_MENU_SHELL(option_menu->menu), menu_item);
		return TRUE;
...
static void gtk_cmoption_menu_changed(GtkCMOptionMenu *option_menu)
{
	cm_return_if_fail(GTK_IS_CMOPTION_MENU(option_menu));
...
static void gtk_cmoption_menu_select_first_sensitive(GtkCMOptionMenu *option_menu)
{
	if (option_menu->menu) {
...
static void gtk_cmoption_menu_item_state_changed_cb(GtkWidget *widget, GtkStateType previous_state, GtkCMOptionMenu *option_menu)
{
...
static void gtk_cmoption_menu_item_destroy_cb(GtkWidget *widget, GtkCMOptionMenu *option_menu)
{
	GtkWidget *child = gtk_bin_get_child(GTK_BIN(option_menu));
...
static void gtk_cmoption_menu_update_contents(GtkCMOptionMenu *option_menu)
{
	...
	if (option_menu->menu) {
		...
		if (option_menu->menu_item) {
			...
			g_signal_connect(option_menu->menu_item, "state_changed", G_CALLBACK(gtk_cmoption_menu_item_state_changed_cb), option_menu);
			g_signal_connect(option_menu->menu_item, "destroy", G_CALLBACK(gtk_cmoption_menu_item_destroy_cb), option_menu);

			gtk_widget_size_request(child, &child_requisition);
			gtk_widget_get_allocation(GTK_WIDGET(option_menu), &allocation);
			gtk_widget_size_allocate(GTK_WIDGET(option_menu), &allocation);
...
static void gtk_cmoption_menu_remove_contents(GtkCMOptionMenu *option_menu)
{
	...
	if (option_menu->menu_item) {
		...
		g_signal_handlers_disconnect_by_func(option_menu->menu_item, gtk_cmoption_menu_item_state_changed_cb, option_menu);
		g_signal_handlers_disconnect_by_func(option_menu->menu_item, gtk_cmoption_menu_item_destroy_cb, option_menu);
...
static void gtk_cmoption_menu_position(GtkMenu *menu, gint *x, gint *y, gboolean *push_in, gpointer user_data)
{
	GtkCMOptionMenu *option_menu;
	GtkWidget *active;
	GtkWidget *child;
...
static gint gtk_cmoption_menu_scroll_event(GtkWidget *widget, GdkEventScroll *event)
{
	...
	if (index != -1) {
		...
		while (TRUE) {
			...
			if (gtk_widget_get_visible(GTK_WIDGET(item)) && gtk_widget_is_sensitive(GTK_WIDGET(item))) {
				gtk_cmoption_menu_set_history(option_menu, index);
