...
static gboolean description_window_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data);
static gboolean description_window_focus_in_event(GtkWidget *widget, GdkEventFocus *event, gpointer data);
static gboolean description_window_focus_out_event(GtkWidget *widget, GdkEventFocus *event, gpointer data);
static void description_window_destroy(GtkWidget *parent, gpointer data);
...
static void description_create(DescriptionWindow *dwindow)
{
	...
	gtk_window_set_title(GTK_WINDOW(dwindow->window), gettext(dwindow->title));
	...
	for (i = 0; dwindow->symbol_table[i] != NULL; i = i + dwindow->columns) {
		if (dwindow->symbol_table[i][0] != '\0') {
			...
			for (j = 0; j < dwindow->columns; j++) {
				...
				while ((colend < dwindow->columns) && (dwindow->symbol_table[i + colend] == NULL)) {
					colend++;
					j++;
				}
				...
				gtk_table_attach(GTK_TABLE(table), label, col, colend, line, line + 1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 2);
			...
			gtk_table_attach(GTK_TABLE(table), separator, 0, dwindow->columns, line, line + 1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 4);
		}
		line++;
	}
	...
	gtkut_stock_button_set_create(&hbbox, &close_btn, GTK_STOCK_CLOSE, NULL, NULL, NULL, NULL);
	...
	gtk_box_pack_start(GTK_BOX(vbox), GTK_WIDGET(scrolledwin), TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(vbox), GTK_WIDGET(hbbox), FALSE, FALSE, 3);

	gtk_widget_grab_default(close_btn);

	g_signal_connect(G_OBJECT(close_btn), "clicked", G_CALLBACK(description_window_destroy), dwindow);
	g_signal_connect(G_OBJECT(dwindow->window), "key_press_event", G_CALLBACK(description_window_key_pressed), dwindow);
	g_signal_connect(G_OBJECT(dwindow->window), "focus_in_event", G_CALLBACK(description_window_focus_in_event), NULL);
	g_signal_connect(G_OBJECT(dwindow->window), "focus_out_event", G_CALLBACK(description_window_focus_out_event), NULL);
	g_signal_connect(G_OBJECT(dwindow->window), "destroy", G_CALLBACK(description_window_destroy), dwindow);

	if (dwindow->parent)
		g_signal_connect(G_OBJECT(dwindow->parent), "hide", G_CALLBACK(description_window_destroy), dwindow);

	gtk_widget_show_all(vbox);
	gtk_widget_set_size_request(dwindow->window, (width < 400) ? 400 : width, 450);
}
...
static void description_window_destroy(GtkWidget *widget, gpointer data)
{
	...
	if (dwindow->parent) {
		...
		g_signal_handlers_disconnect_by_func(G_OBJECT(dwindow->parent), description_window_destroy, dwindow->parent);
	}
}
