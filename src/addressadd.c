...
{
	addressadd_dlg.fiSelected = gtkut_tree_view_get_selected_pointer(GTK_TREE_VIEW(addressadd_dlg.tree_folder), ADDRADD_COL_PTR, NULL, NULL, NULL);
}
...
static void addressadd_tree_row_activated(GtkTreeView *view, GtkTreePath *path, GtkTreeViewColumn *col, gpointer user_data)
{
	addressadd_ok(NULL, NULL);
}

static void addressadd_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation)
{
...
static void addressadd_create(void)
{
	...
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(addressadd_size_allocate_cb), NULL);
	...
	store = gtk_tree_store_new(N_ADDRADD_COLS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_POINTER);
	...
	gtk_tree_view_set_search_column(GTK_TREE_VIEW(tree_folder), ADDRADD_COL_NAME);
	...
	gtk_tree_view_column_set_attributes(col, rdr, "pixbuf", ADDRADD_COL_ICON, NULL);
	rdr = gtk_cell_renderer_text_new();
	gtk_tree_view_column_pack_start(col, rdr, TRUE);
	gtk_tree_view_column_set_attributes(col, rdr, "markup", ADDRADD_COL_NAME, NULL);
	...
	g_signal_connect(G_OBJECT(tree_folder), "row-activated", G_CALLBACK(addressadd_tree_row_activated), NULL);
	...
	gtk_widget_set_size_request(window, prefs_common.addressaddwin_width, prefs_common.addressaddwin_height);
...
#ifndef USE_ALT_ADDRBOOK
static void addressadd_load_folder(GtkTreeIter *parent_iter, ItemFolder *parentFolder, FolderInfo *fiParent)
{
	GtkWidget *view = addressadd_dlg.tree_folder;
	GtkTreeIter iter;
	GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(view));
	...
	while (list) {
		...
		gtk_tree_store_set(GTK_TREE_STORE(model), &iter, ADDRADD_COL_ICON, folderXpm, ADDRADD_COL_NAME, name, ADDRADD_COL_PTR, fi, -1);
		g_free(name);
...
static void addressadd_load_data(AddressIndex *addrIndex)
{
	...
	while (list) {
		...
		if (ainterface->type == ADDR_IF_BOOK || ainterface->type == ADDR_IF_LDAP) {
			...
			while (nodeDS) {
				...
				gtk_tree_store_set(GTK_TREE_STORE(model), &iter, ADDRADD_COL_ICON, bookXpm, ADDRADD_COL_NAME, name, ADDRADD_COL_PTR, fi, -1);
				g_free(name);
...
static void addressadd_load_data()
{
	...
	for (; list; list = g_slist_next(list)) {
		...
		gtk_tree_store_append(GTK_TREE_STORE(model), &iter, NULL);
		gtk_tree_store_set(GTK_TREE_STORE(model), &iter, ADDRADD_COL_ICON, bookXpm, ADDRADD_COL_NAME, (gchar *)list->data, ADDRADD_COL_PTR, fi, -1);
	}
	...
	if (!addressadd_cancelled) {
		if (addressadd_dlg.fiSelected) {
...
#ifndef USE_ALT_ADDRBOOK
			person = addrbook_add_contact(fi->book, fi->folder, returned_name, address, returned_remarks);
			...
			if (addressbook_dbus_add_contact(contact, &error) == 0) {
				...
				retVal = FALSE;
				if (error) {
					GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(addressadd_dlg.window),
			...
			if (person != NULL && fi->book->type == ADBOOKTYPE_LDAP) {
				...
				if (server->retVal != LDAPRC_SUCCESS) {
					alertpanel(_("Add address(es)"), _("Can't add the specified address"), GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST);
