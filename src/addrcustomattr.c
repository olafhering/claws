...
static gint custom_attr_cmp_func(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b, gpointer userdata)
{
	gchar *name1, *name2;
	gint res;
...
static GtkListStore *custom_attr_window_create_data_store(void)
{
	...
	gtk_tree_sortable_set_sort_func(sortable, 0, custom_attr_cmp_func, NULL, NULL);
...
static void custom_attr_window_create_list_view_columns(GtkWidget *list_view)
{
	...
	column = gtk_tree_view_column_new_with_attributes(_("Attribute name"), renderer, "text", CUSTOM_ATTR_NAME, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(list_view), column);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_set_search_column(GTK_TREE_VIEW(list_view), CUSTOM_ATTR_NAME);
	g_signal_connect(G_OBJECT(renderer), "edited", G_CALLBACK(custom_attr_selected_attr_edited), list_view);
}

static void custom_attr_window_list_view_clear_list(GtkWidget *list_view, gboolean warn)
{
	if (!warn || alertpanel(_("Delete all attribute names"), _("Do you really want to delete all attribute names?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_FIRST) == G_ALERTALTERNATE) {
...
static void custom_attr_popup_delete(void *obj, void *data)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(custom_attr_window.attr_list)), &model, &sel))
		return;

	if (alertpanel(_("Delete attribute name"), _("Do you really want to delete this attribute name?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_FIRST) == G_ALERTALTERNATE) {
...
static gint custom_attr_list_btn_pressed(GtkWidget *widget, GdkEventButton *event, GtkTreeView *list_view)
{
	if (event && event->button == 3) {
		...
		if (!custom_attr_popup_menu) {
			custom_attr_popup_action = cm_menu_create_action_group("CustomAttrPopup", custom_attr_popup_entries, G_N_ELEMENTS(custom_attr_popup_entries), (gpointer)list_view);
			MENUITEM_ADDUI("/Menus", "CustomAttrPopup", "CustomAttrPopup", GTK_UI_MANAGER_MENU)
			    ...
			    custom_attr_popup_menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(gtkut_ui_manager(), "/Menus/CustomAttrPopup")));
		}
		...
		gtk_menu_popup(GTK_MENU(custom_attr_popup_menu), NULL, NULL, NULL, NULL, event->button, event->time);
...
static GtkWidget *custom_attr_window_list_view_create(void)
{
	...
	gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(model), CUSTOM_ATTR_NAME, GTK_SORT_ASCENDING);
	...
	g_signal_connect(G_OBJECT(list_view), "popup-menu", G_CALLBACK(custom_attr_list_popup_menu), list_view);
	g_signal_connect(G_OBJECT(list_view), "button-press-event", G_CALLBACK(custom_attr_list_btn_pressed), list_view);
...
static void custom_attr_window_cancel_cb(GtkWidget *widget, gpointer data)
{
	dirty = FALSE;
	custom_attr_window_close();
}

static void custom_attr_window_ok_cb(GtkWidget *widget, gpointer data)
{
	custom_attr_window_close();
}

static void custom_attr_selected_attr_edited(GtkCellRendererText *widget, gchar *path, gchar *new_text, GtkWidget *list_view)
{
	GtkTreeIter iter;
	...
	gtk_list_store_set(GTK_LIST_STORE(model), &iter, CUSTOM_ATTR_NAME, new_text, -1);
	dirty = TRUE;
}
...
static gboolean find_attr_in_store(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, FindAttrInStore *data)
{
	gchar *attr;
...
static void custom_attr_window_add_attr(void)
{
	...
	if (new_attr && *new_attr) {
		GtkListStore *list_store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(custom_attr_window.attr_list)));
		FindAttrInStore fis;

		fis.attr = new_attr;
		fis.path = NULL;
		gtk_tree_model_foreach(gtk_tree_view_get_model(GTK_TREE_VIEW(custom_attr_window.attr_list)), (GtkTreeModelForeachFunc) find_attr_in_store, &fis);

		if (fis.path) {
			/* activate existing one */
			GtkTreeSelection *selection;
			GtkTreePath *path;
			GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(custom_attr_window.attr_list));
			...
			gtk_tree_view_set_cursor(GTK_TREE_VIEW(custom_attr_window.attr_list), path, NULL, FALSE);

			gtk_list_store_set(list_store, &fis.iter, CUSTOM_ATTR_NAME, new_attr, -1);
			...
			gtk_list_store_set(list_store, &iter, CUSTOM_ATTR_NAME, new_attr, -1);
		}
		dirty = TRUE;
	} else {
...
static void custom_attr_window_add_attr_cb(GtkWidget *widget, gpointer data)
{
...
static void custom_attr_window_del_attr_cb(GtkWidget *widget, gpointer data)
{
	custom_attr_popup_delete(NULL, NULL);
	gtk_widget_grab_focus(custom_attr_window.attr_list);
}

static gboolean custom_attr_window_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
...
static gboolean custom_attr_window_add_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
...
static void custom_attr_window_create(void)
{
	...
	gtk_window_set_title(GTK_WINDOW(window), C_("Dialog title", "Edit attribute names"));

	gtk_container_set_border_width(GTK_CONTAINER(window), 8);
	gtk_window_set_position(GTK_WINDOW(window), GTK_WIN_POS_CENTER);
	gtk_window_set_resizable(GTK_WINDOW(window), TRUE);
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(custom_attr_window_cancel_cb), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(custom_attr_window_key_pressed), NULL);
	...
	g_signal_connect(G_OBJECT(new_attr_entry), "key_press_event", G_CALLBACK(custom_attr_window_add_key_pressed), NULL);
	...
	gtkut_stock_button_set_create(&hbox2, &cancel_btn, GTK_STOCK_CANCEL, &ok_btn, GTK_STOCK_OK, NULL, NULL);
	...
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(custom_attr_window_cancel_cb), NULL);
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(custom_attr_window_ok_cb), NULL);
	g_signal_connect(G_OBJECT(add_btn), "clicked", G_CALLBACK(custom_attr_window_add_attr_cb), NULL);
	g_signal_connect(G_OBJECT(del_btn), "clicked", G_CALLBACK(custom_attr_window_del_attr_cb), NULL);

	attr_list = custom_attr_window_list_view_create();

	label = gtk_label_new(_("Adding or removing attribute names won't " "affect attributes already set for contacts."));
...
static void custom_attr_window_load_list(GList *list)
{
	...
	while (cur) {
		gtk_list_store_append(list_store, &iter);
		gtk_list_store_set(list_store, &iter, CUSTOM_ATTR_NAME, cur->data, -1);
		cur = cur->next;
	}
}
...
static void custom_attr_window_save_list(void)
{
	...
	gtk_tree_model_foreach(gtk_tree_view_get_model(GTK_TREE_VIEW(custom_attr_window.attr_list)), (GtkTreeModelForeachFunc) custom_attr_store_to_glist, NULL);
...
static GList *custom_attr_default_list(void)
{
	...
	while (default_addressbook_attributes_table[i]) {
		list = g_list_prepend(list, g_strdup(gettext(default_addressbook_attributes_table[i])));
		i++;
	}
...
GList *addressbook_update_custom_attr_from_prefs(void)
{
	...
	list = prefs_common_read_history_from_dir_with_defaults(ADDRBOOK_DIR, ADDRESSBOOK_CUSTOM_ATTRIBUTES, default_attr_list);
