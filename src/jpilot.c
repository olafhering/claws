...
static gboolean jpilot_check_files(JPilotFile *pilotFile)
{
	...
	if (0 == g_stat(pcFile, &filestat)) {
		if (filestat.st_mtime == pilotFile->pc3ModifyTime)
			retVal = FALSE;
	}
	g_free(pcFile);
	return retVal;
...
/* returns 1 if found */
/*         0 if eof */
static int find_next_offset(mem_rec_header *mem_rh, long fpos, unsigned int *next_offset, unsigned char *attrib, unsigned int *unique_id)
{
		...
		*buffer = '\0';
		...
		while (node) {
			...
			if (convert_charcode) {
				...
				convertBuff = conv_codeset_strdup(labelEntry, jpilot_get_charset(), CS_INTERNAL);
				if (convertBuff)
...
static void jpilot_load_address(JPilotFile *pilotFile, buf_rec *buf, ItemFolder *folderInd[])
{
	...
	*fullName = '\0';
	...
	if (name_order == FAMILY_LAST) {
		g_snprintf(fullName, FULLNAME_BUFSIZE, "%s %s", firstName ? firstName[0] : "", lastName ? lastName[0] : "");
	} else {
		g_snprintf(fullName, FULLNAME_BUFSIZE, "%s %s", lastName ? lastName[0] : "", firstName ? firstName[0] : "");
	}
	...
	if (convert_charcode) {
		gchar *nameConv = NULL;
		nameConv = conv_codeset_strdup(fullName, jpilot_get_charset(), CS_INTERNAL);
		if (nameConv)
	...
	if (person->listEMail) {
		...
		addrcache_folder_add_person(pilotFile->addressCache, folderInd[cat_id], person);
	} else {
	...
	if (pilotFile->readMetadata) {
		...
		while (node) {
			...
			for (i = 0; i < JPILOT_NUM_LABELS; i++) {
				...
				if (convert_charcode) {
					gchar *convertBuff = NULL;
					convertBuff = conv_codeset_strdup(labelName, jpilot_get_charset(), CS_INTERNAL);
					if (convertBuff) {
...
static void jpilot_build_category_list(JPilotFile *pilotFile)
{
	...
	for (i = 0; i < JPILOT_NUM_CATEG; i++) {
		...
		if (convert_charcode) {
			gchar *convertBuff = NULL;
			convertBuff = conv_codeset_strdup(cat->name[i], jpilot_get_charset(), CS_INTERNAL);
			if (convertBuff) {
...
static gint jpilot_read_file(JPilotFile *pilotFile)
{
	...
	while (node) {
		br = node->data;
		if ((br->rt != DELETED_PC_REC) && (br->rt != DELETED_PALM_REC) && (br->rt != MODIFIED_PALM_REC) && (br->rt != DELETED_DELETED_PALM_REC)) {
...
gint jpilot_read_data(JPilotFile *pilotFile)
{
	...
	if (g_ascii_strncasecmp(cur_locale, "hu", 2) == 0 || g_ascii_strncasecmp(cur_locale, "ja", 2) == 0 || g_ascii_strncasecmp(cur_locale, "ko", 2) == 0 || g_ascii_strncasecmp(cur_locale, "vi", 2) == 0 || g_ascii_strncasecmp(cur_locale, "zh", 2) == 0) {
