...
static void set_sensitivity(GtkUIManager *ui_manager, FolderItem *item)
{
	gboolean folder_is_normal = item != NULL && item->stype == F_NORMAL && !folder_has_parent_of_type(item, F_OUTBOX) && !folder_has_parent_of_type(item, F_DRAFT) && !folder_has_parent_of_type(item, F_QUEUE) && !folder_has_parent_of_type(item, F_TRASH);
...
#define SET_SENS(name, sens) \
	...
	if (item && item->folder && item->folder->account)
		cm_toggle_menu_set_active_full(ui_manager, "Popup/FolderViewPopup/Subscriptions/ShowOnlySubs", item->folder->account->imap_subsonly);
...
static void new_folder_cb(GtkAction *action, gpointer data)
{
	...
	new_folder = input_dialog_with_checkbtn(_("New folder"), _("Input the name of new folder:\n" "(if you want to create a folder to store subfolders\n" "only and no mail, append '/' to the folder name)"), _("NewFolder"), _("Inherit properties and processing rules from parent folder"), &(prefs_common.inherit_folder_props));
	...
	if (p && *(p + 1) != '\0') {
		alertpanel_error(_("'%c' can't be included in folder name."), separator);
		return;
	}
	p = strchr(new_folder, '/');
	if (p && *(p + 1) != '\0') {
...
static void move_folder_cb(GtkAction *action, gpointer data)
{
	...
	msg = g_strdup_printf(_("Select folder to move folder '%s' to"), from_folder->name);
...
static void copy_folder_cb(GtkAction *action, gpointer data)
{
	...
	msg = g_strdup_printf(_("Select folder to copy folder '%s' to"), from_folder->name);
	to_folder = foldersel_folder_sel(NULL, FOLDER_SEL_MOVE, NULL, TRUE, msg);
	g_free(msg);
	if (!to_folder)
		return;
...
static void delete_folder_cb(GtkAction *action, gpointer data)
{
	...
	message = g_markup_printf_escaped(_("All folders and messages under '%s' will be permanently deleted. " "Recovery will not be possible.\n\n" "Do you really want to delete?"), name);
	avalue = alertpanel_full(_("Delete folder"), message, GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
	g_free(message);
...
static void subscribe_cb_full(FolderView *folderview, guint action)
{
	...
	if (action && item->folder->account->imap_subsonly) {
		...
		message = g_markup_printf_escaped(_("Do you want to search for unsubscribed subfolders of '%s'?"), name);

		rec_chk = gtk_check_button_new_with_label(_("Search recursively"));

		g_signal_connect(G_OBJECT(rec_chk), "toggled", G_CALLBACK(chk_update_val), &recurse);

		avalue = alertpanel_full(_("Subscriptions"), message, GTK_STOCK_CANCEL, _("_Search"), NULL, ALERTFOCUS_SECOND, FALSE, rec_chk, ALERT_QUESTION);
		g_free(message);
		if (avalue != G_ALERTALTERNATE)
			return;
		...
		if (child_list) {
			...
			for (cur = child_list; cur; cur = cur->next) {
				transc_list = g_list_append(transc_list, imap_modified_utf7_to_utf8(cur->data, FALSE));
			}

			transc_list = g_list_sort(transc_list, g_str_equal);

			child_folder = input_dialog_combo(_("Subscribe"), msg, transc_list->next ? _("All of them") : transc_list->data, transc_list);
			g_free(msg);
			...
			alertpanel_notice(_("This folder is already subscribed and " "has no unsubscribed subfolders.\n\nIf there are new folders, " "created and subscribed to from another client, use \"Check " "for new folders\" at the mailbox's root folder."));
		}
		g_list_free(child_list);
		return;
	}
	message = g_markup_printf_escaped(_("Do you want to %s the '%s' folder?"), action ? _("subscribe") : _("unsubscribe"), name);

	rec_chk = gtk_check_button_new_with_label(_("Apply to subfolders"));

	g_signal_connect(G_OBJECT(rec_chk), "toggled", G_CALLBACK(chk_update_val), &recurse);

	avalue = alertpanel_full(_("Subscriptions"), message, GTK_STOCK_CANCEL, action ? _("_Subscribe") : _("_Unsubscribe"), NULL, ALERTFOCUS_SECOND, FALSE, rec_chk, ALERT_QUESTION);
	g_free(message);
	if (avalue != G_ALERTALTERNATE)
		return;
	...
	if (!action) {
		if (opened == selected || folder_is_child_of(selected, opened)) {
			summary_clear_all(folderview->summaryview);
			folderview_close_opened(folderview, TRUE);
		}
	}
	...
	if (recurse) {
		g_node_traverse(item->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, imap_gtk_subscribe_func, GINT_TO_POINTER(action));
	} else {
		imap_subscribe(item->folder, item, NULL, action);
	}
...
static void subscribed_cb(GtkAction *action, gpointer data)
{
	...
	if (opened == item || folder_is_child_of(item, opened)) {
		summary_clear_all(folderview->summaryview);
		folderview_close_opened(folderview, TRUE);
	}
