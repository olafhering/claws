...
static void export_ldif_status_show(gchar *msg)
{
	if (expldif_dlg.statusbar != NULL) {
		gtk_statusbar_pop(GTK_STATUSBAR(expldif_dlg.statusbar), expldif_dlg.status_cid);
		if (msg) {
			gtk_statusbar_push(GTK_STATUSBAR(expldif_dlg.statusbar), expldif_dlg.status_cid, msg);
		}
	}
}
...
static gboolean exp_ldif_move_file(void)
{
	...
	/* Test for directory */
	if (g_file_test(_exportCtl_->dirOutput, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR)) {
		return TRUE;
	}

	/* Prompt to create */
	msg = g_strdup_printf(_("LDIF Output Directory '%s'\n" "does not exist. OK to create new directory?"), _exportCtl_->dirOutput);
	aval = alertpanel(_("Create Directory"), msg, GTK_STOCK_NO, GTK_STOCK_YES, NULL, ALERTFOCUS_FIRST);
	g_free(msg);
	...
	if (!exportldif_create_dir(_exportCtl_)) {
		...
		msg = g_strdup_printf(_("Could not create output directory for LDIF file:\n%s"), reason);
		aval = alertpanel_full(_("Failed to Create Directory"), msg, GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_ERROR);
		g_free(msg);
		return FALSE;
	}
...
static gboolean exp_ldif_move_dn(void)
{
	...
	exportldif_set_use_dn(_exportCtl_, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(expldif_dlg.checkUseDN)));
	exportldif_set_exclude_email(_exportCtl_, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(expldif_dlg.checkEMail)));

	if (*suffix == '\0' || strlen(suffix) < 1) {
		AlertValue aval;

		aval = alertpanel(_("Suffix was not supplied"), _("A suffix is required if data is to be used " "for an LDAP server. Are you sure you wish " "to proceed without a suffix?"), GTK_STOCK_NO, GTK_STOCK_YES, NULL, ALERTFOCUS_FIRST);
		if (aval != G_ALERTALTERNATE) {
...
static void export_ldif_prev(GtkWidget *widget)
{
	...
	if (pageNum == PAGE_DN) {
		...
		/* Goto format page */
		gtk_notebook_set_current_page(GTK_NOTEBOOK(expldif_dlg.notebook), PAGE_DN);
...
static void export_ldif_next(GtkWidget *widget)
{
	...
	if (pageNum == PAGE_FILE_INFO) {
		...
		/* Goto finish page */
		if (exp_ldif_move_dn()) {
			gtk_notebook_set_current_page(GTK_NOTEBOOK(expldif_dlg.notebook), PAGE_FINISH);
			gtk_button_set_label(GTK_BUTTON(expldif_dlg.btnCancel), GTK_STOCK_CLOSE);
...
static void export_ldif_page_file(gint pageNum, gchar *pageLbl)
{
	...
	gtk_notebook_set_tab_label(GTK_NOTEBOOK(expldif_dlg.notebook), gtk_notebook_get_nth_page(GTK_NOTEBOOK(expldif_dlg.notebook), pageNum), label);
	...
	gtk_table_attach(GTK_TABLE(table), labelBook, 1, 2, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table), entryLdif, 1, 2, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	btnFile = gtkut_get_browse_file_btn(_("B_rowse"));
	gtk_table_attach(GTK_TABLE(table), btnFile, 2, 3, top, (top + 1), GTK_FILL, 0, 3, 0);

	gtk_widget_show_all(vbox);

	/* Button handler */
	g_signal_connect(G_OBJECT(btnFile), "clicked", G_CALLBACK(exp_ldif_file_select), NULL);
...
static void export_ldif_relative_dn_changed(GtkWidget *widget, gpointer data)
{
	...
	case EXPORT_LDIF_ID_UID:
		gtk_label_set_text(label, _("The address book Unique ID is used to create a DN that is " "formatted similar to:\n" "  uid=102376,ou=people,dc=claws-mail,dc=org"));
		break;
	case EXPORT_LDIF_ID_DNAME:
		gtk_label_set_text(label, _("The address book Display Name is used to create a DN that " "is formatted similar to:\n" "  cn=John Doe,ou=people,dc=claws-mail,dc=org"));
		break;
	case EXPORT_LDIF_ID_EMAIL:
		gtk_label_set_text(label, _("The first Email Address belonging to a person is used to " "create a DN that is formatted similar to:\n" "  mail=john.doe@domain.com,ou=people,dc=claws-mail,dc=org"));
		break;
	}
...
static void export_ldif_page_dn(gint pageNum, gchar *pageLbl)
{
	...
	gtk_table_attach(GTK_TABLE(table), entrySuffix, 1, 2, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(entrySuffix, _("The suffix is used to create a \"Distinguished Name\" " "(or DN) for an LDAP entry. Examples include:\n" "  dc=claws-mail,dc=org\n" "  ou=people,dc=domainname,dc=com\n" "  o=Organization Name,c=Country\n"));
	...
	gtk_table_attach(GTK_TABLE(table), optmenuRDN, 1, 2, top, (top + 1), GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(optmenuRDN, _("The LDIF file contains several data records that " "are usually loaded into an LDAP server. Each data " "record in the LDIF file is uniquely identified by " "a \"Distinguished Name\" (or DN). The suffix is " "appended to the \"Relative Distinguished Name\" " "(or RDN) to create the DN. Please select one of " "the available RDN options that will be used to " "create the DN."));
	...
	gtk_table_attach(GTK_TABLE(table), labelRDN, 0, 2, top, (top + 1), GTK_FILL, 0, 0, 0);

	/* Fourth row */
	top++;
	checkUseDN = gtk_check_button_new_with_label(_("Use DN attribute if present in data"));
	gtk_table_attach(GTK_TABLE(table), checkUseDN, 1, 2, top, (top + 1), GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(checkUseDN, _("The addressbook may contain entries that were " "previously imported from an LDIF file. The " "\"Distinguished Name\" (DN) user attribute, if " "present in the address book data, may be used in " "the exported LDIF file. The RDN selected above " "will be used if the DN user attribute is not found."));

	/* Fifth row */
	top++;
	checkEMail = gtk_check_button_new_with_label(_("Exclude record if no Email Address"));
	gtk_table_attach(GTK_TABLE(table), checkEMail, 1, 2, top, (top + 1), GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(checkEMail, _("An addressbook may contain entries without " "Email Addresses. Check this option to ignore " "these records."));

	gtk_widget_show_all(vbox);

	g_signal_connect(G_OBJECT(optmenuRDN), "changed", G_CALLBACK(export_ldif_relative_dn_changed), labelRDN);
...
static void export_ldif_dialog_create(void)
{
	...
	gtk_window_set_title(GTK_WINDOW(window), _("Export Address Book to LDIF File"));
	gtk_window_set_position(GTK_WINDOW(window), GTK_WIN_POS_CENTER);
	gtk_window_set_type_hint(GTK_WINDOW(window), GDK_WINDOW_TYPE_HINT_DIALOG);
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(export_ldif_delete_event), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(export_ldif_key_pressed), NULL);
	...
	/* Button panel */
	gtkut_stock_button_set_create(&hbbox, &btnPrev, GTK_STOCK_GO_BACK, &btnNext, GTK_STOCK_GO_FORWARD, &btnCancel, GTK_STOCK_CANCEL);
	gtk_box_pack_end(GTK_BOX(vbox), hbbox, FALSE, FALSE, 0);
	gtk_container_set_border_width(GTK_CONTAINER(hbbox), 2);
	gtk_widget_grab_default(btnNext);

	/* Button handlers */
	g_signal_connect(G_OBJECT(btnPrev), "clicked", G_CALLBACK(export_ldif_prev), NULL);
	g_signal_connect(G_OBJECT(btnNext), "clicked", G_CALLBACK(export_ldif_next), NULL);
	g_signal_connect(G_OBJECT(btnCancel), "clicked", G_CALLBACK(export_ldif_cancel), NULL);
	...
	expldif_dlg.status_cid = gtk_statusbar_get_context_id(GTK_STATUSBAR(statusbar), "Export LDIF Dialog");
}
...
static void export_ldif_fill_fields(ExportLdifCtl *ctl)
{
	...
	if (ctl->path) {
		gtk_entry_set_text(GTK_ENTRY(expldif_dlg.entryLdif), ctl->path);
	}
	gtk_entry_set_text(GTK_ENTRY(expldif_dlg.entrySuffix), "");
	if (ctl->suffix) {
		gtk_entry_set_text(GTK_ENTRY(expldif_dlg.entrySuffix), ctl->suffix);
	}

	gtk_combo_box_set_active(GTK_COMBO_BOX(expldif_dlg.optmenuRDN), ctl->rdnIndex);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(expldif_dlg.checkUseDN), ctl->useDN);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(expldif_dlg.checkEMail), ctl->excludeEMail);
}
...
void addressbook_exp_ldif(AddressCache *cache)
{
	...
	gtk_button_set_label(GTK_BUTTON(expldif_dlg.btnCancel), GTK_STOCK_CANCEL);
