...
static gint pop3_getrange_uidl_recv(Pop3Session *session, const gchar *data, guint len);
static gint pop3_getsize_list_send(Pop3Session *session);
static gint pop3_getsize_list_recv(Pop3Session *session, const gchar *data, guint len);
...
static gint pop3_write_msg_to_file(const gchar *file, const gchar *data, guint len, const gchar *prefix);
...
static gint pop3_session_recv_data_finished(Session *session, guchar *data, guint len);
...
static gint pop3_getauth_apop_send(Pop3Session *session)
{
	...
	if ((start = strchr(session->greeting, '<')) == NULL) {
		log_error(LOG_PROTOCOL, _("Required APOP timestamp not found " "in greeting\n"));
...
static gint pop3_getauth_oauth2_send(Pop3Session *session)
{
	...
	return (oauth2_provider == OAUTH2AUTH_OUTLOOK || oauth2_provider == OAUTH2AUTH_EXCHANGE || oauth2_provider == OAUTH2AUTH_MICROSOFT_GCCHIGH ? pop3_getauth_oauth2_send_microsoft_1(session)
...
static gint pop3_getrange_uidl_recv(Pop3Session *session, const gchar *data, guint len)
{
	...
	while (p < lastp) {
		...
		if (sscanf(buf, "%d %" Xstr(IDLEN) "s", &num, id) != 2 || num <= 0 || num > session->count) {
		...
		recv_time = (time_t)(GPOINTER_TO_INT(g_hash_table_lookup(session->uidl_table, id)));
		...
		partial_recv = (gint)(GPOINTER_TO_INT(g_hash_table_lookup(session->partial_recv_table, id)));

		if (recv_time != RECV_TIME_NONE || partial_recv != POP3_TOTALLY_RECEIVED) {
			session->msg[num].received = (partial_recv != POP3_MUST_COMPLETE_RECV);
			session->msg[num].partial_recv = partial_recv;
			if (partial_recv == POP3_MUST_COMPLETE_RECV)
				session->new_msg_exist = TRUE;
		}
		if (!session->new_msg_exist && (recv_time == RECV_TIME_NONE || session->ac_prefs->rmmail)) {
			session->cur_msg = num;
			session->new_msg_exist = TRUE;
		}
	}
...
static gint pop3_retr_send(Pop3Session *session)
{
	...
	debug_print("retrieving %d [%s]\n", session->cur_msg, session->msg[session->cur_msg].uidl ? session->msg[session->cur_msg].uidl : " ");
...
static gint pop3_retr_recv(Pop3Session *session, const gchar *data, guint len)
{
	...
	file = get_tmp_file();
	if (pop3_write_msg_to_file(file, mail_receive_data.data, mail_receive_data.data_len, NULL) < 0) {
		g_free(file);
	...
	if (session->msg[session->cur_msg].partial_recv == POP3_MUST_COMPLETE_RECV) {
		gchar *old_file = partial_get_filename(session->ac_prefs->recv_server,
	...
	session->msg[session->cur_msg].recv_time = drop_ok == 1 ? RECV_TIME_KEEP : session->current_time;
...
static gint pop3_top_recv(Pop3Session *session, const gchar *data, guint len)
{
	...
	partial_notice = g_strdup_printf("SC-Marked-For-Download: 0\n" "SC-Partially-Retrieved: %s\n" "SC-Account-Server: %s\n" "SC-Account-Login: %s\n" "SC-Message-Size: %d", session->msg[session->cur_msg].uidl, session->ac_prefs->recv_server, session->ac_prefs->userid, session->msg[session->cur_msg].size);
	file = get_tmp_file();
	if (pop3_write_msg_to_file(file, mail_receive_data.data, mail_receive_data.data_len, partial_notice) < 0) {
		g_free(file);
		g_free(mail_receive_data.data);
		session->error_val = PS_IOERR;
...
#ifdef USE_GNUTLS
	if (account->set_gnutls_priority && account->gnutls_priority && strlen(account->gnutls_priority) != 0)
...
static void pop3_get_uidl_table(PrefsAccount *ac_prefs, Pop3Session *session)
{
	...
	path = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, "uidl", G_DIR_SEPARATOR_S, ac_prefs->recv_server, "-", sanitized_uid, NULL);
	...
	if ((fp = claws_fopen(path, "rb")) == NULL) {
		...
		g_free(path);
		path = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, "uidl-", ac_prefs->recv_server, "-", ac_prefs->userid, NULL);
		if ((fp = claws_fopen(path, "rb")) == NULL) {
			if (ENOENT != errno)
				FILE_OP_ERROR(path, "claws_fopen");
			g_free(path);
	...
	while (claws_fgets(buf, sizeof(buf), fp) != NULL) {
		...
		g_hash_table_insert(table, g_strdup(uidl), GINT_TO_POINTER(recv_time));
		if (strlen(tmp) == 1)
			partial_recv = atoi(tmp); /* totally received ? */
		else
			partial_recv = POP3_MUST_COMPLETE_RECV;

		g_hash_table_insert(partial_recv_table, g_strdup(uidl), GINT_TO_POINTER(partial_recv));
	}
...
gint pop3_write_uidl_list(Pop3Session *session)
{
	...
	path = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, "uidl", G_DIR_SEPARATOR_S, session->ac_prefs->recv_server, "-", sanitized_uid, NULL);
	...
	for (n = 1; n <= session->count; n++) {
		msg = &session->msg[n];
		if (msg->uidl && msg->received && (!msg->deleted || session->state != POP3_DONE))
			TRY(fprintf(fp, "%s\t%ld\t%d\n", msg->uidl, (long int)
				    msg->recv_time, msg->partial_recv)
			    > 0);
	}
...
static gint pop3_write_msg_to_file(const gchar *file, const gchar *data, guint len, const gchar *prefix)
{
	...
	while ((cur = (gchar *)my_memmem(prev, len - (prev - data), "\r\n", 2))
	       != NULL) {
		if ((cur > prev && claws_fwrite(prev, 1, cur - prev, fp) < 1) || claws_fputc('\n', fp) == EOF) {
	...
	if (prev - data < len && claws_fwrite(prev, 1, len - (prev - data), fp) < 1) {
		FILE_OP_ERROR(file, "claws_fwrite");
		g_warning("can't write to file: %s", file);
		claws_fclose(fp);
		claws_unlink(file);
...
static Pop3State pop3_lookup_next(Pop3Session *session)
{
	...
	for (;;) {
		...
		size = msg->size;
		size_limit_over = (ac->enable_size_limit && ac->size_limit > 0 && size > ac->size_limit * 1024);

		if (ac->rmmail && msg->recv_time != RECV_TIME_NONE && msg->recv_time != RECV_TIME_KEEP && msg->partial_recv == POP3_TOTALLY_RECEIVED && session->current_time - msg->recv_time >= ((ac->msg_leave_time * 24 * 60 * 60) + (ac->msg_leave_hour * 60 * 60))) {
			log_message(LOG_PROTOCOL, _("POP: Deleting expired message %d [%s]\n"), session->cur_msg, msg->uidl ? msg->uidl : " ");
			session->cur_total_bytes += size;
...
static Pop3ErrorValue pop3_ok(Pop3Session *session, const gchar *msg)
{
	...
	if (!strncmp(msg, "+OK", 3) || !strncmp(msg, "+", 1))
		...
		if (strstr(msg + 4, "lock") || strstr(msg + 4, "Lock") || strstr(msg + 4, "LOCK") || strstr(msg + 4, "wait")) {
...
static gint pop3_session_recv_msg(Session *session, const gchar *msg)
{
	...
	body = msg;
	if (pop3_session->state != POP3_GETRANGE_UIDL_RECV && pop3_session->state != POP3_GETSIZE_LIST_RECV) {
...
static gint pop3_session_recv_data_finished(Session *session, guchar *data, guint len)
{
	...
	case POP3_RETR_RECV:
		...
		if (pop3_session->ac_prefs->rmmail && pop3_session->ac_prefs->msg_leave_time == 0 && pop3_session->ac_prefs->msg_leave_hour == 0 && pop3_session->msg[pop3_session->cur_msg].recv_time != RECV_TIME_KEEP)
