...
static gint prefs_folder_column_delete_event(GtkWidget *widget, GdkEventAny *event, gpointer data);
static gboolean prefs_folder_column_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data);

static GtkListStore *prefs_folder_column_create_store(void);

static void prefs_folder_column_insert_column(GtkListStore *store, gint row, const gchar *name, FolderColumnType type);
...
static void prefs_filtering_create_list_view_columns(GtkWidget *list_view, const gchar *name);

static void drag_data_get(GtkTreeView *tree_view, GdkDragContext *context, GtkSelectionData *data, guint info, guint time, GtkTreeModel *model);

static void drag_data_received(GtkTreeView *tree_view, GdkDragContext *context, gint x, gint y, GtkSelectionData *data, guint info, guint time, GtkTreeModel *model);
...
static void prefs_folder_column_create(void)
{
	...
	gtk_window_set_title(GTK_WINDOW(window), _("Folder list columns configuration"));
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(prefs_folder_column_delete_event), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(prefs_folder_column_key_pressed), NULL);
	...
	label = gtk_label_new(_("Select columns to be displayed in the folder list. You can modify\n" "the order by using the Up / Down buttons or by dragging the items."));
	...
	stock_list_view = prefs_folder_column_list_view_create(_("Hidden columns"));
	g_signal_connect(G_OBJECT(stock_list_view), "cursor-changed", G_CALLBACK(prefs_folder_column_stock_sel_changed), NULL);
	...
	g_signal_connect(G_OBJECT(add_btn), "clicked", G_CALLBACK(prefs_folder_column_add), NULL);
	...
	g_signal_connect(G_OBJECT(remove_btn), "clicked", G_CALLBACK(prefs_folder_column_remove), NULL);
	...
	shown_list_view = prefs_folder_column_list_view_create(_("Displayed columns"));
	g_signal_connect(G_OBJECT(shown_list_view), "cursor-changed", G_CALLBACK(prefs_folder_column_shown_sel_changed), NULL);
	...
	g_signal_connect(G_OBJECT(up_btn), "clicked", G_CALLBACK(prefs_folder_column_up), NULL);
	g_signal_connect(G_OBJECT(down_btn), "clicked", G_CALLBACK(prefs_folder_column_down), NULL);
	...
	g_signal_connect(G_OBJECT(default_btn), "clicked", G_CALLBACK(prefs_folder_column_set_to_default), NULL);
	...
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(prefs_folder_column_ok), NULL);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(prefs_folder_column_cancel), NULL);
...
FolderColumnState *prefs_folder_column_get_config(void)
{
	...
	for (type = 0; type < N_FOLDER_COLS; type++) {
		...
		if (pos < 0 || pos >= N_FOLDER_COLS || state[pos].type != -1) {
...
static void prefs_folder_column_set_dialog(FolderColumnState *state)
{
	...
	stock_store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(folder_col.stock_list_view)));
	shown_store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(folder_col.shown_list_view)));
	...
	for (pos = 0; pos < N_FOLDER_COLS; pos++) {
		...
		if (state[pos].visible)
			prefs_folder_column_insert_column(shown_store, -1, name, type);
		else
			prefs_folder_column_insert_column(stock_store, -1, name, type);
	}
}
...
static void prefs_folder_column_set_view(void)
{
	...
	stock_n_rows = gtk_tree_model_iter_n_children(gtk_tree_view_get_model(GTK_TREE_VIEW(folder_col.stock_list_view)), NULL);
	shown_n_rows = gtk_tree_model_iter_n_children(gtk_tree_view_get_model(GTK_TREE_VIEW(folder_col.shown_list_view)), NULL);

	cm_return_if_fail(stock_n_rows + shown_n_rows == N_FOLDER_COLS);

	for (row = 0; row < stock_n_rows; row++) {
		type = prefs_folder_column_get_column(folder_col.stock_list_view, row);
		state[row].type = type;
	...
	for (row = 0; row < shown_n_rows; row++) {
		type = prefs_folder_column_get_column(folder_col.shown_list_view, row);
...
static void prefs_folder_column_add(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.stock_list_view)), NULL, &stock_sel))
		return;

	shown_sel_valid = gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.shown_list_view)), NULL, &shown_sel);

	gtk_tree_model_get(GTK_TREE_MODEL(stock_store), &stock_sel, SUMCOL_TYPE, &type, -1);

	gtk_list_store_remove(stock_store, &stock_sel);

	gtk_list_store_insert_after(shown_store, &shown_add, shown_sel_valid ? &shown_sel : NULL);

	name = gettext(col_name[type]);

	gtk_list_store_set(shown_store, &shown_add, SUMCOL_NAME, name, SUMCOL_TYPE, type, -1);

	gtk_tree_selection_select_iter(gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.shown_list_view)), &shown_add);
...
static void prefs_folder_column_remove(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.shown_list_view)), NULL, &shown_sel))
		return;

	stock_sel_valid = gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.stock_list_view)), NULL, &stock_sel);

	gtk_tree_model_get(GTK_TREE_MODEL(shown_store), &shown_sel, SUMCOL_TYPE, &type, -1);

	gtk_list_store_remove(shown_store, &shown_sel);

	gtk_list_store_insert_after(stock_store, &stock_add, stock_sel_valid ? &stock_sel : NULL);

	name = gettext(col_name[type]);

	gtk_list_store_set(stock_store, &stock_add, SUMCOL_NAME, name, SUMCOL_TYPE, type, -1);

	gtk_tree_selection_select_iter(gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.stock_list_view)), &stock_add);
	prefs_folder_column_shown_set_active(FALSE);
...
static void prefs_folder_column_up(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.shown_list_view)), NULL, &isel))
		return;

	shown_store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(folder_col.shown_list_view)));

	sel = gtk_tree_model_get_path(GTK_TREE_MODEL(shown_store), &isel);
	if (!sel)
		return;
	...
	gtk_tree_model_get_iter(GTK_TREE_MODEL(shown_store), &iprev, prev);
	gtk_tree_path_free(sel);
	gtk_tree_path_free(prev);
...
static void prefs_folder_column_down(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.shown_list_view)), NULL, &sel))
		return;
...
static GtkListStore *prefs_folder_column_create_store(void)
{
	return gtk_list_store_new(N_SUMCOL_COLUMNS, G_TYPE_STRING, G_TYPE_INT, -1);
}
...
static void prefs_folder_column_insert_column(GtkListStore *store, gint row, const gchar *name, FolderColumnType type)
{
	...
	if (row < 0) {
		...
		gtk_list_store_set(store, &iter, SUMCOL_NAME, name, SUMCOL_TYPE, type, -1);
		return;
	} else {
		/* change existing */
		gtk_list_store_set(store, &iter, SUMCOL_NAME, name, SUMCOL_TYPE, type, -1);
...
static FolderColumnType prefs_folder_column_get_column(GtkWidget *list, gint row)
{
	...
	gtk_tree_model_get(model, &iter, SUMCOL_TYPE, &result, -1);
...
static GtkWidget *prefs_folder_column_list_view_create(const gchar *name)
{
	...
	gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(list_view), prefs_common.use_stripes_everywhere);
	...
	gtk_tree_view_enable_model_drag_source(GTK_TREE_VIEW(list_view), GDK_BUTTON1_MASK, row_targets, G_N_ELEMENTS(row_targets), GDK_ACTION_MOVE);

	gtk_tree_view_enable_model_drag_dest(GTK_TREE_VIEW(list_view), row_targets, G_N_ELEMENTS(row_targets), GDK_ACTION_MOVE);

	g_signal_connect(G_OBJECT(list_view), "drag_data_get", G_CALLBACK(drag_data_get), model);

	g_signal_connect(G_OBJECT(list_view), "drag_data_received", G_CALLBACK(drag_data_received), model);
...
static void prefs_filtering_create_list_view_columns(GtkWidget *list_view, const gchar *name)
{
	...
	column = gtk_tree_view_column_new_with_attributes(name, renderer, "text", SUMCOL_NAME, NULL);
...
static void drag_data_get(GtkTreeView *tree_view, GdkDragContext *context, GtkSelectionData *data, guint info, guint time, GtkTreeModel *model)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(tree_view), &source_model, &iter))
		return;

	gtk_tree_model_get(source_model, &iter, SUMCOL_TYPE, &type, -1);

	/* send the type */
	gtk_selection_data_set(data, gtk_selection_data_get_target(data), 8, (gchar *)&type, sizeof type);
}
...
static void drag_data_received(GtkTreeView *tree_view, GdkDragContext *context, gint x, gint y, GtkSelectionData *data, guint info, guint time, GtkTreeModel *model)
{
	...
	if (source == GTK_WIDGET(tree_view)) {
		...
		if (gtk_tree_selection_get_selected(gtk_tree_view_get_selection(tree_view), NULL, &isel)) {
			...
			if (dst)
				...
				gtk_list_store_move_before(GTK_LIST_STORE(model), &isel, NULL);
			...
			if ((dst && sel) && gtk_tree_path_compare(sel, dst) != 0) {
				if (pos == GTK_TREE_VIEW_DROP_BEFORE || pos == GTK_TREE_VIEW_DROP_INTO_OR_BEFORE)
					...
					gtk_list_store_move_after(GTK_LIST_STORE(model), &isel, &idst);
			}
	...
	} else if (source == folder_col.stock_list_view || source == folder_col.shown_list_view) {
		...
		if (gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(source)), &sel_model, &isel)) {
			...
			if (dst) {
				...
				if (pos == GTK_TREE_VIEW_DROP_BEFORE || pos == GTK_TREE_VIEW_DROP_INTO_OR_BEFORE)
					gtk_list_store_insert_before(GTK_LIST_STORE(model), &isel, &idst);
				else
					gtk_list_store_insert_after(GTK_LIST_STORE(model), &isel, &idst);
			} else
				gtk_list_store_append(GTK_LIST_STORE(model), &isel);

			gtk_list_store_set(GTK_LIST_STORE(model), &isel, SUMCOL_NAME, name, SUMCOL_TYPE, type, -1);
			gtk_tree_path_free(dst);
		}
		gtk_drag_finish(context, TRUE, FALSE, time);
	}
...
static void prefs_folder_column_shown_set_btn_sensitivity(void)
{
	GtkTreeModel *model = GTK_TREE_MODEL(gtk_tree_view_get_model(GTK_TREE_VIEW(folder_col.shown_list_view)));
	GtkTreeSelection *selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.shown_list_view));
	GtkTreeIter iter;
	GtkTreePath *path;
	...
	gtk_widget_set_sensitive(folder_col.down_btn, gtk_tree_model_iter_next(model, &iter));
...
static void prefs_folder_column_shown_set_active(const gboolean active)
{
	...
	if (active == FALSE) {
		selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.shown_list_view));
...
static void prefs_folder_column_stock_set_active(const gboolean active)
{
	...
	if (active == FALSE) {
		selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.stock_list_view));
		gtk_tree_selection_unselect_all(selection);
	}
}
...
static void prefs_folder_column_stock_sel_changed(void)
{
	GtkTreeSelection *selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.stock_list_view));
	prefs_folder_column_stock_set_active((selection != NULL) ? TRUE : FALSE);
	prefs_folder_column_shown_set_active((selection != NULL) ? FALSE : TRUE);
}

static void prefs_folder_column_shown_sel_changed(void)
{
	GtkTreeSelection *selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(folder_col.shown_list_view));
	prefs_folder_column_shown_set_active((selection != NULL) ? TRUE : FALSE);
	prefs_folder_column_stock_set_active((selection != NULL) ? FALSE : TRUE);
}
