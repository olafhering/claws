...
{
	gtk_widget_set_sensitive(privacy_page.save_clear_text_checkbtn, !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(privacy_page.encrypt_to_self_checkbtn)));
}
...
static void update_privacy_system_menu()
{
	...
	menu = GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(privacy_page.default_privacy_system)));

	/* First add "None", as that one is always available. :) */
	gtk_list_store_append(menu, &iter);
	gtk_list_store_set(menu, &iter, COMBOBOX_TEXT, _("None"), COMBOBOX_DATA, 0, COMBOBOX_SENS, TRUE, COMBOBOX_PRIVACY_PLUGIN_ID, "", -1);
	...
	for (cur = system_ids; cur != NULL; cur = g_slist_next(cur)) {
		...
		gtk_list_store_append(menu, &iter);
		gtk_list_store_set(menu, &iter, COMBOBOX_TEXT, name, COMBOBOX_DATA, 1, COMBOBOX_SENS, TRUE, COMBOBOX_PRIVACY_PLUGIN_ID, id, -1);
		g_free(id);
	}
...
static void basic_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	gtk_table_attach(GTK_TABLE(table1), name_entry, 1, 2, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	addr_entry = gtk_entry_new();
	gtk_widget_show(addr_entry);
	gtk_table_attach(GTK_TABLE(table1), addr_entry, 1, 2, 1, 2, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	org_entry = gtk_entry_new();
	gtk_widget_show(org_entry);
	gtk_table_attach(GTK_TABLE(table1), org_entry, 1, 2, 2, 3, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);
	...
	g_signal_connect(G_OBJECT(optmenu), "changed", G_CALLBACK(prefs_account_protocol_changed), NULL);
	...
	g_signal_connect(G_OBJECT(auto_configure_btn), "clicked", G_CALLBACK(auto_configure_cb), NULL);
	g_signal_connect(G_OBJECT(auto_configure_cancel_btn), "clicked", G_CALLBACK(auto_configure_cb), NULL);
#endif

	no_imap_warn_icon = gtk_image_new_from_stock(GTK_STOCK_DIALOG_WARNING, GTK_ICON_SIZE_SMALL_TOOLBAR);
	...
	gtk_table_attach(GTK_TABLE(serv_table), nntpserv_entry, 1, 4, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);
/*  	gtk_table_set_row_spacing (GTK_TABLE (serv_table), 0, 0); */

	nntpauth_checkbtn = gtk_check_button_new_with_label(_("This server requires authentication"));
	gtk_widget_show(nntpauth_checkbtn);

	gtk_table_attach(GTK_TABLE(serv_table), nntpauth_checkbtn, 0, 2, 6, 7, GTK_FILL, 0, 0, 0);

	nntpauth_onconnect_checkbtn = gtk_check_button_new_with_label(_("Authenticate on connect"));
	gtk_widget_show(nntpauth_onconnect_checkbtn);

	gtk_table_attach(GTK_TABLE(serv_table), nntpauth_onconnect_checkbtn, 2, 4, 6, 7, GTK_FILL, 0, 0, 0);

	recvserv_entry = gtk_entry_new();
	gtk_widget_show(recvserv_entry);
	gtk_table_attach(GTK_TABLE(serv_table), recvserv_entry, 1, 4, 2, 3, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	localmbox_entry = gtk_entry_new();
	gtk_widget_show(localmbox_entry);
	gtk_table_attach(GTK_TABLE(serv_table), localmbox_entry, 1, 4, 3, 4, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	smtpserv_entry = gtk_entry_new();
	gtk_widget_show(smtpserv_entry);
	gtk_table_attach(GTK_TABLE(serv_table), smtpserv_entry, 1, 4, 4, 5, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	mailcmd_entry = gtk_entry_new();
	gtk_widget_show(mailcmd_entry);
	gtk_table_attach(GTK_TABLE(serv_table), mailcmd_entry, 1, 4, 6, 7, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	uid_entry = gtk_entry_new();
	gtk_widget_show(uid_entry);
	gtk_widget_set_size_request(uid_entry, DEFAULT_ENTRY_WIDTH, -1);
	g_signal_connect(G_OBJECT(uid_entry), "changed", G_CALLBACK(prefs_account_entry_changed_newline_check_cb), GINT_TO_POINTER(ac_prefs->protocol));

	pass_entry = gtk_entry_new();
	gtk_widget_show(pass_entry);
	gtk_widget_set_size_request(pass_entry, DEFAULT_ENTRY_WIDTH, -1);
	g_signal_connect(G_OBJECT(pass_entry), "changed", G_CALLBACK(prefs_account_entry_changed_newline_check_cb), GINT_TO_POINTER(ac_prefs->protocol));

#ifndef GENERIC_UMPC
	gtk_table_attach(GTK_TABLE(serv_table), uid_entry, 1, 2, 7, 8, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	gtk_table_attach(GTK_TABLE(serv_table), pass_entry, 3, 4, 7, 8, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);
#else
	gtk_table_attach(GTK_TABLE(serv_table), uid_entry, 1, 4, 7, 8, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);
	gtk_table_attach(GTK_TABLE(serv_table), pass_entry, 1, 4, 8, 9, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);
#endif
	gtk_entry_set_visibility(GTK_ENTRY(pass_entry), FALSE);

	nntpserv_label = gtk_label_new(_("News server"));
	gtk_widget_show(nntpserv_label);
	gtk_table_attach(GTK_TABLE(serv_table), nntpserv_label, 0, 1, 0, 1, GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(serv_table), recvserv_label, 0, 1, 2, 3, GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(serv_table), localmbox_label, 0, 1, 3, 4, GTK_FILL, 0, 0, 0);
...
/*  	gtk_table_set_row_spacing (GTK_TABLE (serv_table), 2, 0); */
	...
	gtk_table_attach(GTK_TABLE(serv_table), smtpserv_label, 0, 1, 4, 5, GTK_FILL, 0, 0, 0);
	gtk_misc_set_alignment(GTK_MISC(smtpserv_label), 1, 0.5);
/*  	gtk_table_set_row_spacing (GTK_TABLE (serv_table), 2, 0); */

	mailcmd_checkbtn = gtk_check_button_new_with_label(_("Use mail command rather than SMTP server"));
	gtk_widget_show(mailcmd_checkbtn);
	gtk_table_attach(GTK_TABLE(serv_table), mailcmd_checkbtn, 0, 4, 5, 6, GTK_EXPAND | GTK_FILL, 0, 0, TABLE_YPAD);
	g_signal_connect(G_OBJECT(mailcmd_checkbtn), "toggled", G_CALLBACK(prefs_account_mailcmd_toggled), NULL);

	mailcmd_label = gtk_label_new(_("command to send mails"));
	gtk_widget_show(mailcmd_label);
	gtk_table_attach(GTK_TABLE(serv_table), mailcmd_label, 0, 1, 6, 7, GTK_FILL, 0, 0, 0);
	gtk_misc_set_alignment(GTK_MISC(mailcmd_label), 1, 0.5);
/*  	gtk_table_set_row_spacing (GTK_TABLE (serv_table), 2, 0); */

	uid_label = gtk_label_new(_("User ID"));
	gtk_widget_show(uid_label);
	gtk_table_attach(GTK_TABLE(serv_table), uid_label, 0, 1, 7, 8, GTK_FILL, 0, 0, 0);
	gtk_misc_set_alignment(GTK_MISC(uid_label), 1, 0.5);

	pass_label = gtk_label_new(_("Password"));
	gtk_widget_show(pass_label);
#ifndef GENERIC_UMPC
	gtk_table_attach(GTK_TABLE(serv_table), pass_label, 2, 3, 7, 8, 0, 0, 0, 0);
#else
	gtk_misc_set_alignment(GTK_MISC(pass_label), 1, 0.5);
	gtk_table_attach(GTK_TABLE(serv_table), pass_label, 0, 1, 8, 9, GTK_FILL, 0, 0, 0);
#endif
	...
	gtk_table_attach(GTK_TABLE(serv_table), showpwd_checkbtn, 3, 4, 9, 10, GTK_FILL, 0, 0, 0);
	g_signal_connect(G_OBJECT(showpwd_checkbtn), "toggled", G_CALLBACK(prefs_account_showpwd_checkbtn_toggled), pass_entry);
	...
	if (new_account) {
		...
		buf = passwd_store_get_account(ac_prefs->account_id, PWS_ACCOUNT_RECV);
...
static void receive_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	CLAWS_SET_TIP(local_inbox_entry, _("Unfiltered messages will be stored in this folder"));
	...
	CLAWS_SET_TIP(local_inbox_btn, _("Unfiltered messages will be stored in this folder"));
	gtk_box_pack_start(GTK_BOX(local_hbox), local_inbox_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(local_inbox_btn), "clicked", G_CALLBACK(prefs_account_select_folder_cb), local_inbox_entry);

	vbox2 = gtkut_get_options_frame(vbox1, &frame1, _("POP"));

	PACK_CHECK_BUTTON(vbox2, pop_auth_checkbtn, _("Authenticate before POP connection"));
	...
	PACK_CHECK_BUTTON(vbox2, rmmail_checkbtn, _("Remove messages on server when received"));
	...
	CLAWS_SET_TIP(leave_time_spinbtn, _("0 days and 0 hours : remove immediately"));
	...
	CLAWS_SET_TIP(leave_hour_spinbtn, _("0 days and 0 hours : remove immediately"));
	...
	CLAWS_SET_TIP(size_limit_checkbtn, _("Messages over this limit will be partially retrieved. " "When selecting them you will be able to download them fully " "or delete them."));
	...
	CLAWS_SET_TIP(inbox_entry, _("Unfiltered messages will be stored in this folder"));
	...
	CLAWS_SET_TIP(inbox_btn, _("Unfiltered messages will be stored in this folder"));
	gtk_box_pack_start(GTK_BOX(hbox1), inbox_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(inbox_btn), "clicked", G_CALLBACK(prefs_account_select_folder_cb), inbox_entry);
	...
	maxarticle_label = gtk_label_new(_("Maximum number of articles to download"));
	gtk_widget_show(maxarticle_label);
	gtk_box_pack_start(GTK_BOX(hbox2), maxarticle_label, FALSE, FALSE, 0);

	maxarticle_spinbtn_adj = GTK_ADJUSTMENT(gtk_adjustment_new(300, 0, 10000, 10, 100, 0));
	maxarticle_spinbtn = gtk_spin_button_new(GTK_ADJUSTMENT(maxarticle_spinbtn_adj), 10, 0);
	gtk_widget_show(maxarticle_spinbtn);
	CLAWS_SET_TIP(maxarticle_spinbtn, _("unlimited if 0 is specified"));
	gtk_box_pack_start(GTK_BOX(hbox2), maxarticle_spinbtn, FALSE, FALSE, 0);
	gtk_widget_set_size_request(maxarticle_spinbtn, 64, -1);
	gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(maxarticle_spinbtn), TRUE);
	...
	PACK_CHECK_BUTTON(hbox1, subsonly_checkbtn, _("Show subscribed folders only"));

	hbox1 = gtk_hbox_new(FALSE, 8);
	gtk_widget_show(hbox1);
	gtk_box_pack_start(GTK_BOX(vbox2), hbox1, FALSE, FALSE, 4);

	PACK_CHECK_BUTTON(hbox1, low_bandwidth_checkbtn, _("Bandwidth-efficient mode (prevents retrieving remote tags)"));
	CLAWS_SET_TIP(low_bandwidth_checkbtn, _("This mode uses less bandwidth, but can be slower with some servers."));
	...
	PACK_CHECK_BUTTON(vbox4, autochk_use_default_checkbtn, _("Use global settings"));

	hbox2 = gtk_hbox_new(FALSE, 8);
	gtk_box_pack_start(GTK_BOX(vbox4), hbox2, FALSE, FALSE, 0);

	PACK_CHECK_BUTTON(hbox2, autochk_checkbtn, _("Check for new mail every"));
	...
	PACK_CHECK_BUTTON(vbox1, filter_on_recv_checkbtn, _("Filter messages on receiving"));

	g_signal_connect(G_OBJECT(filter_on_recv_checkbtn), "toggled", G_CALLBACK(prefs_account_filter_on_recv_toggled), NULL);
	g_signal_connect(G_OBJECT(autochk_hour_spinbtn), "value-changed", G_CALLBACK(prefs_account_receive_itv_spinbutton_value_changed_cb), (gpointer)page);
	g_signal_connect(G_OBJECT(autochk_min_spinbtn), "value-changed", G_CALLBACK(prefs_account_receive_itv_spinbutton_value_changed_cb), (gpointer)page);
	g_signal_connect(G_OBJECT(autochk_sec_spinbtn), "value-changed", G_CALLBACK(prefs_account_receive_itv_spinbutton_value_changed_cb), (gpointer)page);

	PACK_CHECK_BUTTON(vbox1, filterhook_on_recv_checkbtn, _("Allow filtering using plugins on receiving"));

	PACK_CHECK_BUTTON(vbox1, recvatgetall_checkbtn, _("'Get Mail' checks for new messages on this account"));
...
static void send_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	PACK_CHECK_BUTTON(vbox2, checkbtn_msgid_with_addr, _("Send account mail address in Message-ID"));

	PACK_CHECK_BUTTON(vbox2, xmailer_checkbtn, _("Add user agent header"));

	hbox = gtk_hbox_new(FALSE, 12);
	gtk_widget_show(hbox);
	gtk_box_pack_start(GTK_BOX(vbox2), hbox, FALSE, FALSE, 0);

	PACK_CHECK_BUTTON(hbox, customhdr_checkbtn, _("Add user-defined header"));

	customhdr_edit_btn = gtk_button_new_from_stock(GTK_STOCK_EDIT);
	gtk_widget_show(customhdr_edit_btn);
	gtk_box_pack_start(GTK_BOX(hbox), customhdr_edit_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(customhdr_edit_btn), "clicked", G_CALLBACK(prefs_account_edit_custom_header), NULL);

	SET_TOGGLE_SENSITIVITY(customhdr_checkbtn, customhdr_edit_btn);

	vbox3 = gtkut_get_options_frame(vbox1, &frame, _("Authentication"));

	PACK_CHECK_BUTTON(vbox3, smtp_auth_checkbtn, _("SMTP Authentication (SMTP AUTH)"));
	...
	g_signal_connect(G_OBJECT(smtp_uid_entry), "changed", G_CALLBACK(prefs_account_entry_changed_newline_check_cb), GINT_TO_POINTER(ac_prefs->protocol));
	...
	g_signal_connect(G_OBJECT(smtp_pass_entry), "changed", G_CALLBACK(prefs_account_entry_changed_newline_check_cb), GINT_TO_POINTER(ac_prefs->protocol));

	showpwd_checkbtn = gtk_check_button_new_with_label(_("Show password"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(showpwd_checkbtn), FALSE);
	gtk_widget_show(showpwd_checkbtn);
	gtk_box_pack_start(GTK_BOX(hbox), showpwd_checkbtn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(showpwd_checkbtn), "toggled", G_CALLBACK(prefs_account_showpwd_checkbtn_toggled), smtp_pass_entry);
	...
	label = gtk_label_new(_("If you leave these entries empty, the same " "user ID and password as receiving will be used."));
	gtk_widget_show(label);
	gtk_box_pack_start(GTK_BOX(hbox), label, FALSE, FALSE, 0);
	...
	PACK_CHECK_BUTTON(vbox3, pop_bfr_smtp_checkbtn, _("Authenticate with POP before sending"));

	g_signal_connect(G_OBJECT(pop_bfr_smtp_checkbtn), "clicked", G_CALLBACK(pop_bfr_smtp_tm_set_sens), NULL);
	...
	if (new_account) {
		...
		/* Passwords are handled outside of PrefParams. */
		buf = passwd_store_get_account(ac_prefs->account_id, PWS_ACCOUNT_SEND);
...
#ifdef USE_OAUTH2
static void oauth2_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	auth_vbox = gtkut_get_options_frame(vbox1, &auth_frame, _("Authorization"));
	...
	gtk_table_attach(GTK_TABLE(table1), oauth2_client_id_entry, 1, 2, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	oauth2_client_secret_entry = gtk_entry_new();
	gtk_widget_show(oauth2_client_secret_entry);
	gtk_table_attach(GTK_TABLE(table1), oauth2_client_secret_entry, 1, 2, 1, 2, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);
	...
	gtk_widget_set_tooltip_text(oauth2_authcode_entry, _("Paste complete URL from browser or the provided authorization token"));
	...
	g_signal_connect(G_OBJECT(oauth2_authorise_btn), "clicked", G_CALLBACK(prefs_account_oauth2_obtain_tokens), NULL);
	...
	g_signal_connect(G_OBJECT(oauth2_auth_optmenu), "changed", G_CALLBACK(prefs_account_oauth2_set_sensitivity), NULL);
	g_signal_connect(G_OBJECT(oauth2_authcode_entry), "changed", G_CALLBACK(prefs_account_oauth2_set_auth_sensitivity), NULL);
	...
	if (new_account) {
		...
		/* Passwords are handled outside of PrefParams. */
		buf = passwd_store_get_account(ac_prefs->account_id, PWS_ACCOUNT_OAUTH2_AUTH);
...
static void compose_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	PACK_CHECK_BUTTON(vbox_sig, checkbtn_autosig, _("Automatically insert signature"));
	...
	gtk_box_pack_start(GTK_BOX(sig_hbox), sigfile_radiobtn, FALSE, FALSE, 0);
	g_object_set_data(G_OBJECT(sigfile_radiobtn), MENU_VAL_ID, GINT_TO_POINTER(SIG_FILE));
	g_signal_connect(G_OBJECT(sigfile_radiobtn), "clicked", G_CALLBACK(prefs_account_sigfile_radiobtn_cb), NULL);

	sigcmd_radiobtn = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(sigfile_radiobtn), _("Command output"));
	gtk_widget_show(sigcmd_radiobtn);
	gtk_box_pack_start(GTK_BOX(sig_hbox), sigcmd_radiobtn, FALSE, FALSE, 0);
	g_object_set_data(G_OBJECT(sigcmd_radiobtn), MENU_VAL_ID, GINT_TO_POINTER(SIG_COMMAND));
	g_signal_connect(G_OBJECT(sigcmd_radiobtn), "clicked", G_CALLBACK(prefs_account_sigcmd_radiobtn_cb), NULL);
	...
	g_signal_connect(G_OBJECT(signature_browse_button), "clicked", G_CALLBACK(prefs_account_signature_browse_cb), NULL);
	...
	g_signal_connect(G_OBJECT(signature_edit_button), "clicked", G_CALLBACK(prefs_account_signature_edit_cb), entry_sigpath);
	...
	autocc_checkbtn = gtk_check_button_new_with_label(prefs_common_translated_header_name("Cc:"));
	gtk_widget_show(autocc_checkbtn);
	gtk_table_attach(GTK_TABLE(table), autocc_checkbtn, 0, 1, 0, 1, GTK_FILL, 0, 0, 0);

	autocc_entry = gtk_entry_new();
	gtk_widget_show(autocc_entry);
	gtk_table_attach(GTK_TABLE(table), autocc_entry, 1, 2, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	SET_TOGGLE_SENSITIVITY(autocc_checkbtn, autocc_entry);

	autobcc_checkbtn = gtk_check_button_new_with_label(prefs_common_translated_header_name("Bcc:"));
	gtk_widget_show(autobcc_checkbtn);
	gtk_table_attach(GTK_TABLE(table), autobcc_checkbtn, 0, 1, 1, 2, GTK_FILL, 0, 0, 0);

	autobcc_entry = gtk_entry_new();
	gtk_widget_show(autobcc_entry);
	gtk_table_attach(GTK_TABLE(table), autobcc_entry, 1, 2, 1, 2, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	SET_TOGGLE_SENSITIVITY(autobcc_checkbtn, autobcc_entry);

	autoreplyto_checkbtn = gtk_check_button_new_with_label(prefs_common_translated_header_name("Reply-To:"));
	gtk_widget_show(autoreplyto_checkbtn);
	gtk_table_attach(GTK_TABLE(table), autoreplyto_checkbtn, 0, 1, 2, 3, GTK_FILL, 0, 0, 0);

	autoreplyto_entry = gtk_entry_new();
	gtk_widget_show(autoreplyto_entry);
	gtk_table_attach(GTK_TABLE(table), autoreplyto_entry, 1, 2, 2, 3, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table_dict), checkbtn_enable_default_dictionary, 0, 1, 0, 1, GTK_SHRINK | GTK_FILL, GTK_SHRINK, 0, 0);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbtn_enable_default_dictionary), tmp_ac_prefs.enable_default_dictionary);

	combo_default_dictionary = gtkaspell_dictionary_combo_new(TRUE);
	gtk_table_attach(GTK_TABLE(table_dict), combo_default_dictionary, 1, 2, 0, 1, GTK_EXPAND | GTK_FILL, GTK_SHRINK, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table_dict), checkbtn_enable_default_alt_dictionary, 0, 1, 1, 2, GTK_SHRINK | GTK_FILL, GTK_SHRINK, 0, 0);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbtn_enable_default_alt_dictionary), tmp_ac_prefs.enable_default_alt_dictionary);

	combo_default_alt_dictionary = gtkaspell_dictionary_combo_new(FALSE);
	gtk_table_attach(GTK_TABLE(table_dict), combo_default_alt_dictionary, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL, GTK_SHRINK, 0, 0);
	...
	if (compose_page.combo_default_dictionary != NULL) {
		gtk_combo_box_set_model(GTK_COMBO_BOX(compose_page.combo_default_dictionary), gtkaspell_dictionary_store_new());
		gtk_combo_box_set_model(GTK_COMBO_BOX(compose_page.combo_default_alt_dictionary), gtkaspell_dictionary_store_new_with_refresh(FALSE));
	}
#endif
...
static void templates_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	quotefmt_create_new_msg_fmt_widgets(window, vbox2, &page->checkbtn_compose_with_format, NULL, &page->compose_subject_format, &page->compose_body_format, TRUE, NULL);
	...
	quotefmt_create_reply_fmt_widgets(window, vbox2, &page->checkbtn_reply_with_format, NULL, &page->reply_quotemark, &page->reply_body_format, TRUE, NULL);
	...
	quotefmt_create_forward_fmt_widgets(window, vbox2, &page->checkbtn_forward_with_format, NULL, &page->forward_quotemark, &page->forward_body_format, TRUE, NULL);
...
static void privacy_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	menu = gtk_list_store_new(4, G_TYPE_STRING, G_TYPE_INT, G_TYPE_BOOLEAN, G_TYPE_STRING);	/* This is where we store the privacy plugin id. */
	...
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(default_privacy_system), rend, "text", COMBOBOX_TEXT, "sensitive", COMBOBOX_SENS, NULL);
	...
	g_signal_connect(G_OBJECT(default_privacy_system), "changed", G_CALLBACK(privacy_system_activated), NULL);

	PACK_CHECK_BUTTON(vbox2, default_sign_checkbtn, _("Always sign messages"));
	PACK_CHECK_BUTTON(vbox2, default_encrypt_checkbtn, _("Always encrypt messages"));
	PACK_CHECK_BUTTON(vbox2, default_sign_reply_checkbtn, _("Always sign messages when replying to a " "signed message"));
	PACK_CHECK_BUTTON(vbox2, default_encrypt_reply_checkbtn, _("Always encrypt messages when replying to an " "encrypted message"));
	PACK_CHECK_BUTTON(vbox2, encrypt_to_self_checkbtn, _("Encrypt sent messages with your own key in addition to recipient's"));
	PACK_CHECK_BUTTON(vbox2, save_clear_text_checkbtn, _("Save sent encrypted messages as clear text"));
...
static void pop_ssltunnel_toggled(GtkToggleButton *button, gpointer data)
{
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.popport_checkbtn)) == TRUE)
		return;
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(advanced_page.popport_spinbtn), active ? 995 : 110);
}
...
static void imap_ssltunnel_toggled(GtkToggleButton *button, gpointer data)
{
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.imapport_checkbtn)) == TRUE)
		return;
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(advanced_page.imapport_spinbtn), active ? 993 : 143);
}
...
static void nntp_ssltunnel_toggled(GtkToggleButton *button, gpointer data)
{
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.nntpport_checkbtn)) == TRUE)
		return;
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(advanced_page.nntpport_spinbtn), active ? 563 : 119);
}
...
static void smtp_ssltunnel_toggled(GtkToggleButton *button, gpointer data)
{
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.smtpport_checkbtn)) == TRUE)
		return;
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(advanced_page.smtpport_spinbtn), active ? 465 : 25);
}

static void ssl_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	CREATE_RADIO_BUTTONS(vbox2, pop_nossl_radiobtn, _("Don't use TLS"), SSL_NONE, pop_ssltunnel_radiobtn, _("Use TLS"), SSL_TUNNEL, pop_starttls_radiobtn, _("Use STARTTLS command to start encrypted session"), SSL_STARTTLS);
	g_signal_connect(G_OBJECT(pop_ssltunnel_radiobtn), "toggled", G_CALLBACK(pop_ssltunnel_toggled), NULL);

	vbox3 = gtkut_get_options_frame(vbox1, &imap_frame, _("IMAP"));

	CREATE_RADIO_BUTTONS(vbox3, imap_nossl_radiobtn, _("Don't use TLS"), SSL_NONE, imap_ssltunnel_radiobtn, _("Use TLS"), SSL_TUNNEL, imap_starttls_radiobtn, _("Use STARTTLS command to start encrypted session"), SSL_STARTTLS);
	g_signal_connect(G_OBJECT(imap_ssltunnel_radiobtn), "toggled", G_CALLBACK(imap_ssltunnel_toggled), NULL);

	vbox4 = gtkut_get_options_frame(vbox1, &nntp_frame, _("NNTP"));

	nntp_nossl_radiobtn = gtk_radio_button_new_with_label(NULL, _("Don't use TLS"));
	gtk_widget_show(nntp_nossl_radiobtn);
	gtk_box_pack_start(GTK_BOX(vbox4), nntp_nossl_radiobtn, FALSE, FALSE, 0);
	g_object_set_data(G_OBJECT(nntp_nossl_radiobtn), MENU_VAL_ID, GINT_TO_POINTER(SSL_NONE));

	CREATE_RADIO_BUTTON(vbox4, nntp_ssltunnel_radiobtn, nntp_nossl_radiobtn, _("Use TLS"), SSL_TUNNEL);
	g_signal_connect(G_OBJECT(nntp_ssltunnel_radiobtn), "toggled", G_CALLBACK(nntp_ssltunnel_toggled), NULL);

	vbox5 = gtkut_get_options_frame(vbox1, &send_frame, _("Send (SMTP)"));

	CREATE_RADIO_BUTTONS(vbox5, smtp_nossl_radiobtn, _("Don't use TLS (but, if necessary, use STARTTLS)"), SSL_NONE, smtp_ssltunnel_radiobtn, _("Use TLS"), SSL_TUNNEL, smtp_starttls_radiobtn, _("Use STARTTLS command to start encrypted session"), SSL_STARTTLS);
	g_signal_connect(G_OBJECT(smtp_ssltunnel_radiobtn), "toggled", G_CALLBACK(smtp_ssltunnel_toggled), NULL);
	...
	CLAWS_SET_TIP(label, _("Client certificate file as a PKCS12 or PEM file"));
	CLAWS_SET_TIP(entry_in_cert_file, _("Client certificate file as a PKCS12 or PEM file"));
	...
	showpwd_checkbtn = gtk_check_button_new_with_label(_("Show password"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(showpwd_checkbtn), FALSE);
	g_signal_connect(G_OBJECT(showpwd_checkbtn), "toggled", G_CALLBACK(prefs_account_showpwd_checkbtn_toggled), entry_in_cert_pass);
	...
	gtk_table_attach(GTK_TABLE(cert_table), showpwd_checkbtn, 2, 3, 1, 2, GTK_FILL, 0, 0, 0);
	...
	CLAWS_SET_TIP(label, _("Client certificate file as a PKCS12 or PEM file"));
	CLAWS_SET_TIP(entry_out_cert_file, _("Client certificate file as a PKCS12 or PEM file"));
	...
	showpwd_checkbtn = gtk_check_button_new_with_label(_("Show password"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(showpwd_checkbtn), FALSE);
	g_signal_connect(G_OBJECT(showpwd_checkbtn), "toggled", G_CALLBACK(prefs_account_showpwd_checkbtn_toggled), entry_out_cert_pass);
	...
	gtk_table_attach(GTK_TABLE(cert_table), showpwd_checkbtn, 2, 3, 3, 4, GTK_FILL, 0, 0, 0);

	gtk_widget_show_all(cert_table);

	g_signal_connect(G_OBJECT(in_ssl_cert_browse_button), "clicked", G_CALLBACK(prefs_account_in_cert_browse_cb), NULL);
	g_signal_connect(G_OBJECT(out_ssl_cert_browse_button), "clicked", G_CALLBACK(prefs_account_out_cert_browse_cb), NULL);

	vbox7 = gtk_vbox_new(FALSE, 0);
	gtk_widget_show(vbox7);
	gtk_box_pack_start(GTK_BOX(vbox1), vbox7, FALSE, FALSE, 0);

	PACK_CHECK_BUTTON(vbox7, ssl_certs_auto_accept_checkbtn, _("Automatically accept valid TLS certificates"));

	PACK_CHECK_BUTTON(vbox7, use_nonblocking_ssl_checkbtn, _("Use non-blocking TLS"));
	...
	label = gtk_label_new(_("Turn this off if you have TLS connection problems"));
	gtk_widget_show(label);
	gtk_box_pack_start(GTK_BOX(hbox), label, FALSE, FALSE, 0);
	gtkut_widget_set_small_font_size(label);
	...
	if (new_account) {
		...
		/* Passwords are handled outside of PrefParams. */
		buf = passwd_store_get_account(ac_prefs->account_id, PWS_ACCOUNT_RECV_CERT);
		...
		buf = passwd_store_get_account(ac_prefs->account_id, PWS_ACCOUNT_SEND_CERT);
...
static void proxy_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	default_proxy_checkbtn = gtk_check_button_new_with_label(C_("In account preferences, referring to whether or not use proxy settings from common preferences", "Use default settings"));
	CLAWS_SET_TIP(default_proxy_checkbtn, _("Use global proxy server settings"));
	...
	g_object_set_data(G_OBJECT(socks4_radiobtn), MENU_VAL_ID, GINT_TO_POINTER(PROXY_SOCKS4));

	CREATE_RADIO_BUTTON(hbox, socks5_radiobtn, socks4_radiobtn, "SOCKS5", PROXY_SOCKS5);
	...
	gtk_table_attach(GTK_TABLE(table), proxy_name_entry, 1, 2, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	label = gtk_label_new(_("Password"));
	gtk_table_attach(GTK_TABLE(table), label, 2, 3, 0, 1, GTK_SHRINK | GTK_FILL, GTK_SHRINK | GTK_FILL, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table), proxy_pass_entry, 3, 4, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0);

	button = gtk_check_button_new_with_label(_("Show password"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), FALSE);
	gtk_table_attach(GTK_TABLE(table), button, 3, 4, 1, 2, GTK_SHRINK | GTK_FILL, GTK_SHRINK | GTK_FILL, 0, 0);
	g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(prefs_account_showpwd_checkbtn_toggled), proxy_pass_entry);

	gtk_box_pack_start(GTK_BOX(vbox2), gtk_hseparator_new(), FALSE, FALSE, 0);

	PACK_CHECK_BUTTON(vbox2, proxy_send_checkbtn, _("Use proxy server for sending"));
	CLAWS_SET_TIP(proxy_send_checkbtn, _("If disabled, messages will be sent using direct connection to configured outgoing server, bypassing any configured proxy server."));
	...
	if (new_account) {
		...
		/* Passwords are handled outside of PrefParams. */
		buf = passwd_store_get_account(ac_prefs->account_id, PWS_ACCOUNT_PROXY_PASS);
		gtk_entry_set_text(GTK_ENTRY(page->proxy_pass_entry), buf != NULL ? buf : "");
		if (buf != NULL) {
			memset(buf, 0, strlen(buf));
			g_free(buf);
		}
...
static void advanced_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
...
#define PACK_PORT_SPINBTN(box, spinbtn) \
	...
	PACK_CHECK_BUTTON(hbox_popport, checkbtn_popport, _("POP port"));
	...
	PACK_CHECK_BUTTON(hbox_imapport, checkbtn_imapport, _("IMAP port"));
	...
	PACK_CHECK_BUTTON(hbox_nntpport, checkbtn_nntpport, _("NNTP port"));
	...
	tip_domain = _("The domain name will be used in the generated " "Message-ID, and when connecting to SMTP servers.");
...
#ifndef G_OS_WIN32
	PACK_HBOX(hbox1);
	PACK_CHECK_BUTTON(hbox1, checkbtn_tunnelcmd, _("Use command to communicate with server"));
	...
	PACK_HBOX(hbox1);
	PACK_CHECK_BUTTON(hbox1, checkbtn_crosspost, _("Mark cross-posted messages as read and color:"));
	g_signal_connect(G_OBJECT(checkbtn_crosspost), "toggled", G_CALLBACK(crosspost_color_toggled), NULL);
...
{									\
	...
	SET_CHECK_BTN_AND_ENTRY(_("Put sent messages in"), sent_folder_checkbtn, sent_folder_entry, 0);
	SET_CHECK_BTN_AND_ENTRY(_("Put queued messages in"), queue_folder_checkbtn, queue_folder_entry, 1);
	SET_CHECK_BTN_AND_ENTRY(_("Put draft messages in"), draft_folder_checkbtn, draft_folder_entry, 2);
	SET_CHECK_BTN_AND_ENTRY(_("Put deleted messages in"), trash_folder_checkbtn, trash_folder_entry, 3);
...
static gint prefs_basic_apply(void)
{
	...
	if (((protocol == A_POP3) || (protocol == A_LOCAL && !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(basic_page.mailcmd_checkbtn))) || (protocol == A_NONE)) && *gtk_entry_get_text(GTK_ENTRY(basic_page.smtpserv_entry)) == '\0') {
		alertpanel_error(_("SMTP server is not entered."));
		return -1;
	}
	if ((protocol == A_POP3 || protocol == A_IMAP4) && *gtk_entry_get_text(GTK_ENTRY(basic_page.uid_entry)) == '\0') {
		alertpanel_error(_("User ID is not entered."));
		return -1;
	}
	if (protocol == A_POP3 && *gtk_entry_get_text(GTK_ENTRY(basic_page.recvserv_entry)) == '\0') {
	...
	if (protocol == A_POP3 || protocol == A_LOCAL) {
		...
		if (!inbox) {
			...
			gtk_entry_set_text(GTK_ENTRY(receive_page.inbox_entry), id);
			gtk_entry_set_text(GTK_ENTRY(receive_page.local_inbox_entry), id);
			g_free(id);
	...
	if (protocol == A_IMAP4 && *gtk_entry_get_text(GTK_ENTRY(basic_page.recvserv_entry)) == '\0') {
		alertpanel_error(_("IMAP server is not entered."));
		return -1;
	}
	if (protocol == A_NNTP && *gtk_entry_get_text(GTK_ENTRY(basic_page.nntpserv_entry)) == '\0') {
		alertpanel_error(_("NNTP server is not entered."));
		return -1;
	}

	if (protocol == A_LOCAL && *gtk_entry_get_text(GTK_ENTRY(basic_page.localmbox_entry)) == '\0') {
		alertpanel_error(_("local mailbox filename is not entered."));
		return -1;
	}

	if (protocol == A_LOCAL && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(basic_page.mailcmd_checkbtn)) && *gtk_entry_get_text(GTK_ENTRY(basic_page.mailcmd_entry)) == '\0') {
		alertpanel_error(_("mail command is not entered."));
		return -1;
	}

	if (protocol == A_IMAP4 || protocol == A_NNTP)
		old_id = g_strdup_printf("#%s/%s", protocol == A_IMAP4 ? "imap" : "news", tmp_ac_prefs.account_name ? tmp_ac_prefs.account_name : "(null)");
	...
	passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_RECV, gtk_entry_get_text(GTK_ENTRY(basic_page.pass_entry)), FALSE);
	...
	if (protocol == A_IMAP4 || protocol == A_NNTP) {
		new_id = g_strdup_printf("#%s/%s", protocol == A_IMAP4 ? "imap" : "news", tmp_ac_prefs.account_name);
...
static gint prefs_send_apply(void)
{
	...
	/* Passwords are stored outside of PrefParams. */
	passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_SEND, gtk_entry_get_text(GTK_ENTRY(send_page.smtp_pass_entry)), FALSE);
#ifdef USE_OAUTH2
	/* Manual password change - reset expiry on OAUTH2 tokens */
	passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_OAUTH2_EXPIRY, "0", FALSE);
#endif
...
static gint prefs_oauth2_apply(void)
{
	...
	/* Passwords are stored outside of PrefParams. */
	passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_OAUTH2_AUTH, gtk_entry_get_text(GTK_ENTRY(oauth2_page.oauth2_authcode_entry)), FALSE);
...
static gint prefs_ssl_apply(void)
{
	...
	/* Passwords are stored outside of PrefParams. */
	passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_RECV_CERT, gtk_entry_get_text(GTK_ENTRY(ssl_page.entry_in_cert_pass)), FALSE);
	passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_SEND_CERT, gtk_entry_get_text(GTK_ENTRY(ssl_page.entry_out_cert_pass)), FALSE);
...
static gint prefs_proxy_apply(void)
{
	...
	/* Passwords are stored outside of PrefParams. */
	passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_PROXY_PASS, gtk_entry_get_text(GTK_ENTRY(proxy_page.proxy_pass_entry)), FALSE);
...
{
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.domain_checkbtn)) && *gtk_entry_get_text(GTK_ENTRY(advanced_page.domain_entry)) == '\0') {
		alertpanel_error(_("domain is not specified."));
		return -1;
	}
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.sent_folder_checkbtn)) && *gtk_entry_get_text(GTK_ENTRY(advanced_page.sent_folder_entry)) == '\0') {
		alertpanel_error(_("sent folder is not selected."));
		return -1;
	}
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.queue_folder_checkbtn)) && *gtk_entry_get_text(GTK_ENTRY(advanced_page.queue_folder_entry)) == '\0') {
		alertpanel_error(_("queue folder is not selected."));
		return -1;
	}
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.draft_folder_checkbtn)) && *gtk_entry_get_text(GTK_ENTRY(advanced_page.draft_folder_entry)) == '\0') {
		alertpanel_error(_("draft folder is not selected."));
		return -1;
	}
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.trash_folder_checkbtn)) && *gtk_entry_get_text(GTK_ENTRY(advanced_page.trash_folder_entry)) == '\0') {
...
static void templates_save_func(PrefsPage *_page)
{
	...
	quotefmt_check_new_msg_formats(tmp_ac_prefs.compose_with_format, NULL, tmp_ac_prefs.compose_subject_format, tmp_ac_prefs.compose_body_format);
	quotefmt_check_reply_formats(tmp_ac_prefs.reply_with_format, NULL, tmp_ac_prefs.reply_quotemark, tmp_ac_prefs.reply_body_format);
	quotefmt_check_forward_formats(tmp_ac_prefs.forward_with_format, NULL, tmp_ac_prefs.forward_quotemark, tmp_ac_prefs.forward_body_format);
...
PrefsAccount *prefs_account_new_from_config(const gchar *label)
{
	...
	*ac_prefs = tmp_ac_prefs;
	...
	id = atoi(p);
	if (id < 0)
		g_warning("wrong account id: %d", id);
	...
	if (ac_prefs->passwd != NULL && strlen(ac_prefs->passwd) > 1) {
		passwd_store_set_account(ac_prefs->account_id, PWS_ACCOUNT_RECV, ac_prefs->passwd, TRUE);
		passwords_migrated = TRUE;
	}
	if (ac_prefs->smtp_passwd != NULL && strlen(ac_prefs->smtp_passwd) > 1) {
		passwd_store_set_account(ac_prefs->account_id, PWS_ACCOUNT_SEND, ac_prefs->smtp_passwd, TRUE);
		passwords_migrated = TRUE;
	}
	if (ac_prefs->in_ssl_client_cert_pass != NULL && strlen(ac_prefs->in_ssl_client_cert_pass) > 1) {
		passwd_store_set_account(ac_prefs->account_id, PWS_ACCOUNT_RECV_CERT, ac_prefs->in_ssl_client_cert_pass, TRUE);
		passwords_migrated = TRUE;
	}
	if (ac_prefs->out_ssl_client_cert_pass != NULL && strlen(ac_prefs->out_ssl_client_cert_pass) > 1) {
		passwd_store_set_account(ac_prefs->account_id, PWS_ACCOUNT_SEND_CERT, ac_prefs->out_ssl_client_cert_pass, TRUE);
		passwords_migrated = TRUE;
	}
	...
	if (!ac_prefs->autochk_use_default && ac_prefs->autochk_use_custom && ac_prefs->autochk_itv > PREFS_RECV_AUTOCHECK_MIN_INTERVAL) {
...
PrefsAccount *prefs_account_open(PrefsAccount *ac_prefs, gboolean *dirty)
{
	...
	if (new_account)
		...
		title = g_strdup_printf(_("%s - Account preferences"), ac_prefs->account_name);

	prefswindow_open_full(title, prefs_pages, ac_prefs, destroy_dialog, &prefs_common.editaccountwin_width, &prefs_common.editaccountwin_height, TRUE, NULL, NULL, NULL);
	g_free(title);
	gtk_main();
...
static void crosspost_color_toggled(void)
{
	...
	is_active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(advanced_page.crosspost_checkbtn));
...
static void prefs_account_crosspost_set_data_from_colormenu(PrefParam *pparam)
{
	...
	*((gint *)pparam->data) = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(menuitem), "color"));
}
...
static void pop_bfr_smtp_tm_set_sens(GtkWidget *widget, gpointer data)
{
	gtk_widget_set_sensitive(send_page.pop_bfr_smtp_tm_spinbtn, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(send_page.pop_bfr_smtp_checkbtn)));
	gtk_widget_set_sensitive(send_page.pop_auth_timeout_lbl, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(send_page.pop_bfr_smtp_checkbtn)));
...
static void auto_configure_cb(GtkWidget *widget, gpointer data)
{
	...
	if (strchr(address, '@') == NULL) {
		g_free(address);
		gtk_label_set_text(GTK_LABEL(basic_page.auto_configure_lbl), _("Failed (wrong address)"));
		return;
	}
...
static void prefs_account_enum_set_data_from_radiobtn(PrefParam *pparam)
{
	...
	while (group != NULL) {
		...
		if (gtk_toggle_button_get_active(btn)) {
			*((gint *)pparam->data) = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(btn), MENU_VAL_ID));
			break;
		}
		group = group->next;
	}
...
static void prefs_account_protocol_set_data_from_optmenu(PrefParam *pparam)
{
	struct BasicProtocol *protocol_optmenu = (struct BasicProtocol *)*pparam->widget;
	GtkWidget *optmenu = protocol_optmenu->combobox;

	*((RecvProtocol *) pparam->data) = combobox_get_active_data(GTK_COMBO_BOX(optmenu));
}
...
static void prefs_account_protocol_set_optmenu(PrefParam *pparam)
{
	...
	if (new_account) {
		...
		if (protocol == A_IMAP4) {
			if (new_account)
				gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(send_page.msgid_checkbtn), TRUE);
...
static void prefs_account_imap_auth_type_set_data_from_optmenu(PrefParam *pparam)
{
	*((RecvProtocol *) pparam->data) = combobox_get_active_data(GTK_COMBO_BOX(*pparam->widget));
}
...
static void prefs_account_oauth2_provider_set_data_from_optmenu(PrefParam *pparam)
{
	*((Oauth2Service *) pparam->data) = combobox_get_active_data(GTK_COMBO_BOX(*pparam->widget));
}
...
static void prefs_account_oauth2_start_listener_task(GtkWidget *optmenu)
{
	...
	oauth2_listener_data->OAUTH2Data = g_malloc0(sizeof(*oauth2_listener_data->OAUTH2Data));
	oauth2_listener_data->OAUTH2Data->custom_client_secret = g_strdup(gtk_entry_get_text((GtkEntry *)oauth2_page.oauth2_client_secret_entry));
	oauth2_listener_data->OAUTH2Data->custom_client_id = g_strdup(gtk_entry_get_text((GtkEntry *)oauth2_page.oauth2_client_id_entry));
...
static void prefs_account_oauth2_open_url(GtkButton *button, gpointer data)
{
	...
	url = oauth2_authorisation_url(service, custom_client_id);

	open_uri(url, prefs_common_get_uri_cmd());
	prefs_account_oauth2_start_listener_task(optmenu);
}
...
static void prefs_account_oauth2_obtain_tokens(GtkButton *button, gpointer data)
{
	...
	if (!ret) {
		...
		if (OAUTH2Data->access_token != NULL) {
			...
			passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_SEND, OAUTH2Data->access_token, FALSE);
...
static void prefs_account_set_autochk_interval_from_widgets(PrefParam *pparam)
{
	struct AutocheckWidgets *autochk_widgets = (struct AutocheckWidgets *)*pparam->widget;

	*(gint *)pparam->data = (3600 * gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(autochk_widgets->autochk_hour_spinbtn)))
	    + (60 * gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(autochk_widgets->autochk_min_spinbtn)))
	    + gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(autochk_widgets->autochk_sec_spinbtn));
}
...
static void prefs_account_set_autochk_interval_to_widgets(PrefParam *pparam)
{
	...
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(autochk_widgets->autochk_hour_spinbtn), val / 3600);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(autochk_widgets->autochk_min_spinbtn), (val % 3600) / 60);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(autochk_widgets->autochk_sec_spinbtn), (val % 3600) % 60);
}
...
static void prefs_account_set_string_from_combobox(PrefParam *pparam)
{
	...
	cm_return_if_fail(gtk_combo_box_get_active_iter(GTK_COMBO_BOX(combobox), &iter));

	str = (gchar **)pparam->data;
	g_free(*str);
	menu = GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(combobox)));
	gtk_tree_model_get(GTK_TREE_MODEL(menu), &iter, COMBOBOX_PRIVACY_PLUGIN_ID, &(*str), -1);
}
...
static gboolean _privacy_system_set_func(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, PrivacySystemSetCtx *ctx)
{
...
static void prefs_account_set_privacy_combobox_from_string(PrefParam *pparam)
{
	...
	gtk_tree_model_foreach(GTK_TREE_MODEL(menu), (GtkTreeModelForeachFunc) _privacy_system_set_func, ctx);
	found = ctx->found;
	g_free(ctx);
	...
	if (!found) {
		...
		gtk_list_store_append(menu, &iter);
		gtk_list_store_set(menu, &iter, COMBOBOX_TEXT, name, COMBOBOX_DATA, 0, COMBOBOX_SENS, TRUE, COMBOBOX_PRIVACY_PLUGIN_ID, prefsid, -1);
		g_free(name);
...
static void prefs_account_protocol_changed(GtkComboBox *combobox, gpointer data)
{
	...
	case A_NNTP:
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 0, VSPACING_NARROW);
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 1, VSPACING_NARROW);
		gtk_widget_hide(basic_page.recvserv_label);
		gtk_widget_hide(basic_page.recvserv_entry);
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 2, 0);
		gtk_widget_show(basic_page.smtpserv_label);
		gtk_widget_show(basic_page.smtpserv_entry);
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 4, VSPACING_NARROW);
		gtk_widget_hide(basic_page.localmbox_label);
		gtk_widget_hide(basic_page.localmbox_entry);
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 3, 0);
		gtk_widget_hide(basic_page.mailcmd_label);
		gtk_widget_hide(basic_page.mailcmd_entry);
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 6, 0);
		gtk_widget_hide(basic_page.mailcmd_checkbtn);
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 5, 0);
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 7, VSPACING_NARROW);
		...
		prefs_account_nntpauth_toggled(GTK_TOGGLE_BUTTON(basic_page.nntpauth_checkbtn), NULL);
		...
		prefs_account_filter_on_recv_toggled(GTK_TOGGLE_BUTTON(receive_page.filter_on_recv_checkbtn), NULL);
		gtk_widget_set_sensitive(receive_page.recvatgetall_checkbtn, TRUE);
		/* update pop_before_smtp sensitivity */
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(send_page.pop_bfr_smtp_checkbtn), FALSE);
		...
		if (!tmp_ac_prefs.account_name) {
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.filter_on_recv_checkbtn), TRUE);
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.filterhook_on_recv_checkbtn), TRUE);
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.recvatgetall_checkbtn), FALSE);
		}
	...
	case A_LOCAL:
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 0, 0);
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 1, 0);
		gtk_widget_hide(basic_page.recvserv_label);
		gtk_widget_hide(basic_page.recvserv_entry);
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 2, 0);
		gtk_widget_show(basic_page.smtpserv_label);
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 3, VSPACING_NARROW);
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 5, VSPACING_NARROW);
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 7, 0);
		...
		prefs_account_filter_on_recv_toggled(GTK_TOGGLE_BUTTON(receive_page.filter_on_recv_checkbtn), NULL);
		gtk_widget_set_sensitive(receive_page.recvatgetall_checkbtn, TRUE);
		prefs_account_mailcmd_toggled(GTK_TOGGLE_BUTTON(basic_page.mailcmd_checkbtn), NULL);
		...
		if (!tmp_ac_prefs.account_name) {
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.filter_on_recv_checkbtn), TRUE);
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.filterhook_on_recv_checkbtn), TRUE);
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.recvatgetall_checkbtn), TRUE);
		}
	...
	case A_IMAP4:
		...
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 2, VSPACING_NARROW);
		gtk_widget_show(basic_page.smtpserv_label);
		gtk_widget_show(basic_page.smtpserv_entry);
		gtk_table_set_row_spacing(GTK_TABLE(basic_page.serv_table), 4, VSPACING_NARROW);
		gtk_widget_hide(basic_page.localmbox_label);
	...
	case A_NONE:
		...
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.filter_on_recv_checkbtn), FALSE);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.filterhook_on_recv_checkbtn), FALSE);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(receive_page.recvatgetall_checkbtn), FALSE);
...
static void prefs_account_entry_changed_newline_check_cb(GtkWidget *entry, gpointer user_data)
{
	...
	if (strchr(gtk_entry_get_text(GTK_ENTRY(entry)), '\n') != NULL) {
		...
		if (!colors_initialised) {
			...
			colors_initialised = gdk_colormap_alloc_color(gdk_colormap_get_system(), &red, FALSE, TRUE);
		}
...
static void prefs_account_compose_default_dictionary_set_optmenu_from_string(PrefParam *pparam) {
	...
	if (dictionary && *dictionary)
		gtkaspell_set_dictionary_menu_active_item(GTK_COMBO_BOX(combo), dictionary);
	else {
		GtkTreeModel *model;
		GtkTreeIter iter;
...
gchar *prefs_account_generate_msgid(PrefsAccount *account)
{
	...
	if (account->set_domain && account->domain) {
		...
		buf = g_strdup(strchr(account->address, '@') ? strchr(account->address, '@') + 1 : account->address);
	}

	if (user_addr != NULL) {
		addr = g_strdup_printf(".%s", user_addr);
	} else {
		addr = g_strdup_printf("@%s", buf != NULL && strlen(buf) > 0 ? buf : get_domain_name());
	}
	...
	now = (prefs_common.hide_timezone) ? g_date_time_new_now_utc() : g_date_time_new_now_local();
	...
	if (ac_prefs->protocol == A_IMAP4) {
		...
		if (for_server) {
			...
			dir = g_strconcat(get_imap_cache_dir(), G_DIR_SEPARATOR_S,
					  ...
					  ac_prefs->recv_server,
#endif
					  G_DIR_SEPARATOR_S, ac_prefs->userid, NULL);
		}
#ifdef G_OS_WIN32
...
static void prefs_account_receive_itv_spinbutton_value_changed_cb(GtkWidget *w, gpointer data)
{
	...
	gint seconds = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(page->autochk_widgets->autochk_sec_spinbtn));
	gint minutes = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(page->autochk_widgets->autochk_min_spinbtn));
	gint hours = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(page->autochk_widgets->autochk_hour_spinbtn));
	if (seconds < PREFS_RECV_AUTOCHECK_MIN_INTERVAL && minutes == 0 && hours == 0) {
		gtk_spin_button_set_value(GTK_SPIN_BUTTON(page->autochk_widgets->autochk_sec_spinbtn), PREFS_RECV_AUTOCHECK_MIN_INTERVAL);
	}
}
...
static void prefs_account_oauth2_listener(GTask *task, gpointer source, gpointer task_data, GCancellable *cancellable)
{
	...
	do {
		...
		if (FD_ISSET(socket_desc, &rfds)) {
			...
			reply_message = g_strconcat("<html><head><title>", title, "</title><meta charset=\"utf-8\"></head><body><h1>", title, "</h1><p>", body, "</p></body></html>", NULL);
			reply = g_strdup_printf("HTTP/1.0 200 OK\r\nContent-Type: text/html\r\nContent-Length: %" G_GSIZE_FORMAT "\r\n\r\n%s", strlen(reply_message), reply_message);
	...
	if (oauth2_listener_data->success) {
		...
		if (oauth2_listener_data->OAUTH2Data->access_token != NULL) {
			passwd_store_set_account(tmp_ac_prefs.account_id,
						 ...
						 FALSE);
			passwd_store_set_account(tmp_ac_prefs.account_id, PWS_ACCOUNT_SEND, oauth2_listener_data->OAUTH2Data->access_token, FALSE);
			log_message(LOG_PROTOCOL, "OAuth2 access token stored\n");
		}
