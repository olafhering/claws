...
static gboolean from_entry_focus_evt_in(GtkWidget *widget, GdkEventFocus *event, gpointer data);
static gboolean from_entry_focus_evt_out(GtkWidget *widget, GdkEventFocus *event, gpointer data);
static gboolean to_entry_focus_evt_in(GtkWidget *widget, GdkEventFocus *event, gpointer data);
static gboolean to_entry_focus_evt_out(GtkWidget *widget, GdkEventFocus *event, gpointer data);
static gboolean subject_entry_focus_evt_in(GtkWidget *widget, GdkEventFocus *event, gpointer data);
static gboolean subject_entry_focus_evt_out(GtkWidget *widget, GdkEventFocus *event, gpointer data);
...
static gboolean adv_condition_entry_focus_evt_in(GtkWidget *widget, GdkEventFocus *event, gpointer data);
static gboolean adv_condition_entry_focus_evt_out(GtkWidget *widget, GdkEventFocus *event, gpointer data);
...
static void summary_hide_stop_button(void)
{
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(search_window.adv_search_checkbtn))) {
...
static void summary_search_create(void)
{
	...
	g_signal_connect(G_OBJECT(bool_optmenu), "changed", G_CALLBACK(optmenu_changed), NULL);
	...
	if (prefs_common.summary_search_from_history)
		combobox_set_popdown_strings(GTK_COMBO_BOX_TEXT(from_entry), prefs_common.summary_search_from_history);
	gtk_widget_show(from_entry);
	gtk_table_attach(GTK_TABLE(table1), from_entry, 1, 3, 0, 1, GTK_EXPAND | GTK_FILL, 0, 0, 0);
	g_signal_connect(G_OBJECT(from_entry), "changed", G_CALLBACK(from_changed), NULL);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((from_entry)))), "focus_in_event", G_CALLBACK(from_entry_focus_evt_in), NULL);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((from_entry)))), "focus_out_event", G_CALLBACK(from_entry_focus_evt_out), NULL);
	...
	if (prefs_common.summary_search_to_history)
		combobox_set_popdown_strings(GTK_COMBO_BOX_TEXT(to_entry), prefs_common.summary_search_to_history);
	gtk_widget_show(to_entry);
	gtk_table_attach(GTK_TABLE(table1), to_entry, 1, 3, 1, 2, GTK_EXPAND | GTK_FILL, 0, 0, 0);
	g_signal_connect(G_OBJECT(to_entry), "changed", G_CALLBACK(to_changed), NULL);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((to_entry)))), "focus_in_event", G_CALLBACK(to_entry_focus_evt_in), NULL);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((to_entry)))), "focus_out_event", G_CALLBACK(to_entry_focus_evt_out), NULL);
	...
	if (prefs_common.summary_search_subject_history)
		combobox_set_popdown_strings(GTK_COMBO_BOX_TEXT(subject_entry), prefs_common.summary_search_subject_history);
	gtk_widget_show(subject_entry);
	gtk_table_attach(GTK_TABLE(table1), subject_entry, 1, 3, 2, 3, GTK_EXPAND | GTK_FILL, 0, 0, 0);
	g_signal_connect(G_OBJECT(subject_entry), "changed", G_CALLBACK(subject_changed), NULL);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((subject_entry)))), "focus_in_event", G_CALLBACK(subject_entry_focus_evt_in), NULL);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((subject_entry)))), "focus_out_event", G_CALLBACK(subject_entry_focus_evt_out), NULL);
	...
	gtk_table_attach(GTK_TABLE(table1), body_entry, 1, 3, 3, 4, GTK_EXPAND | GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table1), adv_condition_entry, 1, 2, 4, 5, GTK_EXPAND | GTK_FILL, 0, 0, 0);
	g_signal_connect(G_OBJECT(adv_condition_entry), "changed", G_CALLBACK(adv_condition_changed), NULL);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((adv_condition_entry)))), "focus_in_event", G_CALLBACK(adv_condition_entry_focus_evt_in), NULL);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN((adv_condition_entry)))), "focus_out_event", G_CALLBACK(adv_condition_entry_focus_evt_out), NULL);

	adv_condition_btn = gtk_button_new_with_label(" ... ");
	gtk_widget_show(adv_condition_btn);
	gtk_table_attach(GTK_TABLE(table1), adv_condition_btn, 2, 3, 4, 5, GTK_FILL, 0, 0, 0);
	g_signal_connect(G_OBJECT(adv_condition_btn), "clicked", G_CALLBACK(adv_condition_btn_clicked), search_window.window);

	CLAWS_SET_TIP(adv_condition_btn, _("Edit search criteria"));

	from_label = gtk_label_new(_("From:"));
	gtk_widget_show(from_label);
	gtk_table_attach(GTK_TABLE(table1), from_label, 0, 1, 0, 1, GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table1), to_label, 0, 1, 1, 2, GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table1), subject_label, 0, 1, 2, 3, GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table1), body_label, 0, 1, 3, 4, GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table1), adv_condition_label, 0, 1, 4, 5, GTK_FILL, 0, 0, 0);
	...
	g_signal_connect(G_OBJECT(case_checkbtn), "changed", G_CALLBACK(case_changed), NULL);

	adv_search_checkbtn = gtk_check_button_new_with_label(_("Extended Search"));
	gtk_widget_show(adv_search_checkbtn);
	gtk_box_pack_start(GTK_BOX(checkbtn_hbox), adv_search_checkbtn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(adv_search_checkbtn), "changed", G_CALLBACK(case_changed), NULL);
	...
	g_signal_connect(G_OBJECT(clear_btn), "clicked", G_CALLBACK(summary_search_clear), NULL);
	g_signal_connect(G_OBJECT(all_btn), "clicked", G_CALLBACK(summary_search_all_clicked), NULL);
	g_signal_connect(G_OBJECT(prev_btn), "clicked", G_CALLBACK(summary_search_prev_clicked), NULL);
	g_signal_connect(G_OBJECT(next_btn), "clicked", G_CALLBACK(summary_search_next_clicked), NULL);
	g_signal_connect_closure(G_OBJECT(close_btn), "clicked", g_cclosure_new_swap(G_CALLBACK(gtk_widget_hide), window, NULL), FALSE);
	g_signal_connect(G_OBJECT(stop_btn), "clicked", G_CALLBACK(summary_search_stop_clicked), NULL);
...
static gboolean summary_search_prepare_matcher()
{
	...
	if (search_window.advsearch == NULL) {
		...
		advsearch_set_on_progress_cb(search_window.advsearch, summary_search_progress_cb, NULL);
	}

	adv_search = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(search_window.adv_search_checkbtn));
	...
	advsearch_set(search_window.advsearch, ADVANCED_SEARCH_EXTENDED, matcher_str);
...
static gboolean summary_search_prereduce_msg_list()
{
	...
	result = advsearch_search_msgs_in_folders(search_window.advsearch, &msglist, item, FALSE);
	statusbar_pop_all();
...
static void summary_search_execute(gboolean backward, gboolean search_all)
{
	...
	for (; search_window.is_searching; i++) {
		if (!node) {
			...
			if (all_searched) {
				alertpanel_full(_("Search failed"), _("Search string not found."), GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
				break;
			}
...
static gboolean key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
	...
	if (event && (event->keyval == GDK_KEY_Down || event->keyval == GDK_KEY_Up)) {
		if (search_window.from_entry_has_focus) {
			combobox_set_value_from_arrow_key(GTK_COMBO_BOX(search_window.from_entry), event->keyval);
			return TRUE;
		}
		if (search_window.to_entry_has_focus) {
			combobox_set_value_from_arrow_key(GTK_COMBO_BOX(search_window.to_entry), event->keyval);
			return TRUE;
		}
		if (search_window.subject_entry_has_focus) {
			combobox_set_value_from_arrow_key(GTK_COMBO_BOX(search_window.subject_entry), event->keyval);
			return TRUE;
		}
		if (search_window.body_entry_has_focus) {
		...
		if (search_window.adv_condition_entry_has_focus) {
			combobox_set_value_from_arrow_key(GTK_COMBO_BOX(search_window.adv_condition_entry), event->keyval);
			return TRUE;
		}
	}
