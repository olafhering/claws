...
static int etpan_thread_manager_op_schedule(struct etpan_thread_manager *manager, struct etpan_thread_op *op);
...
static struct etpan_thread *etpan_thread_manager_create_thread(struct etpan_thread_manager *manager)
{
	struct etpan_thread *thread;
	int r;
...
static void etpan_thread_manager_terminate_thread(struct etpan_thread_manager *manager, struct etpan_thread *thread)
{
	unsigned int i;
	int r;
...
static void loop_thread_list(carray *op_to_notify, carray *thread_list)
{
	unsigned int i;
	int r;
...
static int etpan_thread_manager_is_stopped(struct etpan_thread_manager *manager)
{
	return ((carray_count(manager->thread_pending) == 0) && (carray_count(manager->thread_pool) == 0));
}
