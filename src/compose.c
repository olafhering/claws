...
static Compose *compose_create(PrefsAccount *account, FolderItem *item, ComposeMode mode, gboolean batch);

static void compose_entry_indicate(Compose *compose, const gchar *address);
static Compose *compose_followup_and_reply_to(MsgInfo *msginfo, ComposeQuoteMode quote_mode, gboolean to_all, gboolean to_sender, const gchar *body);
static Compose *compose_forward_multiple(PrefsAccount *account, GSList *msginfo_list);
static Compose *compose_reply(MsgInfo *msginfo, ComposeQuoteMode quote_mode, gboolean to_all, gboolean to_ml, gboolean to_sender, const gchar *body);
...
static gchar *compose_quote_fmt(Compose *compose, MsgInfo *msginfo, const gchar *fmt, const gchar *qmark, const gchar *body, gboolean rewrap, gboolean need_unescape, const gchar *err_msg);

static void compose_reply_set_entry(Compose *compose, MsgInfo *msginfo, gboolean to_all, gboolean to_ml, gboolean to_sender, gboolean followup_and_reply_to);
...
static gboolean compose_attach_append(Compose *compose, const gchar *file, const gchar *type, const gchar *content_type, const gchar *charset);
...
static void compose_select_account(Compose *compose, PrefsAccount *account, gboolean init);
...
static ComposeQueueResult compose_queue_sub(Compose *compose, gint *msgnum, FolderItem **item, gchar **msgpath, gboolean perform_checks, gboolean remove_reedit_target);
...
static void compose_convert_header(Compose *compose, gchar *dest, gint len, gchar *src, gint header_len, gboolean addr_field);
...
static void compose_template_apply(Compose *compose, Template *tmpl, gboolean replace);
...
static gboolean attach_property_key_pressed(GtkWidget *widget, GdkEventKey *event, gboolean *cancelled);

static void compose_exec_ext_editor(Compose *compose);
static gboolean compose_ext_editor_kill(Compose *compose);
static void compose_ext_editor_closed_cb(GPid pid, gint exit_status, gpointer data);
static void compose_set_ext_editor_sensitive(Compose *compose, gboolean sensitive);
...
static void compose_undo_state_changed(UndoMain *undostruct, gint undo_state, gint redo_state, gpointer data);

static void compose_create_header_entry(Compose *compose);
static void compose_add_header_entry(Compose *compose, const gchar *header, gchar *text, ComposePrefType pref_type);
...
static void compose_notebook_size_alloc(GtkNotebook *notebook, GtkAllocation *allocation, GtkPaned *paned);
static gboolean compose_edit_size_alloc(GtkEditable *widget, GtkAllocation *allocation, GtkSHRuler *shruler);
static void account_activated(GtkComboBox *optmenu, gpointer data);
static void attach_selected(GtkTreeView *tree_view, GtkTreePath *tree_path, GtkTreeViewColumn *column, Compose *compose);
static gboolean attach_button_pressed(GtkWidget *widget, GdkEventButton *event, gpointer data);
static gboolean attach_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data);
...
static gint compose_delete_cb(GtkWidget *widget, GdkEventAny *event, gpointer data);
...
static void compose_advanced_action_cb(GtkAction *action, gpointer data);
...
static void compose_toggle_autowrap_cb(GtkToggleAction *action, gpointer data);
static void compose_toggle_autoindent_cb(GtkToggleAction *action, gpointer data);
...
static void compose_attach_drag_received_cb(GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, GtkSelectionData *data, guint info, guint time, gpointer user_data);
static void compose_insert_drag_received_cb(GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, GtkSelectionData *data, guint info, guint time, gpointer user_data);
static void compose_header_drag_received_cb(GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, GtkSelectionData *data, guint info, guint time, gpointer user_data);

static gboolean compose_drag_drop(GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, guint time, gpointer user_data);
static gboolean completion_set_focus_to_subject(GtkWidget *widget, GdkEventKey *event, Compose *user_data);

static void text_inserted(GtkTextBuffer *buffer, GtkTextIter *iter, const gchar *text, gint len, Compose *compose);
static Compose *compose_generic_reply(MsgInfo *msginfo, ComposeQuoteMode quote_mode, gboolean to_all, gboolean to_ml, gboolean to_sender, gboolean followup_and_reply_to, const gchar *body);

static void compose_headerentry_changed_cb(GtkWidget *entry, ComposeHeaderEntry *headerentry);
static gboolean compose_headerentry_key_press_event_cb(GtkWidget *entry, GdkEventKey *event, ComposeHeaderEntry *headerentry);
static gboolean compose_headerentry_button_clicked_cb(GtkWidget *button, ComposeHeaderEntry *headerentry);
...
#ifdef USE_ENCHANT
static void compose_set_dictionaries_from_folder_prefs(Compose *compose, FolderItem *folder_item);
#endif
static void compose_attach_update_label(Compose *compose);
static void compose_set_folder_prefs(Compose *compose, FolderItem *folder, gboolean respect_default_to);
...
static gboolean compose_put_existing_to_front(MsgInfo *info)
{
	...
	if (compose_list) {
		for (elem = compose_list; elem != NULL && elem->data != NULL; elem = elem->next) {
			...
			if (!c->targetinfo || !c->targetinfo->msgid || !info->msgid)
				continue;
...
static void compose_create_tags(GtkTextView *text, Compose *compose)
{
	...
	if (prefs_common.enable_color) {
		...
		gtkut_convert_int_to_gdk_color(prefs_common.color[COL_QUOTE_LEVEL1], &quote_color1);
		gtkut_convert_int_to_gdk_color(prefs_common.color[COL_QUOTE_LEVEL2], &quote_color2);
		gtkut_convert_int_to_gdk_color(prefs_common.color[COL_QUOTE_LEVEL3], &quote_color3);
		gtkut_convert_int_to_gdk_color(prefs_common.color[COL_QUOTE_LEVEL1_BG], &quote_bgcolor1);
		gtkut_convert_int_to_gdk_color(prefs_common.color[COL_QUOTE_LEVEL2_BG], &quote_bgcolor2);
		gtkut_convert_int_to_gdk_color(prefs_common.color[COL_QUOTE_LEVEL3_BG], &quote_bgcolor3);
		...
		signature_color = quote_color1 = quote_color2 = quote_color3 = quote_bgcolor1 = quote_bgcolor2 = quote_bgcolor3 = uri_color = black;
	}

	if (prefs_common.enable_color && prefs_common.enable_bgcolor) {
		compose->quote0_tag = gtk_text_buffer_create_tag(buffer, "quote0", "foreground-gdk", &quote_color1, "paragraph-background-gdk", &quote_bgcolor1, NULL);
		compose->quote1_tag = gtk_text_buffer_create_tag(buffer, "quote1", "foreground-gdk", &quote_color2, "paragraph-background-gdk", &quote_bgcolor2, NULL);
		compose->quote2_tag = gtk_text_buffer_create_tag(buffer, "quote2", "foreground-gdk", &quote_color3, "paragraph-background-gdk", &quote_bgcolor3, NULL);
	} else {
		compose->quote0_tag = gtk_text_buffer_create_tag(buffer, "quote0", "foreground-gdk", &quote_color1, NULL);
		compose->quote1_tag = gtk_text_buffer_create_tag(buffer, "quote1", "foreground-gdk", &quote_color2, NULL);
		compose->quote2_tag = gtk_text_buffer_create_tag(buffer, "quote2", "foreground-gdk", &quote_color3, NULL);
	}

	compose->signature_tag = gtk_text_buffer_create_tag(buffer, "signature", "foreground-gdk", &signature_color, NULL);

	compose->uri_tag = gtk_text_buffer_create_tag(buffer, "link", "foreground-gdk", &uri_color, NULL);
...
static void compose_set_save_to(Compose *compose, const gchar *folderidentifier)
{
	...
	if (folderidentifier) {
		...
		prefs_common.compose_save_to_history = add_history(prefs_common.compose_save_to_history, folderidentifier);
		combobox_set_popdown_strings(GTK_COMBO_BOX_TEXT(compose->savemsg_combo), prefs_common.compose_save_to_history);
	}
...
static gchar *compose_get_save_to(Compose *compose)
{
	...
	if (result) {
		combobox_unset_popdown_strings(GTK_COMBO_BOX_TEXT(compose->savemsg_combo));
		prefs_common.compose_save_to_history = add_history(prefs_common.compose_save_to_history, result);
		combobox_set_popdown_strings(GTK_COMBO_BOX_TEXT(compose->savemsg_combo), prefs_common.compose_save_to_history);
	}
	return result;
}

Compose *compose_generic_new(PrefsAccount *account, const gchar *mailto, FolderItem *item, GList *attach_files, GList *listAddress)
{
	Compose *compose;
	...
	if (privacy_system_can_sign(compose->privacy_system) == FALSE && (account->default_encrypt || account->default_sign))
	...
	if (item && item->prefs && item->prefs->compose_with_format && item->prefs->compose_override_from_format && *item->prefs->compose_override_from_format != '\0') {
...
#ifdef USE_ENCHANT
		quote_fmt_init(dummyinfo, NULL, NULL, FALSE, compose->account, FALSE, compose->gtkaspell);
#else
	...
	if (subject_format || body_format) {
		...
		if (subject_format && *subject_format != '\0') {
			gchar *subject = NULL;
			gchar *tmp = NULL;
			gchar *buf = NULL;
...
#ifdef USE_ENCHANT
			quote_fmt_init(dummyinfo, NULL, subject, FALSE, compose->account, FALSE, compose->gtkaspell);
#else
		...
		if (body_format && *body_format != '\0') {
			...
			compose_quote_fmt(compose, dummyinfo, body_format, NULL, tmp, FALSE, TRUE, _("The body of the \"New message\" template has an error at line %d."));
			compose_attach_from_list(compose, quote_fmt_get_attachments_list(), FALSE);
			quote_fmt_reset_vartable();
	...
	if (attach_files) {
		...
		for (curr = attach_files; curr != NULL; curr = curr->next) {
			...
			if (ainfo->insert)
				...
				compose_attach_append(compose, ainfo->file, ainfo->file, ainfo->content_type, ainfo->charset);
		}
	}
...
static void compose_force_encryption(Compose *compose, PrefsAccount *account, gboolean override_pref, const gchar *system)
{
	...
	if (privacy_system_can_encrypt(compose->privacy_system) == FALSE || (override_pref == FALSE && account->default_encrypt_reply == FALSE))
		return;
...
static Compose *compose_reply_mode(ComposeMode mode, GSList *msginfo_list, gchar *body)
{
	...
	case COMPOSE_REPLY_TO_ADDRESS:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_CHECK, FALSE, prefs_common.default_reply_list, FALSE, body);
		break;
	case COMPOSE_REPLY_WITH_QUOTE:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_FORCED, FALSE, prefs_common.default_reply_list, FALSE, body);
		break;
	case COMPOSE_REPLY_WITHOUT_QUOTE:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_SKIP, FALSE, prefs_common.default_reply_list, FALSE, NULL);
		break;
	case COMPOSE_REPLY_TO_SENDER:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_CHECK, FALSE, FALSE, TRUE, body);
		break;
	case COMPOSE_FOLLOWUP_AND_REPLY_TO:
		compose = compose_followup_and_reply_to(msginfo, COMPOSE_QUOTE_CHECK, FALSE, FALSE, body);
		break;
	case COMPOSE_REPLY_TO_SENDER_WITH_QUOTE:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_FORCED, FALSE, FALSE, TRUE, body);
		break;
	case COMPOSE_REPLY_TO_SENDER_WITHOUT_QUOTE:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_SKIP, FALSE, FALSE, TRUE, NULL);
		break;
	case COMPOSE_REPLY_TO_ALL:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_CHECK, TRUE, FALSE, FALSE, body);
		break;
	case COMPOSE_REPLY_TO_ALL_WITH_QUOTE:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_FORCED, TRUE, FALSE, FALSE, body);
		break;
	case COMPOSE_REPLY_TO_ALL_WITHOUT_QUOTE:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_SKIP, TRUE, FALSE, FALSE, NULL);
		break;
	case COMPOSE_REPLY_TO_LIST:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_CHECK, FALSE, TRUE, FALSE, body);
		break;
	case COMPOSE_REPLY_TO_LIST_WITH_QUOTE:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_FORCED, FALSE, TRUE, FALSE, body);
		break;
	case COMPOSE_REPLY_TO_LIST_WITHOUT_QUOTE:
		compose = compose_reply(msginfo, COMPOSE_QUOTE_SKIP, FALSE, TRUE, FALSE, NULL);
		break;
...
static Compose *compose_reply(MsgInfo *msginfo, ComposeQuoteMode quote_mode, gboolean to_all, gboolean to_ml, gboolean to_sender, const gchar *body)
{
	return compose_generic_reply(msginfo, quote_mode, to_all, to_ml, to_sender, FALSE, body);
}

static Compose *compose_followup_and_reply_to(MsgInfo *msginfo, ComposeQuoteMode quote_mode, gboolean to_all, gboolean to_sender, const gchar *body)
{
	return compose_generic_reply(msginfo, quote_mode, to_all, FALSE, to_sender, TRUE, body);
}
...
static Compose *compose_generic_reply(MsgInfo *msginfo, ComposeQuoteMode quote_mode, gboolean to_all, gboolean to_ml, gboolean to_sender, gboolean followup_and_reply_to, const gchar *body)
{
	...
	/* override from name according to folder properties */
	if (msginfo->folder && msginfo->folder->prefs && msginfo->folder->prefs->reply_with_format && msginfo->folder->prefs->reply_override_from_format && *msginfo->folder->prefs->reply_override_from_format != '\0') {
...
#ifdef USE_ENCHANT
		quote_fmt_init(compose->replyinfo, NULL, NULL, FALSE, compose->account, FALSE, compose->gtkaspell);
#else
	...
	if (quote_mode == COMPOSE_QUOTE_FORCED || (quote_mode == COMPOSE_QUOTE_CHECK && prefs_common.reply_with_quote)) {
		...
		quote = TRUE;
		if (msginfo->folder && msginfo->folder->prefs && msginfo->folder->prefs->reply_with_format) {
	...
	if (quote) {
		...
		if (qmark == NULL || *qmark == '\0')
			qmark = "> ";
		compose_quote_fmt(compose, compose->replyinfo, body_fmt, qmark, body, FALSE, TRUE, _("The body of the \"Reply\" template has an error at line %d."));
		compose_attach_from_list(compose, quote_fmt_get_attachments_list(), FALSE);
		quote_fmt_reset_vartable();
	}
	...
	if (privacy_system_can_sign(compose->privacy_system) == FALSE && ((account->default_encrypt || account->default_sign) || (account->default_encrypt_reply && MSG_IS_ENCRYPTED(compose->replyinfo->flags)) || (account->default_sign_reply && MSG_IS_SIGNED(compose->replyinfo->flags))))
		COMPOSE_PRIVACY_WARNING();
...
Compose *compose_forward(PrefsAccount *account, MsgInfo *msginfo, gboolean as_attach, const gchar *body, gboolean no_extedit, gboolean batch)
{
	...
	/* override from name according to folder properties */
	if (msginfo->folder && msginfo->folder->prefs && msginfo->folder->prefs->forward_with_format && msginfo->folder->prefs->forward_override_from_format && *msginfo->folder->prefs->forward_override_from_format != '\0') {
...
#ifdef USE_ENCHANT
		gtkaspell_block_check(compose->gtkaspell);
		quote_fmt_init(full_msginfo, NULL, NULL, FALSE, compose->account, FALSE, compose->gtkaspell);
#else
	...
	if (as_attach) {
		...
		if (!is_file_exist(msgfile))
			...
			compose_attach_append(compose, msgfile, msgfile, "message/rfc822", NULL);
		...
		/* use the forward format of folder (if enabled), or the account's one
		   ...
		   enabled (even if empty), and use the relevant quotemark */
		if (msginfo->folder && msginfo->folder->prefs && msginfo->folder->prefs->forward_with_format) {
		...
		compose_quote_fmt(compose, full_msginfo, body_fmt, qmark, body, FALSE, TRUE, _("The body of the \"Forward\" template has an error at line %d."));
		compose_attach_from_list(compose, quote_fmt_get_attachments_list(), FALSE);
		quote_fmt_reset_vartable();
...
static Compose *compose_forward_multiple(PrefsAccount *account, GSList *msginfo_list)
{
	...
	if (!account && !(account = compose_find_account(msginfo_list->data)))
		account = cur_account;
...
#ifdef USE_ENCHANT
			quote_fmt_init(msginfo, NULL, NULL, FALSE, compose->account, FALSE, compose->gtkaspell);
#else
	...
	if (single_mail) {
		...
		gtk_entry_set_text(GTK_ENTRY(compose->subject_entry), _("Fw: multiple emails"));
	}
...
static gboolean compose_is_sig_separator(Compose *compose, GtkTextBuffer *textbuf, GtkTextIter *iter)
{
	...
	gtk_text_buffer_get_iter_at_offset(textbuf, &end_iter, start_pos + strlen(compose->account->sig_sep));
	...
	if (!strcmp(str, compose->account->sig_sep)) {
		gchar *tmp = NULL;
		/* check end of line (\n) */
		gtk_text_buffer_get_iter_at_offset(textbuf, &start, start_pos + strlen(compose->account->sig_sep));
		gtk_text_buffer_get_iter_at_offset(textbuf, &end_iter, start_pos + strlen(compose->account->sig_sep) + 1);
...
Compose *compose_reedit(MsgInfo *msginfo, gboolean batch)
{
	...
	if (folder_has_parent_of_type(msginfo->folder, F_QUEUE) || folder_has_parent_of_type(msginfo->folder, F_DRAFT) || folder_has_parent_of_type(msginfo->folder, F_OUTBOX)) {
		gchar *queueheader_buf = NULL;
		gint id, param;

		/* Select Account from queue headers */
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Claws-Account-Id:")) {
			id = atoi(&queueheader_buf[strlen("X-Claws-Account-Id:")]);
			account = account_find_from_id(id);
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Sylpheed-Account-Id:")) {
			id = atoi(&queueheader_buf[strlen("X-Sylpheed-Account-Id:")]);
			account = account_find_from_id(id);
			g_free(queueheader_buf);
		}
		if (!account && !procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "NAID:")) {
			id = atoi(&queueheader_buf[strlen("NAID:")]);
			account = account_find_from_id(id);
			g_free(queueheader_buf);
		}
		if (!account && !procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "MAID:")) {
			id = atoi(&queueheader_buf[strlen("MAID:")]);
			account = account_find_from_id(id);
			g_free(queueheader_buf);
		}
		if (!account && !procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "S:")) {
			account = account_find_from_address(queueheader_buf, FALSE);
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Claws-Sign:")) {
			param = atoi(&queueheader_buf[strlen("X-Claws-Sign:")]);
			use_signing = param;
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Sylpheed-Sign:")) {
		...
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Sylpheed-Encrypt:")) {
			param = atoi(&queueheader_buf[strlen("X-Sylpheed-Encrypt:")]);
			use_encryption = param;
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Claws-Auto-Wrapping:")) {
			param = atoi(&queueheader_buf[strlen("X-Claws-Auto-Wrapping:")]);
			autowrap = param;
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Claws-Auto-Indent:")) {
			param = atoi(&queueheader_buf[strlen("X-Claws-Auto-Indent:")]);
			autoindent = param;
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Claws-Privacy-System:")) {
			privacy_system = g_strdup(&queueheader_buf[strlen("X-Claws-Privacy-System:")]);
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Sylpheed-Privacy-System:")) {
			privacy_system = g_strdup(&queueheader_buf[strlen("X-Sylpheed-Privacy-System:")]);
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Priority: ")) {
			param = atoi(&queueheader_buf[strlen("X-Priority: ")]);	/* mind the space */
			priority = param;
			g_free(queueheader_buf);
		}
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "RMID:")) {
		...
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "FMID:")) {
		...
		/* Get manual headers */
		if (!procheader_get_header_from_msginfo(msginfo, &queueheader_buf, "X-Claws-Manual-Headers:")) {
	...
	g_signal_handlers_block_by_func(G_OBJECT(textbuf), G_CALLBACK(compose_changed_cb), compose);
	...
	g_signal_handlers_unblock_by_func(G_OBJECT(textbuf), G_CALLBACK(compose_changed_cb), compose);
...
Compose *compose_redirect(PrefsAccount *account, MsgInfo *msginfo, gboolean batch)
{
	...
	if (msginfo->subject)
		gtk_entry_set_text(GTK_ENTRY(compose->subject_entry), msginfo->subject);
	gtk_editable_set_editable(GTK_EDITABLE(compose->subject_entry), FALSE);

	compose_quote_fmt(compose, msginfo, "%M", NULL, NULL, FALSE, FALSE, _("The body of the \"Redirect\" template has an error at line %d."));
	quote_fmt_reset_vartable();
...
static void compose_entry_indicate(Compose *compose, const gchar *mailto)
{
	...
	for (h_list = compose->header_list; h_list != NULL; h_list = h_list->next) {
		...
		if (gtk_entry_get_text(entry) && !g_utf8_collate(gtk_entry_get_text(entry), mailto)) {
			gtk_widget_modify_base(GTK_WIDGET(((ComposeHeaderEntry *)h_list->data)->entry), GTK_STATE_NORMAL, &default_header_bgcolor);
			gtk_widget_modify_text(GTK_WIDGET(((ComposeHeaderEntry *)h_list->data)->entry), GTK_STATE_NORMAL, &default_header_color);
		}
	}
}
...
static MailField compose_entries_set(Compose *compose, const gchar *mailto, ComposeEntryType to_type)
{
	...
	if (attach) {
		...
		if (warn_files) {
			alertpanel_notice(ngettext("The following file has been attached: \n%s", "The following files have been attached: \n%s", att), warn_files);
			g_free(warn_files);
		}
	}
	if (inreplyto)
...
static gint compose_parse_header(Compose *compose, MsgInfo *msginfo)
{
	...
	if (hentry[H_REPLY_TO].body != NULL) {
		if (hentry[H_REPLY_TO].body[0] != '\0') {
			compose->replyto = conv_unmime_header(hentry[H_REPLY_TO].body, NULL, TRUE);
		}
	...
	if (hentry[H_REFERENCES].body != NULL) {
		if (compose->mode == COMPOSE_REEDIT)
			...
			compose->references = compose_parse_references(hentry[H_REFERENCES].body, msginfo->msgid);
	...
	if (hentry[H_BCC].body != NULL) {
		if (compose->mode == COMPOSE_REEDIT)
			compose->bcc = conv_unmime_header(hentry[H_BCC].body, NULL, TRUE);
	...
	if (hentry[H_FOLLOWUP_TO].body != NULL) {
		if (hentry[H_FOLLOWUP_TO].body[0] != '\0') {
			compose->followup_to = conv_unmime_header(hentry[H_FOLLOWUP_TO].body, NULL, TRUE);
		}
	...
	if (hentry[H_LIST_POST].body != NULL) {
		...
		if (hentry[H_LIST_POST].body[0] != '\0') {
			...
			scan_mailto_url(start ? start : hentry[H_LIST_POST].body, NULL, &to, NULL, NULL, NULL, NULL, NULL, NULL);
	...
	if (compose->mode == COMPOSE_REEDIT) {
		...
		if (msginfo->msgid && *msginfo->msgid && compose->folder != NULL && compose->folder->stype == F_DRAFT)
			compose->msgid = g_strdup(msginfo->msgid);
	} else {
		if (msginfo->msgid && *msginfo->msgid && (compose->mode != COMPOSE_FORWARD && compose->mode != COMPOSE_FORWARD_INLINE && compose->mode != COMPOSE_FORWARD_AS_ATTACH))
			compose->inreplyto = g_strdup(msginfo->msgid);

		if (!compose->references) {
			if (msginfo->msgid && *msginfo->msgid) {
				if (msginfo->inreplyto && *msginfo->inreplyto)
					compose->references = g_strdup_printf("<%s>\n\t<%s>", msginfo->inreplyto, msginfo->msgid);
				else
					compose->references = g_strconcat("<", msginfo->msgid, ">", NULL);
			} else if (msginfo->inreplyto && *msginfo->inreplyto) {
				compose->references = g_strconcat("<", msginfo->inreplyto, ">", NULL);
			}
		}
	}
...
static gchar *compose_parse_references(const gchar *ref, const gchar *msgid)
{
	...
	for (;;) {
		...
		if (len > MAX_REFERENCES_LEN) {
			...
			if (ref_id_list && ref_id_list->next && ref_id_list->next->next) {
				g_free(ref_id_list->next->data);
				ref_id_list = g_slist_remove(ref_id_list, ref_id_list->next->data);
			} else {
...
static gchar *compose_quote_fmt(Compose *compose, MsgInfo *msginfo, const gchar *fmt, const gchar *qmark, const gchar *body, gboolean rewrap, gboolean need_unescape, const gchar *err_msg)
{
...
#ifdef USE_ENCHANT
		quote_fmt_init(msginfo, quote_str, trimmed_body, FALSE, compose->account, FALSE, compose->gtkaspell);
#else
	...
	if (g_utf8_validate(buf, -1, NULL)) {
		...
		gchar *tmpout = NULL;
		tmpout = conv_codeset_strdup(buf, conv_get_locale_charset_str_no_utf8(), CS_INTERNAL);
...
static gboolean is_subscription(const gchar *ml_post, const gchar *from)
{
	...
	if (right_ml && right_from && !strncmp(left_from, left_ml, strlen(left_ml))
...
static void compose_set_folder_prefs(Compose *compose, FolderItem *folder, gboolean respect_default_to)
{
	...
	if (respect_default_to && folder->prefs->enable_default_to) {
		compose_entry_append(compose, folder->prefs->default_to, COMPOSE_TO, PREF_FOLDER);
		compose_entry_indicate(compose, folder->prefs->default_to);
	}
	if (folder->prefs->enable_default_cc) {
		compose_entry_append(compose, folder->prefs->default_cc, COMPOSE_CC, PREF_FOLDER);
		compose_entry_indicate(compose, folder->prefs->default_cc);
	}
	if (folder->prefs->enable_default_bcc) {
		compose_entry_append(compose, folder->prefs->default_bcc, COMPOSE_BCC, PREF_FOLDER);
		compose_entry_indicate(compose, folder->prefs->default_bcc);
	}
	if (folder->prefs->enable_default_replyto) {
		compose_entry_append(compose, folder->prefs->default_replyto, COMPOSE_REPLYTO, PREF_FOLDER);
...
static void compose_reply_set_entry(Compose *compose, MsgInfo *msginfo, gboolean to_all, gboolean to_ml, gboolean to_sender, gboolean followup_and_reply_to)
{
	...
	default_reply_to = msginfo->folder && msginfo->folder->prefs->enable_default_reply_to;
	...
	if (compose->account->protocol != A_NNTP) {
		...
		if (reply_to_ml && !default_reply_to) {
			...
			if (!is_subscr) {
				...
				compose_entry_append(compose, compose->ml_post, COMPOSE_TO, PREF_ML);
				if (compose->replyto)
					compose_entry_append(compose, compose->replyto, COMPOSE_CC, PREF_ML);
			} else {
				/* answer to subscription confirmation */
				if (compose->replyto)
					compose_entry_append(compose, compose->replyto, COMPOSE_TO, PREF_ML);
				else if (msginfo->from)
					compose_entry_append(compose, msginfo->from, COMPOSE_TO, PREF_ML);
			}
		} else if (!(to_all || to_sender) && default_reply_to) {
			compose_entry_append(compose, msginfo->folder->prefs->default_reply_to, COMPOSE_TO, PREF_FOLDER);
			compose_entry_indicate(compose, msginfo->folder->prefs->default_reply_to);
		} else {
			gchar *tmp1 = NULL;
			if (!msginfo->from)
				return;
			if (to_sender)
				compose_entry_append(compose, msginfo->from, COMPOSE_TO, PREF_NONE);
			else if (to_all) {
				Xstrdup_a(tmp1, msginfo->from, return);
				extract_address(tmp1);
				compose_entry_append(compose, (!account_find_from_address(tmp1, FALSE))
						     ? msginfo->from : msginfo->to, COMPOSE_TO, PREF_NONE);
				if (compose->replyto)
					compose_entry_append(compose, compose->replyto, COMPOSE_CC, PREF_NONE);
			} else {
				if (!folder_has_parent_of_type(msginfo->folder, F_QUEUE) && !folder_has_parent_of_type(msginfo->folder, F_OUTBOX) && !folder_has_parent_of_type(msginfo->folder, F_DRAFT)) {
					if (compose->replyto) {
						compose_entry_append(compose, compose->replyto, COMPOSE_TO, PREF_NONE);
					} else {
						compose_entry_append(compose, msginfo->from ? msginfo->from : "", COMPOSE_TO, PREF_NONE);
					}
				} else {
					/* replying to own mail, use original recp */
					compose_entry_append(compose, msginfo->to ? msginfo->to : "", COMPOSE_TO, PREF_NONE);
					compose_entry_append(compose, msginfo->cc ? msginfo->cc : "", COMPOSE_CC, PREF_NONE);
				}
			}
		}
	} else {
		if (to_sender || (compose->followup_to && !strncmp(compose->followup_to, "poster", 6)))
			compose_entry_append(compose, (compose->replyto ? compose->replyto : msginfo->from ? msginfo->from : ""), COMPOSE_TO, PREF_NONE);

		else if (followup_and_reply_to || to_all) {
			compose_entry_append(compose, (compose->replyto ? compose->replyto : msginfo->from ? msginfo->from : ""), COMPOSE_TO, PREF_NONE);

			compose_entry_append(compose, compose->followup_to ? compose->followup_to : compose->newsgroups ? compose->newsgroups : "", COMPOSE_NEWSGROUPS, PREF_NONE);
	...
	if (to_ml && compose->ml_post)
		return;
	if (!to_all || compose->account->protocol == A_NNTP)
		return;
	...
	if (to_all && msginfo->folder && msginfo->folder->prefs->enable_default_reply_to)
		cc_list = address_list_append_with_comments(cc_list, msginfo->folder->prefs->default_reply_to);
	...
	if (cc_list) {
		for (cur = cc_list; cur != NULL; cur = cur->next) {
			...
			if (strcmp(ac_email, addr))
				compose_entry_append(compose, (gchar *)cur->data, COMPOSE_CC, PREF_NONE);
			else
...
static void compose_insert_sig(Compose *compose, gboolean replace)
{
	...
	g_signal_handlers_block_by_func(G_OBJECT(buffer), G_CALLBACK(compose_changed_cb), compose);
	...
	if (replace) {
		...
		if (!exists || compose->sig_str[0] == '\0')
			...
			found = gtk_text_iter_forward_to_tag_toggle(&first_iter, compose->signature_tag);
		...
		if (found) {
			...
			end_iter = first_iter;
			/* skip re-start */
			found = gtk_text_iter_forward_to_tag_toggle(&end_iter, compose->signature_tag);
			found &= gtk_text_iter_forward_to_tag_toggle(&end_iter, compose->signature_tag);
			if (found) {
	...
	if (compose->set_cursor_pos < 0)
		...
		gtk_text_buffer_get_iter_at_offset(buffer, &iter, compose->set_cursor_pos);

	compose->set_cursor_pos = -1;
	gtk_text_buffer_place_cursor(buffer, &iter);
	g_signal_handlers_unblock_by_func(G_OBJECT(buffer), G_CALLBACK(compose_changed_cb), compose);
...
static ComposeInsertResult compose_insert_file(Compose *compose, const gchar *file)
{
	...
	if (ret != 0) {
		...
		if (prefs_common.warn_large_insert_size < 0 || size > ((goffset) prefs_common.warn_large_insert_size * 1024)) {
			AlertValue aval;
			gchar *msg;

			msg = g_strdup_printf(_("You are about to insert a file of %s " "in the message body. Are you sure you want to do that?"), to_human_readable(size));
			aval = alertpanel_full(_("Are you sure?"), msg, GTK_STOCK_CANCEL, _("_Insert"), NULL, ALERTFOCUS_SECOND, TRUE, NULL, ALERT_QUESTION);
			g_free(msg);
	...
	g_signal_handlers_block_by_func(G_OBJECT(buffer), G_CALLBACK(text_inserted), compose);
	...
	if (result == COMPOSE_INSERT_SUCCESS) {
		...
		g_signal_handlers_unblock_by_func(G_OBJECT(buffer), G_CALLBACK(text_inserted), compose);
		compose->autowrap = prev_autowrap;
		if (compose->autowrap)
			compose_wrap_all(compose);
	} else {
...
static gboolean compose_attach_append(Compose *compose, const gchar *file, const gchar *filename, const gchar *content_type, const gchar *charset)
{
	AttachInfo *ainfo;
	GtkTreeIter iter;
	FILE *fp;
	...
	if (!is_file_exist(file)) {
		...
		if (file_from_uri && is_file_exist(file_from_uri)) {
			result = compose_attach_append(compose, file_from_uri, filename, content_type, charset);
		}
	...
	auto_ainfo = g_auto_pointer_new_with_free(ainfo, (GFreeFunc) compose_attach_info_free);
	...
	if (ainfo->name != NULL && !strcmp(ainfo->name, ".")) {
		g_free(ainfo->name);
		ainfo->name = NULL;
	}
	...
	store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(compose->attach_clist)));

	gtk_list_store_append(store, &iter);
	gtk_list_store_set(store, &iter, COL_MIMETYPE, ainfo->content_type, COL_SIZE, size_text, COL_NAME, ainfo->name, COL_CHARSET, ainfo->charset, COL_DATA, ainfo, COL_AUTODATA, auto_ainfo, -1);
...
static void compose_attach_parts(Compose *compose, MsgInfo *msginfo)
{
	...
	if (child) {
		...
		} else if (compose->mode == COMPOSE_REEDIT && child->type == MIMETYPE_APPLICATION && !g_ascii_strcasecmp(child->subtype, "pgp-encrypted")) {
	...
	while (child != NULL) {
		...
		if ((err = procmime_get_part(outfile, child)) < 0)
			 ...
			 * we force signing. */
			if ((strcmp(content_type, "application/pgp-signature") && strcmp(content_type, "application/pkcs7-signature") && strcmp(content_type, "application/x-pkcs7-signature"))
				...
				if (partname == NULL)
					partname = "";
				compose_attach_append(compose, outfile, partname, content_type, procmime_mimeinfo_get_parameter(child, "charset"));
			} else {
...
static gchar *compose_get_quote_str(GtkTextBuffer *buffer, const GtkTextIter *start, gint *len)
{
...
static int compose_itemized_length(GtkTextBuffer *buffer, const GtkTextIter *start)
{
	GtkTextIter iter = *start;
	gunichar wc;
	gchar ch[6];
	...
	clen = g_unichar_to_utf8(wc, ch);
	if (!((clen == 1 && strchr("*-+", ch[0])) || (clen == 3 && (wc == 0x2022 || /* BULLET */
...
static gchar *compose_get_itemized_chars(GtkTextBuffer *buffer, const GtkTextIter *start)
{
	GtkTextIter iter = *start;
	gunichar wc;
	gint len = 0;
...
static int compose_left_offset_length(GtkTextBuffer *buffer, const GtkTextIter *start)
{
	GtkTextIter iter = *start;
	gunichar wc;
	gint len = 0;
...
static gboolean compose_get_line_break_pos(GtkTextBuffer *buffer, const GtkTextIter *start, GtkTextIter *break_pos, gint max_col, gint quote_len)
{
...
static gboolean compose_join_next_line(Compose *compose, GtkTextBuffer *buffer, GtkTextIter *iter, const gchar *quote_str)
{
	...
	if (!gtk_text_iter_forward_line(&iter_) || gtk_text_iter_ends_line(&iter_)) {
		return FALSE;
	}
	next_quote_str = compose_get_quote_str(buffer, &iter_, &quote_len);

	if ((quote_str || next_quote_str) && g_strcmp0(quote_str, next_quote_str) != 0) {
	...
	if (!attrs[1].is_line_break || (!g_unichar_iswide(wc1) || !g_unichar_iswide(wc2))) {
...
static gboolean compose_beautify_paragraph(Compose *compose, GtkTextIter *par_iter, gboolean force)
{
	...
	if (compose->draft_timeout_tag == COMPOSE_DRAFT_TIMEOUT_FORBIDDEN) {
		if (gtk_text_iter_ends_line(&iter)) {
			while (gtk_text_iter_ends_line(&iter) && gtk_text_iter_forward_line(&iter)) ;
		} else {
...
/*			debug_print("compose_beautify_paragraph(): quote_str = '%s'\n", quote_str); */
		...
		if (gtk_text_iter_ends_line(&iter)) {
			goto colorize;
		} else if (compose_get_line_break_pos(buffer, &iter, &break_pos, prefs_common.linewrap_len, quote_len)) {
			...
			if (quote_str)
				gtk_text_buffer_insert(buffer, &break_pos, quote_str, -1);
 ...
 colorize:
		...
		if (scanpos) {
			...
			if (parser[last_index].parse(walk, scanpos, (const gchar **)&bp, (const gchar **)&ep, FALSE)
		...
		if (startq_offset != -1) {
			...
			gtk_text_buffer_get_iter_at_offset(buffer, &startquote, startq_offset);
			endquote = iter;

			switch (quotelevel) {
			case 0:
				if (!gtk_text_iter_has_tag(&startquote, compose->quote0_tag) || !gtk_text_iter_has_tag(&end_of_line, compose->quote0_tag)) {
					gtk_text_buffer_apply_tag_by_name(buffer, "quote0", &startquote, &endquote);
					gtk_text_buffer_remove_tag_by_name(buffer, "quote1", &startquote, &endquote);
					gtk_text_buffer_remove_tag_by_name(buffer, "quote2", &startquote, &endquote);
					modified = TRUE;
				}
				break;
			case 1:
				if (!gtk_text_iter_has_tag(&startquote, compose->quote1_tag) || !gtk_text_iter_has_tag(&end_of_line, compose->quote1_tag)) {
					gtk_text_buffer_apply_tag_by_name(buffer, "quote1", &startquote, &endquote);
					gtk_text_buffer_remove_tag_by_name(buffer, "quote0", &startquote, &endquote);
					gtk_text_buffer_remove_tag_by_name(buffer, "quote2", &startquote, &endquote);
					modified = TRUE;
				}
				break;
			case 2:
				if (!gtk_text_iter_has_tag(&startquote, compose->quote2_tag) || !gtk_text_iter_has_tag(&end_of_line, compose->quote2_tag)) {
					gtk_text_buffer_apply_tag_by_name(buffer, "quote2", &startquote, &endquote);
					gtk_text_buffer_remove_tag_by_name(buffer, "quote0", &startquote, &endquote);
					gtk_text_buffer_remove_tag_by_name(buffer, "quote1", &startquote, &endquote);
					modified = TRUE;
				}
			...
			gtk_text_buffer_get_iter_at_offset(buffer, &startnoquote, noq_offset);
			endnoquote = iter;

			if ((gtk_text_iter_has_tag(&startnoquote, compose->quote0_tag)
			     && gtk_text_iter_has_tag(&end_of_line, compose->quote0_tag)) || (gtk_text_iter_has_tag(&startnoquote, compose->quote1_tag)
											      && gtk_text_iter_has_tag(&end_of_line, compose->quote1_tag)) || (gtk_text_iter_has_tag(&startnoquote, compose->quote2_tag)
																			       && gtk_text_iter_has_tag(&end_of_line, compose->quote2_tag))) {
				gtk_text_buffer_remove_tag_by_name(buffer, "quote0", &startnoquote, &endnoquote);
				gtk_text_buffer_remove_tag_by_name(buffer, "quote1", &startnoquote, &endnoquote);
				gtk_text_buffer_remove_tag_by_name(buffer, "quote2", &startnoquote, &endnoquote);
				modified = TRUE;
			}
			noq_offset = -1;
		}
		...
		if (uri_start != nouri_start && uri_stop != nouri_stop) {
			...
			gtk_text_buffer_get_iter_at_offset(buffer, &nouri_start_iter, nouri_start);
			gtk_text_buffer_get_iter_at_offset(buffer, &nouri_end_iter, nouri_stop);
			if (gtk_text_iter_has_tag(&nouri_start_iter, compose->uri_tag) && gtk_text_iter_has_tag(&nouri_end_iter, compose->uri_tag)) {
				gtk_text_buffer_remove_tag_by_name(buffer, "link", &nouri_start_iter, &nouri_end_iter);
		...
		if (uri_start >= 0 && uri_stop > 0) {
			...
			gtk_text_buffer_get_iter_at_offset(buffer, &uri_start_iter, uri_start);
			gtk_text_buffer_get_iter_at_offset(buffer, &uri_end_iter, uri_stop);
			back = uri_end_iter;
			gtk_text_iter_backward_char(&back);
			if (!gtk_text_iter_has_tag(&uri_start_iter, compose->uri_tag) || !gtk_text_iter_has_tag(&back, compose->uri_tag)) {
				gtk_text_buffer_apply_tag_by_name(buffer, "link", &uri_start_iter, &uri_end_iter);
				modified = TRUE;
...
static void compose_select_account(Compose *compose, PrefsAccount *account, gboolean init)
{
	...
	if (account->name && *account->name) {
		...
		from = g_strdup_printf("%s <%s>", qbuf, account->address);
		if (qbuf != buf)
			g_free(qbuf);
		gtk_entry_set_text(GTK_ENTRY(compose->from_name), from);
	} else {
		from = g_strdup_printf("<%s>", account->address);
		gtk_entry_set_text(GTK_ENTRY(compose->from_name), from);
	}
	...
	if (account->default_sign && privacy_system_can_sign(compose->privacy_system) && compose->mode != COMPOSE_REDIRECT)
	...
	if (account->default_encrypt && privacy_system_can_encrypt(compose->privacy_system) && compose->mode != COMPOSE_REDIRECT)
	...
	if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(header_entry->combo), &iter))
		gtk_tree_model_get(gtk_combo_box_get_model(GTK_COMBO_BOX(header_entry->combo)), &iter, COMBOBOX_TEXT, &header, -1);

	if (header && !strlen(gtk_entry_get_text(GTK_ENTRY(header_entry->entry)))) {
		if (account->protocol == A_NNTP) {
			if (!strcmp(header, _("To:")))
				combobox_select_by_text(GTK_COMBO_BOX(header_entry->combo), _("Newsgroups:"));
		} else {
			if (!strcmp(header, _("Newsgroups:")))
				combobox_select_by_text(GTK_COMBO_BOX(header_entry->combo), _("To:"));
		}
	...
	if (compose->gtkaspell) {
		if (account->enable_default_dictionary)
			gtkaspell_change_dict(compose->gtkaspell, account->default_dictionary, FALSE);
		if (account->enable_default_alt_dictionary)
			gtkaspell_change_alt_dict(compose->gtkaspell, account->default_alt_dictionary);
		if (account->enable_default_dictionary || account->enable_default_alt_dictionary)
...
static gboolean compose_check_for_set_recipients(Compose *compose)
{
	if (compose->account->set_autocc && compose->account->auto_cc) {
		...
		if (!found_other) {
			...
			text = g_strdup_printf(_("The only recipient is the default '%s' address. Send anyway?"), prefs_common_translated_header_name("Cc"));
			aval = alertpanel(_("Send"), text, GTK_STOCK_CANCEL, _("_Send"), NULL, ALERTFOCUS_SECOND);
			g_free(text);
			if (aval != G_ALERTALTERNATE)
				return FALSE;
		}
	...
	if (compose->account->set_autobcc && compose->account->auto_bcc) {
		...
		if (!found_other) {
			...
			text = g_strdup_printf(_("The only recipient is the default '%s' address. Send anyway?"), prefs_common_translated_header_name("Bcc"));
			aval = alertpanel(_("Send"), text, GTK_STOCK_CANCEL, _("_Send"), NULL, ALERTFOCUS_SECOND);
			g_free(text);
			if (aval != G_ALERTALTERNATE)
				return FALSE;
...
static gboolean compose_check_entries(Compose *compose, gboolean check_everything)
{
	...
	if (!compose->batch && prefs_common.warn_empty_subj == TRUE) {
		...
		if (*str == '\0' && check_everything == TRUE && compose->mode != COMPOSE_REDIRECT) {
			AlertValue aval;
			gchar *message;

			message = g_strdup_printf(_("Subject is empty. %s"), compose->sending ? _("Send it anyway?") : _("Queue it anyway?"));

			aval = alertpanel_full(compose->sending ? _("Send") : _("Send later"), message, GTK_STOCK_CANCEL, compose->sending ? _("_Send") : _("_Queue"), NULL, ALERTFOCUS_FIRST, TRUE, NULL, ALERT_QUESTION);
			g_free(message);
			if (aval & G_ALERTDISABLE) {
				aval &= ~G_ALERTDISABLE;
	...
	if (!compose->batch && prefs_common.warn_sending_many_recipients_num > 0 && check_everything == TRUE) {
		GSList *list;
		gint cnt = 0;
		...
		for (list = compose->header_list; list; list = list->next) {
			...
			g_strstrip(header);
			g_strstrip(entry);
			if ((entry[0] != '\0') && (!strcmp(header, prefs_common_translated_header_name("To:")) || !strcmp(header, prefs_common_translated_header_name("Cc:")))) {
				cnt++;
			}
			g_free(header);
			g_free(entry);
		...
		if (cnt > prefs_common.warn_sending_many_recipients_num) {
			...
			message = g_strdup_printf(_("Sending to %d recipients. %s"), cnt, compose->sending ? _("Send it anyway?") : _("Queue it anyway?"));
...
static void _display_queue_error(ComposeQueueResult val)
{
	...
	case COMPOSE_QUEUE_ERROR_SIGNING_FAILED:
		alertpanel_error(_("Could not queue message for sending:\n\n" "Signature failed: %s"), privacy_peek_error()? privacy_get_error() : _("Unknown error"));
		break;
	case COMPOSE_QUEUE_ERROR_ENCRYPT_FAILED:
		alertpanel_error(_("Could not queue message for sending:\n\n" "Encryption failed: %s"), privacy_peek_error()? privacy_get_error() : _("Unknown error"));
		break;
...
gint compose_send(Compose *compose)
{
	...
	if (prefs_common.send_dialog_invisible || compose->batch == TRUE)
		discard_window = TRUE;
	...
	if (val == 0) {
		...
		if (errstr) {
			alertpanel_error_log(_("%s\nYou can try to \"Send\" again " "or queue the message with \"Send later\""), errstr);
			g_free(errstr);
		} else {
			alertpanel_error_log(_("The message was queued but could not be " "sent.\nUse \"Send queued messages\" from " "the main window to retry."));
		}
		if (!discard_window) {
			goto bail;
		}
...
{
	GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(compose->attach_clist));
	return gtk_tree_model_iter_n_children(model, NULL) > 0;
}

static gint compose_redirect_write_headers_from_headerlist(Compose *compose, FILE *fp)
{
	gchar buf[BUFFSIZE];
	gchar *str;
	...
	for (list = compose->header_list; list; list = list->next) {
		...
		if (g_utf8_collate(headerentryname, to_hdr) == 0) {
			...
			g_strstrip(str);
			if (str[0] != '\0') {
				compose_convert_header(compose, buf, sizeof(buf), str, strlen("Resent-To") + 2, TRUE);
	...
	for (list = compose->header_list; list; list = list->next) {
		...
		if (g_utf8_collate(headerentryname, cc_hdr) == 0) {
			...
			g_strstrip(str);
			if (str[0] != '\0') {
				compose_convert_header(compose, buf, sizeof(buf), str, strlen("Resent-Cc") + 2, TRUE);
...
static gint compose_redirect_write_headers(Compose *compose, FILE *fp)
{
	...
	if (compose->account->name && *compose->account->name) {
		compose_convert_header(compose, buf, sizeof(buf), compose->account->name, strlen("From: "), TRUE);
	...
	if (*entstr != '\0') {
		...
		g_strstrip(str);
		if (*str != '\0') {
			compose_convert_header(compose, buf, sizeof(buf), str, strlen("Subject: "), FALSE);
...
static gint compose_redirect_write_to_file(Compose *compose, FILE *fdest)
{
	...
	while ((ret = procheader_get_one_field_asis(&buf, fp)) != -1) {
		...
		for (i = 0; not_included[i] != NULL; i++) {
			if (g_ascii_strncasecmp(buf, not_included[i], strlen(not_included[i])) == 0) {
				skip = TRUE;
				break;
			}
		}
		...
		if (!prefs_common.redirect_keep_from) {
			if (g_ascii_strncasecmp(buf, "From:", strlen("From:")) == 0) {
				...
				if (compose->account->name && *compose->account->name) {
					...
					compose_convert_header(compose, buffer, sizeof(buffer), compose->account->name, strlen("From: "), FALSE);
					err |= (fprintf(fdest, "%s <%s>", buffer, compose->account->address) < 0);
				} else
					err |= (fprintf(fdest, "%s", compose->account->address) < 0);
...
static gint compose_write_to_file(Compose *compose, FILE *fp, gint action, gboolean attach_parts)
{
	...
	if (action == COMPOSE_WRITE_FOR_SEND) {
		 ...
		 * if necessary. */
		if (compose->msgid == NULL && compose->account->gen_msgid) {
	...
	} else if (prefs_common.outgoing_fallback_to_ascii && is_ascii_str(chars)) {
		out_codeset = CS_US_ASCII;
		encoding = ENC_7BIT;
	}
	...
	if (!out_codeset) {
		...
		if (!test_conv_global_out && compose->orig_charset && strcmp(compose->orig_charset, CS_US_ASCII)) {
	...
	if (action == COMPOSE_WRITE_FOR_SEND) {
		...
		if (!buf) {
			...
			aval = alertpanel_full(_("Error"), msg, GTK_STOCK_CANCEL, _("_Send"), NULL, ALERTFOCUS_SECOND, FALSE, NULL, ALERT_ERROR);
			g_free(msg);
	...
	if (prefs_common.rewrite_first_from && (encoding == ENC_8BIT || encoding == ENC_7BIT)) {
		if (!strncmp(buf, "From ", sizeof("From ") - 1) || strstr(buf, "\nFrom ") != NULL) {
			encoding = ENC_QUOTED_PRINTABLE;
		}
	}
	...
	g_hash_table_insert(mimetext->typeparameters, g_strdup("charset"), g_strdup(out_codeset));

	/* protect trailing spaces when signing message */
	if (action == COMPOSE_WRITE_FOR_SEND && compose->use_signing && privacy_system_can_sign(compose->privacy_system)) {
		encoding = ENC_QUOTED_PRINTABLE;
	}
	...
	if (action == COMPOSE_WRITE_FOR_SEND && encoding != ENC_QUOTED_PRINTABLE && encoding != ENC_BASE64 && check_line_length(buf, 1000, &line) < 0) {
		AlertValue aval;

		msg = g_strdup_printf(_("Line %d exceeds the line length limit (998 bytes).\n" "The contents of the message might be broken on the way to the delivery.\n" "\n" "Send it anyway?"), line + 1);
		aval = alertpanel(_("Warning"), msg, GTK_STOCK_CANCEL, GTK_STOCK_OK, NULL, ALERTFOCUS_FIRST);
		g_free(msg);
		if (aval != G_ALERTALTERNATE) {
			g_free(buf);
	...
	if (compose_use_attach(compose) && attach_parts) {
		...
		g_hash_table_insert(mimempart->typeparameters, g_strdup("boundary"), boundary);
	...
	if (action == COMPOSE_WRITE_FOR_SEND && compose->use_signing && privacy_system_can_sign(compose->privacy_system))
		if (!privacy_sign(compose->privacy_system, mimemsg, compose->account, from_addr)) {
			g_free(from_name);
			g_free(from_addr);
	...
	if (compose->use_encryption) {
		if (compose->encdata != NULL && strcmp(compose->encdata, "_DONT_ENCRYPT_")) {
...
static gint compose_write_body_to_file(Compose *compose, const gchar *file)
{
	...
	chars = conv_codeset_strdup(tmp, CS_INTERNAL, conv_get_locale_charset_str());
...
static gint compose_remove_reedit_target(Compose *compose, gboolean force)
{
	...
	if (procmsg_msg_exist(msginfo) && (folder_has_parent_of_type(item, F_QUEUE) || folder_has_parent_of_type(item, F_DRAFT)
...
static gboolean compose_warn_encryption(Compose *compose)
{
	...
	val = alertpanel_full(_("Encryption warning"), warning, GTK_STOCK_CANCEL, _("C_ontinue"), NULL, ALERTFOCUS_SECOND, TRUE, NULL, ALERT_WARNING);
	if (val & G_ALERTDISABLE) {
		val &= ~G_ALERTDISABLE;
		if (val == G_ALERTALTERNATE)
			privacy_inhibit_encrypt_warning(compose->privacy_system, TRUE);
	}
...
static ComposeQueueResult compose_queue_sub(Compose *compose, gint *msgnum, FolderItem **item, gchar **msgpath, gboolean perform_checks, gboolean remove_reedit_target)
{
	...
	/* write queue header */
	tmp = g_strdup_printf("%s%cqueue.%p%08x", get_tmp_dir(), G_DIR_SEPARATOR, compose, (guint)rand());
	...
	if (compose->mode == COMPOSE_REEDIT && compose->targetinfo) {
		...
		if (mi) {
			procmsg_msginfo_change_flags(mi, compose->targetinfo->flags.perm_flags, compose->targetinfo->flags.tmp_flags & ~(MSG_COPY | MSG_MOVE | MSG_MOVE_DONE), 0, 0);
...
static int compose_add_attachments(Compose *compose, MimeInfo *parent, gint action)
{
	...
	do {
		...
		*(subtype - 1) = '\0';
		...
		if (mimepart->type == MIMETYPE_MESSAGE && !g_ascii_strcasecmp(mimepart->subtype, "rfc822")) {
			...
			if (!ainfo->name && g_ascii_strcasecmp(mimepart->subtype, "plain")) {
				...
				ainfo->name = g_strdup_printf(_("Original %s part"), mimepart->subtype);
			}
			if (ainfo->charset)
				g_hash_table_insert(mimepart->typeparameters, g_strdup("charset"), g_strdup(ainfo->charset));
		}
		if (ainfo->name && mimepart->type != MIMETYPE_MESSAGE) {
			if (mimepart->type == MIMETYPE_APPLICATION && !g_strcmp0(mimepart->subtype, "octet-stream"))
				g_hash_table_insert(mimepart->typeparameters, g_strdup("name"), g_strdup(ainfo->name));
			g_hash_table_insert(mimepart->dispositionparameters, g_strdup("filename"), g_strdup(ainfo->name));
			mimepart->disposition = DISPOSITIONTYPE_ATTACHMENT;
		}

		if (mimepart->type == MIMETYPE_MESSAGE || mimepart->type == MIMETYPE_MULTIPART)
...
static const gchar *compose_untranslated_header_name(gchar *header_name)
{
	...
	for (i = 0; HEADERS[i].header_name != NULL; i++) {
		...
		if (!strcmp(header_name, untranslated_header_name) || !strcmp(header_name, translated_header_name)) {
			return untranslated_header_name;
		} else {
			if (!strcmp(header_name, untranslated_header_name_wcolon) || !strcmp(header_name, translated_header_name_wcolon)) {
...
static void compose_add_headerfield_from_headerlist(Compose *compose, GString *header, const gchar *fieldname, const gchar *seperator)
{
	...
	if (add_field) {
		...
		compose_convert_header(compose, buf, fieldstr->len * 4 + 256, fieldstr->str, strlen(fieldname) + 2, TRUE);
...
static gchar *compose_get_header(Compose *compose)
{
	...
	if (compose->account->name && *compose->account->name) {
		...
		tmp = g_strdup_printf("%s <%s>", buf, compose->account->address);
	} else {
		tmp = g_strdup_printf("%s", compose->account->address);
	}
	...
	if (from_name && *from_name) {
		gchar *qname;
		compose_convert_header(compose, buf, sizeof(buf), from_name, strlen("From: "), TRUE);
		QUOTE_IF_REQUIRED(name, buf);
		qname = escape_internal_quotes(name, '"');

		g_string_append_printf(header, "From: %s <%s>\n", qname, from_address);
		if (!IS_IN_CUSTOM_HEADER("Disposition-Notification-To") && compose->return_receipt) {
			compose_convert_header(compose, buf, sizeof(buf), from_name, strlen("Disposition-Notification-To: "), TRUE);
	...
	/* Organization */
	if (compose->account->organization && strlen(compose->account->organization) && !IS_IN_CUSTOM_HEADER("Organization")) {
		compose_convert_header(compose, buf, sizeof(buf), compose->account->organization, strlen("Organization: "), FALSE);
	...
	if (compose->account->add_customhdr) {
		...
		for (cur = compose->account->customhdr_list; cur != NULL; cur = cur->next) {
			CustomHeader *chdr = (CustomHeader *)cur->data;

			if (custom_header_is_allowed(chdr->name)
			    && chdr->value != NULL && *(chdr->value) != '\0') {
				compose_convert_header(compose, buf, sizeof(buf), chdr->value, strlen(chdr->name) + 2, FALSE);
				g_string_append_printf(header, "%s: %s\n", chdr->name, buf);
			}
		}
	}
	...
	case PRIORITY_HIGHEST:
		g_string_append_printf(header, "Importance: high\n" "X-Priority: 1 (Highest)\n");
		break;
	case PRIORITY_HIGH:
		g_string_append_printf(header, "Importance: high\n" "X-Priority: 2 (High)\n");
		break;
	case PRIORITY_NORMAL:
		break;
	case PRIORITY_LOW:
		g_string_append_printf(header, "Importance: low\n" "X-Priority: 4 (Low)\n");
		break;
	case PRIORITY_LOWEST:
		g_string_append_printf(header, "Importance: low\n" "X-Priority: 5 (Lowest)\n");
		break;
	default:
		debug_print("compose: priority unknown : %d\n", compose->priority);
	}
	...
	for (list = compose->header_list; list; list = list->next) {
		...
		Xstrdup_a(headervalue, entry_str, {
			  g_free(headername); g_free(headername_wcolon); g_string_free(header, TRUE); return NULL;}
		);
...
static void compose_convert_header(Compose *compose, gchar *dest, gint len, gchar *src, gint header_len, gboolean addr_field)
{
	...
	codeconv_set_strict(TRUE);
	conv_encode_header_full(dest, len, tmpstr, header_len, addr_field, conv_get_charset_str(compose->out_encoding));
	codeconv_set_strict(FALSE);
	...
	if (!dest || *dest == '\0') {
		...
		g_free(test_conv_global_out);
		g_free(test_conv_reply);
		conv_encode_header_full(dest, len, tmpstr, header_len, addr_field, out_codeset);
		codeconv_set_strict(FALSE);
	}
	g_free(tmpstr);
}
...
static void compose_entry_popup_extend(GtkEntry *entry, GtkMenu *menu, gpointer user_data)
{
	...
	g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(compose_add_to_addressbook_cb), entry);
	gtk_widget_show(menuitem);
}
...
static void compose_create_header_entry(Compose *compose)
{
	...
	COMBOBOX_ADD(model, prefs_common_translated_header_name("To:"), COMPOSE_TO);
	COMBOBOX_ADD(model, prefs_common_translated_header_name("Cc:"), COMPOSE_CC);
	COMBOBOX_ADD(model, prefs_common_translated_header_name("Bcc:"), COMPOSE_BCC);
	COMBOBOX_ADD(model, prefs_common_translated_header_name("Newsgroups:"), COMPOSE_NEWSGROUPS);
	COMBOBOX_ADD(model, prefs_common_translated_header_name("Reply-To:"), COMPOSE_REPLYTO);
	COMBOBOX_ADD(model, prefs_common_translated_header_name("Followup-To:"), COMPOSE_FOLLOWUPTO);
	...
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN(combo))), "grab_focus", G_CALLBACK(compose_grab_focus_cb), compose);
	...
	if (compose->header_last && (compose->draft_timeout_tag != COMPOSE_DRAFT_TIMEOUT_FORBIDDEN)) {
		const gchar *last_header_entry = gtk_entry_get_text(GTK_ENTRY(gtk_bin_get_child(GTK_BIN((compose->header_last->combo)))));
		string = headers;
		while (*string != NULL) {
	...
	gtk_editable_set_editable(GTK_EDITABLE(gtk_bin_get_child(GTK_BIN((combo)))), prefs_common.type_any_header);

	g_signal_connect_after(G_OBJECT(gtk_bin_get_child(GTK_BIN((combo)))), "grab_focus", G_CALLBACK(compose_grab_focus_cb), compose);

	/* Entry field with cleanup button */
	button = gtk_button_new();
	gtk_button_set_image(GTK_BUTTON(button), gtk_image_new_from_stock(GTK_STOCK_CLEAR, GTK_ICON_SIZE_MENU));
	gtk_widget_show(button);
	CLAWS_SET_TIP(button, _("Delete entry contents"));
	entry = gtk_entry_new();
	gtk_widget_show(entry);
	CLAWS_SET_TIP(entry, _("Use <tab> to autocomplete from addressbook"));
	hbox = gtk_hbox_new(FALSE, 0);
	gtk_widget_show(hbox);
	gtk_box_pack_start(GTK_BOX(hbox), entry, TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 0);
	gtk_table_attach(GTK_TABLE(compose->header_table), hbox, 1, 2, compose->header_nextrow, compose->header_nextrow + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, 0, 0);

	g_signal_connect(G_OBJECT(entry), "key-press-event", G_CALLBACK(compose_headerentry_key_press_event_cb), headerentry);
	g_signal_connect(G_OBJECT(entry), "changed", G_CALLBACK(compose_headerentry_changed_cb), headerentry);
	g_signal_connect_after(G_OBJECT(entry), "grab_focus", G_CALLBACK(compose_grab_focus_cb), compose);

	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(compose_headerentry_button_clicked_cb), headerentry);

	/* email dnd */
	gtk_drag_dest_set(entry, GTK_DEST_DEFAULT_ALL, compose_mime_types, sizeof(compose_mime_types) / sizeof(compose_mime_types[0]), GDK_ACTION_COPY | GDK_ACTION_MOVE);
	g_signal_connect(G_OBJECT(entry), "drag_data_received", G_CALLBACK(compose_header_drag_received_cb), entry);
	g_signal_connect(G_OBJECT(entry), "drag-drop", G_CALLBACK(compose_drag_drop), compose);
	g_signal_connect(G_OBJECT(entry), "populate-popup", G_CALLBACK(compose_entry_popup_extend), NULL);
	...
	compose->header_list = g_slist_append(compose->header_list, headerentry);
}
...
static void compose_add_header_entry(Compose *compose, const gchar *header, gchar *text, ComposePrefType pref_type)
{
	...
	replyto_hdr = (!strcasecmp(header, prefs_common_translated_header_name("Reply-To:")) || !strcasecmp(header, prefs_common_translated_header_name("Followup-To:")) || !strcasecmp(header, prefs_common_translated_header_name("In-Reply-To:")));

	extract_address(tmp);
	email = g_utf8_strdown(tmp, -1);

	if (replyto_hdr == FALSE && g_hash_table_lookup(compose->email_hashtable, email) != NULL) {
	...
	if (replyto_hdr == FALSE)
		g_hash_table_insert(compose->email_hashtable, email, GUINT_TO_POINTER(1));
	else
		g_free(email);
...
static void compose_destroy_headerentry(Compose *compose, ComposeHeaderEntry *headerentry)
{
...
static GtkWidget *compose_create_header(Compose *compose)
{
	...
	gtk_table_attach(GTK_TABLE(header_table_main), from_optmenu_hbox, 0, 2, 0, 1, GTK_EXPAND | GTK_FILL, GTK_SHRINK, 0, 0);
	...
	gtk_container_set_focus_vadjustment(GTK_CONTAINER(header_table), gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(header_scrolledwin)));
	gtk_viewport_set_shadow_type(GTK_VIEWPORT(gtk_bin_get_child(GTK_BIN(header_scrolledwin))), GTK_SHADOW_NONE);

	gtk_table_attach(GTK_TABLE(header_table_main), header_scrolledwin, 0, 2, 1, 2, GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 2);
...
static GtkWidget *compose_create_attach(Compose *compose)
{
	...
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(attach_scrwin), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	gtk_widget_set_size_request(attach_scrwin, -1, 80);

	store = gtk_list_store_new(N_ATTACH_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER, G_TYPE_AUTO_POINTER, -1);
	attach_clist = GTK_WIDGET(gtk_tree_view_new_with_model(GTK_TREE_MODEL(store)));
	gtk_container_add(GTK_CONTAINER(attach_scrwin), attach_clist);
	g_object_unref(store);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(_("Mime type"), renderer, "text", COL_MIMETYPE, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(attach_clist), column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(_("Size"), renderer, "text", COL_SIZE, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(attach_clist), column);
	...
	gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(attach_clist), prefs_common.use_stripes_everywhere);
	selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(attach_clist));
	gtk_tree_selection_set_mode(selection, GTK_SELECTION_MULTIPLE);

	g_signal_connect(G_OBJECT(attach_clist), "row_activated", G_CALLBACK(attach_selected), compose);
	g_signal_connect(G_OBJECT(attach_clist), "button_press_event", G_CALLBACK(attach_button_pressed), compose);
	g_signal_connect(G_OBJECT(attach_clist), "popup-menu", G_CALLBACK(popup_attach_button_pressed), compose);
	g_signal_connect(G_OBJECT(attach_clist), "key_press_event", G_CALLBACK(attach_key_pressed), compose);

	/* drag and drop */
	gtk_drag_dest_set(attach_clist, GTK_DEST_DEFAULT_ALL, compose_mime_types, sizeof(compose_mime_types) / sizeof(compose_mime_types[0]), GDK_ACTION_COPY | GDK_ACTION_MOVE);
	g_signal_connect(G_OBJECT(attach_clist), "drag_data_received", G_CALLBACK(compose_attach_drag_received_cb), compose);
	g_signal_connect(G_OBJECT(attach_clist), "drag-drop", G_CALLBACK(compose_drag_drop), compose);
...
static GtkWidget *compose_create_others(Compose *compose)
{
	...
	if (prefs_common.compose_save_to_history)
		combobox_set_popdown_strings(GTK_COMBO_BOX_TEXT(savemsg_combo), prefs_common.compose_save_to_history);
	...
	g_signal_connect_after(G_OBJECT(savemsg_combo), "grab_focus", G_CALLBACK(compose_grab_focus_cb), compose);
	if (account_get_special_folder(compose->account, F_OUTBOX)) {
		...
		folderidentifier = folder_item_get_identifier(account_get_special_folder(compose->account, F_OUTBOX));
		compose_set_save_to(compose, folderidentifier);
		g_free(folderidentifier);
	}
	...
	g_signal_connect(G_OBJECT(savemsg_select), "clicked", G_CALLBACK(compose_savemsg_select_cb), compose);
...
static void compose_savemsg_select_cb(GtkWidget *widget, Compose *compose)
{
	...
	dest = foldersel_folder_sel(NULL, FOLDER_SEL_COPY, NULL, FALSE, _("Select folder to save message to"));
	if (!dest)
		return;
...
static void entry_paste_clipboard(Compose *compose, GtkWidget *entry, gboolean wrap, GdkAtom clip, GtkTextIter *insert_place);

static gboolean text_clicked(GtkWidget *text, GdkEventButton *event, Compose *compose)
{
	gint prev_autowrap;
	...
	if (event->button == 3) {
		 ...
		 * under the mouse */
		if (event->x && event->y) {
			gtk_text_view_window_to_buffer_coords(GTK_TEXT_VIEW(text), GTK_TEXT_WINDOW_TEXT, event->x, event->y, &x, &y);
		...
		/* get selection */
		stuff_selected = gtk_text_buffer_get_selection_bounds(buffer, &sel_start, &sel_end);

		gtk_text_buffer_place_cursor(buffer, &iter);
		/* reselect stuff */
		if (stuff_selected && gtk_text_iter_in_range(&iter, &sel_start, &sel_end)) {
	...
	if (event->button == 2) {
		...
		entry_paste_clipboard(compose, text, prefs_common.linewrap_pastes, GDK_SELECTION_PRIMARY, &iter);
		UNBLOCK_WRAP();
		return TRUE;
	}
	return FALSE;
...
static void compose_spell_menu_changed(void *data)
{
	...
	parent_item = gtk_ui_manager_get_widget(compose->ui_manager, "/Menu/Spelling/Options");
	...
	for (items = spell_menu; items; items = items->next) {
...
static Compose *compose_create(PrefsAccount *account, FolderItem *folder, ComposeMode mode, gboolean batch)
{
	Compose *compose;
	GtkWidget *window;
	GtkWidget *vbox;
	...
	gtkut_convert_int_to_gdk_color(prefs_common.color[COL_DEFAULT_HEADER_BG], &default_header_bgcolor);
	gtkut_convert_int_to_gdk_color(prefs_common.color[COL_DEFAULT_HEADER], &default_header_color);
	...
	if (compose_force_window_origin)
		gtk_window_move(GTK_WINDOW(window), prefs_common.compose_x, prefs_common.compose_y);
#endif
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(compose_delete_cb), compose);
	...
	action_group = cm_menu_create_action_group_full(compose->ui_manager, "Menu", compose_entries, G_N_ELEMENTS(compose_entries), (gpointer)compose);
	gtk_action_group_add_toggle_actions(action_group, compose_toggle_entries, G_N_ELEMENTS(compose_toggle_entries), (gpointer)compose);
	gtk_action_group_add_radio_actions(action_group, compose_radio_rm_entries, G_N_ELEMENTS(compose_radio_rm_entries), COMPOSE_REPLY, G_CALLBACK(compose_reply_change_mode_cb), (gpointer)compose);
	gtk_action_group_add_radio_actions(action_group, compose_radio_prio_entries, G_N_ELEMENTS(compose_radio_prio_entries), PRIORITY_NORMAL, G_CALLBACK(compose_set_priority_cb), (gpointer)compose);
...
/* Help menu */
	...
	compose->toolbar = toolbar_create(TOOLBAR_COMPOSE, handlebox, (gpointer)compose);
	...
	gtk_notebook_append_page(GTK_NOTEBOOK(notebook), compose_create_header(compose), gtk_label_new_with_mnemonic(_("Hea_der")));
	/* attachment list */
	...
	gtk_notebook_append_page(GTK_NOTEBOOK(notebook), compose_create_attach(compose), attach_hbox);
	/* Others Tab */
	gtk_notebook_append_page(GTK_NOTEBOOK(notebook), compose_create_others(compose), gtk_label_new_with_mnemonic(_("Othe_rs")));
	...
	g_signal_connect_after(G_OBJECT(subject_entry), "grab_focus", G_CALLBACK(compose_grab_focus_cb), compose);
	...
	gtk_box_pack_start(GTK_BOX(ruler_hbox), ruler, TRUE, TRUE, BORDER_WIDTH);
	...
	g_signal_connect_after(G_OBJECT(text), "size_allocate", G_CALLBACK(compose_edit_size_alloc), ruler);
	g_signal_connect(G_OBJECT(buffer), "changed", G_CALLBACK(compose_changed_cb), compose);
	g_signal_connect(G_OBJECT(text), "grab_focus", G_CALLBACK(compose_grab_focus_cb), compose);
	g_signal_connect(G_OBJECT(buffer), "insert_text", G_CALLBACK(text_inserted), compose);
	g_signal_connect(G_OBJECT(text), "button_press_event", G_CALLBACK(text_clicked), compose);
	g_signal_connect(G_OBJECT(text), "popup-menu", G_CALLBACK(compose_popup_menu), compose);
	g_signal_connect(G_OBJECT(subject_entry), "changed", G_CALLBACK(compose_changed_cb), compose);
	g_signal_connect(G_OBJECT(subject_entry), "activate", G_CALLBACK(compose_subject_entry_activated), compose);

	/* drag and drop */
	gtk_drag_dest_set(text, GTK_DEST_DEFAULT_ALL, compose_mime_types, sizeof(compose_mime_types) / sizeof(compose_mime_types[0]), GDK_ACTION_COPY | GDK_ACTION_MOVE);
	g_signal_connect(G_OBJECT(text), "drag_data_received", G_CALLBACK(compose_insert_drag_received_cb), compose);
	g_signal_connect(G_OBJECT(text), "drag-drop", G_CALLBACK(compose_drag_drop), compose);
	g_signal_connect(G_OBJECT(text), "key-press-event", G_CALLBACK(completion_set_focus_to_subject), compose);
	...
	g_signal_connect(G_OBJECT(notebook), "size_allocate", G_CALLBACK(compose_notebook_size_alloc), paned);
	...
	gtk_action_group_add_actions(action_group, compose_popup_entries, G_N_ELEMENTS(compose_popup_entries), (gpointer)compose);
	...
	undo_set_change_state_func(undostruct, &compose_undo_state_changed, compose);
	...
	compose->email_hashtable = g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify) g_free, NULL);
	...
	compose->folder_update_callback_id = hooks_register_hook(FOLDER_UPDATE_HOOKLIST, compose_update_folder_hook, (gpointer)compose);

#if USE_ENCHANT
	cm_menu_set_sensitive_full(compose->ui_manager, "Menu/Spelling", FALSE);
	if (mode != COMPOSE_REDIRECT) {
		if (prefs_common.enable_aspell && prefs_common.dictionary && strcmp(prefs_common.dictionary, "")) {
			gtkaspell = gtkaspell_new(prefs_common.dictionary, prefs_common.alt_dictionary, conv_get_locale_charset_str(), prefs_common.color[COL_MISSPELLED], prefs_common.check_while_typing, prefs_common.recheck_when_changing_dict, prefs_common.use_alternate, prefs_common.use_both_dicts, GTK_TEXT_VIEW(text), GTK_WINDOW(compose->window), compose_dict_changed, compose_spell_menu_changed, compose);
			if (!gtkaspell) {
	...
	if (account->protocol != A_NNTP)
		...
		gtk_entry_set_text(GTK_ENTRY(gtk_bin_get_child(GTK_BIN((compose->header_last->combo)))), prefs_common_translated_header_name("Newsgroups:"));
...
static GtkWidget *compose_account_option_menu_create(Compose *compose)
{
	...
	g_signal_connect_after(G_OBJECT(from_name), "grab_focus", G_CALLBACK(compose_grab_focus_cb), compose);
	g_signal_connect_after(G_OBJECT(from_name), "activate", G_CALLBACK(from_name_activate_cb), optmenu);
	...
	for (; accounts != NULL; accounts = accounts->next, num++) {
		...
		name = g_markup_printf_escaped("<i>%s</i>", ac->account_name);

		if (ac == compose->account) {
			if (ac->name && *ac->name) {
				gchar *buf;
				QUOTE_IF_REQUIRED_NORMAL(buf, ac->name, return NULL);
				from = g_strdup_printf("%s <%s>", buf, ac->address);
				gtk_entry_set_text(GTK_ENTRY(from_name), from);
			} else {
				from = g_strdup_printf("%s", ac->address);
				gtk_entry_set_text(GTK_ENTRY(from_name), from);
			}
			if (cur_account != compose->account) {
				gtk_widget_modify_base(GTK_WIDGET(from_name), GTK_STATE_NORMAL, &default_header_bgcolor);
				gtk_widget_modify_text(GTK_WIDGET(from_name), GTK_STATE_NORMAL, &default_header_color);
			}
		}
	...
	g_signal_connect(G_OBJECT(optmenu), "changed", G_CALLBACK(account_activated), compose);
	g_signal_connect(G_OBJECT(from_name), "populate-popup", G_CALLBACK(compose_entry_popup_extend), NULL);
	...
	CLAWS_SET_TIP(optmenubox, _("Account to use for this email"));
	CLAWS_SET_TIP(from_name, _("Sender address to be used"));
...
static void compose_update_priority_menu_item(Compose *compose)
{
	...
	switch (compose->priority) {
	case PRIORITY_HIGHEST:
		menuitem = gtk_ui_manager_get_widget(compose->ui_manager, "/Menu/Options/Priority/Highest");
		break;
	case PRIORITY_HIGH:
		menuitem = gtk_ui_manager_get_widget(compose->ui_manager, "/Menu/Options/Priority/High");
		break;
	case PRIORITY_NORMAL:
		menuitem = gtk_ui_manager_get_widget(compose->ui_manager, "/Menu/Options/Priority/Normal");
		break;
	case PRIORITY_LOW:
		menuitem = gtk_ui_manager_get_widget(compose->ui_manager, "/Menu/Options/Priority/Low");
		break;
	case PRIORITY_LOWEST:
		menuitem = gtk_ui_manager_get_widget(compose->ui_manager, "/Menu/Options/Priority/Lowest");
		break;
	}
...
static void compose_set_privacy_system_cb(GtkWidget *widget, gpointer data)
{
	...
	if (compose->toolbar->privacy_sign_btn != NULL) {
		gtk_widget_set_sensitive(GTK_WIDGET(compose->toolbar->privacy_sign_btn), can_sign);
	...
	if (compose->toolbar->privacy_encrypt_btn != NULL) {
		gtk_widget_set_sensitive(GTK_WIDGET(compose->toolbar->privacy_encrypt_btn), can_encrypt);
...
static void compose_update_privacy_system_menu_item(Compose *compose, gboolean warn)
{
	...
	if (compose->privacy_system != NULL) {
		gchar *systemid;
		menuitem = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(compose->ui_manager, branch_path)));
		...
		while (amenu != NULL) {
			...
			if (systemid != NULL) {
				if (strcmp(systemid, compose->privacy_system) == 0 && GTK_IS_CHECK_MENU_ITEM(amenu->data)) {
			...
			} else if (strlen(compose->privacy_system) == 0 && GTK_IS_CHECK_MENU_ITEM(amenu->data)) {
				menuitem = GTK_WIDGET(amenu->data);
		...
		if (warn && !found && strlen(compose->privacy_system)) {
			alertpanel_warning(_("The privacy system '%s' cannot be loaded. You " "will not be able to sign or encrypt this message."), compose->privacy_system);
		}
	}
...
static void compose_set_out_encoding(Compose *compose)
{
	...
	switch (out_encoding) {
	case C_AUTO:
		branch = "Menu/Options/Encoding/" CS_AUTO;
		break;
	case C_US_ASCII:
		branch = "Menu/Options/Encoding/" CS_US_ASCII;
		break;
	case C_UTF_8:
		branch = "Menu/Options/Encoding/" CS_UTF_8;
		break;
	case C_ISO_8859_2:
		branch = "Menu/Options/Encoding/" CS_ISO_8859_2;
		break;
	case C_ISO_8859_7:
		branch = "Menu/Options/Encoding/" CS_ISO_8859_7;
		break;
	case C_ISO_8859_9:
		branch = "Menu/Options/Encoding/" CS_ISO_8859_9;
		break;
	case C_ISO_8859_1:
		branch = "Menu/Options/Encoding/Western/" CS_ISO_8859_1;
		break;
	case C_ISO_8859_15:
		branch = "Menu/Options/Encoding/Western/" CS_ISO_8859_15;
		break;
	case C_WINDOWS_1252:
		branch = "Menu/Options/Encoding/Western/" CS_WINDOWS_1252;
		break;
	case C_ISO_8859_13:
		branch = "Menu/Options/Encoding/Baltic/" CS_ISO_8859_13;
		break;
	case C_ISO_8859_4:
		branch = "Menu/Options/Encoding/Baltic" CS_ISO_8859_4;
		break;
	case C_ISO_8859_8:
		branch = "Menu/Options/Encoding/Hebrew/" CS_ISO_8859_8;
		break;
	case C_WINDOWS_1255:
		branch = "Menu/Options/Encoding/Hebrew/" CS_WINDOWS_1255;
		break;
	case C_ISO_8859_6:
		branch = "Menu/Options/Encoding/Arabic/" CS_ISO_8859_6;
		break;
	case C_WINDOWS_1256:
		branch = "Menu/Options/Encoding/Arabic/" CS_WINDOWS_1256;
		break;
	case C_ISO_8859_5:
		branch = "Menu/Options/Encoding/Cyrillic/" CS_ISO_8859_5;
		break;
	case C_KOI8_R:
		branch = "Menu/Options/Encoding/Cyrillic/" CS_KOI8_R;
		break;
	case C_MACCYR:
		branch = "Menu/Options/Encoding/Cyrillic/" CS_MACCYR;
		break;
	case C_KOI8_U:
		branch = "Menu/Options/Encoding/Cyrillic/" CS_KOI8_U;
		break;
	case C_WINDOWS_1251:
		branch = "Menu/Options/Encoding/Cyrillic/" CS_WINDOWS_1251;
		break;
	case C_ISO_2022_JP:
		branch = "Menu/Options/Encoding/Japanese/" CS_ISO_2022_JP;
		break;
	case C_ISO_2022_JP_2:
		branch = "Menu/Options/Encoding/Japanese/" CS_ISO_2022_JP_2;
		break;
	case C_EUC_JP:
		branch = "Menu/Options/Encoding/Japanese/" CS_EUC_JP;
		break;
	case C_SHIFT_JIS:
		branch = "Menu/Options/Encoding/Japanese/" CS_SHIFT_JIS;
		break;
	case C_GB18030:
		branch = "Menu/Options/Encoding/Chinese/" CS_GB18030;
		break;
	case C_GB2312:
		branch = "Menu/Options/Encoding/Chinese/" CS_GB2312;
		break;
	case C_GBK:
		branch = "Menu/Options/Encoding/Chinese/" CS_GBK;
		break;
	case C_BIG5:
		branch = "Menu/Options/Encoding/Chinese/" CS_BIG5;
		break;
	case C_EUC_TW:
		branch = "Menu/Options/Encoding/Chinese/" CS_EUC_TW;
		break;
	case C_EUC_KR:
		branch = "Menu/Options/Encoding/Korean/" CS_EUC_KR;
		break;
	case C_ISO_2022_KR:
		branch = "Menu/Options/Encoding/Korean/" CS_ISO_2022_KR;
		break;
	case C_TIS_620:
		branch = "Menu/Options/Encoding/Thai/" CS_TIS_620;
		break;
	case C_WINDOWS_874:
		branch = "Menu/Options/Encoding/Thai/" CS_WINDOWS_874;
		break;
	default:
		branch = "Menu/Options/Encoding/" CS_AUTO;
		break;
...
static void compose_set_template_menu(Compose *compose)
{
	...
	for (cur = tmpl_list; cur != NULL; cur = cur->next) {
		...
		g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(compose_template_activate_cb), compose);
...
static void compose_update_privacy_systems_menu(Compose *compose)
{
	...
	g_signal_connect(G_OBJECT(system_none), "activate", G_CALLBACK(compose_set_privacy_system_cb), compose);
	...
	for (cur = systems; cur != NULL; cur = g_slist_next(cur)) {
		...
		widget = gtk_radio_menu_item_new_with_label(group, privacy_system_get_name(systemid));
		g_object_set_data_full(G_OBJECT(widget), "privacy_system", g_strdup(systemid), g_free);
		g_signal_connect(G_OBJECT(widget), "activate", G_CALLBACK(compose_set_privacy_system_cb), compose);
...
static const gchar *compose_quote_char_from_context(Compose *compose)
{
	...
	case COMPOSE_FORWARD_AS_ATTACH:
	case COMPOSE_FORWARD_INLINE:
		if (compose->folder && compose->folder->prefs && compose->folder->prefs->forward_with_format)
	...
	default:
		if (compose->folder && compose->folder->prefs && compose->folder->prefs->reply_with_format)
...
static void compose_template_apply(Compose *compose, Template *tmpl, gboolean replace)
{
	...
	if (tmpl->value) {
		...
		if (compose->replyinfo != NULL) {
			...
			parsed_str = compose_quote_fmt(compose, compose->replyinfo, tmpl->value, qmark, NULL, FALSE, FALSE, err_msg);
			...
			parsed_str = compose_quote_fmt(compose, compose->fwdinfo, tmpl->value, qmark, NULL, FALSE, FALSE, err_msg);
...
static void compose_template_apply_fields_error(const gchar *header)
{
	...
	tr = g_strdup(C_("'%s' stands for a header name", "Template '%s' format error."));
...
static void compose_destroy(Compose *compose)
{
	...
	g_slist_foreach(compose->passworded_ldap_servers, _ldap_srv_func, &enable);
	...
	if (!compose->batch) {
		gtk_widget_get_allocation(GTK_WIDGET(compose->window), &allocation);
...
static void compose_attach_remove_selected(GtkAction *action, gpointer data)
{
	...
	for (cur = sel; cur != NULL; cur = cur->next) {
		...
		GtkTreeRowReference *ref = gtk_tree_row_reference_new(model, cur->data);
		cur->data = ref;
...
static void compose_attach_property(GtkAction *action, gpointer data)
{
	...
	gtk_window_set_transient_for(GTK_WINDOW(attach_prop.window), GTK_WINDOW(compose->window));
	...
	gtk_entry_set_text(GTK_ENTRY(attach_prop.mimetype_entry), ainfo->content_type ? ainfo->content_type : "");
	gtk_entry_set_text(GTK_ENTRY(attach_prop.path_entry), ainfo->file ? ainfo->file : "");
	gtk_entry_set_text(GTK_ENTRY(attach_prop.filename_entry), ainfo->name ? ainfo->name : "");
	...
	for (;;) {
		...
		if (*entry_text != '\0') {
			if (is_file_exist(entry_text) && (size = get_file_size(entry_text)) > 0)
				file = g_strdup(entry_text);
			else {
				alertpanel_error(_("File doesn't exist or is empty."));
				g_free(cnttype);
				continue;
			}
		}
		...
		gtk_tree_model_get_iter(model, &iter, path);
		gtk_list_store_set(GTK_LIST_STORE(model), &iter, COL_MIMETYPE, ainfo->content_type, COL_SIZE, text, COL_NAME, ainfo->name, COL_CHARSET, ainfo->charset, -1);
...
static void compose_attach_property_create(gboolean *cancelled)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(attach_property_delete_event), cancelled);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(attach_property_key_pressed), cancelled);
	...
	gtk_table_attach(GTK_TABLE(table), label, 0, 1, 0, (0 + 1), GTK_FILL, 0, 0, 0);
	gtk_misc_set_alignment(GTK_MISC(label), 0, 0.5);
	mimetype_entry = gtk_combo_box_text_new_with_entry();
	gtk_table_attach(GTK_TABLE(table), mimetype_entry, 1, 2, 0, (0 + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);
	...
	for (; mime_type_list != NULL; mime_type_list = mime_type_list->next) {
		...
		if (g_list_find_custom(strlist, tmp, (GCompareFunc) g_strcmp0))
			...
			strlist = g_list_insert_sorted(strlist, (gpointer)tmp, (GCompareFunc) g_strcmp0);
	}

	for (mime_type_list = strlist; mime_type_list != NULL; mime_type_list = mime_type_list->next) {
	...
	hbox = gtk_hbox_new(FALSE, 0);
	gtk_table_attach(GTK_TABLE(table), hbox, 1, 2, 1, 2, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);
	...
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(attach_property_ok), cancelled);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(attach_property_cancel), cancelled);
...
static void compose_exec_ext_editor(Compose *compose)
{
	...
	tmp = g_strdup_printf("%s%ctmpmsg.%p", get_tmp_dir(), G_DIR_SEPARATOR, compose);
	...
	if (compose_get_ext_editor_uses_socket()) {
		...
		g_signal_connect(GTK_OBJECT(socket), "plug-removed", G_CALLBACK(compose_ext_editor_plug_removed_cb), compose);
	...
	if (compose_get_ext_editor_cmd_valid()) {
		...
		if (prefs_common_get_ext_editor_cmd())
			g_warning("external editor command-line is invalid: '%s'", prefs_common_get_ext_editor_cmd());
		cmd = g_strdup_printf(DEFAULT_EDITOR_CMD, tmp);
	}

	argv = strsplit_with_quote(cmd, " ", 0);

	if (!g_spawn_async(NULL, argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD | G_SPAWN_SEARCH_PATH, NULL, NULL, &pid, &error)) {
		alertpanel_error(_("Could not spawn the following " "external editor command:\n%s\n%s"), cmd, error ? error->message : _("Unknown error"));
		if (error)
			g_error_free(error);
		g_free(tmp);
		g_free(cmd);
	...
	compose->exteditor_tag = g_child_watch_add(pid, compose_ext_editor_closed_cb, compose);
...
#endif
		alertpanel_error(_("External editor stopped with an error: %s"), error ? error->message : _("Unknown error"));
		if (error)
			g_error_free(error);
	}
...
static gboolean compose_ext_editor_kill(Compose *compose)
{
	...
	if (pid > 0) {
		...
		msg = g_strdup_printf(_("The external editor is still working.\n" "Force terminating the process?\n" "%s"), pidmsg);
		val = alertpanel_full(_("Notice"), msg, GTK_STOCK_NO, GTK_STOCK_YES, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
		g_free(msg);
		...
		if (val == G_ALERTALTERNATE) {
			...
			g_warning("terminated %s, temporary file: %s", pidmsg, compose->exteditor_file);
			g_spawn_close_pid(compose->exteditor_pid);
...
static void compose_set_ext_editor_sensitive(Compose *compose, gboolean sensitive)
{
	...
	for (i = 0; ext_editor_menu_entries[i]; ++i) {
		cm_menu_set_sensitive_full(compose->ui_manager, ext_editor_menu_entries[i], sensitive);
	}
...
static gboolean compose_get_ext_editor_uses_socket()
{
	return (prefs_common_get_ext_editor_cmd() && strstr(prefs_common_get_ext_editor_cmd(), "%w"));
}
...
static gboolean compose_edit_size_alloc(GtkEditable *widget, GtkAllocation *allocation, GtkSHRuler *shruler)
{
	if (prefs_common.show_ruler) {
		...
		gtkut_get_font_size(GTK_WIDGET(widget), &char_width, &char_height);
		line_width_in_chars = (allocation->width - allocation->x) / char_width;

		/* got the maximum */
		gtk_shruler_set_range(GTK_SHRULER(shruler), 0.0, line_width_in_chars, 0);
	}
...
static void account_activated(GtkComboBox *optmenu, gpointer data)
{
	...
	if (ac != compose->account) {
		...
		for (list = compose->header_list; list; list = list->next) {
			...
			state->header = gtk_editable_get_chars(GTK_EDITABLE(gtk_bin_get_child(GTK_BIN(hentry->combo))), 0, -1);
			state->entry = gtk_editable_get_chars(GTK_EDITABLE(hentry->entry), 0, -1);
		...
		if (ac->set_autocc && ac->auto_cc)
			compose_entry_append(compose, ac->auto_cc, COMPOSE_CC, PREF_ACCOUNT);
		if (ac->set_autobcc && ac->auto_bcc)
			compose_entry_append(compose, ac->auto_bcc, COMPOSE_BCC, PREF_ACCOUNT);
		if (ac->set_autoreplyto && ac->auto_replyto)
			compose_entry_append(compose, ac->auto_replyto, COMPOSE_REPLYTO, PREF_ACCOUNT);

		for (list = saved_list; list; list = list->next) {
			state = (HeaderEntryState *) list->data;

			compose_add_header_entry(compose, state->header, state->entry, state->type);
		...
		combobox_select_by_data(GTK_COMBO_BOX(compose->header_last->combo), (ac->protocol == A_NNTP) ? COMPOSE_NEWSGROUPS : COMPOSE_TO);
	}
...
static gboolean attach_button_pressed(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
	...
	if (event->button == 3) {
		...
		gtk_menu_popup(GTK_MENU(compose->popupmenu), NULL, NULL, NULL, NULL, event->button, event->time);
		return TRUE;
	}
...
#endif
	if (prefs_common.work_offline && !inc_offline_should_override(TRUE, _("Claws Mail needs network access in order " "to send this email.")))
		return;
...
gboolean compose_draft(gpointer data, guint action)
{
	...
	tmp = g_strdup_printf("%s%cdraft.%p", get_tmp_dir(), G_DIR_SEPARATOR, compose);
 ...
 warn_err:
		...
		if (action != COMPOSE_AUTO_SAVE) {
			if (action != COMPOSE_DRAFT_FOR_EXIT)
				...
				val = alertpanel_full(_("Could not save draft"), _("Could not save draft.\n" "Do you want to cancel exit or discard this email?"), _("_Cancel exit"), _("_Discard email"), NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_QUESTION);
				if (val == G_ALERTALTERNATE) {
					lock = FALSE;
	...
	if (action == COMPOSE_QUIT_EDITING || action == COMPOSE_DRAFT_FOR_EXIT) {
		...
		fi = g_file_query_info(f, "standard::size,time::modified", G_FILE_QUERY_INFO_NONE, NULL, &error);
		if (error != NULL) {
...
#ifdef USE_ENCHANT
	if (files_inserted > 0 && compose->gtkaspell && compose->gtkaspell->check_while_typing)
		gtkaspell_highlight_all(compose->gtkaspell);
#endif
}
...
static void compose_close_cb(GtkAction *action, gpointer data)
{
	...
	if (compose->modified) {
		...
		if (!reedit || (compose->folder != NULL && compose->folder->stype == F_DRAFT)) {
			val = alertpanel(_("Discard message"), _("This message has been modified. Discard it?"), _("_Discard"), _("_Save to Drafts"), GTK_STOCK_CANCEL, ALERTFOCUS_FIRST);
		} else {
			val = alertpanel(_("Save changes"), _("This message has been modified. Save the latest changes?"), _("_Don't save"), _("_Save to Drafts"), GTK_STOCK_CANCEL, ALERTFOCUS_SECOND);
		}
		g_mutex_unlock(&compose->mutex);
		switch (val) {
		case G_ALERTDEFAULT:
...
static void compose_template_activate_cb(GtkWidget *widget, gpointer data)
{
	...
	msg = g_strdup_printf(_("Do you want to apply the template '%s'?"), tmpl->name);
	val = alertpanel(_("Apply template"), msg, _("_Replace"), _("_Insert"), GTK_STOCK_CANCEL, ALERTFOCUS_FIRST);
	g_free(msg);
...
static void entry_cut_clipboard(GtkWidget *entry)
{
	if (GTK_IS_EDITABLE(entry))
		...
		gtk_text_buffer_cut_clipboard(gtk_text_view_get_buffer(GTK_TEXT_VIEW(entry)), gtk_clipboard_get(GDK_SELECTION_CLIPBOARD), TRUE);
}
...
static void entry_copy_clipboard(GtkWidget *entry)
{
	if (GTK_IS_EDITABLE(entry))
		...
		gtk_text_buffer_copy_clipboard(gtk_text_view_get_buffer(GTK_TEXT_VIEW(entry)), gtk_clipboard_get(GDK_SELECTION_CLIPBOARD));
}
...
static void entry_allsel(GtkWidget *entry)
{
	if (GTK_IS_EDITABLE(entry))
		...
		gtk_text_buffer_move_mark_by_name(textbuf, "selection_bound", &startiter);
...
static void compose_paste_cb(GtkAction *action, gpointer data)
{
	...
	if (compose->focused_editable
		...
		entry_paste_clipboard(compose, compose->focused_editable, prefs_common.linewrap_pastes, GDK_SELECTION_CLIPBOARD, NULL);
	UNBLOCK_WRAP();
...
#endif
		   compose->gtkaspell && compose->gtkaspell->check_while_typing)
		gtkaspell_highlight_all(compose->gtkaspell);
#endif
}
...
static void compose_paste_as_quote_cb(GtkAction *action, gpointer data)
{
	...
	if (compose->focused_editable
		...
		gint paste_as_quotation = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(compose->focused_editable),
									    "paste_as_quotation"));
		g_object_set_data(G_OBJECT(compose->focused_editable), "paste_as_quotation", GINT_TO_POINTER(paste_as_quotation + 1));
...
static void compose_paste_no_wrap_cb(GtkAction *action, gpointer data)
{
	...
	if (compose->focused_editable
		...
		entry_paste_clipboard(compose, compose->focused_editable, FALSE, GDK_SELECTION_CLIPBOARD, NULL);
	UNBLOCK_WRAP();
...
static void compose_paste_wrap_cb(GtkAction *action, gpointer data)
{
	...
	if (compose->focused_editable
		...
		entry_paste_clipboard(compose, compose->focused_editable, TRUE, GDK_SELECTION_CLIPBOARD, NULL);
	UNBLOCK_WRAP();
...
static void compose_advanced_action_cb(GtkAction *gaction, gpointer data)
{
	...
	if (action >= COMPOSE_CALL_ADVANCED_ACTION_MOVE_BEGINNING_OF_LINE && action <= COMPOSE_CALL_ADVANCED_ACTION_DELETE_TO_LINE_END) {
...
static void compose_grab_focus_cb(GtkWidget *widget, Compose *compose)
{
	...
	if (GTK_IS_EDITABLE(widget)) {
		...
		gtk_editable_set_position(GTK_EDITABLE(widget), strlen(str));
		g_free(str);
		if ((parent = gtk_widget_get_parent(widget))
			...
			if (GTK_IS_SCROLLED_WINDOW(parent)) {
				...
				GtkAdjustment *shown = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(parent));
				...
				if ((y + height) > ((int)value + (int)page_size)) {
					if ((y - height - 1) < ((int)upper - (int)page_size)) {
						gtk_adjustment_set_value(shown, y + height - (int)page_size - 1);
					} else {
						gtk_adjustment_set_value(shown, (int)upper - (int)page_size - 1);
					}
				}
			}
		}
	...
	if (GTK_IS_TEXT_VIEW(widget)
		...
		g_signal_handlers_block_by_func(G_OBJECT(widget), G_CALLBACK(compose_grab_focus_cb), compose);
		gtk_widget_grab_focus(widget);
		g_signal_handlers_unblock_by_func(G_OBJECT(widget), G_CALLBACK(compose_grab_focus_cb), compose);
...
{
	if (folder_item != NULL) {
		if (folder_item->prefs->always_sign != SIGN_OR_ENCRYPT_DEFAULT && privacy_system_can_sign(compose->privacy_system)) {
			compose_use_signing(compose, (folder_item->prefs->always_sign == SIGN_OR_ENCRYPT_ALWAYS) ? TRUE : FALSE);
		}
		if (folder_item->prefs->always_encrypt != SIGN_OR_ENCRYPT_DEFAULT && privacy_system_can_encrypt(compose->privacy_system)) {
			compose_use_encryption(compose, (folder_item->prefs->always_encrypt == SIGN_OR_ENCRYPT_ALWAYS) ? TRUE : FALSE);
		}
	}
}
...
static void compose_attach_drag_received_cb(GtkWidget *widget, GdkDragContext *context, gint x, gint y, GtkSelectionData *data, guint info, guint time, gpointer user_data)
{
	...
	if ((gdk_atom_name(type) && !strcmp(gdk_atom_name(type), "text/uri-list"))
	    && gtk_drag_get_source_widget(context) != summary_get_main_widget(mainwindow_get_mainwindow()->summaryview)) {
		...
		for (tmp = list; tmp != NULL; tmp = tmp->next) {
			...
			compose_attach_append(compose, (const gchar *)tmp->data, utf8_filename, NULL, NULL);
			g_free(utf8_filename);
		}
		if (list)
			compose_changed_cb(NULL, compose);
		...
		for (cur = list; cur; cur = cur->next) {
			...
			gchar *file = NULL;
			if (msginfo)
				file = procmsg_get_message_file_full(msginfo, TRUE, TRUE);
			if (file) {
				compose_attach_append(compose, (const gchar *)file, (const gchar *)file, "message/rfc822", NULL);
				g_free(file);
			}
		}
		g_slist_free(list);
...
static gboolean completion_set_focus_to_subject(GtkWidget *widget, GdkEventKey *event, Compose *compose) {
...
static void compose_insert_drag_received_cb(GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, GtkSelectionData *data, guint info, guint time, gpointer user_data)
{
	...
	if (gdk_atom_name(type) && !strcmp(gdk_atom_name(type), "text/uri-list")) {
		...
		case COMPOSE_DND_ASK:
			msg = g_strdup_printf(ngettext("Do you want to insert the contents of the file " "into the message body, or attach it to the email?", "Do you want to insert the contents of the %d files " "into the message body, or attach them to the email?", num_files), num_files);
			val = alertpanel_full(_("Insert or attach?"), msg, GTK_STOCK_CANCEL, _("_Insert"), _("_Attach"), ALERTFOCUS_SECOND, TRUE, NULL, ALERT_QUESTION);
			g_free(msg);
			break;
...
static gboolean compose_headerentry_key_press_event_cb(GtkWidget *entry, GdkEventKey *event, ComposeHeaderEntry *headerentry)
{
	if ((g_slist_length(headerentry->compose->header_list) > 0) && ((headerentry->headernum + 1) != headerentry->compose->header_nextrow) && !(event->state & GDK_MODIFIER_MASK) && (event->keyval == GDK_KEY_BackSpace) && (strlen(gtk_entry_get_text(GTK_ENTRY(entry))) == 0)) {
...
static void compose_headerentry_changed_cb(GtkWidget *entry, ComposeHeaderEntry *headerentry)
{
	if (strlen(gtk_entry_get_text(GTK_ENTRY(entry))) != 0) {
		...
		g_signal_handlers_disconnect_matched(G_OBJECT(entry), G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL, headerentry);
...
static void compose_show_first_last_header(Compose *compose, gboolean show_first)
{
	...
	vadj = gtk_viewport_get_vadjustment(GTK_VIEWPORT(gtk_widget_get_parent(compose->header_table)));
	gtk_adjustment_set_value(vadj, (show_first ? gtk_adjustment_get_lower(vadj) : (gtk_adjustment_get_upper(vadj) - gtk_adjustment_get_page_size(vadj))));
	gtk_adjustment_changed(vadj);
}

static void text_inserted(GtkTextBuffer *buffer, GtkTextIter *iter, const gchar *text, gint len, Compose *compose)
{
	gint paste_as_quotation = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(compose->text), "paste_as_quotation"));
	GtkTextMark *mark;
	...
	if (paste_as_quotation) {
		...
		compose_quote_fmt(compose, NULL, "%Q", qmark, new_text, TRUE, FALSE, _("Quote format error at line %d."));
		quote_fmt_reset_vartable();
		g_free(new_text);
		g_object_set_data(G_OBJECT(compose->text), "paste_as_quotation", GINT_TO_POINTER(paste_as_quotation - 1));
		...
		if (strcmp(text, "\n") || compose->automatic_break || gtk_text_iter_starts_line(iter)) {
			...
			if (is_normal) {
				...
				gtk_text_buffer_insert_with_tags_by_name(buffer, iter, text, len, "no_join", NULL);
			}
		}
	}
	...
	if (compose_can_autosave(compose) && gtk_text_buffer_get_char_count(buffer) % prefs_common.autosave_length == 0 && compose->draft_timeout_tag != COMPOSE_DRAFT_TIMEOUT_FORBIDDEN /* disabled while loading */ )
		compose->draft_timeout_tag = g_timeout_add(500, (GSourceFunc) compose_defer_auto_save_draft, compose);
}
...
static void compose_check_all(GtkAction *action, gpointer data)
{
	...
	if (gtk_widget_has_focus(compose->subject_entry))
		claws_spell_entry_check_all(CLAWS_SPELL_ENTRY(compose->subject_entry));
	else
...
static void compose_check_backwards(GtkAction *action, gpointer data)
{
	...
	if (gtk_widget_has_focus(compose->subject_entry))
		claws_spell_entry_check_backwards(CLAWS_SPELL_ENTRY(compose->subject_entry));
	else
...
static void compose_check_forwards_go(GtkAction *action, gpointer data)
{
	...
	if (gtk_widget_has_focus(compose->subject_entry))
		claws_spell_entry_check_forwards_go(CLAWS_SPELL_ENTRY(compose->subject_entry));
	else
...
static PrefsAccount *compose_find_account(MsgInfo *msginfo)
{
	...
	if (!account && prefs_common.forward_account_autosel) {
		gchar *cc = NULL;
		if (!procheader_get_header_from_msginfo(msginfo, &cc, "Cc:")) {
	...
	if (!account && prefs_common.forward_account_autosel) {
		...
		if (!procheader_get_header_from_msginfo(msginfo, &deliveredto, "Delivered-To:")) {
...
gboolean compose_close(Compose *compose)
{
	...
	if (!g_mutex_trylock(&compose->mutex)) {
		...
		if (compose->close_timeout_tag == 0) {
			compose->close_timeout_tag = g_timeout_add(500, (GSourceFunc) compose_close, compose);
		}
		return TRUE;
	}
...
static void compose_reply_from_messageview_real(MessageView *msgview, GSList *msginfo_list, guint action, gboolean opening_multiple)
{
	gchar *body = NULL;
	...
	if (g_slist_length(msginfo_list) == 1 && !opening_multiple && msgview != NULL) {
		...
		if (mimeinfo != NULL && mimeinfo->type == MIMETYPE_MESSAGE && !g_ascii_strcasecmp(mimeinfo->subtype, "rfc822")) {
			tmp_msginfo = procmsg_msginfo_new_from_mimeinfo(orig_msginfo, mimeinfo);
			if (tmp_msginfo != NULL) {
...
void compose_reply_from_messageview(MessageView *msgview, GSList *msginfo_list, guint action)
{
	if ((!prefs_common.forward_as_attachment || action != COMPOSE_FORWARD)
	    && msginfo_list != NULL && action != COMPOSE_FORWARD_AS_ATTACH && g_slist_length(msginfo_list) > 1) {
		GSList *cur = msginfo_list;
		gchar *msg = g_strdup_printf(_("You are about to reply to %d " "messages. Opening the windows " "could take some time. Do you " "want to continue?"),
					     g_slist_length(msginfo_list));
		if (g_slist_length(msginfo_list) > 9 && alertpanel(_("Warning"), msg, GTK_STOCK_CANCEL, GTK_STOCK_YES, NULL, ALERTFOCUS_SECOND) != G_ALERTALTERNATE) {
			g_free(msg);
			return;
		}
		g_free(msg);
...
void compose_check_for_email_account(Compose *compose)
{
	...
	if (compose->account && compose->account->protocol == A_NNTP) {
		...
		combobox_select_by_data(GTK_COMBO_BOX(compose->account_combo), ac->account_id);
	}
}
...
static MsgInfo *compose_msginfo_new_from_compose(Compose *compose)
{
	...
	for (list = compose->header_list; list; list = list->next) {
		gchar *header = gtk_editable_get_chars(GTK_EDITABLE(gtk_bin_get_child(GTK_BIN((((ComposeHeaderEntry *)list->data)->combo)))), 0, -1);
		gchar *entry = gtk_editable_get_chars(GTK_EDITABLE(((ComposeHeaderEntry *)list->data)->entry), 0, -1);
		...
		} else if (strcasecmp(header, prefs_common_translated_header_name("Cc:")) == 0) {
		...
		} else if (strcasecmp(header, prefs_common_translated_header_name("Newsgroups:")) == 0) {
...
static void compose_set_dictionaries_from_folder_prefs(Compose *compose, FolderItem *folder_item)
{
	...
	if (compose->gtkaspell && folder_item && folder_item->prefs) {
		...
		if (prefs->enable_default_dictionary)
			gtkaspell_change_dict(compose->gtkaspell, prefs->default_dictionary, FALSE);
		if (folder_item->prefs->enable_default_alt_dictionary)
			gtkaspell_change_alt_dict(compose->gtkaspell, prefs->default_alt_dictionary);
		if (prefs->enable_default_dictionary || prefs->enable_default_alt_dictionary)
			compose_spell_menu_changed(compose);
	}
}
#endif
