...
gint compose_send(Compose *compose)
{
	...
	if (val == 0) {
		...
		if (errstr) {
			alertpanel_error_log(_("%s\nUse \"Send queued messages\" from " "the main window to retry."), errstr);
			g_free(errstr);
		} else {
...
static void compose_exec_ext_editor(Compose *compose)
{
	...
	if (!g_spawn_async(NULL, argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD | G_SPAWN_SEARCH_PATH, NULL, NULL, &pid, &error)) {
		alertpanel_error(_("Could not spawn the following " "command:\n%s\n%s"), cmd, error ? error->message : _("Unknown error"));
		if (error)
			g_error_free(error);
		g_free(tmp);
		g_free(cmd);
...
static gboolean compose_ext_editor_kill(Compose *compose)
{
	...
	if (pid > 0) {
		...
		if (val == G_ALERTALTERNATE) {
			...
			g_warning("terminated process id: %" G_PID_FORMAT ", " "temporary file: %s", pid, compose->exteditor_file);
			g_spawn_close_pid(compose->exteditor_pid);
