 ...
 */
static gint browse_callback_entry(LdapQuery *qry, gint queryID, GList *listValues, gpointer data)
{
	GList *node;
	NameValuePair *nvp;
	...
	while (node) {
		...
		debug_print("adding to list: %s->%s\n", nvp->name ? nvp->name : "null", nvp->value ? nvp->value : "null");
 ...
 * \param data    User data.
 */
static gint browse_callback_end(LdapQuery *qry, gint queryID, gint status, gpointer data)
{
 ...
 */
static gint browse_delete_event(GtkWidget *widget, GdkEventAny *event, gboolean *cancelled)
{
	gtk_main_quit();
	return TRUE;
}
 ...
 * \param cancelled Cancelled flag.
 */
static void browse_key_pressed(GtkWidget *widget, GdkEventKey *event, gboolean *cancelled)
{
...
static void browse_create(void)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(browse_delete_event), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(browse_key_pressed), NULL);
	...
	store = gtk_list_store_new(N_COLS, G_TYPE_STRING, G_TYPE_STRING, -1);
	...
	col = gtk_tree_view_column_new_with_attributes(_("LDAP Name"), rdr, "markup", COL_NAME, NULL);
	gtk_tree_view_column_set_min_width(col, COL_WIDTH_NAME);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), col);

	rdr = gtk_cell_renderer_text_new();
	col = gtk_tree_view_column_new_with_attributes(_("Attribute Value"), rdr, "markup", COL_VALUE, NULL);
	...
	g_signal_connect(G_OBJECT(close_btn), "clicked", G_CALLBACK(browse_close), NULL);
...
static gboolean browse_idle(gpointer data)
{
	...
	if (_displayQueue_) {
		...
		while (node) {
			...
			gtk_list_store_set(store, &iter, COL_NAME, nvp->name, COL_VALUE, nvp->value, -1);
...
gboolean browseldap_entry(AddressDataSource *ds, const gchar *dn)
{
	...
	if (dn) {
		gtk_label_set_text(GTK_LABEL(browseldap_dlg.label_address), dn);
	}
	gtk_label_set_text(GTK_LABEL(browseldap_dlg.label_server), ldapsvr_get_name(server));

	debug_print("browsing server: %s\n", ldapsvr_get_name(server));
	/* Setup search */
	_searchComplete_ = FALSE;
	_queryID_ = ldaplocate_search_setup(server, dn, browse_callback_entry, browse_callback_end);
