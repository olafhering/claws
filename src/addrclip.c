...
*/
void addrclip_set_index(AddressClipboard *clipBoard, AddressIndex *addrIndex)
{
	cm_return_if_fail(clipBoard != NULL);
...
void addrclip_add(AddressClipboard *clipBoard, AddrSelectList *asl)
{
	...
	while (node) {
		...
		clipBoard->objectList = g_list_append(clipBoard->objectList, itemCopy);
		node = g_list_next(node);
	}
}
 ...
 */
static GList *addrclip_cache_add_person(AddressCache *cache, ItemFolder *folder, ItemPerson *person, GList *copyList)
{
	...
	if (strcmp(ADDRITEM_ID(newPerson), ADDRITEM_ID(person))) {
		...
		pictureFile = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, ADDRBOOK_DIR, G_DIR_SEPARATOR_S, person->picture, ".png", NULL);
		newPictureFile = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, ADDRBOOK_DIR, G_DIR_SEPARATOR_S, newPerson->picture, ".png", NULL);
 ...
 *         not found.
 */
static ItemEMail *addrclip_find_copied_email(GList *copyList, ItemEMail *emailOrig)
{
	ItemEMail *emailCopy;
	GList *node;
	AddrClip_EMail *em;
 ...
 * Return: Group added.
 */
static ItemGroup *addrclip_cache_add_group(AddressCache *cache, ItemFolder *folder, ItemGroup *group, GList *copyList)
{
	ItemGroup *newGroup;
 ...
 * Return: Folder added.
 */
static ItemFolder *addrclip_cache_copy_folder(AddressCache *cache, ItemFolder *targetFolder, ItemFolder *folder)
{
	...
	while (node) {
		...
		node = g_list_next(node);
		copyList = addrclip_cache_add_person(cache, newFolder, item, copyList);
	}
	...
	while (node) {
		...
		node = g_list_next(node);
		newGroup = addrclip_cache_add_group(cache, newFolder, item, copyList);
		if (newGroup == NULL) {
...
*/
static GList *addrclip_cache_add_folder(AddressCache *cache, ItemFolder *folder, GList *itemList, AddressClipboard *clipBoard)
{
	GList *folderGroup;
	GList *node;
	AddrSelectItem *item;
	...
	while (node) {
		...
		cacheFrom = addrindex_get_cache(clipBoard->addressIndex, item->cacheID);
		if (cacheFrom == NULL)
			continue;
		if (item->uid) {
			...
			if (aio) {
				if (ADDRITEM_TYPE(aio) == ITEMTYPE_PERSON) {
					...
					copyList = addrclip_cache_add_person(cache, folder, person, copyList);
				}
				/*
				   else if( ADDRITEM_TYPE(aio) == ITEMTYPE_EMAIL ) {
				   } 
				 */
					...
					if (!addrclip_is_subfolder_of(itemFolder, folder)) {
						newFolder = addrclip_cache_copy_folder(cache, folder, itemFolder);
						folderGroup = g_list_append(folderGroup, newFolder);
					} else {
						alertpanel_error(_("Cannot copy a folder to itself or to its sub-structure."));
					}
				}
			}
		} else {
			if (item->objectType == ITEMTYPE_DATASOURCE) {
				...
				if (cache != cacheFrom) {
					...
					newFolder = addrclip_cache_copy_folder(cache, folder, itemFolder);
					addritem_folder_set_name(newFolder, addrcache_get_name(cacheFrom));
					folderGroup = g_list_append(folderGroup, newFolder);
				} else {
					alertpanel_error(_("Cannot copy an address book to itself."));
				}
			}
		}
	}
		...
		while (node) {
			...
			if (aio) {
				if (ADDRITEM_TYPE(aio) == ITEMTYPE_GROUP) {
					...
					group = (ItemGroup *)aio;
					newGroup = addrclip_cache_add_group(cache, folder, group, copyList);
					folderGroup = g_list_append(folderGroup, newGroup);
				}
			}
		}
	}
...
* Return: List of group or folder items added.
*/
static GList *addrclip_cache_move_items(AddressCache *cache, ItemFolder *targetFolder, GList *itemList, AddressClipboard *clipBoard)
{
	GList *folderGroup;
	GList *node;
	AddrSelectItem *item;
	...
	while (node) {
		...
		if (aio) {
			if (ADDRITEM_TYPE(aio) == ITEMTYPE_PERSON) {
				...
				person = (ItemPerson *)aio;
				addrcache_folder_move_person(cache, person, targetFolder);
			} else if (ADDRITEM_TYPE(aio) == ITEMTYPE_GROUP) {
				ItemGroup *group;

				group = (ItemGroup *)aio;
				addrcache_folder_move_group(cache, group, targetFolder);
				...
				if (!addrclip_is_subfolder_of(folder, targetFolder)) {
					addrcache_folder_move_folder(cache, folder, targetFolder);
					folderGroup = g_list_append(folderGroup, folder);
				} else {
					alertpanel_error(_("Cannot move a folder to itself or to its sub-structure."));
				}
			}
		}
	}
...
* Return: List of group or folder items added.
*/
GList *addrclip_paste_copy(AddressClipboard *clipBoard, AddressBookFile *book, ItemFolder *folder)
{
	AddressCache *cache;
	GList *itemList;
	GList *folderGroup;
	...
	if (folder == NULL)
		folder = cache->rootFolder;

	folderGroup = NULL;
	itemList = clipBoard->objectList;
	folderGroup = addrclip_cache_add_folder(cache, folder, itemList, clipBoard);
	...
	while (node) {
		...
		if (aio) {
			if (ADDRITEM_TYPE(aio) == ITEMTYPE_PERSON) {
				...
				itemFolder = (ItemFolder *)aio;
				itemFolder = addrcache_remove_folder_delete(cacheFrom, itemFolder);
...
* Return: List of group or folder items added.
*/
GList *addrclip_paste_cut(AddressClipboard *clipBoard, AddressBookFile *book, ItemFolder *folder)
{
	...
	if (cacheFrom && cacheFrom == cache) {
		...
		itemList = clipBoard->objectList;
		folderGroup = addrclip_cache_move_items(cache, folder, itemList, clipBoard);
