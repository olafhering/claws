...
static void quote_fmt_add_buttons(GtkWindow *parent_window, GtkWidget *parent_box, gboolean add_info_button, void (*set_defaults_func)(void))
{
	GtkWidget *hbox_btns;
...
void quotefmt_create_new_msg_fmt_widgets(GtkWindow *parent_window, GtkWidget *parent_box, GtkWidget **checkbtn_compose_with_format, GtkWidget **override_from_format, GtkWidget **edit_subject_format, GtkWidget **edit_body_format, gboolean add_info_button, void (*set_defaults_func)(void))
{
	...
	if (checkbtn_compose_with_format)
		PACK_CHECK_BUTTON(parent_box, checkbtn_use_format, _("Use template when composing new messages"));
	...
	if (override_from_format) {
		...
		CLAWS_SET_TIP(entry_from, _("Override From header. This doesn't change the account used to compose the new message."));
	}
	...
	gtk_box_pack_start(GTK_BOX(vbox_format), scrolledwin_format, TRUE, TRUE, 0);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledwin_format), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrolledwin_format), GTK_SHADOW_IN);
	...
	quote_fmt_add_buttons(parent_window, vbox_format, add_info_button, set_defaults_func);
...
void quotefmt_create_reply_fmt_widgets(GtkWindow *parent_window, GtkWidget *parent_box, GtkWidget **checkbtn_reply_with_format, GtkWidget **override_from_format, GtkWidget **edit_reply_quotemark, GtkWidget **edit_reply_format, gboolean add_info_button, void (*set_defaults_func)(void))
{
	...
	if (checkbtn_reply_with_format)
		PACK_CHECK_BUTTON(parent_box, checkbtn_use_format, _("Use template when replying to messages"));
	...
	if (override_from_format) {
		...
		CLAWS_SET_TIP(entry_from, _("Override From header. This doesn't change the account used to reply."));
	}
	...
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolledwin_quotefmt), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
	...
	quote_fmt_add_buttons(parent_window, vbox_quote, add_info_button, set_defaults_func);
...
void quotefmt_create_forward_fmt_widgets(GtkWindow *parent_window, GtkWidget *parent_box, GtkWidget **checkbtn_forward_with_format, GtkWidget **override_from_format, GtkWidget **edit_fw_quotemark, GtkWidget **edit_fw_format, gboolean add_info_button, void (*set_defaults_func)(void))
{
	...
	if (checkbtn_forward_with_format)
		PACK_CHECK_BUTTON(parent_box, checkbtn_use_format, _("Use template when forwarding messages"));
	...
	if (override_from_format) {
		...
		CLAWS_SET_TIP(entry_from, _("Override From header. This doesn't change the account used to forward."));
	}
...
void quotefmt_add_info_button(GtkWindow *parent_window, GtkWidget *parent_box)
{
	...
	g_signal_connect(G_OBJECT(btn_formatdesc), "clicked", G_CALLBACK(quote_fmt_quote_description), GTK_WIDGET(parent_window));
}
...
void quotefmt_add_defaults_button(GtkWindow *parent_window, GtkWidget *parent_box, void (*set_defaults_func)(void))
{
	...
	gtk_button_set_image(GTK_BUTTON(btn_formatdesc), gtk_image_new_from_stock(GTK_STOCK_UNDO, GTK_ICON_SIZE_BUTTON));
	gtk_widget_show(btn_formatdesc);
	gtk_box_pack_end(GTK_BOX(parent_box), btn_formatdesc, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(btn_formatdesc), "clicked", G_CALLBACK(set_defaults_func), GTK_WIDGET(parent_window));
}
