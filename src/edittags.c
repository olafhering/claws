...
void tags_window_open(GSList *msglist)
{
	...
	if (msglist == NULL && tagswindow.has_tag_col) {
		gtk_tree_view_remove_column(GTK_TREE_VIEW(tagswindow.taglist), gtk_tree_view_get_column(GTK_TREE_VIEW(tagswindow.taglist), 0));
		tagswindow.has_tag_col = FALSE;
	}
	if (msglist == NULL) {
		gtk_widget_hide(tagswindow.label);
		gtk_widget_show(tagswindow.del_btn);
		gtk_window_set_title(GTK_WINDOW(tagswindow.window), C_("Window title", "Tags configuration"));
	} else {
		gtk_widget_show(tagswindow.label);
		gtk_widget_hide(tagswindow.del_btn);
		gtk_window_set_title(GTK_WINDOW(tagswindow.window), C_("Window title", "Modify tags"));
	}
...
static GtkListStore *tags_window_create_data_store(void)
{
	...
	gtk_tree_sortable_set_sort_func(sortable, 0, tag_cmp_func, NULL, NULL);

	return store;
}

static void tags_selected_toggled(GtkCellRendererToggle *widget, gchar *path, GtkWidget *list_view);
static void tags_selected_edited(GtkCellRendererText *widget, gchar *arg1, gchar *arg2, GtkWidget *list_view);
...
static void tags_window_insert_check_column(GtkWidget *list_view)
{
	...
	column = gtk_tree_view_column_new_with_attributes("", renderer, "active", TAG_SELECTED, "inconsistent", TAG_SELECTED_INCONSISTENT, NULL);
	gtk_tree_view_column_set_alignment(column, 0.5);
	gtk_tree_view_insert_column(GTK_TREE_VIEW(list_view), column, 0);
	g_signal_connect(G_OBJECT(renderer), "toggled", G_CALLBACK(tags_selected_toggled), list_view);
}
...
static void tags_window_create_list_view_columns(GtkWidget *list_view)
{
	...
	column = gtk_tree_view_column_new_with_attributes(_("Tag"), renderer, "text", TAG_NAME, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(list_view), column);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_set_search_column(GTK_TREE_VIEW(list_view), TAG_NAME);
	g_signal_connect(G_OBJECT(renderer), "edited", G_CALLBACK(tags_selected_edited), list_view);
...
static void tags_popup_delete(GtkAction *action, gpointer data)
{
	...
	GtkTreeSelection *selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(tagswindow.taglist));
	model = gtk_tree_view_get_model(GTK_TREE_VIEW(tagswindow.taglist));

	if (!gtk_tree_selection_get_selected(selection, NULL, &sel))
		return;

	if (alertpanel(_("Delete tag"), _("Do you really want to delete this tag?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_FIRST) != G_ALERTALTERNATE)
		return;
	...
	gtk_tree_model_get(model, &sel, TAG_DATA, &id, -1);
...
static void tags_popup_delete_all(GtkAction *action, gpointer data)
{
	...
	if (alertpanel(_("Delete all tags"), _("Do you really want to delete all tags?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_FIRST) != G_ALERTALTERNATE)
		return;
...
static gint tags_list_btn_pressed(GtkWidget *widget, GdkEventButton *event, GtkTreeView *list_view)
{
	...
	if (event && event->button == 3) {
		...
		if (!tags_popup_menu) {
			(void)cm_menu_create_action_group("EditTags", tags_popup_actions, G_N_ELEMENTS(tags_popup_actions), (gpointer)list_view);

			MENUITEM_ADDUI("/Menus", "EditTags", "EditTags", GTK_UI_MANAGER_MENU)
			    MENUITEM_ADDUI("/Menus/EditTags", "Delete", "EditTags/Delete", GTK_UI_MANAGER_MENUITEM)
			    MENUITEM_ADDUI("/Menus/EditTags", "DeleteAll", "EditTags/DeleteAll", GTK_UI_MANAGER_MENUITEM)
			    tags_popup_menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(gui_manager, "/Menus/EditTags")));
		}
		...
		gtk_menu_popup(GTK_MENU(tags_popup_menu), NULL, NULL, NULL, NULL, event->button, event->time);
...
static GtkWidget *tags_window_list_view_create(void)
{
	...
	g_signal_connect(G_OBJECT(list_view), "popup-menu", G_CALLBACK(tags_list_popup_menu), list_view);
	g_signal_connect(G_OBJECT(list_view), "button-press-event", G_CALLBACK(tags_list_btn_pressed), list_view);
...
static void tags_window_close_cb(GtkWidget *widget, gpointer data)
{
	tags_window_close();
}

static void tags_window_list_view_insert_tag(GtkWidget *list_view, GtkTreeIter *row_iter, gint tag);
...
static gboolean find_tag_in_store(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, FindTagInStore *data)
{
	gpointer tmp;
	...
	if (*new_tag) {
		...
		if (id == -1) {
			...
			if (mainwindow_get_mainwindow())
				summary_set_tag(mainwindow_get_mainwindow()->summaryview, id, NULL);
			main_window_reflect_tags_changes(mainwindow_get_mainwindow());
			tags_window_list_view_insert_tag(tagswindow.taglist, NULL, id);
		}
		fis.tag_id = id;
		fis.path = NULL;
		gtk_tree_model_foreach(gtk_tree_view_get_model(GTK_TREE_VIEW(tagswindow.taglist)), (GtkTreeModelForeachFunc) find_tag_in_store, &fis);
		if (fis.path) {
			GtkTreeSelection *selection;
			GtkTreePath *path;
			GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(tagswindow.taglist));
 ...
 */
static void tags_window_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation)
{
...
static void tags_window_create(void)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(tags_window_close_cb), NULL);
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(tags_window_size_allocate_cb), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(tags_window_key_pressed), NULL);
	...
	g_signal_connect(G_OBJECT(new_tag_entry), "key_press_event", G_CALLBACK(tags_window_add_key_pressed), NULL);

	add_btn = gtk_button_new_from_stock(GTK_STOCK_ADD);
	gtk_box_pack_start(GTK_BOX(hbox1), add_btn, FALSE, FALSE, 0);
	CLAWS_SET_TIP(add_btn, _("Add the new tag"));

	del_btn = gtk_button_new_from_stock(GTK_STOCK_DELETE);
	gtk_box_pack_start(GTK_BOX(hbox1), del_btn, FALSE, FALSE, 0);
	CLAWS_SET_TIP(del_btn, _("Delete the selected tag"));
	...
	g_signal_connect(G_OBJECT(close_btn), "clicked", G_CALLBACK(tags_window_close_cb), NULL);
	g_signal_connect(G_OBJECT(add_btn), "clicked", G_CALLBACK(tags_window_add_tag_cb), NULL);
	g_signal_connect(G_OBJECT(del_btn), "clicked", G_CALLBACK(tags_window_del_tag_cb), NULL);
	...
	gtk_widget_set_size_request(window, prefs_common.tagswin_width, prefs_common.tagswin_height);
	...
	g_signal_connect(G_OBJECT(new_tag_entry), "changed", G_CALLBACK(new_tag_set_add_sensitivity), NULL);
...
static void tags_selected_toggled(GtkCellRendererToggle *widget, gchar *path, GtkWidget *list_view)
{
	...
	gtk_tree_model_get(model, &iter, TAG_SELECTED, &enabled, TAG_DATA, &tmp, -1);

	set = !enabled;
	tag_id = GPOINTER_TO_INT(tmp);

	gtk_list_store_set(GTK_LIST_STORE(model), &iter, TAG_SELECTED, set, TAG_SELECTED_INCONSISTENT, FALSE, -1);
...
static void tags_selected_edited(GtkCellRendererText *widget, gchar *path, gchar *new_text, GtkWidget *list_view)
{
	GtkTreeIter iter;
	GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(list_view));
	gpointer tmp;
	...
	gtk_tree_model_get(model, &iter, TAG_SELECTED, &selected, TAG_DATA, &tmp, -1);
	...
	gtk_list_store_set(GTK_LIST_STORE(model), &iter, TAG_NAME, new_text, -1);
	if (selected) {
		if (summaryview)
			summary_set_tag(summaryview, tag_id, NULL);
	} else {
...
static void tags_window_list_view_insert_tag(GtkWidget *list_view, GtkTreeIter *row_iter, gint tag)
{
	...
	if (row_iter == NULL) {
		...
		gtk_list_store_set(list_store, &iter, TAG_SELECTED, selected, TAG_SELECTED_INCONSISTENT, selected_inconsistent, TAG_NAME, name, TAG_DATA, GINT_TO_POINTER(tag), -1);
	} else {
		gtk_list_store_set(list_store, row_iter, TAG_SELECTED, selected, TAG_SELECTED_INCONSISTENT, selected_inconsistent, TAG_NAME, name, TAG_DATA, GINT_TO_POINTER(tag), -1);
	}
}
