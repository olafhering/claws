...
static void imap_folder_init(Folder *folder, const gchar *name, const gchar *path);

static Folder *imap_folder_new(const gchar *name, const gchar *path);
static void imap_folder_destroy(Folder *folder);

static IMAPSession *imap_session_new(Folder *folder, const PrefsAccount *account);
static gint imap_session_authenticate(IMAPSession *session, PrefsAccount *account);
static void imap_session_destroy(Session *session);

static gchar *imap_fetch_msg(Folder *folder, FolderItem *item, gint uid);
static gchar *imap_fetch_msg_full(Folder *folder, FolderItem *item, gint uid, gboolean headers, gboolean body);
static void imap_remove_cached_msg(Folder *folder, FolderItem *item, MsgInfo *msginfo);
static gint imap_add_msg(Folder *folder, FolderItem *dest, const gchar *file, MsgFlags *flags);
static gint imap_add_msgs(Folder *folder, FolderItem *dest, GSList *file_list, GHashTable *relation);

static gint imap_copy_msg(Folder *folder, FolderItem *dest, MsgInfo *msginfo);
static gint imap_copy_msgs(Folder *folder, FolderItem *dest, MsgInfoList *msglist, GHashTable *relation);

static gint search_msgs(Folder *folder, FolderItem *container, MsgNumberList **msgs, gboolean *on_server, MatcherList *predicate, SearchProgressNotify progress_cb, gpointer progress_data);

static gint imap_remove_msg(Folder *folder, FolderItem *item, gint uid);
static gint imap_remove_msgs(Folder *folder, FolderItem *dest, MsgInfoList *msglist, GHashTable *relation);
static gint imap_expunge(Folder *folder, FolderItem *dest);
static gint imap_remove_all_msg(Folder *folder, FolderItem *item);

static gboolean imap_is_msg_changed(Folder *folder, FolderItem *item, MsgInfo *msginfo);
...
static FolderItem *imap_create_folder(Folder *folder, FolderItem *parent, const gchar *name);
static gint imap_rename_folder(Folder *folder, FolderItem *item, const gchar *name);
static gint imap_remove_folder(Folder *folder, FolderItem *item);

static FolderItem *imap_folder_item_new(Folder *folder);
static void imap_folder_item_destroy(Folder *folder, FolderItem *item);
...
static FolderItem *imap_create_special_folder(Folder *folder, SpecialFolderItemType stype, const gchar *name);

static gint imap_do_copy_msgs(Folder *folder, FolderItem *dest, MsgInfoList *msglist, GHashTable *relation, gboolean same_dest_ok);

static gint imap_do_remove_msgs(Folder *folder, FolderItem *dest, MsgInfoList *msglist, GHashTable *relation);

static void imap_delete_all_cached_messages(FolderItem *item);
static void imap_set_batch(Folder *folder, FolderItem *item, gboolean batch);
static gint imap_set_message_flags(IMAPSession *session, IMAPFolderItem *item, MsgNumberList *numlist, IMAPFlags flags, GSList *tags, gboolean is_set);
static gint imap_select(IMAPSession *session, IMAPFolder *folder, FolderItem *item, gint *exists, gint *recent, gint *unseen, guint32 *uid_validity, gint *can_create_flags, gboolean block);
static gint imap_status(IMAPSession *session, IMAPFolder *folder, const gchar *path, IMAPFolderItem *item, gint *messages, guint32 *uid_next, guint32 *uid_validity, gint *unseen, gboolean block);
static void imap_commit_tags(FolderItem *item, MsgInfo *msginfo, GSList *set_tags, GSList *unset_tags);
...
static gint imap_cmd_login(IMAPSession *session, const gchar *user, const gchar *pass, const gchar *type);
static gint imap_cmd_noop(IMAPSession *session);
#ifdef USE_GNUTLS
static gint imap_cmd_starttls(IMAPSession *session);
#endif
static gint imap_cmd_select(IMAPSession *session, const gchar *folder, gint *exists, gint *recent, gint *unseen, guint32 *uid_validity, gint *can_create_flags, GSList **ok_flags, gboolean block);
static gint imap_cmd_close(IMAPSession *session);
static gint imap_cmd_examine(IMAPSession *session, const gchar *folder, gint *exists, gint *recent, gint *unseen, guint32 *uid_validity, gboolean block);
static gint imap_cmd_create(IMAPSession *sock, const gchar *folder);
static gint imap_cmd_rename(IMAPSession *sock, const gchar *oldfolder, const gchar *newfolder);
static gint imap_cmd_delete(IMAPSession *session, const gchar *folder);
static gint imap_cmd_fetch(IMAPSession *sock, guint32 uid, const gchar *filename, gboolean headers, gboolean body);
static gint imap_cmd_append(IMAPSession *session, IMAPFolderItem *item, const gchar *destfolder, const gchar *file, IMAPFlags flags, guint32 *new_uid);
static gint imap_cmd_copy(IMAPSession *session, struct mailimap_set *set, const gchar *destfolder, struct mailimap_set **source, struct mailimap_set **dest);
static gint imap_cmd_store(IMAPSession *session, IMAPFolderItem *item, struct mailimap_set *set, IMAPFlags flags, GSList *tags, int do_add);
...
static gint imap_get_num_list(Folder *folder, FolderItem *item, GSList **list, gboolean *old_uids_valid);
static GSList *imap_get_msginfos(Folder *folder, FolderItem *item, GSList *msgnum_list);
static MsgInfo *imap_get_msginfo(Folder *folder, FolderItem *item, gint num);
static gboolean imap_scan_required(Folder *folder, FolderItem *item);
static void imap_change_flags(Folder *folder, FolderItem *item, MsgInfo *msginfo, MsgPermFlags newflags);
static gint imap_get_flags(Folder *folder, FolderItem *item, MsgInfoList *msglist, GHashTable *msgflags);
...
static GSList *imap_list_from_lep(IMAPFolder *folder, clist *list, const gchar *real_path, gboolean all);
...
static void imap_flags_hash_from_lep_uid_flags_tab(carray *list, GHashTable *hash, GHashTable *tags_hash);
static MsgInfo *imap_envelope_from_lep(struct imap_fetch_env_info *info, FolderItem *item);
...
static void imap_handle_error(Session *session, const gchar *server, int libetpan_errcode)
{
	...
	case MAILIMAP_NO_ERROR_AUTHENTICATED:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("authenticated"), NULL), session_server)
		    break;
	case MAILIMAP_NO_ERROR_NON_AUTHENTICATED:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("not authenticated"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_BAD_STATE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("bad state"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_STREAM:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("stream error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_PARSE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("parse error " "(very probably non-RFC compliance from the server)"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_CONNECTION_REFUSED:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("connection refused"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_MEMORY:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("memory error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_FATAL:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("fatal error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_PROTOCOL:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("protocol error " "(very probably non-RFC compliance from the server)"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_DONT_ACCEPT_CONNECTION:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("connection not accepted"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_APPEND:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("APPEND error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_NOOP:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("NOOP error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_LOGOUT:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("LOGOUT error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_CAPABILITY:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("CAPABILITY error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_CHECK:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("CHECK error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_CLOSE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("CLOSE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_EXPUNGE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("EXPUNGE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_COPY:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("COPY error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_UID_COPY:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("UID COPY error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_CREATE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("CREATE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_DELETE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("DELETE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_EXAMINE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("EXAMINE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_FETCH:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("FETCH error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_UID_FETCH:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("UID FETCH error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_LIST:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("LIST error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_LOGIN:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("LOGIN error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_LSUB:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("LSUB error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_RENAME:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("RENAME error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_SEARCH:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("SEARCH error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_UID_SEARCH:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("UID SEARCH error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_SELECT:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("SELECT error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_STATUS:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("STATUS error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_STORE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("STORE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_UID_STORE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("UID STORE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_SUBSCRIBE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("SUBSCRIBE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_UNSUBSCRIBE:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("UNSUBSCRIBE error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_STARTTLS:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("STARTTLS error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_INVAL:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("INVAL error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_EXTENSION:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("EXTENSION error"), NULL), session_server)
		    break;
	case MAILIMAP_ERROR_SASL:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("SASL error"), NULL), session_server)
		    break;
#ifdef USE_GNUTLS
	case MAILIMAP_ERROR_SSL:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("TLS error"), NULL), session_server)
		    break;
#endif
	default:
		MY_LOG_WARNING(g_strconcat(_("IMAP error on %s:"), " ", _("Unknown error [%d]"), NULL), session_server, libetpan_errcode)
		    break;
	}
...
static int imap_get_capabilities(IMAPSession *session)
{
	...
	for (cur = clist_begin(capabilities->cap_list); cur != NULL; cur = clist_next(cur)) {
		struct mailimap_capability *cap = clist_content(cur);
		if (!cap || cap->cap_data.cap_name == NULL)
			continue;
		session->capability = g_slist_append(session->capability, g_strdup(cap->cap_data.cap_name));
...
static gint imap_auth(IMAPSession *session, const gchar *user, const gchar *pass, IMAPAuthType type)
{
	...
	if (ok == MAILIMAP_NO_ERROR)
		...
		if (type == IMAP_AUTH_SCRAM_SHA224) {
			ext_info = _("\n\nSCRAM-SHA-224 logins only work if libetpan has been " "compiled with SASL support and the " "SCRAM SASL plugin is installed.");
		}

		if (type == IMAP_AUTH_SCRAM_SHA256) {
			ext_info = _("\n\nSCRAM-SHA-256 logins only work if libetpan has been " "compiled with SASL support and the " "SCRAM SASL plugin is installed.");
		}

		if (type == IMAP_AUTH_SCRAM_SHA384) {
			ext_info = _("\n\nSCRAM-SHA-384 logins only work if libetpan has been " "compiled with SASL support and the " "SCRAM SASL plugin is installed.");
		}

		if (type == IMAP_AUTH_SCRAM_SHA512) {
			ext_info = _("\n\nSCRAM-SHA-512 logins only work if libetpan has been " "compiled with SASL support and the " "SCRAM SASL plugin is installed.");
		}
		...
		if (time(NULL) - last_login_err > 10) {
			if (!prefs_common.no_recv_err_panel) {
				alertpanel_error_log(_("Connection to %s failed: " "login refused.%s"), server, ext_info);
			} else {
				log_error(LOG_PROTOCOL, _("Connection to %s failed: " "login refused.%s\n"), server, ext_info);
			}
		}
...
static IMAPSession *imap_reconnect_if_possible(Folder *folder, IMAPSession *session)
{
	...
	if (rfolder->session == NULL) {
		log_warning(LOG_PROTOCOL, _("Connecting to %s failed"), folder->account->recv_server);
		SESSION(session)->sock = NULL;
		imap_safe_destroy(session);
		session = NULL;
	} else {
		rfolder->session = NULL;
		log_warning(LOG_PROTOCOL, _("IMAP connection to %s has been" " disconnected. Reconnecting...\n"), folder->account->recv_server);
		statusbar_print_all(_("IMAP connection to %s has been" " disconnected. Reconnecting...\n"), folder->account->recv_server);
		SESSION(session)->state = SESSION_DISCONNECTED;
		SESSION(session)->sock = NULL;
		imap_safe_destroy(session);
...
static IMAPSession *imap_session_get(Folder *folder)
{
	...
	if (prefs_common.work_offline && !inc_offline_should_override(FALSE, _("Claws Mail needs network access in order " "to access the IMAP server."))) {
		return NULL;
	}
...
static IMAPSession *imap_session_new(Folder *folder, const PrefsAccount *account)
{
	...
	port = account->set_imapport ? account->imapport : account->ssl_imap == SSL_TUNNEL ? IMAPS_PORT : IMAP4_PORT;
	...
	buf = g_strdup_printf(_("Account '%s': Connecting to IMAP server: %s:%d..."), folder->account->account_name, folder->account->recv_server, port);
	...
	if (account->use_proxy) {
		if (account->use_default_proxy) {
			...
			if (proxy_info->use_proxy_auth)
				proxy_info->proxy_pass = passwd_store_get_account(account->account_id, PWS_ACCOUNT_PROXY_PASS);
		}
	}

#ifndef G_OS_WIN32
	if (account->set_tunnelcmd) {
		r = imap_threaded_connect_cmd(folder, account->tunnelcmd, account->recv_server, port);
	} else
#endif
	{
#ifdef USE_GNUTLS
		...
		if (ssl_type == SSL_TUNNEL) {
			r = imap_threaded_connect_ssl(folder, account->recv_server, port, proxy_info);
		} else
#endif
		{
			r = imap_threaded_connect(folder, account->recv_server, port, proxy_info);
		}
	}
	...
	if (r == MAILIMAP_NO_ERROR_AUTHENTICATED) {
		...
		if (!prefs_common.no_recv_err_panel) {
			alertpanel_error_log(_("Can't connect to IMAP server: %s:%d"), account->recv_server, port);
		} else {
...
static gint imap_session_authenticate(IMAPSession *session, PrefsAccount *account)
{
...
#endif
	if (!password_get(account->userid, account->recv_server, "imap", SESSION(session)->port, &acc_pass)) {
		acc_pass = passwd_store_get_account(account->account_id, PWS_ACCOUNT_RECV);
	}
 ...
 try_again:
	...
	if (!pass && account->imap_auth_type != IMAP_AUTH_ANON && account->imap_auth_type != IMAP_AUTH_GSSAPI) {
		gchar *tmp_pass;
		tmp_pass = input_dialog_query_password_keep(account->recv_server, account->userid, &(account->session_passwd));
		if (!tmp_pass) {
			return MAILIMAP_NO_ERROR;
		}
...
static void imap_commit_tags(FolderItem *item, MsgInfo *msginfo, GSList *tags_set, GSList *tags_unset)
{
	...
	ok = imap_select(session, IMAP_FOLDER(folder), item, NULL, NULL, NULL, NULL, &can_create_tags, FALSE);
	...
	if (IMAP_FOLDER_ITEM(item)->batching) {
		...
		for (cur = tags_set; cur; cur = cur->next) {
			...
			if (cur_tag) {
				ht_data = g_hash_table_lookup(IMAP_FOLDER_ITEM(item)->tags_set_table, GINT_TO_POINTER(cur_tag));
				if (ht_data == NULL) {
					ht_data = g_new0(TagsData, 1);
					ht_data->str = g_strdup(tags_get_tag(cur_tag));
					ht_data->item = IMAP_FOLDER_ITEM(item);
					g_hash_table_insert(IMAP_FOLDER_ITEM(item)->tags_set_table, GINT_TO_POINTER(cur_tag), ht_data);
				}
		...
		for (cur = tags_unset; cur; cur = cur->next) {
			gint cur_tag = GPOINTER_TO_INT(cur->data);
			if (cur_tag) {
				ht_data = g_hash_table_lookup(IMAP_FOLDER_ITEM(item)->tags_unset_table, GINT_TO_POINTER(cur_tag));
				if (ht_data == NULL) {
					ht_data = g_new0(TagsData, 1);
					ht_data->str = g_strdup(tags_get_tag(cur_tag));
					ht_data->item = IMAP_FOLDER_ITEM(item);
					g_hash_table_insert(IMAP_FOLDER_ITEM(item)->tags_unset_table, GINT_TO_POINTER(cur_tag), ht_data);
				}
				ht_data->msglist = g_slist_prepend(ht_data->msglist, GINT_TO_POINTER(msginfo->msgnum));
			}
		}
		...
		if (list_set) {
			ok = imap_set_message_flags(session, IMAP_FOLDER_ITEM(item), &numlist, 0, list_set, TRUE);
		...
		if (list_unset) {
			ok = imap_set_message_flags(session, IMAP_FOLDER_ITEM(item), &numlist, 0, list_unset, FALSE);
...
static gchar *imap_fetch_msg_full(Folder *folder, FolderItem *item, gint uid, gboolean headers, gboolean body)
{
	...
	ok = imap_select(session, IMAP_FOLDER(folder), item, NULL, NULL, NULL, NULL, NULL, FALSE);
	if (ok != MAILIMAP_NO_ERROR) {
...
static gint imap_add_msgs(Folder *folder, FolderItem *dest, GSList *file_list, GHashTable *relation)
{
	...
	for (cur = file_list; cur != NULL; cur = cur->next) {
		...
		if (folder_has_parent_of_type(dest, F_QUEUE) || folder_has_parent_of_type(dest, F_OUTBOX) || folder_has_parent_of_type(dest, F_DRAFT) || folder_has_parent_of_type(dest, F_TRASH))
			iflags |= IMAP_FLAG_SEEN;

		ok = imap_cmd_append(session, IMAP_FOLDER_ITEM(dest), destdir, real_file, iflags, &new_uid);
		...
		if (ok != MAILIMAP_NO_ERROR) {
			...
			if (new_uid > 0) {
				...
				if (is_dir_exist(cache_path)) {
					gchar *cache_file = g_strconcat(cache_path, G_DIR_SEPARATOR_S,
									itos(new_uid), NULL);
		...
		if (relation != NULL)
			g_hash_table_insert(relation, fileinfo->msginfo != NULL ? (gpointer)fileinfo->msginfo : (gpointer)fileinfo, GINT_TO_POINTER(new_uid));
		if (last_uid < new_uid) {
			last_uid = new_uid;
		}
	...
	if (missing_uids) {
		gint a;
		ok = imap_select(session, IMAP_FOLDER(folder), dest, &a, NULL, NULL, NULL, NULL, FALSE);
	}
	return last_uid;
}
...
static gint imap_do_copy_msgs(Folder *folder, FolderItem *dest, MsgInfoList *msglist, GHashTable *relation, gboolean same_dest_ok)
{
	...
	ok = imap_select(session, IMAP_FOLDER(folder), msginfo->folder, NULL, NULL, NULL, NULL, NULL, FALSE);
	if (ok != MAILIMAP_NO_ERROR) {
		return ok;
	}
	...
	for (cur = seq_list; cur != NULL; cur = g_slist_next(cur)) {
		...
		lock_session(session); /* unlocked later in the function */
		ok = imap_cmd_copy(session, seq_set, destdir, &source, &dest);
		...
		if (ok == MAILIMAP_NO_ERROR) {
			...
			if (relation && source && dest) {
				...
				if (g_slist_length(s_list) == g_slist_length(d_list)) {
					...
					for (s_cur = s_list, d_cur = d_list; s_cur && d_cur; s_cur = s_cur->next, d_cur = d_cur->next) {
	...
	for (cur = msglist; cur != NULL; cur = g_slist_next(cur)) {
		...
		if (hashval != NULL) {
			...
			if (num > 0) {
				...
				gchar *real_file = g_strconcat(cache_path, G_DIR_SEPARATOR_S,
							       itos(msginfo->msgnum), NULL);
				gchar *cache_file = NULL;
				g_free(cache_path);
				cache_path = folder_item_get_path(dest);
				cache_file = g_strconcat(cache_path, G_DIR_SEPARATOR_S, itos(num), NULL);
				if (!is_dir_exist(cache_path))
					make_dir_hier(cache_path);
			...
			g_hash_table_insert(relation, msginfo, GINT_TO_POINTER(0));
	}
	statusbar_pop_all();
...
static IMAPSearchKey *search_make_key(MatcherProp *match, gboolean *is_all)
{
	if (match->matchtype == MATCHTYPE_MATCHCASE || match->matchtype == MATCHTYPE_MATCH) {
		...
		case MATCHCRITERIA_NOT_NEW:
			invert = TRUE;
			matchertype = MATCHCRITERIA_NEW;
			break;
		case MATCHCRITERIA_NOT_MARKED:
			invert = TRUE;
			matchertype = MATCHCRITERIA_MARKED;
			break;
		case MATCHCRITERIA_NOT_FORWARDED:
			invert = TRUE;
			matchertype = MATCHCRITERIA_FORWARDED;
			break;
		case MATCHCRITERIA_NOT_SPAM:
			invert = TRUE;
			matchertype = MATCHCRITERIA_SPAM;
			break;
		case MATCHCRITERIA_NOT_SUBJECT:
			invert = TRUE;
			matchertype = MATCHCRITERIA_SUBJECT;
			break;
		case MATCHCRITERIA_NOT_FROM:
			invert = TRUE;
			matchertype = MATCHCRITERIA_FROM;
			break;
		case MATCHCRITERIA_NOT_TO:
			invert = TRUE;
			matchertype = MATCHCRITERIA_TO;
			break;
		case MATCHCRITERIA_NOT_CC:
			invert = TRUE;
			matchertype = MATCHCRITERIA_CC;
			break;
		case MATCHCRITERIA_NOT_REFERENCES:
			invert = TRUE;
			matchertype = MATCHCRITERIA_REFERENCES;
			break;
		case MATCHCRITERIA_NOT_HEADER:
			invert = TRUE;
			matchertype = MATCHCRITERIA_HEADER;
			break;
		case MATCHCRITERIA_NOT_TAG:
			invert = TRUE;
			matchertype = MATCHCRITERIA_TAG;
			break;
		case MATCHCRITERIA_NOT_HEADERS_PART:
			invert = TRUE;
			matchertype = MATCHCRITERIA_HEADERS_PART;
			break;
		case MATCHCRITERIA_NOT_HEADERS_CONT:
			invert = TRUE;
			matchertype = MATCHCRITERIA_HEADERS_CONT;
			break;
		case MATCHCRITERIA_NOT_MESSAGE:
			invert = TRUE;
			matchertype = MATCHCRITERIA_MESSAGE;
			break;
		case MATCHCRITERIA_NOT_BODY_PART:
			invert = TRUE;
			matchertype = MATCHCRITERIA_BODY_PART;
			break;
		case MATCHCRITERIA_NOT_TO_AND_NOT_CC:
			invert = TRUE;
			matchertype = MATCHCRITERIA_TO_OR_CC;
			break;
		case MATCHCRITERIA_NOT_MESSAGEID:
			invert = TRUE;
			matchertype = MATCHCRITERIA_MESSAGEID;
			break;
		case MATCHCRITERIA_NOT_INREPLYTO:
			invert = TRUE;
			matchertype = MATCHCRITERIA_INREPLYTO;
			break;
		}
		...
		case MATCHCRITERIA_TO_OR_CC:
			result = imap_search_or(imap_search_new(IMAP_SEARCH_CRITERIA_TO, NULL, match->expr, 0), imap_search_new(IMAP_SEARCH_CRITERIA_CC, NULL, match->expr, 0)
			    );
			break;

		case MATCHCRITERIA_HEADERS_PART:
		case MATCHCRITERIA_HEADERS_CONT:
			result = imap_search_and(imap_search_not(imap_search_new(IMAP_SEARCH_CRITERIA_BODY, NULL, match->expr, 0)), imap_search_new(IMAP_SEARCH_CRITERIA_MESSAGE, NULL, match->expr, 0)
			    );
			break;

		case MATCHCRITERIA_SIZE_EQUAL:
			result = imap_search_and(imap_search_not(imap_search_new(IMAP_SEARCH_CRITERIA_SIZE_SMALLER, NULL, NULL, match->value)), imap_search_not(imap_search_new(IMAP_SEARCH_CRITERIA_SIZE_GREATER, NULL, NULL, match->value))
			    );
			break;
...
static MatcherProp *imap_matcher_prop_set_charset(IMAPFolder *folder, MatcherProp *utf8_prop, gchar **charset)
{
	...
	if (imap_matcher_type_is_local(utf8_prop->criteria)
	    || utf8_prop->criteria == MATCHCRITERIA_TAG || utf8_prop->criteria == MATCHCRITERIA_NOT_TAG || utf8_prop->expr == NULL || is_ascii_str(utf8_prop->expr))
		return matcherprop_new(utf8_prop->criteria, utf8_prop->header, utf8_prop->matchtype, utf8_prop->expr, utf8_prop->value);
	else {
		...
		return matcherprop_new(utf8_prop->criteria, utf8_prop->header, utf8_prop->matchtype, conv_expr, utf8_prop->value);
	}
}
...
static gint search_msgs(Folder *folder, FolderItem *container, MsgNumberList **msgs, gboolean *on_server, MatcherList *predicate, SearchProgressNotify progress_cb, gpointer progress_data)
{
	...
	if (on_server == NULL || !*on_server) {
		return folder_item_search_msgs_local(folder, container, msgs, on_server, predicate, progress_cb, progress_data);
	}
	...
	result = imap_select(session, IMAP_FOLDER(folder), FOLDER_ITEM(container), NULL, NULL, NULL, NULL, NULL, TRUE);
	if (result != MAILIMAP_NO_ERROR)
		return -1;
	...
	if (result == MAILIMAP_NO_ERROR) {
		...
		*msgs = imap_uid_list_from_lep(uidlist, &result);
		...
		if (IMAP_FOLDER(folder)->search_charset_supported)
			return search_msgs(folder, container, msgs, on_server, predicate, progress_cb, progress_data);
		else {
			imap_handle_error(SESSION(session), NULL, result);
			return -1;
		}
...
static gint imap_do_remove_msgs(Folder *folder, FolderItem *dest, MsgInfoList *msglist, GHashTable *relation)
{
	...
	if (numlist != NULL) {
		ok = imap_set_message_flags(session, IMAP_FOLDER_ITEM(msginfo->folder), numlist, IMAP_FLAG_DELETED, NULL, TRUE);
		if (ok != MAILIMAP_NO_ERROR) {
...
static gint imap_scan_tree_real(Folder *folder, gboolean subs_only)
{
	...
	if (folder->account->imap_dir && *folder->account->imap_dir) {
		...
		Xstrdup_a(root_folder, folder->account->imap_dir, {
			  return -1;
			  }
		);
		extract_quote(root_folder, '"');
		subst_char(root_folder, imap_get_path_separator(session, IMAP_FOLDER(folder), root_folder, &r), '/');
		if (is_fatal(r))
			return -1;
		strtailchomp(root_folder, '/');
		real_path = imap_get_real_path(session, IMAP_FOLDER(folder), root_folder, &r);
		if (is_fatal(r)) {
			g_free(real_path);
			return -1;
		}
		...
		r = imap_threaded_list(session->folder, "", real_path, &lep_list);
	...
	if (!item || ((item->path || root_folder) && g_strcmp0(item->path, root_folder) != 0)) {
		folder_tree_destroy(folder);
...
static gint imap_scan_tree_recursive_dive(IMAPSession *session, FolderItem *item, gboolean subs_only)
{
	...
	if (session->scan_tree_recurs_depth >= prefs_common.imap_scan_tree_recurs_limit) {
		g_warning("IMAP scan tree recursion limit reached (%d, folder '%s')", prefs_common.imap_scan_tree_recurs_limit, item->folder->name);
		return -1;
	}
	...
	if (r != MAILIMAP_NO_ERROR) {
		...
		item_list = imap_list_from_lep(imapfolder, lep_list, real_path, FALSE);
		mailimap_list_result_free(lep_list);
	}
	...
	for (cur = item_list; cur != NULL; cur = cur->next) {
		...
		for (node = item->node->children; node != NULL; node = node->next) {
			if (!g_strcmp0(FOLDER_ITEM(node->data)->path, cur_item->path)) {
		...
		if (new_item->no_sub == FALSE) {
			...
			if (session->scan_tree_recurs_depth < 0)
				g_error("IMAP scan tree recursion underflow (%d)", session->scan_tree_recurs_depth);
		}
	}
...
GList *imap_scan_subtree(Folder *folder, FolderItem *item, gboolean unsubs_only, gboolean recursive)
{
	...
	if (unsubs_only)
		statusbar_print_all(_("Looking for unsubscribed folders in %s..."), item->path ? item->path : item->name);
	else
		statusbar_print_all(_("Looking for subfolders of %s..."), item->path ? item->path : item->name);
	...
	item_list = imap_list_from_lep(IMAP_FOLDER(folder), lep_list, real_path, FALSE);
	mailimap_list_result_free(lep_list);

	for (cur = item_list; cur != NULL; cur = cur->next) {
		FolderItem *cur_item = FOLDER_ITEM(cur->data);
		if (recursive) {
			tmplist = imap_scan_subtree(folder, cur_item, unsubs_only, recursive);
			if (tmplist)
				child_list = g_list_concat(child_list, tmplist);
		}
		child_list = g_list_prepend(child_list, imap_get_real_path(session, IMAP_FOLDER(folder), cur_item->path, &r));
		if (is_fatal(r)) {
			g_free(real_path);
			statusbar_pop_all();
			return NULL;
	...
	if (unsubs_only) {
		...
		sub_list = imap_list_from_lep(IMAP_FOLDER(folder), lep_list, real_path, FALSE);
		mailimap_list_result_free(lep_list);
		...
		for (cur = sub_list; cur != NULL; cur = cur->next) {
			...
			folder_item_destroy(cur_item);
			oldlitem = g_list_find_custom(child_list, tmp, (GCompareFunc) g_strcmp0);
			if (oldlitem) {
...
static void imap_create_missing_folders(Folder *folder)
{
	...
	if (!folder->inbox)
		folder->inbox = imap_create_special_folder(folder, F_INBOX, "INBOX");
	if (!folder->trash)
		folder->trash = imap_create_special_folder(folder, F_TRASH, "Trash");
	if (!folder->queue)
		folder->queue = imap_create_special_folder(folder, F_QUEUE, "Queue");
	if (!folder->outbox)
		folder->outbox = imap_create_special_folder(folder, F_OUTBOX, "Sent");
	if (!folder->draft)
		folder->draft = imap_create_special_folder(folder, F_DRAFT, "Drafts");
}
...
static gchar *imap_item_get_path(Folder *folder, FolderItem *item)
{
	...
	if (g_path_is_absolute(folder_path)) {
		if (item_path)
			path = g_strconcat(folder_path, G_DIR_SEPARATOR_S, item_path, NULL);
		else
			path = g_strdup(folder_path);
	} else {
		if (item_path)
			path = g_strconcat(get_home_dir(), G_DIR_SEPARATOR_S, folder_path, G_DIR_SEPARATOR_S, item_path, NULL);
		else
			path = g_strconcat(get_home_dir(), G_DIR_SEPARATOR_S, folder_path, NULL);
	}
	g_free(folder_path);
	g_free(item_path);
#ifdef G_OS_WIN32
...
static FolderItem *imap_create_folder(Folder *folder, FolderItem *parent, const gchar *name)
{
	...
	if (!folder_item_parent(parent) && strcasecmp(name, "INBOX") == 0) {
		...
		Xstrdup_a(imap_dir, folder->account->imap_dir, {
			  return NULL;
			  }
		);
	...
	Xstrdup_a(new_name, name, {
		  g_free(dirpath); g_free(imap_path); return NULL;
		  }
	);
...
static gint imap_rename_folder(Folder *folder, FolderItem *item, const gchar *name)
{
	...
	if (strchr(name, imap_get_path_separator(session, IMAP_FOLDER(folder), item->path, &ok)) != NULL || is_fatal(ok)) {
	...
	session->recent = 0;
	session->expunge = 0;
	ok = imap_cmd_examine(session, "INBOX", &exists, &recent, &unseen, &uid_validity, FALSE);
	if (ok != MAILIMAP_NO_ERROR) {
		g_free(real_oldpath);
		return -1;
	}
	...
	paths[1] = newpath;
	g_node_traverse(item->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, imap_rename_folder_func, paths);
...
static gint imap_remove_folder_real(Folder *folder, FolderItem *item)
{
	...
	selected_folder = (session->mbox != NULL) && (!strcmp(session->mbox, item->path));
	if (selected_folder) {
	...
	if (ok != MAILIMAP_NO_ERROR && !is_fatal(ok)) {
		...
		tmp = g_strdup_printf("%s%c", path, imap_get_path_separator(session, IMAP_FOLDER(folder), path, &ok));
		g_free(path);
		path = tmp;
		if (!is_fatal(ok))
			ok = imap_cmd_delete(session, path);
...
static void *imap_get_uncached_messages_thread(void *data)
{
	...
	if (session == NULL || item == NULL || item->folder == NULL || FOLDER_CLASS(item->folder) != &imap_class) {
		stuff->done = TRUE;
		return NULL;
	}
	...
	for (cur = seq_list; cur != NULL; cur = g_slist_next(cur)) {
		...
		r = imap_threaded_fetch_env(session->folder, imapset, &env_list);
		if (r != MAILIMAP_NO_ERROR) {
			imap_handle_error(SESSION(session), NULL, r);
			if (is_fatal(r)) {
				stuff->ok = r;
		...
		for (i = 0; i < carray_count(env_list); i += 2) {
			...
			for (cur = tags; cur; cur = cur->next) {
				...
				if (!g_slist_find(msginfo->tags, GINT_TO_POINTER(id))) {
					msginfo->tags = g_slist_prepend(msginfo->tags, GINT_TO_POINTER(id));
				}
				g_free(real_tag);
			}
			if (msginfo->tags)
...
static GSList *imap_get_uncached_messages(IMAPSession *session, FolderItem *item, MsgNumberList *numlist, int *r)
{
	GSList *result = NULL;
	GSList *cur;
	...
	while (cur != NULL) {
		...
		partial_result = (GSList *)imap_get_uncached_messages_thread(data);
		*r = data->ok;
...
static gint imap_set_message_flags(IMAPSession *session, IMAPFolderItem *item, MsgNumberList *numlist, IMAPFlags flags, GSList *tags, gboolean is_set)
{
	...
	for (cur = seq_list; cur != NULL; cur = g_slist_next(cur)) {
		...
		ok = imap_cmd_store(session, item, imapset, flags, tags, is_set);
...
static gint imap_select(IMAPSession *session, IMAPFolder *folder, FolderItem *item, gint *exists, gint *recent, gint *unseen, guint32 *uid_validity, gint *can_create_flags, gboolean block)
{
	...
	ok = imap_cmd_select(session, real_path, exists, recent, unseen, uid_validity, can_create_flags, &(IMAP_FOLDER_ITEM(item)->ok_flags), block);
	if (ok != MAILIMAP_NO_ERROR) {
		...
		debug_print("select: exists %d recent %d expunge %d uid_validity %d can_create_flags %d\n", session->exists, session->recent, session->expunge, session->uid_validity, *can_create_flags);
	}
	if (*can_create_flags) {
...
static gint imap_status(IMAPSession *session, IMAPFolder *folder, const gchar *path, IMAPFolderItem *item, gint *messages, guint32 *uid_next, guint32 *uid_validity, gint *unseen, gboolean block)
{
	...
	if (session->mbox != NULL && !strcmp(session->mbox, item->item.path)) {
	...
	r = imap_threaded_status(FOLDER(folder), real_path, &data_status, mask);
	...
	if (data_status->st_info_list) {
		for (iter = clist_begin(data_status->st_info_list); iter != NULL; iter = clist_next(iter)) {
...
#else
			log_error(LOG_PROTOCOL, _("Connection to %s failed: " "server requires STARTTLS, but Claws Mail " "has been compiled without STARTTLS " "support.\n"), SESSION(session)->server);
...
static gint imap_cmd_starttls(IMAPSession *session)
{
	...
	r = imap_threaded_starttls(session->folder, SESSION(session)->server, SESSION(session)->port);
	if (r != MAILIMAP_NO_ERROR) {
		imap_handle_error(SESSION(session), NULL, r);
...
static gint imap_cmd_select(IMAPSession *session, const gchar *folder, gint *exists, gint *recent, gint *unseen, guint32 *uid_validity, gint *can_create_flags, GSList **ok_flags, gboolean block)
{
	...
	r = imap_threaded_select(session->folder, folder, exists, recent, unseen, uid_validity, can_create_flags, ok_flags);
	if (r != MAILIMAP_NO_ERROR) {
		imap_handle_error(SESSION(session), NULL, r);
...
static gint imap_cmd_examine(IMAPSession *session, const gchar *folder, gint *exists, gint *recent, gint *unseen, guint32 *uid_validity, gboolean block)
{
	...
	r = imap_threaded_examine(session->folder, folder, exists, recent, unseen, uid_validity);
	if (r != MAILIMAP_NO_ERROR) {
		imap_handle_error(SESSION(session), NULL, r);
...
static gint imap_cmd_rename(IMAPSession *session, const gchar *old_folder, const gchar *new_folder)
{
	int r;

	r = imap_threaded_rename(session->folder, old_folder, new_folder);
	if (r != MAILIMAP_NO_ERROR) {
		imap_handle_error(SESSION(session), NULL, r);
		return r;
	}
...
static void *imap_cmd_fetch_thread(void *data)
{
	...
	if (stuff->body) {
		r = imap_threaded_fetch_content(session->folder, uid, 1, filename);
	} else {
		r = imap_threaded_fetch_content(session->folder, uid, 0, filename);
	}
	if (r != MAILIMAP_NO_ERROR) {
		imap_handle_error(SESSION(session), NULL, r);
...
static gint imap_cmd_fetch(IMAPSession *session, guint32 uid, const gchar *filename, gboolean headers, gboolean body)
{
...
static gint imap_cmd_append(IMAPSession *session, IMAPFolderItem *item, const gchar *destfolder, const gchar *file, IMAPFlags flags, guint32 *new_uid)
{
	...
	r = imap_threaded_append(session->folder, destfolder, file, flag_list, (int *)new_uid);
...
static gint get_list_of_uids(IMAPSession *session, Folder *folder, IMAPFolderItem *item, GSList **msgnum_list)
{
	...
	ok = imap_select(session, IMAP_FOLDER(folder), FOLDER_ITEM(item), NULL, NULL, NULL, NULL, NULL, TRUE);
	if (ok != MAILIMAP_NO_ERROR) {
		return -1;
	}
	...
	if (folder->account && folder->account->low_bandwidth) {
		r = imap_threaded_search(folder, IMAP_SEARCH_TYPE_SIMPLE, NULL, NULL, NULL, &lep_uidlist);
	}
	...
	if (r == MAILIMAP_NO_ERROR) {
		...
		r = imap_threaded_fetch_uid(folder, 1, &lep_uidtab);
		if (r == MAILIMAP_NO_ERROR) {
...
gint imap_get_num_list(Folder *folder, FolderItem *_item, GSList **msgnum_list, gboolean *old_uids_valid)
{
	...
	if (!item->should_update) {
		...
		*old_uids_valid = TRUE;
		if (known_list_len == item->item.total_msgs && known_list_len > 0) {
	...
	if (FOLDER_ITEM(item)->path)
		statusbar_print_all(_("Scanning folder %s/%s..."), FOLDER_ITEM(item)->folder->name, FOLDER_ITEM(item)->path);
	else
		statusbar_print_all(_("Scanning folder %s..."), FOLDER_ITEM(item)->folder->name);
...
GSList *imap_get_msginfos(Folder *folder, FolderItem *item, GSList *msgnum_list)
{
	...
	if (!(folder_has_parent_of_type(item, F_DRAFT) || folder_has_parent_of_type(item, F_QUEUE))) {
		ret = g_slist_concat(ret, imap_get_uncached_messages(session, item, msgnum_list, &ok));
		if (ok != MAILIMAP_NO_ERROR)
			return NULL;
		unlock_session(session);
	} else {
...
gboolean imap_scan_required(Folder *folder, FolderItem *_item)
{
	...
	selected_folder = (session->mbox != NULL) && (!strcmp(session->mbox, item->item.path));
	if (selected_folder) {
		...
		ok = imap_status(session, IMAP_FOLDER(folder), item->item.path, IMAP_FOLDER_ITEM(item), &exists, &uid_next, &uid_val, &unseen, FALSE);
		if (ok != MAILIMAP_NO_ERROR) {
			return FALSE;
		}

		debug_print("exists %d, item->item.total_msgs %d\n" "\tunseen %d, item->item.unread_msgs %d\n" "\tuid_next %d, item->uid_next %d\n" "\tuid_val %d, item->item.mtime %ld\n", exists, item->item.total_msgs, unseen, item->item.unread_msgs, uid_next, item->uid_next, uid_val, (long)(item->item.mtime));
		if (exists != item->item.total_msgs || unseen != item->item.unread_msgs || uid_next != item->uid_next || uid_val != item->item.mtime) {
...
void imap_change_flags(Folder *folder, FolderItem *item, MsgInfo *msginfo, MsgPermFlags newflags)
{
	...
	if ((ok = imap_select(session, IMAP_FOLDER(folder), msginfo->folder, NULL, NULL, NULL, NULL, NULL, FALSE)) != MAILIMAP_NO_ERROR) {
		return;
	}
	numlist.next = NULL;
	numlist.data = GINT_TO_POINTER(msginfo->msgnum);
	...
	if (IMAP_FOLDER_ITEM(item)->batching) {
		...
		if (flags_set) {
			ht_data = g_hash_table_lookup(IMAP_FOLDER_ITEM(item)->flags_set_table, GINT_TO_POINTER(flags_set));
			if (ht_data == NULL) {
				ht_data = g_new0(hashtable_data, 1);
				ht_data->item = IMAP_FOLDER_ITEM(item);
				g_hash_table_insert(IMAP_FOLDER_ITEM(item)->flags_set_table, GINT_TO_POINTER(flags_set), ht_data);
			}
			ht_data->msglist = g_slist_prepend(ht_data->msglist, GINT_TO_POINTER(msginfo->msgnum));
		}
		if (flags_unset) {
			ht_data = g_hash_table_lookup(IMAP_FOLDER_ITEM(item)->flags_unset_table, GINT_TO_POINTER(flags_unset));
			if (ht_data == NULL) {
				ht_data = g_new0(hashtable_data, 1);
				ht_data->item = IMAP_FOLDER_ITEM(item);
				g_hash_table_insert(IMAP_FOLDER_ITEM(item)->flags_unset_table, GINT_TO_POINTER(flags_unset), ht_data);
			}
			ht_data->msglist = g_slist_prepend(ht_data->msglist, GINT_TO_POINTER(msginfo->msgnum));
		}
	} else {
...
static gint imap_remove_msg(Folder *folder, FolderItem *item, gint uid)
{
	...
	ok = imap_set_message_flags(session, IMAP_FOLDER_ITEM(item), &numlist, IMAP_FLAG_DELETED, NULL, TRUE);
	if (ok != MAILIMAP_NO_ERROR) {
	...
	IMAP_FOLDER_ITEM(item)->uid_list = g_slist_remove(IMAP_FOLDER_ITEM(item)->uid_list, numlist.data);
	dir = folder_item_get_path(item);
	if (is_dir_exist(dir))
...
static /*gint */ void *imap_get_flags_thread(void *data)
{
	...
	selected_folder = (session->mbox != NULL) && (!strcmp(session->mbox, fitem->path));

	lock_session(session);
	if (!selected_folder) {
		ok = imap_select(session, IMAP_FOLDER(folder), fitem, &exists_cnt, NULL, &unseen_cnt, NULL, NULL, TRUE);
		if (ok != MAILIMAP_NO_ERROR) {
			stuff->done = TRUE;
			return GINT_TO_POINTER(-1);
		}
	...
	if (folder->account && folder->account->low_bandwidth) {
		for (cur = seq_list; cur != NULL; cur = g_slist_next(cur)) {
			...
			imapset = cur->data;
			if (reverse_seen) {
				r = imap_threaded_search(folder, IMAP_SEARCH_TYPE_SEEN, NULL, NULL, full_search ? NULL : imapset, &lep_uidlist);
			} else {
				r = imap_threaded_search(folder, IMAP_SEARCH_TYPE_UNSEEN, NULL, NULL, full_search ? NULL : imapset, &lep_uidlist);
			}
			if (r == MAILIMAP_NO_ERROR) {
				GSList *uidlist;
			...
			r = imap_threaded_search(folder, IMAP_SEARCH_TYPE_FLAGGED, NULL, NULL, full_search ? NULL : imapset, &lep_uidlist);
			if (r == MAILIMAP_NO_ERROR) {
				GSList *uidlist;
			...
			if (fitem->opened || fitem->processing_pending || fitem == folder->inbox) {
				r = imap_threaded_search(folder, IMAP_SEARCH_TYPE_ANSWERED, NULL, NULL, full_search ? NULL : imapset, &lep_uidlist);
				if (r == MAILIMAP_NO_ERROR) {
					GSList *uidlist;
				...
				if (flag_ok(IMAP_FOLDER_ITEM(fitem), IMAP_FLAG_FORWARDED)) {
					r = imap_threaded_search(folder, IMAP_SEARCH_TYPE_FORWARDED, NULL, NULL, full_search ? NULL : imapset, &lep_uidlist);
					if (r == MAILIMAP_NO_ERROR) {
						GSList *uidlist;
				...
				if (flag_ok(IMAP_FOLDER_ITEM(fitem), IMAP_FLAG_SPAM)) {
					r = imap_threaded_search(folder, IMAP_SEARCH_TYPE_SPAM, NULL, NULL, full_search ? NULL : imapset, &lep_uidlist);
					if (r == MAILIMAP_NO_ERROR) {
						GSList *uidlist;
				...
				r = imap_threaded_search(folder, IMAP_SEARCH_TYPE_DELETED, NULL, NULL, full_search ? NULL : imapset, &lep_uidlist);
				if (r == MAILIMAP_NO_ERROR) {
					GSList *uidlist;
	...
	for (elem = sorted_list; elem != NULL; elem = g_slist_next(elem)) {
		...
		if (folder->account && folder->account->low_bandwidth) {
			...
			if (flags_hash != NULL) {
				...
				flags = GPOINTER_TO_INT(g_hash_table_lookup(flags_hash, GINT_TO_POINTER(msginfo->msgnum)));
			}
...
static gint imap_get_flags(Folder *folder, FolderItem *item, MsgInfoList *msginfo_list, GHashTable *msgflags)
{
	gint result;
...
static gboolean process_flags(gpointer key, gpointer value, gpointer user_data)
{
	...
	debug_print("IMAP %ssetting flags to %d for %d messages\n", flags_set ? "" : "un", flags_value, g_slist_length(data->msglist));

	lock_session(session);
	if (session) {
		ok = imap_select(session, IMAP_FOLDER(item->folder), item, NULL, NULL, NULL, NULL, NULL, FALSE);
	}
	if (ok == MAILIMAP_NO_ERROR) {
		ok = imap_set_message_flags(session, IMAP_FOLDER_ITEM(item), data->msglist, flags_value, NULL, flags_set);
	} else {
		g_warning("can't select mailbox %s", item->path);
	}
...
static gboolean process_tags(gpointer key, gpointer value, gpointer user_data)
{
	...
	if (ok == MAILIMAP_NO_ERROR) {
		...
		list.data = str;
		list.next = NULL;
		ok = imap_set_message_flags(session, IMAP_FOLDER_ITEM(item), data->msglist, 0, &list, tags_set);
	} else {
		g_warning("can't select mailbox %s", item->path);
	}
...
static int imap_flags_to_flags(struct mailimap_mbx_list_flags *imap_flags)
{
	...
	if (imap_flags->mbf_oflags) {
		for (cur = clist_begin(imap_flags->mbf_oflags); cur != NULL; cur = clist_next(cur)) {
...
static GSList *imap_list_from_lep(IMAPFolder *folder, clist *list, const gchar *real_path, gboolean all)
{
	...
	if (list) {
		for (iter = clist_begin(list); iter != NULL; iter = clist_next(iter)) {
			...
			if (strcasecmp(dup_name, "INBOX") != 0 && ((flags & ETPAN_IMAP_MB_NOSELECT) != 0))
...
static GSList *imap_get_lep_set_from_numlist(IMAPFolder *folder, MsgNumberList *numlist)
{
	...
	for (cur = sorted_list; cur != NULL; cur = g_slist_next(cur)) {
		...
		if (last + 1 != next || next == 0 || item_count >= folder->max_set_size) {
			...
			if (item_count >= folder->max_set_size) {
				if (ret_list == NULL)
					llast = ret_list = g_slist_append(ret_list, current_set);
				else {
	...
	if (clist_count(current_set->set_list) > 0) {
		ret_list = g_slist_append(ret_list, current_set);
	}
...
static struct mailimap_flag_list *imap_flag_to_lep(IMAPFolderItem *item, IMAPFlags flags, GSList *tags)
{
	...
	if (IMAP_IS_FORWARDED(flags) && flag_ok(item, IMAP_FLAG_FORWARDED))
		mailimap_flag_list_add(flag_list, mailimap_flag_new_flag_keyword(strdup(RTAG_FORWARDED)));
	if (IMAP_IS_SPAM(flags) && flag_ok(item, IMAP_FLAG_SPAM))
		mailimap_flag_list_add(flag_list, mailimap_flag_new_flag_keyword(strdup(RTAG_JUNK)));
	else if (IMAP_IS_HAM(flags) && flag_ok(item, IMAP_FLAG_HAM))
		mailimap_flag_list_add(flag_list, mailimap_flag_new_flag_keyword(strdup(RTAG_NON_JUNK)));

	for (; cur; cur = cur->next) {
		gchar *enc_str = imap_utf8_to_modified_utf7(cur->data, TRUE);
		g_strstrip(enc_str);

		mailimap_flag_list_add(flag_list, mailimap_flag_new_flag_keyword(enc_str));
	}
...
static Folder *imap_folder_new(const gchar *name, const gchar *path)
{
	...
	if (missing_imap_warning) {
		...
		alertpanel_error(_("You have one or more IMAP accounts " "defined. However this version of " "Claws Mail has been built without " "IMAP support; your IMAP accounts are " "disabled.\n\n" "You probably need to " "install libetpan and recompile " "Claws Mail."));
	}
	return NULL;
}
...
#ifdef HAVE_LIBETPAN
	xml_tag_add_attr(tag, xml_attr_new_int("uidnext", IMAP_FOLDER_ITEM(item)->uid_next));
	xml_tag_add_attr(tag, xml_attr_new_int("last_sync", IMAP_FOLDER_ITEM(item)->last_sync));
	xml_tag_add_attr(tag, xml_attr_new_int("last_change", IMAP_FOLDER_ITEM(item)->last_change));
