...
MsgCache *msgcache_new(void)
{
	...
	cache = g_new0(MsgCache, 1), cache->msgnum_table = g_hash_table_new(g_int_hash, g_int_equal);
...
static FILE *msgcache_open_data_file(const gchar *file, guint version, DataOpenMode mode, gchar *buf, size_t buf_size)
{
	...
	if ((fp = claws_fopen(file, "rb")) == NULL)
		...
		if (claws_fread(&data_ver, sizeof(data_ver), 1, fp) != 1 || version != bswap_32(data_ver)) {
	...
	if (fp) {
		...
		fp = msgcache_open_data_file(file, version, DATA_WRITE, buf, buf_size);
	}
...
static gint msgcache_read_cache_data_str(FILE *fp, gchar **str, StringConverter *conv)
{
	...
	*str = NULL;
	if (!swapping) {
		if ((ni = claws_fread(&len, sizeof(len), 1, fp) != 1) || len > G_MAXINT) {
			g_warning("read_data_str: cache data (len) corrupted, read %" G_GSIZE_FORMAT " of %" G_GSIZE_FORMAT " bytes at offset %ld", ni, sizeof(len), ftell(fp));
			return -1;
		}
	} else {
		if ((ni = claws_fread(&len, sizeof(len), 1, fp) != 1) || bswap_32(len) > G_MAXINT) {
			g_warning("read_data_str: cache data (len) corrupted, read %" G_GSIZE_FORMAT " of %" G_GSIZE_FORMAT " bytes at offset %ld", ni, sizeof(len), ftell(fp));
			return -1;
		}
		len = bswap_32(len);
	...
	if ((ni = claws_fread(tmpstr, 1, len, fp)) != len) {
		g_warning("read_data_str: cache data corrupted, read %" G_GSIZE_FORMAT " of %u " "bytes at offset %ld", ni, len, ftell(fp));
		g_free(tmpstr);
		return -1;
	}
	tmpstr[len] = 0;
...
MsgCache *msgcache_read_cache(FolderItem *item, const gchar *cache_file)
{
	...
	if ((fp = msgcache_open_data_file(cache_file, CACHE_VERSION, DATA_READ, file_buf, sizeof(file_buf))) == NULL) {
		if ((fp = msgcache_open_data_file(cache_file, bswap_32(CACHE_VERSION), DATA_READ, file_buf, sizeof(file_buf))) == NULL)
			return NULL;
		else
			swapping = FALSE;
	}
...
gint msgcache_write(const gchar *cache_file, const gchar *mark_file, const gchar *tags_file, MsgCache *cache)
{
	...
	if (cache_file) {
		write_fps.cache_fp = msgcache_open_data_file(new_cache, CACHE_VERSION, DATA_WRITE, NULL, 0);
	...
	if (mark_file) {
		write_fps.mark_fp = msgcache_open_data_file(new_mark, MARK_VERSION, DATA_WRITE, NULL, 0);
	...
	if (tags_file) {
		write_fps.tags_fp = msgcache_open_data_file(new_tags, TAGS_VERSION, DATA_WRITE, NULL, 0);
