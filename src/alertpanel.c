...
static void alertpanel_create(const gchar *title, const gchar *message, const gchar *button1_label, const gchar *button2_label, const gchar *button3_label, AlertFocus focus, gboolean can_disable, GtkWidget *custom_widget, gint alert_type);
...
{
	return alertpanel_full(title, message, button1_label, button2_label, button3_label, focus, can_disable, widget, ALERT_QUESTION);
}
...
AlertValue alertpanel_full(const gchar *title, const gchar *message, const gchar *button1_label, const gchar *button2_label, const gchar *button3_label, AlertFocus focus, gboolean can_disable, GtkWidget *widget, AlertType alert_type)
{
	...
	alertpanel_create(title, message, button1_label, button2_label, button3_label, focus, can_disable, widget, alert_type);
	alertpanel_show();
...
{
	return alertpanel_full(title, message, button1_label, button2_label, button3_label, focus, FALSE, NULL, ALERT_QUESTION);
}
...
static void alertpanel_message(const gchar *title, const gchar *message, gint type)
{
	...
	alertpanel_create(title, message, GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, FALSE, NULL, type);
	alertpanel_show();
}
...
void alertpanel_error_log(const gchar *format, ...)
{
	...
	if (mainwin && mainwin->logwin) {
		...
		val = alertpanel_full(_("Error"), buf, GTK_STOCK_CLOSE, _("_View log"), NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_ERROR);
...
static void alertpanel_create(const gchar *title, const gchar *message, const gchar *button1_label, const gchar *button2_label, const gchar *button3_label, AlertFocus focus, gboolean can_disable, GtkWidget *custom_widget, gint alert_type)
{
	...
	gchar *tmp = title ? g_markup_printf_escaped("%s", title)
	    : g_strdup("");
	gchar *title_full = g_strdup_printf("<span weight=\"bold\" " "size=\"larger\">%s</span>",
					    tmp);
	g_free(tmp);
	...
	/* Claws: custom widget */
	if (custom_widget) {
		gtk_box_pack_start(GTK_BOX(vbox), custom_widget, FALSE, FALSE, 0);
	}
	...
	if (can_disable) {
		...
		disable_checkbtn = gtk_check_button_new_with_label(_("Show this message next time"));
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(disable_checkbtn), TRUE);
		gtk_box_pack_start(GTK_BOX(hbox), disable_checkbtn, FALSE, FALSE, 12);
		g_signal_connect(G_OBJECT(disable_checkbtn), "toggled", G_CALLBACK(alertpanel_button_toggled), GUINT_TO_POINTER(G_ALERTDISABLE));
	}
	...
	gtkut_stock_button_set_create(&confirm_area, &button1, button1_label, button2_label ? &button2 : NULL, label2, button3_label ? &button3 : NULL, label3);
