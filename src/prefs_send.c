...
static gchar *prefs_common_charset_set_data_from_optmenu(GtkWidget *widget)
{
	...
	cm_return_val_if_fail(gtk_combo_box_get_active_iter(GTK_COMBO_BOX(widget), &iter), NULL);
...
static gboolean _select_by_data_func(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, ComboboxSelCtx *ctx)
{
...
static void prefs_common_charset_set_optmenu(GtkWidget *widget, gchar *data)
{
	...
	ctx = g_new(ComboboxSelCtx, sizeof(ComboboxSelCtx));
...
static void checkbtn_warn_multiple_recipients_toggled(GtkToggleButton *button, gpointer user_data)
{
...
static void prefs_send_create_widget(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	PACK_CHECK_BUTTON(vbox2, checkbtn_savemsg, _("Save sent messages"));

	PACK_CHECK_BUTTON(vbox2, checkbtn_never_send_retrcpt, _("Never send Return Receipts"));
	...
	PACK_CHECK_BUTTON(vbox2, checkbtn_confirm_send_queued_messages, _("Confirm before sending queued messages"));

	PACK_CHECK_BUTTON(vbox2, checkbtn_senddialog, _("Show send dialog"));

	PACK_CHECK_BUTTON(vbox2, checkbtn_warn_empty_subj, _("Warn when Subject is empty"));

	hbox1 = gtk_hbox_new(FALSE, 0);
	gtk_widget_show(hbox1);

	PACK_CHECK_BUTTON(hbox1, checkbtn_warn_multiple_recipients, _("Warn when sending to more recipients than"));
	...
	g_signal_connect(G_OBJECT(checkbtn_warn_multiple_recipients), "toggled", G_CALLBACK(checkbtn_warn_multiple_recipients_toggled), spinbtn_warn_multiple_recipients);
	...
	gtk_table_attach(GTK_TABLE(table), label_outcharset, 0, 1, 1, 2, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 0);
	gtk_label_set_justify(GTK_LABEL(label_outcharset), GTK_JUSTIFY_RIGHT);
	gtk_misc_set_alignment(GTK_MISC(label_outcharset), 1, 0.5);

	optmenu = gtk_list_store_new(2, G_TYPE_STRING, /* Menu label */
				     G_TYPE_STRING); /* Actual charset data string */

	combobox_charset = gtk_combo_box_new_with_model(GTK_TREE_MODEL(optmenu));
	rend = gtk_cell_renderer_text_new();
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(combobox_charset), rend, TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combobox_charset), rend, "text", COMBOBOX_TEXT, NULL);

	gtk_combo_box_set_row_separator_func(GTK_COMBO_BOX(combobox_charset), (GtkTreeViewRowSeparatorFunc) _combobox_separator_func, NULL, NULL);

	gtk_widget_show(combobox_charset);
	CLAWS_SET_TIP(combobox_charset, _("If 'Automatic' is selected, the optimal encoding" " for the current locale will be used"));
	gtk_table_attach(GTK_TABLE(table), combobox_charset, 1, 2, 1, 2, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 0);
...
#undef SET_MENUITEM
	...
	gtk_table_attach(GTK_TABLE(table), label_encoding, 0, 1, 2, 3, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 0);
	...
	CLAWS_SET_TIP(combobox_encoding, _("Specify Content-Transfer-Encoding used when" " message body contains non-ASCII characters"));
	gtk_table_attach(GTK_TABLE(table), combobox_encoding, 1, 2, 2, 3, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 0);

	optmenu = GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(combobox_encoding)));
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbtn_savemsg), prefs_common.savemsg);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbtn_confirm_send_queued_messages), prefs_common.confirm_send_queued_messages);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbtn_never_send_retrcpt), prefs_common.never_send_retrcpt);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbtn_senddialog), !prefs_common.send_dialog_invisible);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(checkbtn_warn_empty_subj), prefs_common.warn_empty_subj);
	...
	if (prefs_common.warn_sending_many_recipients_num > 0)
		gtk_spin_button_set_value(GTK_SPIN_BUTTON(spinbtn_warn_multiple_recipients), prefs_common.warn_sending_many_recipients_num);
	else
		gtk_widget_set_sensitive(spinbtn_warn_multiple_recipients, FALSE);

	prefs_common_charset_set_optmenu(combobox_charset, prefs_common.outgoing_charset);
	combobox_select_by_data(GTK_COMBO_BOX(combobox_encoding), prefs_common.encoding_method);
...
static void prefs_send_save(PrefsPage *_page)
{
	...
	prefs_common.savemsg = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->checkbtn_savemsg));
	prefs_common.confirm_send_queued_messages = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->checkbtn_confirm_send_queued_messages));
	prefs_common.never_send_retrcpt = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->checkbtn_never_send_retrcpt));
	prefs_common.send_dialog_invisible = !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->checkbtn_senddialog));
	prefs_common.warn_empty_subj = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->checkbtn_warn_empty_subj));

	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->checkbtn_warn_multiple_recipients)))
		prefs_common.warn_sending_many_recipients_num = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(page->spinbtn_warn_multiple_recipients));
	else
		prefs_common.warn_sending_many_recipients_num = 0;

	g_free(prefs_common.outgoing_charset);
	prefs_common.outgoing_charset = prefs_common_charset_set_data_from_optmenu(page->combobox_charset);
