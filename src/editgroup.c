...
static void edit_group_to_group(GtkWidget *widget, gpointer data)
{
	...
	while (selected) {
		edit_group_move_email(groupeditdlg.clist_avail, groupeditdlg.clist_group, GTK_CMCTREE_NODE(selected->data));
...
static void edit_group_to_avail(GtkWidget *widget, gpointer data)
{
	...
	while (selected) {
		edit_group_move_email(groupeditdlg.clist_group, groupeditdlg.clist_avail, GTK_CMCTREE_NODE(selected->data));
		selected = selected->next;
	}
	g_list_free(selected);
	gtk_cmclist_thaw(groupeditdlg.clist_avail);
...
static void addressbook_edit_group_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation)
{
...
static void addressbook_edit_group_create(gboolean *cancelled)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(edit_group_delete_event), cancelled);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(edit_group_key_pressed), cancelled);
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(addressbook_edit_group_size_allocate_cb), NULL);
	...
	gtk_cmctree_set_expander_style(GTK_CMCTREE(clist_avail), GTK_CMCTREE_EXPANDER_TRIANGLE);
	...
	gtk_cmctree_set_expander_style(GTK_CMCTREE(clist_group), GTK_CMCTREE_EXPANDER_TRIANGLE);
	...
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(edit_group_ok), cancelled);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(edit_group_cancel), cancelled);

	gtk_widget_show_all(vbox);

	/* Event handlers */
	g_signal_connect(G_OBJECT(buttonGroup), "clicked", G_CALLBACK(edit_group_to_group), NULL);
	g_signal_connect(G_OBJECT(buttonAvail), "clicked", G_CALLBACK(edit_group_to_avail), NULL);
	g_signal_connect(G_OBJECT(clist_avail), "button_press_event", G_CALLBACK(edit_group_list_avail_button), NULL);
	g_signal_connect(G_OBJECT(clist_group), "button_press_event", G_CALLBACK(edit_group_list_group_button), NULL);
	...
	gtk_widget_set_size_request(window, prefs_common.addressbookeditgroupwin_width, prefs_common.addressbookeditgroupwin_height);
...
ItemFolder *addressbook_edit_folder(AddressBookFile *abf, ItemFolder *parent, ItemFolder *folder)
{
	...
	if (folder) {
		...
		name = input_dialog(_("New folder"), _("Input the name of new folder:"), _(ADDRESSBOOK_GUESS_FOLDER_NAME));
	}
	if (!name)
		return NULL;
	g_strstrip(name);
