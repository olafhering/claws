 ...
 *        dfl   Default flag.
 */
static void exporthtml_build_entry(ExportHtmlCtl *ctl, const gchar *file, const gint id, const gboolean dfl)
{
...
ExportHtmlCtl *exporthtml_create(void)
{
	...
	exporthtml_build_entry(ctl, FILENAME_NONE, EXPORT_HTML_ID_NONE, FALSE);
	exporthtml_build_entry(ctl, FILENAME_DEFAULT, EXPORT_HTML_ID_DEFAULT, TRUE);
	exporthtml_build_entry(ctl, FILENAME_FULL, EXPORT_HTML_ID_FULL, FALSE);
	exporthtml_build_entry(ctl, FILENAME_CUSTOM, EXPORT_HTML_ID_CUSTOM, FALSE);
	exporthtml_build_entry(ctl, FILENAME_CUSTOM2, EXPORT_HTML_ID_CUSTOM2, FALSE);
	exporthtml_build_entry(ctl, FILENAME_CUSTOM3, EXPORT_HTML_ID_CUSTOM3, FALSE);
	exporthtml_build_entry(ctl, FILENAME_CUSTOM4, EXPORT_HTML_ID_CUSTOM4, FALSE);
...
static void exporthtml_create_css_files(ExportHtmlCtl *ctl)
{
	...
	while (node) {
		...
		if (strlen(entry->fileName)) {
			fileSpec = g_strconcat(ctl->dirOutput, G_DIR_SEPARATOR_S, entry->fileName, NULL);
 ...
 */
static gint exporthtml_compare_name(gconstpointer ptr1, gconstpointer ptr2)
{
	const AddrItemObject *item1 = ptr1;
	const AddrItemObject *item2 = ptr2;
	const gchar *name1 = NULL, *name2 = NULL;
	if (item1)
		name1 = ADDRITEM_NAME(item1);
	if (item2)
		name2 = ADDRITEM_NAME(item2);
	if (!name1)
		return (name2 != NULL);
	if (!name2)
		return -1;
 ...
 * Comparison using linked list elements.
 */
static gint exporthtml_compare_email(gconstpointer ptr1, gconstpointer ptr2)
{
	const ItemEMail *email1 = ptr1;
	const ItemEMail *email2 = ptr2;
	const gchar *name1 = NULL, *name2 = NULL;
	if (email1)
		name1 = email1->address;
	if (email2)
		name2 = email2->address;
	if (!name1)
		return (name2 != NULL);
	if (!name2)
		return -1;
 ...
 * Comparison using linked list elements.
 */
static gint exporthtml_compare_attrib(gconstpointer ptr1, gconstpointer ptr2)
{
...
static GList *exporthtml_sort_name(const GList *list)
{
	...
	node = list;
	while (node) {
		sorted = g_list_insert_sorted(sorted, node->data, exporthtml_compare_name);
		node = g_list_next(node);
	}
	return sorted;
}
...
static GList *exporthtml_sort_email(const GList *list)
{
	...
	node = list;
	while (node) {
		sorted = g_list_insert_sorted(sorted, node->data, exporthtml_compare_email);
		node = g_list_next(node);
	}
	return sorted;
}
...
static GList *exporthtml_sort_attrib(const GList *list)
{
	...
	node = list;
	while (node) {
		sorted = g_list_insert_sorted(sorted, node->data, exporthtml_compare_attrib);
		node = g_list_next(node);
	}
	return sorted;
}
 ...
 */
static void exporthtml_fmt_email(ExportHtmlCtl *ctl, FILE *stream, const GList *listEMail, gboolean sortFlag)
{
	const GList *node;
	GList *list;
	gchar *name;
	...
	while (node) {
		...
		if (ctl->linkEMail) {
			fprintf(stream, "<a href=\"mailto:%s\">", email->address);
		}
 ...
 */
static gboolean exporthtml_fmt_group(ExportHtmlCtl *ctl, FILE *stream, const ItemFolder *folder, gboolean prevFlag)
{
 ...
 */
static void exporthtml_fmt_attribs(ExportHtmlCtl *ctl, FILE *stream, const GList *listAttr)
{
	const GList *node;
	GList *list;
 ...
 */
static void exporthtml_fmt_fullname(ExportHtmlCtl *ctl, gchar *buf, const ItemPerson *person)
{
	gboolean flag;
 ...
 */
static gboolean exporthtml_fmt_person(ExportHtmlCtl *ctl, FILE *stream, const ItemFolder *folder)
{
	gboolean retVal, band;
	GList *node, *list;
		...
		if (aio && aio->type == ITEMTYPE_PERSON) {
			...
			*buf = '\0';
			...
			if (ctl->showAttribs) {
				...
				exporthtml_fmt_attribs(ctl, stream, person->listAttrib);
				fprintf(stream, "</td>\n");
			}
			fprintf(stream, "</tr>\n");
 ...
 *        folder Folder.
 */
static void exporthtml_fmt_folder(ExportHtmlCtl *ctl, FILE *stream, const ItemFolder *folder)
{
	const GList *node;
	GList *listHeir, *list;
	const gchar *name;
 ...
 *         title  Page title.
 */
static void exporthtml_fmt_header(ExportHtmlCtl *ctl, FILE *stream, gchar *title)
{
	StylesheetEntry *entry;

	entry = exporthtml_find_stylesheet(ctl);

	fprintf(stream, "<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\n");
...
static void exporthtml_build_filespec(ExportHtmlCtl *ctl)
{
	...
	fileSpec = g_strconcat(ctl->dirOutput, G_DIR_SEPARATOR_S, ctl->fileHtml, NULL);
...
void exporthtml_parse_filespec(ExportHtmlCtl *ctl, gchar *fileSpec)
{
	...
	ctl->fileHtml = mgu_replace_string(ctl->fileHtml, base);
	g_free(base);
...
static void exporthtml_default_values(ExportHtmlCtl *ctl)
{
	...
	ctl->fileHtml = mgu_replace_string(ctl->fileHtml, DFL_FILE_CLAWS_OUT);
	ctl->encoding = NULL;
...
void exporthtml_load_settings(ExportHtmlCtl *ctl)
{
	...
	*buf = '\0';
	...
	if (rc == 0) {
		...
		ctl->stylesheet = xmlprops_get_property_i(props, EXMLPROP_STYLESHEET);
		ctl->nameFormat = xmlprops_get_property_i(props, EXMLPROP_FMT_NAME);
		ctl->banding = xmlprops_get_property_b(props, EXMLPROP_BANDING);
		ctl->linkEMail = xmlprops_get_property_b(props, EXMLPROP_FMT_EMAIL);
		ctl->showAttribs = xmlprops_get_property_b(props, EXMLPROP_FMT_ATTRIB);
	} else {
		/* Set default values */
