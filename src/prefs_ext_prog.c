...
static void prefs_ext_prog_create_widget(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	gtk_table_attach(GTK_TABLE(table), vbox, 0, 1, 0, 1, (GtkAttachOptions) (GTK_SHRINK), (GtkAttachOptions) (0), 0, 0);
	...
	gtk_box_pack_start(GTK_BOX(vbox), hint_label, FALSE, FALSE, 4);
	...
	gtk_table_attach(GTK_TABLE(table), table2, 0, 1, 1, 2, (GtkAttachOptions) (GTK_EXPAND | GTK_FILL), (GtkAttachOptions) (0), 0, 0);

	cmds_use_system_default_checkbtn = gtk_check_button_new_with_label(_("Use system defaults when possible"));

	gtk_widget_show(cmds_use_system_default_checkbtn);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(cmds_use_system_default_checkbtn), prefs_common.cmds_use_system_default);
	...
	if (!tmp) {
		...
		gtk_widget_set_sensitive(cmds_use_system_default_checkbtn, FALSE);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(cmds_use_system_default_checkbtn), FALSE);
	} else
		g_free(tmp);

	gtk_table_attach(GTK_TABLE(table2), cmds_use_system_default_checkbtn, 0, 2, i, i + 1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 2);

#ifndef G_OS_WIN32
	uri_label = gtk_label_new(_("Web browser"));
	gtk_widget_show(uri_label);
	i++;
	gtk_table_attach(GTK_TABLE(table2), uri_label, 0, 1, i, i + 1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 2);
	gtk_label_set_justify(GTK_LABEL(uri_label), GTK_JUSTIFY_RIGHT);
	gtk_misc_set_alignment(GTK_MISC(uri_label), 1, 0.5);

	uri_combo = combobox_text_new(TRUE, DEFAULT_BROWSER_CMD, "galeon --new-tab '%s'", "galeon '%s'", "mozilla -remote 'openurl(%s,new-window)'", "netscape -remote 'openURL(%s, new-window)'", "netscape '%s'", "gnome-moz-remote --newwin '%s'", "kfmclient openURL '%s'", "opera -newwindow '%s'", "rxvt -e w3m '%s'", "rxvt -e lynx '%s'", NULL);
	gtk_table_attach(GTK_TABLE(table2), uri_combo, 1, 2, i, i + 1, GTK_EXPAND | GTK_FILL, 0, 0, 0);
	...
	i++;
	gtk_table_attach(GTK_TABLE(table2), exteditor_label, 0, 1, i, i + 1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 2);
	gtk_label_set_justify(GTK_LABEL(exteditor_label), GTK_JUSTIFY_RIGHT);
	gtk_misc_set_alignment(GTK_MISC(exteditor_label), 1, 0.5);

	exteditor_combo = combobox_text_new(TRUE, "gedit %s", "kedit %s", "mousepad %s", "nedit %s", "mgedit --no-fork %s", "emacs %s", "xemacs %s",
#ifdef G_OS_UNIX
					    "gvim -f --socketid %w %s", "emacsclient --parent-id %w %s",
#endif
					    "kterm -e jed %s", "kterm -e vi %s", NULL);
	gtk_table_attach(GTK_TABLE(table2), exteditor_combo, 1, 2, i, i + 1, GTK_EXPAND | GTK_FILL, 0, 0, 0);
	...
	i++;
	gtk_table_attach(GTK_TABLE(table2), astextviewer_label, 0, 1, i, i + 1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 2);
	...
	CLAWS_SET_TIP(astextviewer_entry, _("This option enables MIME parts to be displayed in the " "message view via a script when using the 'Display as text' " "contextual menu item"));

	gtk_table_attach(GTK_TABLE(table2), astextviewer_entry, 1, 2, i, i + 1, (GtkAttachOptions) (GTK_EXPAND | GTK_FILL), (GtkAttachOptions) (0), 0, 0);
	gtk_entry_set_text(GTK_ENTRY(astextviewer_entry), prefs_common.mime_textviewer ? prefs_common.mime_textviewer : "");
...
#ifndef G_OS_WIN32
	prefs_common.uri_cmd = gtk_editable_get_chars(GTK_EDITABLE(ext_prog->uri_entry), 0, -1);
#endif
	prefs_common.ext_editor_cmd = gtk_editable_get_chars(GTK_EDITABLE(ext_prog->exteditor_entry), 0, -1);
	prefs_common.mime_textviewer = gtk_editable_get_chars(GTK_EDITABLE(ext_prog->astextviewer_entry), 0, -1);
	prefs_common.cmds_use_system_default = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ext_prog->cmds_use_system_default_checkbtn));
}
