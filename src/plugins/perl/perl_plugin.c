...
static void filter_log_write(gint type, gchar *text)
{
	if (filter_log_verbosity >= type) {
		if (!wrote_filter_log_head) {
			log_message(LOG_PROTOCOL, "From: %s || Subject: %s || Message-ID: %s\n", msginfo->from ? msginfo->from : "<no From header>", msginfo->subject ? msginfo->subject : "<no Subject header>", msginfo->msgid ? msginfo->msgid : "<no message id>");
...
static gint add_to_email_slist(ItemPerson *person, const gchar *bookname)
{
	...
	while (nodeM) {
		...
		if (email->address != NULL)
			ee->address = g_strdup(email->address);
		else
			ee->address = NULL;
		if (bookname != NULL)
			ee->bookname = g_strdup(bookname);
		else
			ee->bookname = NULL;
...
static gboolean addr_in_addressbook(gchar *addr, gchar *bookname)
{
	...
	if (bookname == NULL) {
		...
		for (; walk != NULL; walk = g_slist_next(walk)) {
			...
			if ((!g_utf8_collate(a, b)) && ((bookname == NULL) || (!strcmp(ee->bookname, bookname)))) {
				g_free(a);
				g_free(b);
				return TRUE;
			}
	...
	while (nodeA) {
		...
		if (attrib->name && !strcmp(attrib->name, attribute_key)) {
			...
			while (nodeM) {
				...
				if (email->address != NULL)
					ae->address = g_strdup(email->address);
				else
					ae->address = NULL;
				if (attrib->value != NULL)
					ae->value = g_strdup(attrib->value);
				else
					ae->value = NULL;
				if (bookname != NULL)
					ae->bookname = g_strdup(bookname);
				else
					ae->bookname = NULL;
...
static gchar *get_attribute_value(gchar *email, gchar *attr, gchar *bookname)
{
	...
	for (; walk != NULL; walk = g_slist_next(walk)) {
		...
		if (!g_utf8_collate(a, b)) {
			if ((bookname == NULL) || ((ae->bookname != NULL) && !strcmp(bookname, ae->bookname))) {
				g_free(a);
				g_free(b);
				return ae->value;
			}
...
static XS(XS_ClawsMail_filter_init)
{
	...
	case 12:
		(msginfo->extradata && msginfo->extradata->dispositionnotificationto) ? XSRETURN_PV(msginfo->extradata->dispositionnotificationto) : XSRETURN_UNDEF;
	case 13:
		(msginfo->extradata && msginfo->extradata->returnreceiptto) ? XSRETURN_PV(msginfo->extradata->returnreceiptto) : XSRETURN_UNDEF;
	case 14:
	...
	case 17:
		msginfo->plaintext_file ? XSRETURN_PV(msginfo->plaintext_file) : XSRETURN_UNDEF;
	case 19:
	...
	case 20:
		if ((charp = procmsg_get_message_file_path(msginfo)) != NULL) {
			...
			XSRETURN_UNDEF;
	case 21:
		(msginfo->extradata && msginfo->extradata->partial_recv) ? XSRETURN_PV(msginfo->extradata->partial_recv) : XSRETURN_UNDEF;
	case 22:
		msginfo->total_size ? XSRETURN_IV(msginfo->total_size) : XSRETURN_UNDEF;
	case 23:
		(msginfo->extradata && msginfo->extradata->account_server) ? XSRETURN_PV(msginfo->extradata->account_server) : XSRETURN_UNDEF;
	case 24:
		(msginfo->extradata && msginfo->extradata->account_login) ? XSRETURN_PV(msginfo->extradata->account_login) : XSRETURN_UNDEF;
	case 25:
		msginfo->planned_download ? XSRETURN_IV(msginfo->planned_download) : XSRETURN_UNDEF;
...
static XS(XS_ClawsMail_move)
{
	...
	if (!dest_folder) {
		g_warning("Perl plugin: move: folder not found '%s'", targetfolder ? targetfolder : "");
		XSRETURN_UNDEF;
	}
...
static XS(XS_ClawsMail_copy)
{
	...
	if (!dest_folder) {
		g_warning("Perl plugin: copy: folder not found '%s'", targetfolder ? targetfolder : "");
		XSRETURN_UNDEF;
	}
...
static XS(XS_ClawsMail_forward)
{
	...
	compose = compose_forward(account, msginfo, flag == 1 ? FALSE : TRUE, NULL, TRUE, TRUE);
	compose_entry_append(compose, dest, compose->account->protocol == A_NNTP ? COMPOSE_NEWSGROUPS : COMPOSE_TO, PREF_NONE);

	val = compose_send(compose);

	if (val == 0) {

		logtext = g_strdup_printf("forward%s to %s", flag == 2 ? " as attachment" : "", dest ? dest : "<unknown destination>");
		filter_log_write(LOG_ACTION, logtext);
		g_free(logtext);
...
static XS(XS_ClawsMail_redirect)
{
	...
	if (val == 0) {
		...
		logtext = g_strdup_printf("redirect to %s", dest ? dest : "<unknown destination>");
		filter_log_write(LOG_ACTION, logtext);
		g_free(logtext);
...
static XS(XS_ClawsMail_filter_log_verbosity)
{
	...
	dXSARGS;
	if (items != 1 && items != 0) {
		g_warning("Perl plugin: wrong number of arguments to " "ClawsMail::C::filter_log_verbosity");
		XSRETURN_UNDEF;
	}
...
EXTERN_C void xs_init(pTHX)
{
	...
	newXS("ClawsMail::C::make_sure_folder_exists", XS_ClawsMail_make_sure_folder_exists, "ClawsMail::C");
	...
	newXS("ClawsMail::C::addr_in_addressbook", XS_ClawsMail_addr_in_addressbook, "ClawsMail::C");
	newXS("ClawsMail::C::open_mail_file", XS_ClawsMail_open_mail_file, "ClawsMail::C");
	newXS("ClawsMail::C::close_mail_file", XS_ClawsMail_close_mail_file, "ClawsMail::C");
	newXS("ClawsMail::C::get_next_header", XS_ClawsMail_get_next_header, "ClawsMail::C");
	newXS("ClawsMail::C::get_next_body_line", XS_ClawsMail_get_next_body_line, "ClawsMail::C");
	...
	newXS("ClawsMail::C::get_attribute_value", XS_ClawsMail_get_attribute_value, "ClawsMail::C");
	newXS("ClawsMail::C::filter_log", XS_ClawsMail_filter_log, "ClawsMail::C");
	newXS("ClawsMail::C::filter_log_verbosity", XS_ClawsMail_filter_log_verbosity, "ClawsMail::C");
}
...
static int perl_load_file(void)
{
	...
	call_argv("ClawsMail::Filter::Matcher::filter_init_", G_DISCARD | G_EVAL | G_NOARGS, noargs);
	/* check $@ */
	if (SvTRUE(ERRSV)) {
	...
	args[0] = perlfilter;
	call_argv("ClawsMail::Persistent::eval_file", G_DISCARD | G_EVAL, args);
	g_free(perlfilter);
	if (SvTRUE(ERRSV)) {
		AlertValue val;
		gchar *message;
		...
		debug_print("%s", SvPV(ERRSV, n_a));
		message = g_strdup_printf(_("Error processing Perl script file: " "(line numbers may not be valid)\n%s"), SvPV(ERRSV, n_a));
		val = alertpanel(_("Perl Plugin error"), message, _("Retry"), _("Abort"), _("Edit"), ALERTFOCUS_FIRST);
		g_free(message);
...
gint plugin_init(gchar **error)
{
	...
	/* version check */
	if (!check_plugin_version(MAKE_NUMERIC_VERSION(3, 7, 4, 6), VERSION_NUMERIC, "Perl", error))
		return -1;

	/* register hook for automatic and manual filtering */
	filtering_hook_id = hooks_register_hook(MAIL_FILTERING_HOOKLIST, my_filtering_hook, GUINT_TO_POINTER(AUTO_FILTER));
	if (filtering_hook_id == HOOK_NONE) {
		*error = g_strdup("Failed to register mail filtering hook");
		return -1;
	}
	manual_filtering_hook_id = hooks_register_hook(MAIL_MANUAL_FILTERING_HOOKLIST, my_filtering_hook, GUINT_TO_POINTER(MANU_FILTER));
	...
	if ((fp = claws_fopen(perlfilter, "a")) == NULL) {
		*error = g_strdup("Failed to create blank scriptfile");
		g_free(perlfilter);
		hooks_unregister_hook(MAIL_FILTERING_HOOKLIST, filtering_hook_id);
		hooks_unregister_hook(MAIL_MANUAL_FILTERING_HOOKLIST, manual_filtering_hook_id);
		return -1;
	}
	/* chmod for security */
...
{
	static struct PluginFeature features[] = { {PLUGIN_FILTERING, N_("Perl integration")},
	{PLUGIN_NOTHING, NULL}
	};
	return features;
}
