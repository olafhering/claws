...
static gboolean pdf_viewer_text_search(MimeViewer *_viewer, gboolean backward, const gchar *str, gboolean case_sens);
...
static void pdf_viewer_index_row_activated(GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, gpointer data);
...
static void _poppler_page_render_to_pixbuf(PopplerPage *page, int src_x, int src_y, int src_width, int src_height, double scale, int rotation, gboolean printing, GdkPixbuf *pixbuf)
{
	cairo_t *cr;
 ...
 * Deprecated: 0.16
 **/
static void poppler_page_render_to_pixbuf(PopplerPage *page, int src_x, int src_y, int src_width, int src_height, double scale, int rotation, GdkPixbuf *pixbuf)
{
	g_return_if_fail(POPPLER_IS_PAGE(page));
	g_return_if_fail(scale > 0.0);
	g_return_if_fail(pixbuf != NULL);

	_poppler_page_render_to_pixbuf(page, src_x, src_y, src_width, src_height, scale, rotation, FALSE, pixbuf);
}
#endif
static GtkWidget *pdf_viewer_get_widget(MimeViewer *_viewer)
{
...
static void pdf_viewer_scroll_to(PdfViewer *viewer, gfloat x, gfloat y)
{
	...
	if (y < gtk_adjustment_get_value(vadj)) {
		...
		while (y > gtk_adjustment_get_value(vadj) + gtk_adjustment_get_page_size(vadj)) {
			gtk_adjustment_set_value(vadj, gtk_adjustment_get_value(vadj) + gtk_adjustment_get_page_size(vadj));
		}
	}

	hadj = gtk_scrolled_window_get_hadjustment(GTK_SCROLLED_WINDOW(viewer->scrollwin));

	if (x < gtk_adjustment_get_value(hadj)) {
		gtk_adjustment_set_value(hadj, x);
	} else {
		while (x > gtk_adjustment_get_value(hadj) + gtk_adjustment_get_page_size(hadj)) {
			gtk_adjustment_set_value(hadj, gtk_adjustment_get_value(hadj) + gtk_adjustment_get_page_size(hadj));
		}
	}
...
static void pdf_viewer_render_page(PopplerPage *page, GtkWidget *view, double width, double height, double zoom, gint rotate)
{
	...
	debug_print("width: %f\n", width);
	pb = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, (int)(width * zoom), (int)(height * zoom));

	poppler_page_render_to_pixbuf(page, 0, 0, (int)(width * zoom), (int)(height * zoom), zoom, rotate, pb);
...
static void pdf_viewer_render_selection(PdfViewer *viewer, PopplerRectangle *rect, PageResult *page_results)
{
	...
	gint cur_page_num = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(viewer->cur_page));
	...
	if (cur_page_num != page_results->page_num) {
		...
		gtk_spin_button_set_value(GTK_SPIN_BUTTON(viewer->cur_page), (gdouble)page_results->page_num);
	}
	...
	sel_pb = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, selw, selh);

	gdk_pixbuf_fill(sel_pb, SELECTION_COLOR);

	page_pb = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, (int)(viewer->width * viewer->zoom), (int)(viewer->height * viewer->zoom));

	poppler_page_render_to_pixbuf(viewer->pdf_page, 0, 0, (int)(viewer->width * viewer->zoom), (int)(viewer->height * viewer->zoom), viewer->zoom, viewer->rotate, page_pb);

	gdk_pixbuf_composite(sel_pb, page_pb, x1, y2, selw, selh, 0, 0, viewer->zoom, viewer->zoom, GDK_INTERP_BILINEAR, ALPHA_CHANNEL);
...
static gboolean pdf_viewer_text_search(MimeViewer *_viewer, gboolean backward, const gchar *str, gboolean case_sens)
{
	...
	for (; all_pages_results; all_pages_results = (backward ? all_pages_results->prev : all_pages_results->next)) {
		...
		for (; cur_page_results; cur_page_results = (backward ? cur_page_results->prev : cur_page_results->next)) {
			...
			if (valid) {
				pdf_viewer_render_selection(viewer, (PopplerRectangle *)cur_page_results->data, page_results);
				main_window_cursor_normal(mainwindow_get_mainwindow());
				return TRUE;
			}
...
static void pdf_viewer_get_document_index(PdfViewer *viewer, PopplerIndexIter *index_iter, GtkTreeIter *parentiter)
{
	...
	do {
		...
		if (action->goto_dest.dest->type == POPPLER_DEST_XYZ || action->goto_dest.dest->type == POPPLER_DEST_FITH) {
			...
			PopplerDest *dest = poppler_document_find_dest(viewer->pdf_doc, action->goto_dest.dest->named_dest);
		...
		gtk_tree_store_set(GTK_TREE_STORE(viewer->index_model), &childiter, INDEX_NAME, action->named.title, INDEX_PAGE, page_num, INDEX_TOP, action->goto_dest.dest->top, -1);
		poppler_action_free(action);
...
static void pdf_viewer_index_row_activated(GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, gpointer data)
{
	...
	gtk_tree_model_get(model, &iter, INDEX_PAGE, &page_num, -1);
...
static GtkTable *pdf_viewer_fill_info_table(PdfViewer *viewer)
{
	...
	g_object_get(viewer->pdf_doc, "title", &title, "format", &format, "author", &author, "subject", &subject, "keywords", &keywords, "creation-date", &creation_date, "permissions", &permissions, "mod-date", &mod_date, "creator", &creator, "producer", &producer, "linearized", &linearized, "page-mode", &mode, "page-layout", &layout, "viewer-preferences", &view_prefs, NULL);
...
static void pdf_viewer_button_press_events_cb(GtkWidget *widget, GdkEventButton *event, PdfViewer *viewer)
{
	...
	if (event->button == 1 && viewer->in_link) {
		...
		case POPPLER_ACTION_GOTO_DEST:
			if (viewer->link_action->goto_dest.dest->type == POPPLER_DEST_XYZ || viewer->link_action->goto_dest.dest->type == POPPLER_DEST_FITH) {
				gtk_spin_button_set_value(GTK_SPIN_BUTTON(viewer->cur_page), (gdouble)viewer->link_action->goto_dest.dest->page_num);
			}
#ifdef HAVE_POPPLER_DEST_NAMED
			else if (viewer->link_action->goto_dest.dest->type == POPPLER_DEST_NAMED) {
				dest = poppler_document_find_dest(viewer->pdf_doc, viewer->link_action->goto_dest.dest->named_dest);
				if (dest->type != POPPLER_DEST_XYZ) {
					g_warning("couldn't figure out link");
					poppler_dest_free(dest);
					break;
				}
				gtk_spin_button_set_value(GTK_SPIN_BUTTON(viewer->cur_page), (gdouble)dest->page_num);
				poppler_dest_free(dest);
			}
#endif
			break;
...
#ifdef HAVE_POPPLER_DEST_NAMED
			dest = poppler_document_find_dest(viewer->pdf_doc, viewer->link_action->goto_remote.dest->named_dest);
			if (dest->type != POPPLER_DEST_XYZ) {
				g_warning("couldn't figure out link");
				poppler_dest_free(dest);
				break;
		...
		if (((MimeViewer *)viewer)->mimeview && ((MimeViewer *)viewer)->mimeview->messageview && ((MimeViewer *)viewer)->mimeview->messageview->window && (gdkwin = gtk_widget_get_window(((MimeViewer *)viewer)->mimeview->messageview->window)) != NULL)
...
static void pdf_viewer_move_events_cb(GtkWidget *widget, GdkEventMotion *event, PdfViewer *viewer)
{
	...
	if (viewer->pdf_view_scroll) {
		...
		if (event->x < viewer->last_x && gtk_adjustment_get_value(viewer->pdf_view_hadj) < (gtk_adjustment_get_upper(viewer->pdf_view_hadj) - gtk_adjustment_get_page_size(viewer->pdf_view_hadj))) {
			if (viewer->last_dir_x == -1) {
				gtk_adjustment_set_value(viewer->pdf_view_hadj, gtk_adjustment_get_value(viewer->pdf_view_hadj)
							 + viewer->last_x - event->x);
				g_signal_emit_by_name(G_OBJECT(viewer->pdf_view_hadj), "value_changed", 0);
			}
			viewer->last_dir_x = -1;
		} else if (event->x > viewer->last_x && gtk_adjustment_get_value(viewer->pdf_view_hadj) > 0.0) {
			if (viewer->last_dir_x == +1) {
				gtk_adjustment_set_value(viewer->pdf_view_hadj, gtk_adjustment_get_value(viewer->pdf_view_hadj)
							 + viewer->last_x - event->x);
				g_signal_emit_by_name(G_OBJECT(viewer->pdf_view_hadj), "value_changed", 0);
		...
		if (event->y < viewer->last_y && gtk_adjustment_get_value(viewer->pdf_view_vadj) < (gtk_adjustment_get_upper(viewer->pdf_view_vadj) - gtk_adjustment_get_page_size(viewer->pdf_view_vadj))) {
			if (viewer->last_dir_y == -1) {
				gtk_adjustment_set_value(viewer->pdf_view_vadj, gtk_adjustment_get_value(viewer->pdf_view_vadj)
							 + viewer->last_y - event->y);
				g_signal_emit_by_name(G_OBJECT(viewer->pdf_view_vadj), "value_changed", 0);
			}
			viewer->last_dir_y = -1;
		} else if (event->y > viewer->last_y && gtk_adjustment_get_value(viewer->pdf_view_vadj) > 0.0) {
			if (viewer->last_dir_y == +1) {
				gtk_adjustment_set_value(viewer->pdf_view_vadj, gtk_adjustment_get_value(viewer->pdf_view_vadj)
							 + viewer->last_y - event->y);
				g_signal_emit_by_name(G_OBJECT(viewer->pdf_view_vadj), "value_changed", 0);
...
static gboolean pdf_viewer_scroll_cb(GtkWidget *widget, GdkEventScroll *event, PdfViewer *viewer)
{
	...
	if (event->direction == GDK_SCROLL_UP && gtk_adjustment_get_value(adj) == gtk_adjustment_get_lower(adj) && cur_p > 1) {
		gtk_spin_button_spin(GTK_SPIN_BUTTON(viewer->cur_page), GTK_SPIN_STEP_BACKWARD, 1);
		gtk_adjustment_set_value(adj, gtk_adjustment_get_upper(adj) - gtk_adjustment_get_page_size(adj));
		handled = TRUE;
	} else if (event->direction == GDK_SCROLL_DOWN && gtk_adjustment_get_value(adj) + gtk_adjustment_get_page_size(adj) == gtk_adjustment_get_upper(adj) && cur_p < viewer->num_pages) {
		gtk_spin_button_spin(GTK_SPIN_BUTTON(viewer->cur_page), GTK_SPIN_STEP_FORWARD, 1);
...
static void pdf_viewer_button_document_info_cb(GtkButton *button, PdfViewer *viewer)
{
	alertpanel_full(_("PDF properties"), NULL, GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, FALSE, GTK_WIDGET(pdf_viewer_fill_info_table(viewer)), ALERT_NOTICE);
}
...
static void pdf_viewer_update(MimeViewer *_viewer, gboolean reload_file, int page_num)
{
	...
	if (reload_file) {
		...
		if (pdf_viewer_mimepart_get_type(viewer->to_load) == TYPE_PS) {
			stock_pixbuf_gdk(STOCK_PIXMAP_MIME_PS, &viewer->icon_pixbuf);
			...
			stock_pixbuf_gdk(STOCK_PIXMAP_MIME_PDF, &viewer->icon_pixbuf);
			gtk_image_set_from_pixbuf(GTK_IMAGE(viewer->icon_type), viewer->icon_pixbuf);
		} else {
			stock_pixbuf_gdk(STOCK_PIXMAP_MIME_APPLICATION, &viewer->icon_pixbuf);
			gtk_image_set_from_pixbuf(GTK_IMAGE(viewer->icon_type), viewer->icon_pixbuf);
		}
		...
		if (pdf_viewer_mimepart_get_type(viewer->to_load) == TYPE_PS) {
			...
			if ((gspath = g_find_program_in_path("gs")) != NULL) {
				...
				cmdline = g_strdup_printf("gs -dSAFER -dCompatibilityLevel=1.2 -q -dNOPAUSE -dBATCH " "-sDEVICE=pdfwrite -sOutputFile=%s -f \"%s\"", tmpfile, viewer->filename);
		...
		if (error && g_error_matches(error, POPPLER_ERROR, POPPLER_ERROR_ENCRYPTED)) {
			g_clear_error(&error);
			password = input_dialog_with_invisible(_("Enter password"), _("This document is locked and requires a password before it can be opened."), "");
		...
		CLAWS_SET_TIP(GTK_WIDGET(viewer->icon_type_ebox), tmp);
		g_free(tmp);
	...
	if (page_num > 0 && page_num <= viewer->num_pages) {
		...
		if (viewer->last_rect && viewer->last_page_result && viewer->last_page_result->page_num == page_num) {
			pdf_viewer_render_selection(viewer, viewer->last_rect, viewer->last_page_result);
		} else {
			pdf_viewer_render_page(viewer->pdf_page, viewer->pdf_view, viewer->width, viewer->height, viewer->zoom, viewer->rotate);
...
{
	PdfViewer *viewer = (PdfViewer *)_viewer;
	GtkAdjustment *vadj = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(viewer->scrollwin));
...
static void pdf_viewer_cb_begin_print(GtkPrintOperation *op, GtkPrintContext *context, gpointer user_data)
{
...
static void pdf_viewer_cb_draw_page(GtkPrintOperation *op, GtkPrintContext *context, int page_nr, gpointer user_data)
{
	cairo_t *cr;
	PrintData *print_data;
	PopplerDocument *pdf_doc;
...
static void pdf_viewer_print(MimeViewer *mviewer)
{
	...
	printing_print_full(mainwin ? GTK_WINDOW(mainwin->window) : NULL, pdf_renderer, viewer->pdf_doc, -1, -1, NULL);
...
static MimeViewer *pdf_viewer_create(void)
{
	...
	gtk_widget_set_events(viewer->pdf_view_ebox, GDK_BUTTON_RELEASE_MASK | GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_MOTION_MASK);
	...
	ADD_SEP_TO_TABLE ADD_BUTTON_TO_TABLE(viewer->first_page, STOCK_PIXMAP_FIRST_ARROW)
	...
	gtk_table_attach(GTK_TABLE(viewer->widgets_table), GTK_WIDGET(viewer->cur_page), col, col + 1, 0, 1, 0, 0, BUTTON_H_PADDING, 0);
	col++;
	gtk_table_attach(GTK_TABLE(viewer->widgets_table), GTK_WIDGET(viewer->doc_label), col, col + 1, 0, 1, 0, 0, BUTTON_H_PADDING, 0);
	col++;
	...
	ADD_BUTTON_TO_TABLE(viewer->next_page, STOCK_PIXMAP_RIGHT_ARROW)
	    ...
	    ADD_SEP_TO_TABLE ADD_BUTTON_TO_TABLE(viewer->zoom_fit, STOCK_PIXMAP_ZOOM_FIT)
	    ADD_BUTTON_TO_TABLE(viewer->zoom_in, STOCK_PIXMAP_ZOOM_IN)
	    gtk_table_attach(GTK_TABLE(viewer->widgets_table), GTK_WIDGET(viewer->zoom_scroll), col, col + 1, 0, 1, 0, 0, BUTTON_H_PADDING, 0);
	col++;
	ADD_BUTTON_TO_TABLE(viewer->zoom_out, STOCK_PIXMAP_ZOOM_OUT)
	    ADD_BUTTON_TO_TABLE(viewer->zoom_width, STOCK_PIXMAP_ZOOM_WIDTH)
	    ADD_SEP_TO_TABLE ADD_BUTTON_TO_TABLE(viewer->rotate_left, STOCK_PIXMAP_ROTATE_LEFT)
	    ADD_BUTTON_TO_TABLE(viewer->rotate_right, STOCK_PIXMAP_ROTATE_RIGHT)
	    ADD_SEP_TO_TABLE ADD_BUTTON_TO_TABLE(viewer->print, STOCK_PIXMAP_PRINTER)
	...
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(viewer->scrollwin), GTK_SHADOW_IN);

	gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(viewer->scrollwin), viewer->pdf_view_ebox);

	viewer->vbox = gtk_vbox_new(FALSE, 4);
	viewer->hbox = gtk_hbox_new(FALSE, 4);

	/* treeview */
	tree_store = gtk_tree_store_new(N_INDEX_COLUMNS, G_TYPE_STRING, G_TYPE_INT, G_TYPE_DOUBLE);
	...
	g_signal_connect(G_OBJECT(viewer->index_list), "row_activated", G_CALLBACK(pdf_viewer_index_row_activated), viewer);

	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(viewer->scrollwin_index), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);

	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(viewer->scrollwin_index), GTK_SHADOW_IN);

	gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(viewer->scrollwin_index), viewer->index_list);
	...
	/* Connect Signals */
	g_signal_connect(G_OBJECT(viewer->cur_page), "value-changed", G_CALLBACK(pdf_viewer_spin_change_page_cb), (gpointer)viewer);

	g_signal_connect(G_OBJECT(viewer->first_page), "clicked", G_CALLBACK(pdf_viewer_button_first_page_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->prev_page), "clicked", G_CALLBACK(pdf_viewer_button_prev_page_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->next_page), "clicked", G_CALLBACK(pdf_viewer_button_next_page_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->last_page), "clicked", G_CALLBACK(pdf_viewer_button_last_page_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->zoom_in), "clicked", G_CALLBACK(pdf_viewer_button_zoom_in_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->zoom_out), "clicked", G_CALLBACK(pdf_viewer_button_zoom_out_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->zoom_scroll), "value-changed", G_CALLBACK(pdf_viewer_spin_zoom_scroll_cb), (gpointer)viewer);

	g_signal_connect(G_OBJECT(viewer->zoom_fit), "clicked", G_CALLBACK(pdf_viewer_button_zoom_fit_cb), (gpointer)viewer);

	g_signal_connect(G_OBJECT(viewer->zoom_width), "clicked", G_CALLBACK(pdf_viewer_button_zoom_width_cb), (gpointer)viewer);

	g_signal_connect(G_OBJECT(viewer->rotate_right), "clicked", G_CALLBACK(pdf_viewer_button_rotate_right_cb), (gpointer)viewer);

	g_signal_connect(G_OBJECT(viewer->rotate_left), "clicked", G_CALLBACK(pdf_viewer_button_rotate_left_cb), (gpointer)viewer);

	g_signal_connect(G_OBJECT(viewer->print), "clicked", G_CALLBACK(pdf_viewer_button_print_cb), (gpointer)viewer);

	g_signal_connect(G_OBJECT(viewer->doc_info), "clicked", G_CALLBACK(pdf_viewer_button_document_info_cb), (gpointer)viewer);

	g_signal_connect(G_OBJECT(viewer->doc_index), "clicked", G_CALLBACK(pdf_viewer_show_document_index_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->scrollwin), "scroll-event", G_CALLBACK(pdf_viewer_scroll_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->pdf_view_ebox), "button_press_event", G_CALLBACK(pdf_viewer_button_press_events_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->pdf_view_ebox), "button_release_event", G_CALLBACK(pdf_viewer_mouse_scroll_destroy_cb), (gpointer)viewer);
	g_signal_connect(G_OBJECT(viewer->pdf_view_ebox), "motion_notify_event", G_CALLBACK(pdf_viewer_move_events_cb), (gpointer)viewer);
...
gint plugin_init(gchar **error)
{
	...
	msg = g_strdup_printf(_("This plugin enables the viewing of PDF and PostScript " "attachments using the Poppler %s Lib and the gs tool.\n\n" "Any feedback is welcome: iwkse@claws-mail.org"), poppler_get_version());

	if (!check_plugin_version(MAKE_NUMERIC_VERSION(3, 8, 1, 46), VERSION_NUMERIC, _("PDF Viewer"), error))
		return -1;

	if ((gspath = g_find_program_in_path("gs")) == NULL) {
		gchar *pmsg = msg;
		msg = g_strdup_printf(_("Warning: could not find ghostscript binary (gs) required " "for %s plugin to process PostScript attachments, only PDF " "attachments will be displayed. To enable PostScript " "support please install gs program.\n\n%s"), _("PDF Viewer"), pmsg);
		g_free(pmsg);
	} else {
		g_free(gspath);
	}
...
struct PluginFeature *plugin_provides(void)
{
	static struct PluginFeature features[] = { {PLUGIN_MIMEVIEWER, "application/pdf"},
