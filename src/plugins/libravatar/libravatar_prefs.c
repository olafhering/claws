...
{
	if (stats == NULL)
		return g_strdup(g_strconcat("<span color=\"red\">", _("Error reading cache stats"), "</span>", NULL));

	if (stats->errors > 0)
		return g_markup_printf_escaped(g_strconcat("<span color=\"red\">", _("Using %s in %d files, %d " "directories, %d others and %d errors"), "</span>", NULL), to_human_readable((goffset) stats->bytes), stats->files, stats->dirs, stats->others, stats->errors);
