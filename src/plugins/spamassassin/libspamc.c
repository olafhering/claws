...
static int _opensocket(int flags, int type, int *psock)
{
	...
	if (type == PF_INET && setsockopt(*psock, SOL_SOCKET, SO_RCVTIMEO, (char *)&socktout, sizeof(socktout)) != 0) {
...
static int _try_to_connect_unix(struct transport *tp, int *sockptr)
{
...
#ifdef DO_CONNECT_DEBUG_SYSLOGS
	libspamc_log(tp->flags, CONNECT_DEBUG_LEVEL, "dbg: connect(AF_UNIX) to spamd at %s", addrbuf.sun_path);
#endif
	...
	libspamc_log(tp->flags, LOG_ERR, "connect(AF_UNIX) to spamd using --socket='%s' failed: %s", addrbuf.sun_path, strerror(origerr));
	closesocket(mysock);
	...
	for (numloops = 0; numloops < connect_retries; numloops++) {
		...
		while (res) {
...
#ifdef DO_CONNECT_DEBUG_SYSLOGS
			libspamc_log(tp->flags, CONNECT_DEBUG_LEVEL, "dbg: connect(%s) to spamd (host %s, port %s) (try #%d of %d)", family, host, port, numloops + 1, connect_retries);
#endif
			...
			if (!strcmp(host, "255.255.255.255")) {
				libspamc_log(tp->flags, LOG_ERR, "connect to spamd on %s failed, broadcast addr", host);
				status = -1;
			} else {
...
#ifdef DO_CONNECT_DEBUG_SYSLOGS
		libspamc_log(tp->flags, LOG_DEBUG, "dbg: connect(AF_INET) to spamd at %s (try #%d of %d)", ipaddr, numloops + 1, connect_retries);
#endif

		/* this is special-cased so that we have an address we can
		 * safely use as an "always fail" test case */
		if (!strcmp(ipaddr, "255.255.255.255")) {
			libspamc_log(tp->flags, LOG_ERR, "connect to spamd on %s failed, broadcast addr", ipaddr);
			status = -1;
		} else {
			status = timeout_connect(mysock, (struct sockaddr *)&addrbuf, sizeof(addrbuf));
			if (status != 0)
		...
		if (status != 0) {
			...
			libspamc_log(tp->flags, innocent ? LOG_DEBUG : LOG_ERR, "connect to spamd on %s failed, retrying (#%d of %d): %s", host, numloops + 1, connect_retries,
#ifdef _WIN32
				     origerr
#else
				     strerror(origerr)
...
#ifdef DO_CONNECT_DEBUG_SYSLOGS
			libspamc_log(tp->flags, CONNECT_DEBUG_LEVEL, "dbg: connect(%s) to spamd done", family);
#endif
			*sockptr = mysock;
...
#endif
	if (numloops + 1 < connect_retries && !innocent)
		sleep(retry_sleep);
} /* for(numloops...) */

libspamc_log(tp->flags, LOG_ERR, "connection attempt to spamd aborted after %d retries", connect_retries);
...
static int _message_read_raw(int fd, struct message *m)
{
	...
	if (m->raw_len > (int)m->max_len) {
		libspamc_log(m->priv->flags, LOG_NOTICE, "skipped message, greater than max message size (%d bytes)", m->max_len);
		return EX_TOOBIG;
	}
	m->type = MESSAGE_RAW;
	m->msg = m->raw;
...
static int _message_read_bsmtp(int fd, struct message *m)
{
	...
	while ((p_len = (m->raw_len - (p - m->raw))) > 8) { /* leave room for at least \nDATA\n.\n */
		...
		if (((q[0] | 0x20) == 'd') && /* case-insensitive ASCII comparison */
		    ((q[1] | 0x20) == 'a') && ((q[2] | 0x20) == 't') && ((q[3] | 0x20) == 'a')) {
			q += 4;
			if (q[0] == '\r')
				++q;
...
int message_read(int fd, int flags, struct message *m)
{
	...
	default:
		libspamc_log(flags, LOG_ERR, "message_read: Unknown mode %d", flags & SPAMC_MODE_MASK);
		return EX_USAGE;
	}
}
...
long message_write(int fd, struct message *m)
{
	...
	if (m->priv->flags & (SPAMC_CHECK_ONLY | SPAMC_PING)) {
		if (m->is_spam == EX_ISSPAM || m->is_spam == EX_NOTSPAM) {
			...
			libspamc_log(m->priv->flags, LOG_ERR, "oops! SPAMC_CHECK_ONLY is_spam: %d", m->is_spam);
			return -1;
		}
	}
...
void message_dump(int in_fd, int out_fd, struct message *m, int flags)
{
	...
	while ((bytes = full_read(in_fd, 1, buf, 8192, 8192)) > 0) {
		if (bytes != full_write(out_fd, 1, buf, bytes)) {
			libspamc_log(flags, LOG_ERR, "oops! message_dump of %d returned different", bytes);
		}
	}
}
...
static int _handle_spamd_header(struct message *m, int flags, char *buf, int len, unsigned int *didtellflags)
{
	...
	if (sscanf(buf, "Spam: %5s ; %20s / %20s", is_spam, s_str, t_str) == 3) {
		...
		if (m->content_length < 0) {
			libspamc_log(flags, LOG_ERR, "spamd responded with bad Content-length '%s'", buf);
			return EX_PROTOCOL;
		}
		return EX_OK;
...
#define NLNL_LEN        2
	...
	for (cp = m->raw; cp < cpend; cp++) {
		if (*cp == '\r' && cpend - cp >= CRNLCRNL_LEN && !strncmp(cp, CRNLCRNL, CRNLCRNL_LEN)) {
			bodystart = cp + CRNLCRNL_LEN;
			break;
		} else if (*cp == '\n' && cpend - cp >= NLNL_LEN && !strncmp(cp, NLNL, NLNL_LEN)) {
...
int message_filter(struct transport *tp, const char *username, int flags, struct message *m)
{
	...
	if (version < 1.0) {
		libspamc_log(flags, LOG_ERR, "spamd responded with bad version string '%s'", versbuf);
		failureval = EX_PROTOCOL;
		goto failure;
	}
	...
	if (flags & SPAMC_CHECK_ONLY) {
		...
		if (flags & SPAMC_USE_SSL) {
			len = full_read_ssl(ssl, (unsigned char *)m->out + m->out_len, m->priv->alloced_size - m->out_len, m->priv->alloced_size - m->out_len);
		} else {
			len = full_read(sock, 0, m->out + m->out_len, m->priv->alloced_size - m->out_len, m->priv->alloced_size - m->out_len);
		}
	...
	if (m->out_len != m->content_length) {
		libspamc_log(flags, LOG_ERR, "failed sanity check, %d bytes claimed, %d bytes seen", m->content_length, m->out_len);
		failureval = EX_PROTOCOL;
		goto failure;
	}
...
int transport_setup(struct transport *tp, int flags)
{
	...
	case TRANSPORT_LOCALHOST:
		...
		if ((origerr = getaddrinfo(NULL, port, &hints, &res)) != 0) {
			libspamc_log(flags, LOG_ERR, "getaddrinfo for a loopback address failed: %s", gai_strerror(origerr));
			return EX_OSERR;
		}
		tp->hosts[0] = res;
#else
	...
	case TRANSPORT_TCP:
		...
		do {
			...
			if ((origerr = getaddrinfo(hostname, port, &hints, &res))) {
				libspamc_log(flags, LOG_DEBUG, "getaddrinfo(%s) failed: %s", hostname, gai_strerror(origerr));
				switch (origerr) {
				case EAI_AGAIN:
					errbits |= 1;
					break;
...
/* work around Cygwin IPv6 patch - err codes not defined in Windows aren't in patch */
			...
			if ((hp = gethostbyname(hostname)) == NULL) {
				...
				libspamc_log(flags, LOG_DEBUG, "gethostbyname(%s) failed: h_errno=%d", hostname, origerr);
				switch (origerr) {
				case TRY_AGAIN:
					errbits |= 1;
					break;
			...
			if (res == NULL)
#else
			if (hp->h_addr_list[0] == NULL || hp->h_length != sizeof tp->hosts[0]
			...
			if (tp->nhosts == TRANSPORT_MAX_HOSTS) {
				libspamc_log(flags, LOG_NOTICE, "hit limit of %d hosts, ignoring remainder", TRANSPORT_MAX_HOSTS);
				break;
			}
 ...
 */
 ...
 nexthost:
		...
		if (tp->nhosts == 0) {
			if (errbits & 1) {
				libspamc_log(flags, LOG_ERR, "could not resolve any hosts (%s): a temporary error occurred", tp->hostname);
				return EX_TEMPFAIL;
			} else {
				libspamc_log(flags, LOG_ERR, "could not resolve any hosts (%s): no such host", tp->hostname);
				return EX_NOHOST;
			}
		}
