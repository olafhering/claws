...
#else
G_DEFINE_TYPE_WITH_CODE(GtkHotkeyInfo, gtk_hotkey_info, G_TYPE_OBJECT, G_ADD_PRIVATE(GtkHotkeyInfo))
#endif
/**
...
gboolean gtk_hotkey_info_bind(GtkHotkeyInfo *self, GError **error)
{
	...
	if (gtk_hotkey_info_is_bound(self)) {
		g_set_error(error, GTK_HOTKEY_LISTENER_ERROR, GTK_HOTKEY_LISTENER_ERROR_BIND, "Can not bind hotkey '%s' with signature '%s'. " "It is already bound", gtk_hotkey_info_get_key_id(self), gtk_hotkey_info_get_signature(self));
		return FALSE;
	}
...
gboolean gtk_hotkey_info_unbind(GtkHotkeyInfo *self, GError **error)
{
	...
	if (!gtk_hotkey_info_is_bound(self)) {
		g_set_error(error, GTK_HOTKEY_LISTENER_ERROR, GTK_HOTKEY_LISTENER_ERROR_UNBIND, "Can not unbind hotkey '%s' with signature '%s'." "It is not bound", gtk_hotkey_info_get_key_id(self), gtk_hotkey_info_get_signature(self));
		return FALSE;
	}

	g_return_val_if_fail(GTK_HOTKEY_IS_LISTENER(priv->listener), FALSE);

	result = gtk_hotkey_listener_unbind_hotkey(priv->listener, self, error);
...
gboolean gtk_hotkey_info_equals(GtkHotkeyInfo *hotkey1, GtkHotkeyInfo *hotkey2, gboolean sloppy_equals)
{
	...
	if (!g_str_equal(gtk_hotkey_info_get_application_id(hotkey1), gtk_hotkey_info_get_application_id(hotkey2)))
		return FALSE;

	if (!g_str_equal(gtk_hotkey_info_get_key_id(hotkey1), gtk_hotkey_info_get_key_id(hotkey2)))
		return FALSE;

	if (!g_str_equal(gtk_hotkey_info_get_signature(hotkey1), gtk_hotkey_info_get_signature(hotkey2)))
		return FALSE;
	...
	if (d1 != NULL && d2 != NULL) {
		if (!g_str_equal(gtk_hotkey_info_get_description(hotkey1), gtk_hotkey_info_get_description(hotkey2)))
			return FALSE;
	} else if (d1 != d2)
		return FALSE;
...
GtkHotkeyInfo *gtk_hotkey_info_new(const gchar *app_id, const gchar *key_id, const gchar *signature, GAppInfo *app_info)
{
	...
	self = g_object_new(GTK_HOTKEY_TYPE_INFO, "application-id", app_id, "key-id", key_id, "signature", signature, "app-info", app_info, NULL);
	return self;
}

static void gtk_hotkey_info_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
	GtkHotkeyInfo *self;
	GtkHotkeyInfoPrivate *priv;
	...
	case GTK_HOTKEY_INFO_BOUND:
		g_value_set_boolean(value, (priv->listener != NULL));
		break;
	case GTK_HOTKEY_INFO_APPLICATION_ID:
		g_value_set_string(value, gtk_hotkey_info_get_application_id(self));
		break;
	case GTK_HOTKEY_INFO_KEY_ID:
		g_value_set_string(value, gtk_hotkey_info_get_key_id(self));
		break;
	case GTK_HOTKEY_INFO_APP_INFO:
		g_value_set_object(value, gtk_hotkey_info_get_app_info(self));
		break;
	case GTK_HOTKEY_INFO_SIGNATURE:
		g_value_set_string(value, gtk_hotkey_info_get_signature(self));
		break;
	case GTK_HOTKEY_INFO_DESCRIPTION:
		g_value_set_string(value, gtk_hotkey_info_get_description(self));
		break;
	default:
...
static void gtk_hotkey_info_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
	GtkHotkeyInfo *self;
	GtkHotkeyInfoPrivate *priv;
...
static void gtk_hotkey_info_class_init(GtkHotkeyInfoClass *klass)
{
	gtk_hotkey_info_parent_class = g_type_class_peek_parent(klass);

#if !GLIB_CHECK_VERSION(2, 58, 0)
	g_type_class_add_private(G_OBJECT_CLASS(klass), sizeof(GtkHotkeyInfoPrivate));
#endif
	 ...
	 * by the user.
	 */
	g_object_class_install_property(G_OBJECT_CLASS(klass), GTK_HOTKEY_INFO_BOUND, g_param_spec_boolean("bound", "Is Bound", "Whether or not the hotkey is bound to a GtkHotkeyListener", FALSE, G_PARAM_READABLE));
	 ...
	 */
	g_object_class_install_property(G_OBJECT_CLASS(klass), GTK_HOTKEY_INFO_APPLICATION_ID, g_param_spec_string("application-id", "Application Id", "Globally unique application id", NULL, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
	 ...
	 */
	g_object_class_install_property(G_OBJECT_CLASS(klass), GTK_HOTKEY_INFO_KEY_ID, g_param_spec_string("key-id", "Hotkey Id", "Globally unique identifier for the hotkey", NULL, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
	 ...
	 */
	g_object_class_install_property(G_OBJECT_CLASS(klass), GTK_HOTKEY_INFO_APP_INFO, g_param_spec_object("app-info", "Application Information", "Object holding metadata about " "the hotkey's application", G_TYPE_APP_INFO, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
	 ...
	 */
	g_object_class_install_property(G_OBJECT_CLASS(klass), GTK_HOTKEY_INFO_SIGNATURE, g_param_spec_string("signature", "Signature", "String defining the keyboard shortcut", NULL, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
	 ...
	 */
	g_object_class_install_property(G_OBJECT_CLASS(klass), GTK_HOTKEY_INFO_DESCRIPTION, g_param_spec_string("description", "Description", "Short description of what happens upon activation", "", G_PARAM_READABLE | G_PARAM_WRITABLE));
	 ...
	 */
	info_signals[ACTIVATED] = g_signal_new("activated", GTK_HOTKEY_TYPE_INFO, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);
}

static void gtk_hotkey_info_init(GtkHotkeyInfo *self)
{
	GtkHotkeyInfoPrivate *priv = GTK_HOTKEY_INFO_GET_PRIVATE(self);
