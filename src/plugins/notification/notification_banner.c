...
static gboolean notification_banner_configure(GtkWidget *, GdkEventConfigure *, gpointer);
...
{
	G_LOCK(banner);
	if ((notify_config.banner_show != NOTIFY_BANNER_SHOW_NEVER) && (g_slist_length(msg_list) || (notify_config.banner_show == NOTIFY_BANNER_SHOW_ALWAYS)))
...
static void notification_banner_create(GSList *msg_list)
{
	...
	if (!banner.window) {
		...
		gtk_window_move(GTK_WINDOW(banner.window), notify_config.banner_root_x, notify_config.banner_root_y);
		g_signal_connect(banner.window, "configure-event", G_CALLBACK(notification_banner_configure), NULL);
	} else {
		if (banner.entries) {
			g_free(banner.entries);
			banner.entries = NULL;
	...
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(banner.scrolled_win), GTK_POLICY_NEVER, GTK_POLICY_NEVER);
	...
	if (requisition.width > banner_width) {
		...
		sdata.adj = gtk_scrolled_window_get_hadjustment(GTK_SCROLLED_WINDOW(banner.scrolled_win));
		G_UNLOCK(sdata);
		...
		banner.timeout_id = g_timeout_add(notify_config.banner_speed, scroller, NULL);
	} else {
	...
	banner_action_group = cm_menu_create_action_group_full(banner_ui_manager, "BannerPopup", banner_popup_entries, G_N_ELEMENTS(banner_popup_entries), (gpointer)NULL);
	MENUITEM_ADDUI_MANAGER(banner_ui_manager, "/", "Menus", "Menus", GTK_UI_MANAGER_MENUBAR)
	    ...
	    banner_popup = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(banner_ui_manager, "/Menus/BannerPopup")));
	g_signal_connect(banner_popup, "selection-done", G_CALLBACK(notification_banner_popup_done), NULL);
}

static gboolean notification_banner_configure(GtkWidget *widget, GdkEventConfigure *event, gpointer data)
{
	gtk_window_get_position(GTK_WINDOW(gtk_widget_get_toplevel(widget)), &(notify_config.banner_root_x), &(notify_config.banner_root_y));
	return TRUE;
}
...
static gboolean scroller(gpointer data)
{
	...
	if (sdata.adj && GTK_IS_ADJUSTMENT(sdata.adj)) {
		if (gtk_adjustment_get_value(sdata.adj) != sdata.banner_width)
			gtk_adjustment_set_value(sdata.adj, gtk_adjustment_get_value(sdata.adj) + 1);
		else
...
static GtkWidget *create_entrybox(GSList *msg_list)
{
	...
	if (list_length) {
		...
		for (walk = msg_list; walk != NULL; walk = g_slist_next(walk)) {
			...
			gtk_widget_set_events(ebox, GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK);
			...
			g_signal_connect(ebox, "enter-notify-event", G_CALLBACK(notification_banner_swap_colors), banner.entries[ii].table);
			g_signal_connect(ebox, "leave-notify-event", G_CALLBACK(notification_banner_swap_colors), banner.entries[ii].table);
			g_signal_connect(ebox, "button-press-event", G_CALLBACK(notification_banner_button_press), cmsg);

			label1 = gtk_label_new(prefs_common_translated_header_name("From:"));
			gtk_misc_set_alignment(GTK_MISC(label1), 0, 0.5);
			gtk_table_attach_defaults(GTK_TABLE(banner.entries[ii].table), label1, 0, 1, 0, 1);
			label2 = gtk_label_new(prefs_common_translated_header_name("Subject:"));
			gtk_misc_set_alignment(GTK_MISC(label2), 0, 0.5);
			gtk_table_attach_defaults(GTK_TABLE(banner.entries[ii].table), label2, 0, 1, 1, 2);
			label3 = gtk_label_new(_("Folder:"));
			gtk_misc_set_alignment(GTK_MISC(label3), 0, 0.5);
			gtk_table_attach_defaults(GTK_TABLE(banner.entries[ii].table), label3, 0, 1, 2, 3);

			label4 = gtk_label_new(cmsg->from);
			gtk_misc_set_alignment(GTK_MISC(label4), 0, 0.5);
			gtk_table_attach_defaults(GTK_TABLE(banner.entries[ii].table), label4, 1, 2, 0, 1);
			label5 = gtk_label_new(cmsg->subject);
			gtk_misc_set_alignment(GTK_MISC(label5), 0, 0.5);
			gtk_table_attach_defaults(GTK_TABLE(banner.entries[ii].table), label5, 1, 2, 1, 2);
			label6 = gtk_label_new(cmsg->folderitem_name);
			gtk_misc_set_alignment(GTK_MISC(label6), 0, 0.5);
			gtk_table_attach_defaults(GTK_TABLE(banner.entries[ii].table), label6, 1, 2, 2, 3);
		...
		label = gtk_label_new("");
		gtk_table_attach_defaults(GTK_TABLE(banner.entries[0].table), label, 0, 1, 0, 1);
		label = gtk_label_new("");
		gtk_table_attach_defaults(GTK_TABLE(banner.entries[0].table), label, 0, 1, 1, 2);
		label = gtk_label_new("");
		gtk_table_attach_defaults(GTK_TABLE(banner.entries[0].table), label, 0, 1, 2, 3);
		gtk_box_pack_start(GTK_BOX(entrybox), banner.entries[0].table, FALSE, FALSE, 0);
	}
	return entrybox;
}

static gboolean notification_banner_swap_colors(GtkWidget *widget, GdkEventCrossing *event, gpointer data)
{
	GList *children;
	GList *walk;
	GdkColor *old_bg;
	...
	if (children)
		gtk_widget_modify_bg(widget, GTK_STATE_NORMAL, &(gtk_widget_get_style(GTK_WIDGET(children->data))->fg[GTK_STATE_NORMAL]));
...
static gboolean notification_banner_button_press(GtkWidget *widget, GdkEventButton *button, gpointer data)
{
	gboolean done;
	done = FALSE;
	if (button->type == GDK_BUTTON_PRESS) {
		...
		if (button->button == 1) {
			...
			gtk_window_begin_move_drag(GTK_WINDOW(gtk_widget_get_toplevel(widget)), button->button, button->x_root, button->y_root, button->time);
...
static void notification_banner_show_popup(GtkWidget *widget, GdkEventButton *event)
{
	...
	gtk_menu_popup(GTK_MENU(banner_popup), NULL, NULL, NULL, NULL, button, event_time);
}

static void notification_banner_popup_done(GtkMenuShell *menushell, gpointer user_data)
{
...
static void banner_menu_reply_cb(GtkAction *action, gpointer data)
{
	...
	compose_reply_from_messageview(messageview, msginfo_list, prefs_common_get_prefs()->reply_with_quote ? COMPOSE_REPLY_WITH_QUOTE : COMPOSE_REPLY_WITHOUT_QUOTE);
