...
static gint foldercheck_folder_name_compare(GtkTreeModel *, GtkTreeIter *, GtkTreeIter *, gpointer);
static gboolean foldercheck_selected(GtkTreeSelection *, GtkTreeModel *, GtkTreePath *, gboolean, gpointer);
...
static void foldercheck_insert_gnode_in_store(GtkTreeStore *, GNode *, GtkTreeIter *);
static void foldercheck_append_item(GtkTreeStore *, FolderItem *, GtkTreeIter *, GtkTreeIter *);
...
static void folder_toggle_recurse_tree(GtkTreeStore *, GtkTreeIter *, gint, gboolean);
static gboolean foldercheck_foreach_check(GtkTreeModel *, GtkTreePath *, GtkTreeIter *, gpointer);
static gboolean foldercheck_foreach_update_to_list(GtkTreeModel *, GtkTreePath *, GtkTreeIter *, gpointer);
...
guint notification_register_folder_specific_list(gchar *node_name)
{
	...
	if (!specific_folder_array) {
		specific_folder_array = g_array_new(FALSE, FALSE, sizeof(SpecificFolderArrayEntry *));
		specific_folder_array_size = 0;

		/* Register hook for folder update */
		/* "The hook is registered" is bound to "the array is allocated" */
		hook_folder_update = hooks_register_hook(FOLDER_UPDATE_HOOKLIST, my_folder_update_hook, NULL);
		if (hook_folder_update == 0) {
			debug_print("Warning: Failed to register hook to folder update " "hooklist. " "Strange things can occur when deleting folders.\n");
		}
	}
	...
	entry->tree_store = gtk_tree_store_new(N_FOLDERCHECK_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER, GDK_TYPE_PIXBUF, GDK_TYPE_PIXBUF, G_TYPE_BOOLEAN);
	gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(entry->tree_store), FOLDERCHECK_FOLDERNAME, foldercheck_folder_name_compare, NULL, NULL);
...
void notification_foldercheck_write_array(void)
{
	...
	if ((pfile = prefs_write_open(path)) == NULL) {
		debug_print("Notification plugin error: cannot open " "file " FOLDERCHECK_ARRAY " for writing\n");
		return;
	}
	...
	if (prefs_file_close(pfile) < 0) {
		debug_print("Notification plugin error: failed to write " "file " FOLDERCHECK_ARRAY "\n");
	}
...
gboolean notification_foldercheck_read_array(void)
{
	...
	for (branchnode = rootnode->children; branchnode != NULL; branchnode = branchnode->next) {
		GList *list;
		guint id;
		...
		for (node = branchnode->children; node != NULL; node = node->next) {
			...
			if (!G_NODE_IS_LEAF(node))
				g_warning("subnodes in \"branch\" nodes should all be leaves, " "ignoring deeper subnodes");
			...
			for (; list != NULL; list = list->next) {
				...
				if (attr && attr->name && attr->value && !g_strcmp0(attr->name, "identifier")) {
					item = folder_find_item_from_identifier(attr->value);
					break;
				}
			}
			if ((list == NULL) || (item == NULL)) {
				g_warning("did not find attribute \"identifier\" in tag " "\"folderitem\"");
...
static void folder_checked(guint id)
{
	...
	if (!entry->cancelled) {
		...
		gtk_tree_model_foreach(GTK_TREE_MODEL(entry->tree_store), foldercheck_foreach_check, &checked_list);
...
static void foldercheck_create_window(SpecificFolderArrayEntry *entry)
{
	...
	gtk_window_set_wmclass(GTK_WINDOW(entry->window), "folder_selection", PACKAGE_NAME "");
	g_signal_connect(G_OBJECT(entry->window), "delete_event", G_CALLBACK(delete_event), entry);
	g_signal_connect(G_OBJECT(entry->window), "key_press_event", G_CALLBACK(key_pressed), entry);
	...
	gtk_tree_model_foreach(GTK_TREE_MODEL(entry->tree_store), foldercheck_foreach_update_to_list, entry);

	/* tree view */
	entry->treeview = gtk_tree_view_new_with_model(GTK_TREE_MODEL(entry->tree_store));
	gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(entry->treeview), FALSE);
	gtk_tree_view_set_search_column(GTK_TREE_VIEW(entry->treeview), FOLDERCHECK_FOLDERNAME);
	gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(entry->treeview), prefs_common_get_prefs()->use_stripes_everywhere);
	...
	gtk_tree_selection_set_select_function(selection, foldercheck_selected, NULL, NULL);
	...
	gtk_tree_view_column_set_attributes(column, renderer, "active", FOLDERCHECK_CHECK, NULL);
	...
	gtk_tree_view_column_set_attributes(column, renderer, "pixbuf", FOLDERCHECK_PIXBUF, "pixbuf-expander-open", FOLDERCHECK_PIXBUF_OPEN, "pixbuf-expander-closed", FOLDERCHECK_PIXBUF, NULL);

	/* text */
	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_column_pack_start(column, renderer, TRUE);
	gtk_tree_view_column_set_attributes(column, renderer, "text", FOLDERCHECK_FOLDERNAME, NULL);
	...
	g_signal_connect(G_OBJECT(checkbox), "toggled", G_CALLBACK(foldercheck_recursive_cb), entry);
	...
	g_signal_connect(G_OBJECT(ok_button), "clicked", G_CALLBACK(foldercheck_ok), entry);
	g_signal_connect(G_OBJECT(cancel_button), "clicked", G_CALLBACK(foldercheck_cancel), entry);
	...
	gtk_window_set_geometry_hints(GTK_WINDOW(entry->window), NULL, &geometry, GDK_HINT_MIN_SIZE);
...
static gint foldercheck_folder_name_compare(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b, gpointer context)
{
...
static gboolean foldercheck_selected(GtkTreeSelection *selection, GtkTreeModel *model, GtkTreePath *path, gboolean currently_selected, gpointer data)
{
	GtkTreeIter iter;
	FolderItem *item = NULL;
...
static void foldercheck_set_tree(SpecificFolderArrayEntry *entry)
{
	...
	gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(entry->tree_store), FOLDERCHECK_FOLDERNAME, GTK_SORT_ASCENDING);
...
static void foldercheck_insert_gnode_in_store(GtkTreeStore *store, GNode *node, GtkTreeIter *parent)
{
	FolderItem *item;
	GtkTreeIter child;
	GNode *iter;
...
static void foldercheck_append_item(GtkTreeStore *store, FolderItem *item, GtkTreeIter *iter, GtkTreeIter *parent)
{
	gchar *name, *tmpname;
	...
	gtk_tree_store_set(store, iter, FOLDERCHECK_FOLDERNAME, name, FOLDERCHECK_FOLDERITEM, item, FOLDERCHECK_PIXBUF, pixbuf, FOLDERCHECK_PIXBUF_OPEN, pixbuf_open, -1);
...
static void folder_toggle_cb(GtkCellRendererToggle *cell_renderer, gchar *path_str, gpointer data)
{
	...
	gtk_tree_model_get(GTK_TREE_MODEL(entry->tree_store), &iter, FOLDERCHECK_CHECK, &toggle_item, -1);
	toggle_item = !toggle_item;

	if (!entry->recursive)
		gtk_tree_store_set(entry->tree_store, &iter, FOLDERCHECK_CHECK, toggle_item, -1);
	else {
		GtkTreeIter child;
		gtk_tree_store_set(entry->tree_store, &iter, FOLDERCHECK_CHECK, toggle_item, -1);
		if (gtk_tree_model_iter_children(GTK_TREE_MODEL(entry->tree_store), &child, &iter))
			folder_toggle_recurse_tree(entry->tree_store, &child, FOLDERCHECK_CHECK, toggle_item);
	}
...
static void folder_toggle_recurse_tree(GtkTreeStore *tree_store, GtkTreeIter *iterp, gint column, gboolean toggle_item)
{
	...
	if (gtk_tree_model_iter_children(GTK_TREE_MODEL(tree_store), &next, &iter))
		folder_toggle_recurse_tree(tree_store, &next, FOLDERCHECK_CHECK, toggle_item);

	/* do the same for the next sibling */
	if (gtk_tree_model_iter_next(GTK_TREE_MODEL(tree_store), &iter))
		folder_toggle_recurse_tree(tree_store, &iter, FOLDERCHECK_CHECK, toggle_item);
}
...
static gboolean foldercheck_foreach_check(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
 ...
 * if a node is checked, and adds it to a list if it is. data us a (GSList**)
 * where the result it to be stored */
static gboolean foldercheck_foreach_update_to_list(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
