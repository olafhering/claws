...
void notification_popup_msg(MsgInfo *msginfo)
{
	...
	if (notify_config.popup_folder_specific) {
		...
		id = notification_register_folder_specific_list(POPUP_SPECIFIC_FOLDER_ID_STR);
	...
	if (retval) {
		...
		ppopup->timeout_id = g_timeout_add(notify_config.popup_timeout, popup_timeout_fun, GINT_TO_POINTER(nftype));
	}
...
#ifdef HAVE_LIBNOTIFY
static void default_action_cb(NotifyNotification *notification, const char *action, void *user_data)
{
...
static gboolean notification_libnotify_create(MsgInfo *msginfo, NotificationFolderType nftype)
{
	GdkPixbuf *pixbuf;
	NotificationPopup *ppopup;
	gchar *summary = NULL;
	...
	/* Default action */
	if (support_actions)
		notify_notification_add_action(ppopup->notification, "default", _("Present main window"), (NotifyActionCallback) default_action_cb, GINT_TO_POINTER(nftype), notification_libnotify_free_func);
	...
	if (nftype == F_TYPE_MAIL) {
		if (msginfo && msginfo->folder) {
			...
			ppopup->msg_path = g_strdup_printf("%s%s%u", ident, G_DIR_SEPARATOR_S, msginfo->msgnum);
			g_free(ident);
		} else
			ppopup->msg_path = NULL;
	}
...
static gboolean notification_libnotify_add_msg(MsgInfo *msginfo, NotificationFolderType nftype)
{
	gchar *summary;
	gchar *text;
	gboolean retval;
	...
	case F_TYPE_MAIL:
		...
		text = g_strdup_printf(ngettext("%d new message arrived", "%d new messages arrived", ppopup->count), ppopup->count);
		break;
	case F_TYPE_NEWS:
	...
	case F_TYPE_CALENDAR:
		...
		text = g_strdup_printf(ngettext("%d new calendar message arrived", "%d new calendar messages arrived", ppopup->count), ppopup->count);
		break;
	case F_TYPE_RSS:
		summary = _("RSS news feed");
		text = g_strdup_printf(ngettext("%d new article in a RSS feed arrived", "%d new articles in a RSS feed arrived", ppopup->count), ppopup->count);
		break;
	default:
		/* Should not happen */
	...
	retval = notify_notification_update(ppopup->notification, summary, text, NULL);
	g_free(text);
	if (!retval) {
...
static gboolean notification_popup_add_msg(MsgInfo *msginfo)
{
	...
	message = g_strdup_printf(ngettext("%d new message", "%d new messages", ppopup->count), ppopup->count);
...
static gboolean notification_popup_create(MsgInfo *msginfo)
{
	...
	gtk_window_move(GTK_WINDOW(ppopup->window), notify_config.popup_root_x, notify_config.popup_root_y);
	...
	/* Signals */
	gtk_widget_set_events(ppopup->window, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);
	g_signal_connect(ppopup->window, "button_press_event", G_CALLBACK(notification_popup_button), NULL);
	...
	ppopup->label1 = gtk_label_new(msginfo->from ? msginfo->from : _("(No From)"));
	gtk_box_pack_start(GTK_BOX(ppopup->vbox), ppopup->label1, FALSE, FALSE, 0);

	ppopup->label2 = gtk_label_new(msginfo->subject ? msginfo->subject : _("(No Subject)"));
...
static gboolean notification_popup_button(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
