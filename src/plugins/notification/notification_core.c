...
void notification_core_get_msg_count(GSList *folder_list, NotificationMsgCount *count)
{
	GSList *walk;
...
static void msg_count_hash_update_func(FolderItem *item, gpointer data)
{
	...
	if (!notify_include_folder_type(item->folder->klass->type, item->folder->klass->uistr))
		return;
...
static void msg_count_update_from_hash(gpointer key, gpointer value, gpointer data)
{
...
gboolean notification_notified_hash_msginfo_update(MsgInfoUpdate *msg_update)
{
	...
	if ((msg_update->flags & MSGINFO_UPDATE_FLAGS) && !MSG_IS_NEW(msg_update->msginfo->flags)) {
		...
		if (g_hash_table_lookup(notified_hash, msgid) != NULL) {
			...
			debug_print("Notification Plugin: Removing message id %s from hash " "table\n", msgid);
...
void notification_notified_hash_startup_init(void)
{
	...
	for (walk = folder_list; walk != NULL; walk = g_list_next(walk)) {
		...
		g_node_traverse(folder->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, notification_traverse_hash_startup, NULL);
	}
}
...
static gboolean notification_traverse_hash_startup(GNode *node, gpointer data)
{
	...
	for (walk = msg_list; walk; walk = g_slist_next(walk)) {
		...
		if (MSG_IS_NEW(msg->flags)) {
			...
			g_hash_table_insert(notified_hash, g_strdup(msgid), GINT_TO_POINTER(1));
...
void notification_new_unnotified_msgs(FolderItemUpdateData *update_data)
{
	...
	for (walk = msg_list; walk; walk = g_slist_next(walk)) {
		...
		if (MSG_IS_NEW(msg->flags)) {
			...
			debug_print("Notification Plugin: Found msg %s, " "checking if it is in hash...\n", msgid);
 ...
 */
GSList *notification_collect_msgs(gboolean unread_also, GSList *folder_items, gint max_msgs)
{
	GList *folder_list, *walk;
	Folder *folder;
	...
	for (walk = folder_list; walk != NULL; walk = g_list_next(walk)) {
		...
		g_node_traverse(folder->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, notification_traverse_collect, &collect_data);
	}
...
static gboolean notification_traverse_collect(GNode *node, gpointer data)
{
	...
	if ((cdata->folder_items) && (item->path != NULL) && ((folder_id_cur = folder_item_get_identifier(item)) != NULL)) {
	...
	if (item->new_msgs || (cdata->unread_also && item->unread_msgs)) {
		...
		for (walk = msg_list; walk != NULL; walk = g_slist_next(walk)) {
			...
			if (MSG_IS_NEW(msg_info->flags) || (MSG_IS_UNREAD(msg_info->flags) && cdata->unread_also)) {
...
gchar *notification_libnotify_sanitize_str(gchar *in)
{
	...
	while (*in) {
		if (*in == '<') {
			...
			if (out + 5 > STR_MAX_LEN)
				break;
			memcpy(&(tmp_str[out]), "&amp;", 5);
			in++;
			out += 5;
		} else {
			if (out + 1 > STR_MAX_LEN)
				break;
			tmp_str[out++] = *in++;
		}
	}
...
gchar *notification_validate_utf8_str(gchar *text)
{
	...
	if (!g_utf8_validate(text, -1, NULL)) {
		...
		/* fix it */
		utf8_str = conv_codeset_strdup(text, conv_get_locale_charset_str_no_utf8(), CS_INTERNAL);
