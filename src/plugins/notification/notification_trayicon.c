...
static void notification_trayicon_account_list_reset(const gchar *, gpointer, gboolean);

static GdkPixbuf *notification_trayicon_create(void);

static void notification_trayicon_on_popup_menu(GtkStatusIcon *, guint, guint, gpointer);
static gboolean notification_trayicon_on_size_changed(GtkStatusIcon *, gint, gpointer);
...
static gboolean notification_trayicon_popup_add_msg(MsgInfo *, NotificationFolderType);
static gboolean notification_trayicon_popup_create(MsgInfo *, NotificationFolderType);
static void popup_timeout_fun(NotifyNotification *, gpointer);
static void notification_trayicon_popup_free_func(gpointer);
static void notification_trayicon_popup_default_action_cb(NotifyNotification *, const char *, void *);
...
void notification_trayicon_msg(MsgInfo *msginfo)
{
	...
	if (!msginfo || !notify_config.trayicon_enabled || !notify_config.trayicon_popup_enabled || !MSG_IS_NEW(msginfo->flags))
		return;
	...
	if (notify_config.trayicon_folder_specific) {
		...
		id = notification_register_folder_specific_list(TRAYICON_SPECIFIC_FOLDER_ID_STR);
...
void notification_update_trayicon()
{
	...
	/* Tooltip */
	buf = g_strdup_printf(_("New %d, Unread: %d, Total: %d"), count.new_msgs, count.unread_msgs, count.total_msgs);
	...
	if ((count.new_msgs > 0) && (count.unreadmarked_msgs > 0))
		new_icon = notification_pixbuf_get(NOTIFICATION_TRAYICON_NEWMARKEDMAIL + offset);
	else if (count.new_msgs > 0)
		new_icon = notification_pixbuf_get(NOTIFICATION_TRAYICON_NEWMAIL + offset);
	else if (count.unreadmarked_msgs > 0)
		new_icon = notification_pixbuf_get(NOTIFICATION_TRAYICON_UNREADMARKEDMAIL + offset);
	else if (count.unread_msgs > 0)
		new_icon = notification_pixbuf_get(NOTIFICATION_TRAYICON_UNREADMAIL + offset);
	else
		new_icon = notification_pixbuf_get(NOTIFICATION_TRAYICON_NOMAIL + offset);
...
gboolean notification_trayicon_main_window_got_iconified(gpointer source, gpointer data)
{
	MainWindow *mainwin = mainwindow_get_mainwindow();
	...
	if (notify_config.trayicon_hide_when_iconified && mainwin && gtk_widget_get_visible(GTK_WIDGET(mainwin->window))
...
static void notification_trayicon_account_list_reset(const gchar *menuname, gpointer callback, gboolean receive)
{
	GList *cur_ac;
	...
	for (cur_ac = account_list; cur_ac != NULL; cur_ac = cur_ac->next) {
		...
		g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(callback), ac_prefs);
	}
	gtk_widget_show(submenu);
	gtk_menu_item_set_submenu(GTK_MENU_ITEM(menu), submenu);
}

gboolean notification_trayicon_account_list_changed(gpointer source, gpointer data)
{
	if (notify_config.trayicon_enabled) {
		notification_trayicon_account_list_reset("/Menus/SysTrayiconPopup/GetMailAcc", (gpointer)trayicon_get_from_account_cb, TRUE);
		notification_trayicon_account_list_reset("/Menus/SysTrayiconPopup/EmailAcc", (gpointer)trayicon_compose_acc_cb, FALSE);
	}
	return FALSE;
}
...
static GdkPixbuf *notification_trayicon_create(void)
{
	...
	g_signal_connect(G_OBJECT(trayicon), "activate", G_CALLBACK(notification_trayicon_on_activate), NULL);
	g_signal_connect(G_OBJECT(trayicon), "popup-menu", G_CALLBACK(notification_trayicon_on_popup_menu), NULL);
	g_signal_connect(G_OBJECT(trayicon), "size-changed", G_CALLBACK(notification_trayicon_on_size_changed), NULL);

	/* Popup-Menu */
	action_group = cm_menu_create_action_group("SysTrayiconPopup", trayicon_popup_menu_entries, G_N_ELEMENTS(trayicon_popup_menu_entries), NULL);
	gtk_action_group_add_toggle_actions(action_group, trayicon_popup_toggle_menu_entries, G_N_ELEMENTS(trayicon_popup_toggle_menu_entries), NULL);
	...
	MENUITEM_ADDUI("/Menus", "SysTrayiconPopup", "SysTrayiconPopup", GTK_UI_MANAGER_MENU)
	    ...
	    traymenu_popup = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(gtkut_ui_manager(), "/Menus/SysTrayiconPopup")));
...
static void notification_trayicon_on_popup_menu(GtkStatusIcon *status_icon, guint button, guint activate_time, gpointer user_data)
{
	MainWindow *mainwin = mainwindow_get_mainwindow();
	...
	gtk_menu_popup(GTK_MENU(traymenu_popup), NULL, NULL, NULL, NULL, button, activate_time);
}

static gboolean notification_trayicon_on_size_changed(GtkStatusIcon *icon, gint size, gpointer user_data)
{
...
static void app_exit_cb(MainWindow *mainwin, guint action, GtkWidget *widget)
{
	if (prefs_common_get_prefs()->confirm_on_exit) {
		if (alertpanel(_("Exit"), _("Exit Claws Mail?"), GTK_STOCK_CANCEL, GTK_STOCK_OK, NULL, ALERTFOCUS_FIRST) != G_ALERTALTERNATE) {
			return;
		}
...
#ifdef HAVE_LIBNOTIFY
static gboolean notification_trayicon_popup_add_msg(MsgInfo *msginfo, NotificationFolderType nftype)
{
	gchar *summary;
	gchar *utf8_str;
	gboolean retval;
	...
	retval = notify_notification_update(popup.notification, summary, utf8_str, NULL);
	g_free(summary);
	g_free(utf8_str);
	if (!retval) {
	...
	/* Show the popup */
	notify_notification_set_hint_string(popup.notification, "desktop-entry", PACKAGE_TARNAME);
	...
	debug_print("Notification Plugin: Popup successfully modified " "with libnotify.\n");

	return TRUE;
}

static gboolean notification_trayicon_popup_create(MsgInfo *msginfo, NotificationFolderType nftype)
{
	gchar *summary = NULL;
	gchar *utf8_str = NULL;
	GdkPixbuf *pixbuf;
	...
	if (!notify_is_initted()) {
		if (!notify_init(PACKAGE_TARNAME)) {
			debug_print("Notification Plugin: Failed to initialize libnotify. " "No popups will be shown.\n");
			return FALSE;
		}
	}
	...
	/* Default action */
	if (support_actions)
		notify_notification_add_action(popup.notification, "default", _("Present main window"), (NotifyActionCallback)
					       notification_trayicon_popup_default_action_cb, GINT_TO_POINTER(nftype), notification_trayicon_popup_free_func);
	...
	if (msginfo && msginfo->from) {
		...
		if (is_file_exist(icon_path)) {
			...
			if ((w > 64) || (h > 64))
				pixbuf = gdk_pixbuf_new_from_file_at_scale(icon_path, 64, 64, TRUE, &error);
			else
	...
	if (nftype == F_TYPE_MAIL) {
		if (msginfo && msginfo->folder) {
			...
			popup.msg_path = g_strdup_printf("%s%s%u", ident, G_DIR_SEPARATOR_S, msginfo->msgnum);
			g_free(ident);
		} else
			popup.msg_path = NULL;
	}
...
static void notification_trayicon_popup_default_action_cb(NotifyNotification
							  *notification, const char *action, void *user_data)
{
...
static gchar *notification_trayicon_popup_assemble_body(MsgInfo *msginfo)
{
	...
	if (popup.count == 1) {
		if (popup.num_mail || popup.num_news) {
			...
			from = notification_libnotify_sanitize_str(msginfo->from ? msginfo->from : _("(No From)"));
			subj = notification_libnotify_sanitize_str(msginfo->subject ? msginfo->subject : _("(No Subject)"));
		...
		if (popup.num_mail) {
			msg = g_strdup_printf(ngettext("%d new mail message arrived", "%d new mail messages arrived", popup.num_mail), popup.num_mail);
		...
		if (popup.num_news) {
			msg = g_strdup_printf(ngettext("%d new news post arrived", "%d new news posts arrived", popup.num_news), popup.num_news);
		...
		if (popup.num_calendar) {
			msg = g_strdup_printf(ngettext("%d new calendar message arrived", "%d new calendar messages arrived", popup.num_calendar), popup.num_calendar);
			tmp = g_strdup_printf("%s%s%s", utf8_str, str_empty ? "" : "\n", msg);
			g_free(msg);
			g_free(utf8_str);
			utf8_str = tmp;
		...
		if (popup.num_rss) {
			msg = g_strdup_printf(ngettext("%d new article in RSS feeds arrived", "%d new articles in RSS feeds arrived", popup.num_rss), popup.num_rss);
			tmp = g_strdup_printf("%s%s%s", utf8_str, str_empty ? "" : "\n", msg);
			g_free(msg);
			g_free(utf8_str);
			utf8_str = tmp;
...
gboolean notification_trayicon_is_available(void)
{
	...
	if (trayicon) {
		if (gtk_status_icon_is_embedded(trayicon) && gtk_status_icon_get_visible(trayicon))
			is_available = TRUE;
	}
