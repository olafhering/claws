...
gint plugin_gtk_init(gchar **error)
{
	...
	gtk_action_group_add_actions(mainwin->action_group, mainwindow_add_mailbox, 1, (gpointer)mainwin);
	MENUITEM_ADDUI_ID_MANAGER(mainwin->ui_manager, "/Menu/File/AddMailbox", "Mbox", "File/AddMailbox/Mbox", GTK_UI_MANAGER_MENUITEM, main_menu_id)
	    return 0;
}
...
static void add_mailbox(GtkAction *action, gpointer callback_data)
{
	...
	path = input_dialog(_("Add mailbox"), _("Input the location of mailbox.\n" "If the existing mailbox is specified, it will be\n" "scanned automatically."), "Mail");
	if (!path)
		return;
	...
	folder = folder_new(folder_get_class_from_string("mailmbox"), !strcmp(path, "Mail") ? _("Mailbox") : basename, path);
	g_free(basename);
	g_free(path);
...
static void new_folder_cb(GtkAction *action, gpointer data)
{
	...
	new_folder = input_dialog(_("New folder"), _("Input the name of new folder:"), _("NewFolder"));
	if (!new_folder)
		return;
	...
	if (p == NULL)
		p = strchr(new_folder, '.');
	if (p) {
		alertpanel_error(_("'%c' can't be included in folder name."), p[0]);
		return;
	}
...
static void remove_mailbox_cb(GtkAction *action, gpointer data)
{
	...
	message = g_strdup_printf(_("Really remove the mailbox '%s'?\n" "(The messages are NOT deleted from the disk)"), name);
	avalue = alertpanel_full(_("Remove mailbox"), message, GTK_STOCK_CANCEL, _("_Remove"), NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
	g_free(message);
	g_free(name);
...
static void delete_folder_cb(GtkAction *action, gpointer data)
{
	...
	message = g_strdup_printf(_("All folders and messages under '%s' will be permanently deleted.\n" "Recovery will not be possible.\n\n" "Do you really want to delete?"), name);
	avalue = alertpanel_full(_("Delete folder"), message, GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_NOTICE);
	g_free(message);
	if (avalue != G_ALERTALTERNATE)
		return;
	...
	if (item->folder->klass->remove_folder(item->folder, item) < 0) {
		...
		if (item == opened)
			summary_show(folderview->summaryview, folderview->summaryview->folder_item, FALSE);
		g_free(old_id);
		return;
	}
