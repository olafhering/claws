...
static void set_scroll_position(day_win *dw)
{
	...
	v_adj = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(dw->scroll_win));
...
static gint on_Previous_clicked(GtkWidget *button, GdkEventButton *event, day_win *dw)
{
...
static gint on_Next_clicked(GtkWidget *button, GdkEventButton *event, day_win *dw)
{
	int days = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(dw->day_spin));
	changeSelectedDate(dw, +days);
	refresh_day_win(dw);
...
static void dw_summary_selected(GtkCMCTree *ctree, GtkCMCTreeNode *row, gint column, day_win *dw)
{
		...
		if (event) {
			...
			if (changed) {
				...
				if (tm_start.tm_hour > 2)
					gtk_adjustment_set_value(v_adj, ((gtk_adjustment_get_upper(v_adj) - gtk_adjustment_get_page_size(v_adj)) / ((gdouble)24 / (gdouble)(tm_start.tm_hour - 2))));
				else
...
static void day_view_edit_meeting_cb(day_win *dw, gpointer data_i, gpointer data_s)
{
	...
	vcal_view_select_event(uid, dw->item, TRUE, G_CALLBACK(dw_summary_selected), dw);
}
...
static void on_button_press_event_cb(GtkWidget *widget, GdkEventButton *event, gpointer *user_data)
{
	...
	if (event->button == 1) {
		if (uid)
			vcal_view_select_event(uid, dw->item, (event->type == GDK_2BUTTON_PRESS), G_CALLBACK(dw_summary_selected), dw);
	}
	if (event->button == 3) {
		g_object_set_data(G_OBJECT(dw->Vbox), "menu_win", dw);
		g_object_set_data(G_OBJECT(dw->Vbox), "menu_data_i", offset);
		g_object_set_data(G_OBJECT(dw->Vbox), "menu_data_s", uid);
		g_object_set_data(G_OBJECT(dw->Vbox), "new_meeting_cb", day_view_new_meeting_cb);
		g_object_set_data(G_OBJECT(dw->Vbox), "edit_meeting_cb", day_view_edit_meeting_cb);
		g_object_set_data(G_OBJECT(dw->Vbox), "cancel_meeting_cb", day_view_cancel_meeting_cb);
		g_object_set_data(G_OBJECT(dw->Vbox), "go_today_cb", day_view_today_cb);
		if (uid)
			gtk_menu_popup(GTK_MENU(dw->event_menu), NULL, NULL, NULL, NULL, event->button, event->time);
		else
			gtk_menu_popup(GTK_MENU(dw->view_menu), NULL, NULL, NULL, NULL, event->button, event->time);
	}
}
...
/* we took the date in unnormalized format, so we need to do that now */
	...
	g_signal_connect((gpointer)ev, "button-press-event", G_CALLBACK(on_button_press_event_cb), dw);
	g_free(tip);
	g_free(text);
		...
		for (col = first_col; col <= last_col; col++) {
			...
			for (row = start_row; row <= end_row; row++) {
				...
				if (row == tm_end.tm_hour && col == end_col)
					...
					end_height = height;
				dw->line[row][col] = build_line(1, start_height, 2, end_height - start_height, dw->line[row][col]
								, &dw->line_color);
			}
		}
	}
...
static void fill_days(day_win *dw, gint days, FolderItem *item, gint first_col_day)
{
	...
	for (col = 1; col < days + 1; col++) {
		...
		for (row = 0; row < 24; row++) {
			...
			dw->line[row][col] = build_line(0, 0, 3, height, NULL, &dw->line_color);
		}
	}
	...
	for (col = 1; col < days + 1; col++) {
		...
		gtk_table_attach(GTK_TABLE(dw->dtable_h), hb, col, col + 1, 1, 2, (GTK_FILL), (0), 0, 0);
		...
		for (row = 0; row < 24; row++) {
			...
			gtk_table_attach(GTK_TABLE(dw->dtable), hb, col, col + 1, row, row + 1, (GTK_FILL), (0), 0, 0);
		}
		first_col_day++;
	}
}
...
static void fill_hour(day_win *dw, gint col, gint row, char *text)
{
	...
	if ((row % 2) == 1)
		...
		gtk_widget_modify_bg(ev, GTK_STATE_NORMAL, &dw->bg2);
	gtk_widget_set_size_request(ev, dw->hour_req.width, dw->StartDate_button_req.height);
	if (text)
		gtk_table_attach(GTK_TABLE(dw->dtable), ev, col, col + 1, row, row + 1, (GTK_FILL), (0), 0, 0);
	else /* special, needed for header table full day events */
		gtk_table_attach(GTK_TABLE(dw->dtable_h), ev, col, col + 1, row, row + 1, (GTK_FILL), (0), 0, 0);
}
...
static void build_day_view_table(day_win *dw)
{
	...
	gtk_box_pack_start(GTK_BOX(dw->Vbox), dw->scroll_win_h, TRUE, TRUE, 0);
	...
	gtk_box_pack_start(GTK_BOX(dw->day_view_vbox), dw->dtable_h, FALSE, FALSE, 0);
	...
	gtk_table_attach(GTK_TABLE(dw->dtable_h), dw->Previous_toolbutton, i, i + 1, 0, 1, (GTK_FILL), (0), 0, 0);
	gtk_widget_show_all(dw->Previous_toolbutton);
	g_signal_connect((gpointer)dw->Previous_toolbutton, "button_release_event", G_CALLBACK(on_Previous_clicked), dw);
	...
	for (i = 1; i < days + 1; i++) {
		...
		g_signal_connect((gpointer)button, "clicked", G_CALLBACK(header_button_clicked_cb), dw);
		g_object_set_data(G_OBJECT(button), "offset", GINT_TO_POINTER(tm_date.tm_mday * 1000));
		gtk_table_attach(GTK_TABLE(dw->dtable_h), button, i, i + 1, 0, 1, (GTK_FILL), (0), 0, 0);
	...
	gtk_table_attach(GTK_TABLE(dw->dtable_h), dw->Next_toolbutton, i, i + 1, 0, 1, (GTK_FILL), (0), 0, 0);
	gtk_widget_show_all(dw->Next_toolbutton);
	g_signal_connect((gpointer)dw->Next_toolbutton, "button_release_event", G_CALLBACK(on_Next_clicked), dw);
	...
	gtk_scrolled_window_set_placement(GTK_SCROLLED_WINDOW(dw->scroll_win)
					  , GTK_CORNER_TOP_LEFT);
	gtk_box_pack_start(GTK_BOX(dw->day_view_vbox), dw->scroll_win, TRUE, TRUE, 0);
...
day_win *create_day_win(FolderItem *item, struct tm tmdate)
{
	...
	dw->selsig = vcal_view_set_calendar_page(dw->Vbox, G_CALLBACK(dw_summary_selected), dw);
	vcal_view_create_popup_menus(dw->Vbox, &dw->view_menu, &dw->event_menu, &dw->event_group, &dw->ui_manager);
