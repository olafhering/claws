...
{
	if (vcalprefs.freebusy_get_url == NULL || *vcalprefs.freebusy_get_url == '\0')
		return FALSE;
	else
		return TRUE;
}
...
VCalAttendee *attendee_add(VCalMeeting *meet, gchar *address, gchar *name, gchar *partstat, gchar *cutype, gboolean first)
{
	...
	attendee->avail_img = gtk_image_new_from_stock(GTK_STOCK_DIALOG_WARNING, GTK_ICON_SIZE_SMALL_TOOLBAR);
	...
	g_signal_connect(G_OBJECT(attendee->remove_btn), "clicked", G_CALLBACK(remove_btn_cb), attendee);
	g_signal_connect(G_OBJECT(attendee->add_btn), "clicked", G_CALLBACK(add_btn_cb), attendee);
...
static gboolean meeting_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
	VCalMeeting *meet = (VCalMeeting *)data;
...
static void meeting_start_changed(GtkWidget *widget, gpointer data)
{
	...
	gtk_calendar_select_month(GTK_CALENDAR(meet->end_c), end_lt.tm_mon, end_lt.tm_year + 1900);
...
static void meeting_end_changed(GtkWidget *widget, gpointer data)
{
	...
	gtk_calendar_select_month(GTK_CALENDAR(meet->start_c), start_lt.tm_mon, start_lt.tm_year + 1900);
...
static void att_update_icon(VCalMeeting *meet, VCalAttendee *attendee, gint avail, gchar *text)
{
	...
	if (!gtk_entry_get_text(GTK_ENTRY(attendee->address))
		...
		gtk_image_set_from_stock(GTK_IMAGE(attendee->avail_img), icon, GTK_ICON_SIZE_SMALL_TOOLBAR);
...
static gchar *get_avail_msg(const gchar *unavailable_persons, gboolean multiple, gboolean short_version, gint offset_before, gint offset_after)
{
	...
	if (multiple) {
		...
		if (short_version) {
			...
			if (before && after)
				...
				outro = g_strdup_printf(_(", and isn't available " "in the previous or next 6 hours."));
		}
	}
...
static gboolean find_availability(const gchar *dtstart, const gchar *dtend, GSList *attendees, gboolean for_send, VCalMeeting *meet)
{
	...
	for (cur = attendees; cur; cur = cur->next) {
		...
		if (!attendee_available(attendee, icaltime_as_ical_string(start), icaltime_as_ical_string(end), attendee->cached_contents)) {
			gchar *mail = NULL;
	...
	while (!found && offset >= -3600 * 6) {
		...
		for (cur = attendees; cur; cur = cur->next) {
			...
			if (!attendee_available(attendee, icaltime_as_ical_string(new_start), icaltime_as_ical_string(new_end), attendee->cached_contents)) {
				ok = FALSE;
				break;
			} else {
	...
	if (for_send) {
		...
		val = alertpanel_full(_("Not everyone is available"), msg, GTK_STOCK_CANCEL, _("Send anyway"), NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_QUESTION);
		g_free(msg);
	}
	msg = get_avail_msg(unavailable_persons, TRUE, TRUE, offset_before, offset_after);
	g_free(unavailable_persons);
	gtk_image_set_from_stock(GTK_IMAGE(meet->total_avail_img), GTK_STOCK_DIALOG_WARNING, GTK_ICON_SIZE_SMALL_TOOLBAR);
	gtk_widget_show(meet->total_avail_img);
	gtk_label_set_text(GTK_LABEL(meet->total_avail_msg), _("Not everyone is available. " "See tooltips for more info..."));
	...
	if (find_avail) {
		...
		if (tell_if_ok) {
			if (for_send)
				...
				gtk_image_set_from_stock(GTK_IMAGE(meet->total_avail_img), GTK_STOCK_DIALOG_INFO, GTK_ICON_SIZE_SMALL_TOOLBAR);
				gtk_widget_show(meet->total_avail_img);
				gtk_label_set_text(GTK_LABEL(meet->total_avail_msg), _("Everyone is available."));
				CLAWS_SET_TIP(meet->total_avail_evtbox, NULL);
			} else {
				gtk_image_set_from_stock(GTK_IMAGE(meet->total_avail_img), GTK_STOCK_DIALOG_QUESTION, GTK_ICON_SIZE_SMALL_TOOLBAR);
				gtk_widget_show(meet->total_avail_img);
				gtk_label_set_text(GTK_LABEL(meet->total_avail_msg), _("Everyone is available."));
...
static gboolean send_meeting_cb(GtkButton *widget, gpointer data)
{
	...
	if (meet->uid == NULL && meet->visible && !check_attendees_availability(meet, FALSE, TRUE)) {
		return FALSE;
	}
	...
	event = vcal_manager_new_event(uid, organizer, organizer_name, location, summary, description, dtstart, dtend, NULL, tzid, NULL, meet->method, meet->sequence, ICAL_VEVENT_COMPONENT);

	vcal_manager_update_answer(event, organizer, organizer_name, ICAL_PARTSTAT_ACCEPTED, ICAL_CUTYPE_INDIVIDUAL);
	...
	for (cur = meet->attendees; cur && cur->data; cur = cur->next) {
		...
		if (strlen(email)) {
			...
			vcal_manager_update_answer(event, email, name, status, cutype);
...
static VCalMeeting *vcal_meeting_create_real(VCalEvent *event, gboolean visible)
{
	...
	meet->avail_img = gtk_image_new_from_stock(GTK_STOCK_DIALOG_WARNING, GTK_ICON_SIZE_SMALL_TOOLBAR);
	...
	if (event) {
		...
		meet->method = (event->method == ICAL_METHOD_CANCEL ? ICAL_METHOD_CANCEL : ICAL_METHOD_REQUEST);
	...
	meet->total_avail_img = gtk_image_new_from_stock(GTK_STOCK_DIALOG_WARNING, GTK_ICON_SIZE_SMALL_TOOLBAR);
	...
	g_signal_connect(G_OBJECT(meet->save_btn), "clicked", G_CALLBACK(send_meeting_cb), meet);

	g_signal_connect(G_OBJECT(meet->avail_btn), "clicked", G_CALLBACK(check_avail_cb), meet);

	g_signal_connect(G_OBJECT(meet->window), "destroy", G_CALLBACK(destroy_meeting_cb), meet);
	g_signal_connect(G_OBJECT(meet->window), "key_press_event", G_CALLBACK(meeting_key_pressed), meet);
	...
	if (!event || (event && !event->dtstart && !event->dtend)) {
		...
		mktime(lt);
		gtk_calendar_select_day(GTK_CALENDAR(meet->start_c), lt->tm_mday);
		gtk_calendar_select_month(GTK_CALENDAR(meet->start_c), lt->tm_mon, lt->tm_year + 1900);
		...
		gtk_calendar_select_day(GTK_CALENDAR(meet->end_c), lt->tm_mday);
		gtk_calendar_select_month(GTK_CALENDAR(meet->end_c), lt->tm_mon, lt->tm_year + 1900);

		gtkut_time_select_select_by_time(GTK_COMBO_BOX(meet->end_time), lt->tm_hour, 0);
	} else {
		gtk_calendar_select_day(GTK_CALENDAR(meet->start_c), get_dtdate(event->dtstart, DAY));
		gtk_calendar_select_day(GTK_CALENDAR(meet->end_c), get_dtdate(event->dtend, DAY));

		gtk_calendar_select_month(GTK_CALENDAR(meet->start_c), get_dtdate(event->dtstart, MONTH) - 1, get_dtdate(event->dtstart, YEAR));
		gtk_calendar_select_month(GTK_CALENDAR(meet->end_c), get_dtdate(event->dtend, MONTH) - 1, get_dtdate(event->dtend, YEAR));

		gtkut_time_select_select_by_time(GTK_COMBO_BOX(meet->start_time), get_dtdate(event->dtstart, HOUR), get_dtdate(event->dtstart, MINUTE));

		gtkut_time_select_select_by_time(GTK_COMBO_BOX(meet->end_time), get_dtdate(event->dtend, HOUR), get_dtdate(event->dtend, MINUTE));
	}

	g_signal_connect(G_OBJECT(meet->start_c), "day-selected", G_CALLBACK(meeting_start_changed), meet);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN(meet->start_time))), "changed", G_CALLBACK(meeting_start_changed), meet);

	g_signal_connect(G_OBJECT(meet->end_c), "day-selected", G_CALLBACK(meeting_end_changed), meet);
	g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN(meet->end_time))), "changed", G_CALLBACK(meeting_end_changed), meet);
	...
	for (i = 0; accounts != NULL; accounts = accounts->next) {
		...
		if (ac->name)
			s = g_strdup_printf("%s: %s <%s>", ac->account_name, ac->name, ac->address);
		else
			s = g_strdup_printf("%s: %s", ac->account_name, ac->address);
	...
	gtk_notebook_append_page(GTK_NOTEBOOK(notebook), meet->table1, gtk_label_new_with_mnemonic(_("Event:")));

	gtk_notebook_append_page(GTK_NOTEBOOK(notebook), meet->table2, gtk_label_new_with_mnemonic(_("Time:")));
...
VCalMeeting *vcal_meeting_create_with_start(VCalEvent *event, struct tm *sdate)
{
	...
	gtk_calendar_select_day(GTK_CALENDAR(meet->start_c), sdate->tm_mday);
	gtk_calendar_select_day(GTK_CALENDAR(meet->end_c), sdate->tm_mday);

	gtk_calendar_select_month(GTK_CALENDAR(meet->start_c), sdate->tm_mon, sdate->tm_year + 1900);
	gtk_calendar_select_month(GTK_CALENDAR(meet->end_c), sdate->tm_mon, sdate->tm_year + 1900);
	...
	if (sdate->tm_hour != 0) {
		...
		if (sdate->tm_hour < 23) {
			...
			gtk_calendar_select_day(GTK_CALENDAR(meet->end_c), tm_tomorrow.tm_mday);
			gtk_calendar_select_month(GTK_CALENDAR(meet->end_c), tm_tomorrow.tm_mon, tm_tomorrow.tm_year);
...
gboolean vcal_meeting_alert_check(gpointer data)
{
	...
	for (cur = events; cur; cur = cur->next) {
		...
		if (warn) {
			...
			if (length >= 60)
				hours = g_strdup_printf(ngettext("%d hour", "%d hours", (length / 60) > 1 ? 2 : 1), length / 60);
			if (length % 60)
				minutes = g_strdup_printf(ngettext("%d minute", "%d minutes", length % 60), length % 60);

			duration = g_strdup_printf("%s%s%s", hours ? hours : "", hours && minutes ? " " : "", minutes ? minutes : "");

			g_free(hours);
			g_free(minutes);

			title = g_strdup_printf(_("Upcoming event: %s"), event->summary);
			message = g_strdup_printf(_("You have a meeting or event soon.\n" "It starts at %s and ends %s later.\n" "Location: %s\n" "More information:\n\n" "%s"), estart, duration, event->location ? event->location : "", event->description);
			...
			label = g_strdup_printf(ngettext("Remind me in %d minute", "Remind me in %d minutes", postpone_min > 1 ? 2 : 1), postpone_min);
			aval = alertpanel_full(title, message, label, GTK_STOCK_OK, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_NOTICE);
			g_free(label);
...
gboolean vcal_meeting_export_calendar(const gchar *path, const gchar *user, const gchar *pass, gboolean automatic)
{
	...
	if (g_slist_length(list) == 0 && g_slist_length(subs) == 0) {
		...
		g_slist_free(subs);
		if (!automatic) {
			alertpanel_full(_("Empty calendar"), _("There is nothing to export."), GTK_STOCK_OK, NULL, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_NOTICE);
			g_free(tmpfile);
 ...
 putfile:
	...
	if (file && strncmp(file, "http://", 7)
		...
		if (file[0] != G_DIR_SEPARATOR)
			afile = g_strdup_printf("%s%s%s", get_home_dir(), G_DIR_SEPARATOR_S, file);
		else
			afile = g_strdup(file);
...
gboolean vcal_meeting_export_freebusy(const gchar *path, const gchar *user, const gchar *pass)
{
	...
	icalcomponent_add_property(timezone, icalproperty_new_tzid("UTC"));

	tzc = icalcomponent_new(ICAL_XSTANDARD_COMPONENT);
	icalcomponent_add_property(tzc, icalproperty_new_dtstart(icaltime_from_string("19700101T000000")));
	icalcomponent_add_property(tzc, icalproperty_new_tzoffsetfrom(0.0));
	icalcomponent_add_property(tzc, icalproperty_new_tzoffsetto(0.0));
	icalcomponent_add_property(tzc, icalproperty_new_tzname("Greenwich meridian time"));
	...
	vfreebusy = icalcomponent_vanew(ICAL_VFREEBUSY_COMPONENT, icalproperty_vanew_dtstart(itt_start, 0), icalproperty_vanew_dtend(itt_end, 0), (void *)0);

	debug_print("DTSTART:%s\nDTEND:%s\n", icaltime_as_ical_string(itt_start), icaltime_as_ical_string(itt_end));
