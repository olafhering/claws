...
static gint on_Previous_clicked(GtkWidget *button, GdkEventButton *event, month_win *mw)
{
...
static gint on_Next_clicked(GtkWidget *button, GdkEventButton *event, month_win *mw)
{
	changeSelectedDate(mw, +1);
	refresh_month_win(mw);
	return TRUE;
...
static void mw_summary_selected(GtkCMCTree *ctree, GtkCMCTreeNode *row, gint column, month_win *mw)
{
...
static void month_view_edit_meeting_cb(month_win *mw, gpointer data_i, gpointer data_s)
{
	...
	vcal_view_select_event(uid, mw->item, TRUE, G_CALLBACK(mw_summary_selected), mw);
}
...
static void header_button_clicked_cb(GtkWidget *button, GdkEventButton *event, gpointer *user_data)
{
	...
	if (event->button == 3) {
		g_object_set_data(G_OBJECT(mw->Vbox), "menu_win", mw);
		g_object_set_data(G_OBJECT(mw->Vbox), "menu_data_i", GINT_TO_POINTER(offset));
		g_object_set_data(G_OBJECT(mw->Vbox), "menu_data_s", NULL);
		g_object_set_data(G_OBJECT(mw->Vbox), "new_meeting_cb", month_view_new_meeting_cb);
		g_object_set_data(G_OBJECT(mw->Vbox), "go_today_cb", month_view_today_cb);
		gtk_menu_popup(GTK_MENU(mw->view_menu), NULL, NULL, NULL, NULL, event->button, event->time);
	}
}

static void on_button_press_event_cb(GtkWidget *widget, GdkEventButton *event, gpointer *user_data)
{
	month_win *mw = (month_win *)user_data;
	...
	if (event->button == 1) {
		if (uid)
			vcal_view_select_event(uid, mw->item, (event->type == GDK_2BUTTON_PRESS), G_CALLBACK(mw_summary_selected), mw);
	...
	if (event->button == 3) {
		g_object_set_data(G_OBJECT(mw->Vbox), "menu_win", mw);
		g_object_set_data(G_OBJECT(mw->Vbox), "menu_data_i", offset);
		g_object_set_data(G_OBJECT(mw->Vbox), "menu_data_s", uid);
		g_object_set_data(G_OBJECT(mw->Vbox), "new_meeting_cb", month_view_new_meeting_cb);
		g_object_set_data(G_OBJECT(mw->Vbox), "edit_meeting_cb", month_view_edit_meeting_cb);
		g_object_set_data(G_OBJECT(mw->Vbox), "cancel_meeting_cb", month_view_cancel_meeting_cb);
		g_object_set_data(G_OBJECT(mw->Vbox), "go_today_cb", month_view_today_cb);
		gtk_menu_popup(GTK_MENU(mw->event_menu), NULL, NULL, NULL, NULL, event->button, event->time);
	}
}
...
static void add_row(month_win *mw, VCalEvent *event, gint days)
{
	...
	if (ev && tm_start.tm_mday == tm_today.tm_mday && tm_start.tm_mon == tm_today.tm_mon && tm_start.tm_year == tm_today.tm_year)
		gtk_widget_modify_bg(ev, GTK_STATE_NORMAL, &mw->bg_today);

	if (orage_days_between(&tm_start, &tm_end) == 0)
		tip = g_strdup_printf("%s\n%02d:%02d-%02d:%02d\n%s", text, tm_start.tm_hour, tm_start.tm_min, tm_end.tm_hour, tm_end.tm_min, event->description);
	else {
/* we took the date in unnormalized format, so we need to do that now */
		...
		tip = g_strdup_printf("%s\n%s %02d:%02d - %s %02d:%02d\n%s", text, start_date, tm_start.tm_hour, tm_start.tm_min, end_date, tm_end.tm_hour, tm_end.tm_min, event->description);
		g_free(start_date);
		g_free(end_date);
	}
	if (pack) {
	...
	g_signal_connect((gpointer)ev, "button-press-event", G_CALLBACK(on_button_press_event_cb), mw);
	g_free(tip);
	g_free(text);
	...
	if (TRUE) {
		...
		for (row = first_row; row <= last_row; row++) {
			...
			for (col = first_col; col <= last_col; col++) {
				...
				mw->line[row][col] = build_line(start_width, 0, end_width - start_width, 2, mw->line[row][col]
								, &mw->line_color);
			}
		}
	}
...
static void fill_days(month_win *mw, gint days, FolderItem *item)
{
	...
	for (day = 1; day <= monthdays[t.tm_mon - 1]; day++) {
		...
		mw->line[row][dcol] = build_line(0, 0, width, 3, NULL, &mw->line_color);
		g_date_free(date);
	}
	...
	for (day = 1; day <= monthdays[t.tm_mon - 1]; day++) {
		...
		if (g_date_get_day(date) == 1)
			label = g_strdup_printf("%d %s", g_date_get_day(date), _(monthname[g_date_get_month(date) - 1]));
		else
			label = g_strdup_printf("%d", g_date_get_day(date));
		tmp = g_strdup_printf("%s %d %s %d", _(dayname[col - 1]), g_date_get_day(date), _(monthname[g_date_get_month(date) - 1]), g_date_get_year(date));

		ev = gtk_event_box_new();
		g_object_set_data(G_OBJECT(ev), "day", GINT_TO_POINTER((int)g_date_get_day(date)));
		g_signal_connect((gpointer)ev, "button-press-event", G_CALLBACK(header_button_clicked_cb), mw);
		...
		gtk_table_attach(GTK_TABLE(mw->dtable), vb, col, col + 1, row, row + 1, (GTK_FILL), (0), 0, 0);
		g_date_free(date);
	}
}
...
static void fill_hour(month_win *mw, gint col, gint row, char *text)
{
	...
	gtk_container_add(GTK_CONTAINER(ev), name);
	gtk_widget_set_size_request(ev, mw->hour_req.width, mw->StartDate_button_req.height);
	if (text)
		gtk_table_attach(GTK_TABLE(mw->dtable), ev, col, col + 1, row, row + 1, (GTK_FILL), (0), 0, 0);
	else /* special, needed for header table full day events */
		gtk_table_attach(GTK_TABLE(mw->dtable_h), ev, col, col + 1, row, row + 1, (GTK_FILL), (0), 0, 0);
}
...
static void build_month_view_table(month_win *mw)
{
	...
	if (mainwindow_get_mainwindow()) {
		...
		GTK_EVENTS_FLUSH();
		gtk_widget_get_allocation(summaryview->mainwidget_book, &allocation);
	...
	gtk_widget_set_size_request(mw->StartDate_button, avail_w / avail_d, (avail_h) / 6);
	gtk_widget_size_request(mw->StartDate_button, &mw->StartDate_button_req);
	...
	gtk_box_pack_start(GTK_BOX(mw->Vbox), mw->scroll_win_h, TRUE, TRUE, 0);
	...
	gtk_box_pack_start(GTK_BOX(mw->month_view_vbox), mw->dtable_h, FALSE, FALSE, 0);
	...
	gtk_table_attach(GTK_TABLE(mw->dtable_h), mw->Previous_toolbutton, i, i + 1, 0, 1, (GTK_FILL), (0), 0, 0);
	gtk_widget_show_all(mw->Previous_toolbutton);
	g_signal_connect((gpointer)mw->Previous_toolbutton, "button_release_event", G_CALLBACK(on_Previous_clicked), mw);
	...
	for (i = 1; i < days + 1; i++) {
		...
		gtk_table_attach(GTK_TABLE(mw->dtable_h), button, i, i + 1, 0, 1, (GTK_FILL), (0), 0, 0);
	}
	...
	gtk_table_attach(GTK_TABLE(mw->dtable_h), mw->Next_toolbutton, i, i + 1, 0, 1, (GTK_FILL), (0), 0, 0);
	gtk_widget_show_all(mw->Next_toolbutton);
	g_signal_connect((gpointer)mw->Next_toolbutton, "button_release_event", G_CALLBACK(on_Next_clicked), mw);
	...
	gtk_scrolled_window_set_placement(GTK_SCROLLED_WINDOW(mw->scroll_win)
					  , GTK_CORNER_TOP_LEFT);
	gtk_box_pack_start(GTK_BOX(mw->month_view_vbox), mw->scroll_win, TRUE, TRUE, 0);
...
month_win *create_month_win(FolderItem *item, struct tm tmdate)
{
	...
	mw->selsig = vcal_view_set_calendar_page(mw->Vbox, G_CALLBACK(mw_summary_selected), mw);

	vcal_view_create_popup_menus(mw->Vbox, &mw->view_menu, &mw->event_menu, &mw->event_group, &mw->ui_manager);
	return (mw);
}
