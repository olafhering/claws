...
static void view_new_meeting_cb(GtkAction *action, gpointer user_data);
static void view_edit_meeting_cb(GtkAction *action, gpointer user_data);
static void view_cancel_meeting_cb(GtkAction *action, gpointer user_data);
static void view_go_today_cb(GtkAction *action, gpointer user_data);
...
GtkWidget *build_line(gint start_x, gint start_y, gint width, gint height, GtkWidget *hour_line, GdkColor *line_color)
{
...
gint vcal_view_set_calendar_page(GtkWidget *to_show, GCallback cb, gpointer data)
{
	...
	if (mainwindow_get_mainwindow()) {
		...
		gtk_container_add(GTK_CONTAINER(summaryview->mainwidget_book), to_show);
		gtk_notebook_set_current_page(GTK_NOTEBOOK(summaryview->mainwidget_book), gtk_notebook_page_num(GTK_NOTEBOOK(summaryview->mainwidget_book), to_show));
		main_window_set_menu_sensitive(mainwindow_get_mainwindow());
		toolbar_main_set_sensitive(mainwindow_get_mainwindow());
		selsig = g_signal_connect(G_OBJECT(summaryview->ctree), "tree_select_row", G_CALLBACK(cb), data);
	}
	return selsig;
}
...
void vcal_view_set_summary_page(GtkWidget *to_remove, guint selsig)
{
	...
	if (mainwindow_get_mainwindow()) {
		...
		gtk_container_remove(GTK_CONTAINER(summaryview->mainwidget_book), to_remove);
		gtk_notebook_set_current_page(GTK_NOTEBOOK(summaryview->mainwidget_book), gtk_notebook_page_num(GTK_NOTEBOOK(summaryview->mainwidget_book), summaryview->scrolledwin));
		main_window_set_menu_sensitive(mainwindow_get_mainwindow());
		toolbar_main_set_sensitive(mainwindow_get_mainwindow());
	}
...
void vcal_view_select_event(const gchar *uid, FolderItem *item, gboolean edit, GCallback block_cb, gpointer block_data)
{
	if (edit) {
...
void vcal_view_create_popup_menus(gpointer data, GtkWidget **view_menu, GtkWidget **event_menu, GtkActionGroup **event_group, GtkUIManager **ui_manager)
{
	*ui_manager = gtk_ui_manager_new();
	*event_group = cm_menu_create_action_group_full(*ui_manager, "VcalViewEvent", view_event_popup_entries, G_N_ELEMENTS(view_event_popup_entries), (gpointer)data);
	...
	MENUITEM_ADDUI_MANAGER(*ui_manager, "/", "Menus", "Menus", GTK_UI_MANAGER_MENUBAR)
	    ...
	    * view_menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(*ui_manager, "/Menus/VcalView")));
	*event_menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(*ui_manager, "/Menus/VcalViewEvent")));
}
...
static void view_new_meeting_cb(GtkAction *action, gpointer user_data)
{
	...
	void (*cb)(gpointer win, gpointer data_i, gpointer data_s) = g_object_get_data(G_OBJECT(user_data), "new_meeting_cb");
	if (cb)
...
static void view_edit_meeting_cb(GtkAction *action, gpointer user_data)
{
	...
	gpointer data_s = g_object_get_data(G_OBJECT(user_data), "menu_data_s");
	gpointer win = g_object_get_data(G_OBJECT(user_data), "menu_win");
	void (*cb)(gpointer win, gpointer data_i, gpointer data_s) = g_object_get_data(G_OBJECT(user_data), "edit_meeting_cb");
	if (cb)
		cb(win, data_i, data_s);
}
...
static void view_cancel_meeting_cb(GtkAction *action, gpointer user_data)
{
	...
	gpointer data_s = g_object_get_data(G_OBJECT(user_data), "menu_data_s");
	gpointer win = g_object_get_data(G_OBJECT(user_data), "menu_win");
	void (*cb)(gpointer win, gpointer data_i, gpointer data_s) = g_object_get_data(G_OBJECT(user_data), "cancel_meeting_cb");
	if (cb)
		cb(win, data_i, data_s);
}
...
static void view_go_today_cb(GtkAction *action, gpointer user_data)
{
	...
	gpointer data_s = g_object_get_data(G_OBJECT(user_data), "menu_data_s");
	gpointer win = g_object_get_data(G_OBJECT(user_data), "menu_win");
	void (*cb)(gpointer win, gpointer data_i, gpointer data_s) = g_object_get_data(G_OBJECT(user_data), "go_today_cb");
	if (cb)
		cb(win, data_i, data_s);
}
