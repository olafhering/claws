...
static void keyword_warner_prefs_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(case_sensitive_checkbox), kwarnerprefs.case_sensitive);
	gtk_widget_show(case_sensitive_checkbox);

	CLAWS_SET_TIP(case_sensitive_checkbox, _("Case sensitive when matching for the regular expressions in the list"));
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(skip_quotes_checkbox), kwarnerprefs.skip_quotes);
	gtk_box_pack_start(GTK_BOX(vbox2), skip_quotes_checkbox, FALSE, FALSE, 0);
	gtk_widget_show(skip_quotes_checkbox);

	CLAWS_SET_TIP(skip_quotes_checkbox, _("Exclude quoted lines from checking for the regular " "expressions above. Note that handmade quotes cannot " "be distinguished from quotes generated by replying."));
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(skip_fwd_redir_checkbox), kwarnerprefs.skip_forwards_and_redirections);
	gtk_box_pack_start(GTK_BOX(vbox2), skip_fwd_redir_checkbox, FALSE, FALSE, 0);
	gtk_widget_show(skip_fwd_redir_checkbox);

	CLAWS_SET_TIP(skip_fwd_redir_checkbox, _("Don't check for keywords when forwarding or redirecting messages"));
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(skip_signature_checkbox), kwarnerprefs.skip_signature);
	gtk_box_pack_start(GTK_BOX(vbox2), skip_signature_checkbox, FALSE, FALSE, 0);
	gtk_widget_show(skip_signature_checkbox);

	CLAWS_SET_TIP(skip_signature_checkbox, _("Exclude lines from the first signature-separator onwards " "from checking for the regular expressions above"));
...
static void keyword_warner_prefs_save_func(PrefsPage *_page)
{
	...
	kwarnerprefs.skip_quotes = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->skip_quotes_checkbox));
	kwarnerprefs.skip_forwards_and_redirections = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->skip_forwards_and_redirections));
	kwarnerprefs.skip_signature = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->skip_signature));
	kwarnerprefs.case_sensitive = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->case_sensitive_checkbox));
