...
gchar *ask_user_for_auth_code(const gchar *auth_uri)
{
	...
	dialog = gtk_message_dialog_new_with_markup(mainwin ? GTK_WINDOW(mainwin->window) : NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO, GTK_BUTTONS_NONE, "<span weight=\"bold\" size=\"larger\">%s</span>", _("GData plugin: Authorization required"));
	gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), _("You need to authorize Claws Mail to access your Google contact list to use the GData plugin." "\n\nVisit Google's authorization page by pressing the button below. After you " "confirmed the authorization, you will get an authorization code. Enter that code " "in the field below to grant Claws Mail access to your Google contact list."));
...
static void query_contacts(GDataContactsService *service)
{
	...
	gdata_contacts_service_query_contacts_async(service, GDATA_QUERY(query), NULL, NULL, NULL, NULL, (GAsyncReadyCallback) cm_gdata_query_contacts_ready, NULL);
...
static void query_for_contacts_group_id(GDataAuthorizer *authorizer)
{
	...
	gdata_contacts_service_query_groups_async(service, NULL, NULL, NULL, NULL, NULL, (GAsyncReadyCallback) cm_gdata_query_groups_ready, NULL);
}
...
static void cm_gdata_refresh_ready(GDataOAuth2Authorizer *auth, GAsyncResult *res, gpointer data)
{
	...
	if (gdata_authorizer_refresh_authorization_finish(GDATA_AUTHORIZER(auth), res, &error) == FALSE) {
		...
		start_interactive_auth = g_error_matches(error, GDATA_SERVICE_ERROR, GDATA_SERVICE_ERROR_AUTHENTICATION_REQUIRED) || g_error_matches(error, GDATA_SERVICE_ERROR, GDATA_SERVICE_ERROR_FORBIDDEN);
