...
typedef struct {
	PyObject_HEAD PyObject *ui_manager;
	PyObject *text;
	PyObject *replyinfo;
	PyObject *fwdinfo;
	Compose *compose;
...
static PyObject *ComposeWindow_set_header_list(clawsmail_ComposeWindowObject *self, PyObject *args)
{
	...
	for (iEl = 0; iEl < headerlistsize; iEl++) {
		...
		if (!headerfield || !headercontent || !PyObject_TypeCheck(headerfield, &PyString_Type) || !PyObject_TypeCheck(headercontent, &PyString_Type)) {
			PyErr_SetString(PyExc_LookupError, "Argument to set_header_list() must be a list of tuples with two strings");
			return NULL;
		}
	}
...
static PyObject *ComposeWindow_set_modified(clawsmail_ComposeWindowObject *self, PyObject *args)
{
	...
	if ((strcmp(gtk_window_get_title(GTK_WINDOW(self->compose->window)), _("Compose message")) != 0) && (old_modified != self->compose->modified)) {
		gchar *str;
		gchar *edited;
		gchar *subject;
