...
static void setup_python_console(void)
{
	...
	g_signal_connect(G_OBJECT(python_console), "size_allocate", G_CALLBACK(size_allocate_cb), NULL);
	if (!geometry.min_height) {
		geometry.min_width = 600;
		geometry.min_height = 400;
	}

	gtk_window_set_geometry_hints(GTK_WINDOW(python_console), NULL, &geometry, GDK_HINT_MIN_SIZE);
	gtk_widget_set_size_request(python_console, python_config.console_win_width, python_config.console_win_height);
...
static void run_auto_script_file_if_it_exists(const gchar *autofilename, Compose *compose)
{
	...
	auto_filepath = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, PYTHON_SCRIPTS_BASE_DIR, G_DIR_SEPARATOR_S, PYTHON_SCRIPTS_AUTO_DIR, G_DIR_SEPARATOR_S, autofilename, NULL);
...
static void compose_toolbar_callback(gpointer parent, const gchar *item_name, gpointer data)
{
	...
	filename = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, PYTHON_SCRIPTS_BASE_DIR, G_DIR_SEPARATOR_S, PYTHON_SCRIPTS_COMPOSE_DIR, G_DIR_SEPARATOR_S, item_name, NULL);
...
{
	const char *dirs[] = {
		"", PYTHON_SCRIPTS_MAIN_DIR, PYTHON_SCRIPTS_COMPOSE_DIR, PYTHON_SCRIPTS_AUTO_DIR, NULL
	};
	const char **dir = dirs;
...
static void migrate_scripts_out_of_base_dir(void)
{
	...
	dest_dir = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, PYTHON_SCRIPTS_BASE_DIR, G_DIR_SEPARATOR_S, PYTHON_SCRIPTS_MAIN_DIR, NULL);
...
static void create_mainwindow_menus_and_items(GSList *filenames, gint num_entries)
{
	...
	for (ii = 0; ii < num_entries; ii++) {
		...
		MENUITEM_ADDUI_ID_MANAGER(mainwin->ui_manager, "/Menu/" PYTHON_SCRIPTS_ACTION_PREFIX, entries[ii].label, entries[ii].name, GTK_UI_MANAGER_MENUITEM, id)
...
static gboolean my_compose_create_hook(gpointer cw, gpointer data)
{
	...
	MENUITEM_ADDUI_MANAGER(compose->ui_manager, "/Menu/Tools", "PythonScripts", "Tools/PythonScripts", GTK_UI_MANAGER_MENU)
	    for (ii = 0; ii < num_entries; ii++) {
		MENUITEM_ADDUI_MANAGER(compose->ui_manager, "/Menu/" PYTHON_SCRIPTS_ACTION_PREFIX, entries[ii].label, entries[ii].name, GTK_UI_MANAGER_MENUITEM)
	}
...
static void refresh_scripts_in_dir(const gchar *subdir, ToolbarType toolbar_type)
{
	...
	scripts_dir = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, PYTHON_SCRIPTS_BASE_DIR, G_DIR_SEPARATOR_S, subdir, NULL);
...
static int python_menu_init(char **error)
{
	...
	MENUITEM_ADDUI_ID_MANAGER(mainwin->ui_manager, "/Menu/Tools", "ShowPythonConsole", "Tools/ShowPythonConsole", GTK_UI_MANAGER_MENUITEM, id)
	    menu_id_list = g_slist_prepend(menu_id_list, GUINT_TO_POINTER(id));

	MENUITEM_ADDUI_ID_MANAGER(mainwin->ui_manager, "/Menu/Tools", "PythonScripts", "Tools/PythonScripts", GTK_UI_MANAGER_MENU, id)
	    menu_id_list = g_slist_prepend(menu_id_list, GUINT_TO_POINTER(id));

	MENUITEM_ADDUI_ID_MANAGER(mainwin->ui_manager, "/Menu/Tools/PythonScripts", "Refresh", "Tools/PythonScripts/Refresh", GTK_UI_MANAGER_MENUITEM, id)
	    menu_id_list = g_slist_prepend(menu_id_list, GUINT_TO_POINTER(id));

	MENUITEM_ADDUI_ID_MANAGER(mainwin->ui_manager, "/Menu/Tools/PythonScripts", "Browse", "Tools/PythonScripts/Browse", GTK_UI_MANAGER_MENUITEM, id)
	    menu_id_list = g_slist_prepend(menu_id_list, GUINT_TO_POINTER(id));

	MENUITEM_ADDUI_ID_MANAGER(mainwin->ui_manager, "/Menu/Tools/PythonScripts", "Separator1", "Tools/PythonScripts/---", GTK_UI_MANAGER_SEPARATOR, id)
	    menu_id_list = g_slist_prepend(menu_id_list, GUINT_TO_POINTER(id));
...
{
	static struct PluginFeature features[] = { {PLUGIN_UTILITY, N_("Python integration")},
	{PLUGIN_NOTHING, NULL}
	};
	return features;
}
