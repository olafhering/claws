...
static void create_progress_dialog(struct ArchivePage *page)
{
	...
	progress->position = 0;
	progress->progress_dialog = gtk_dialog_new_with_buttons(title, GTK_WINDOW(mainwin->window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, NULL);

	g_signal_connect(progress->progress_dialog, "response", G_CALLBACK(progress_dialog_cb), page);

	progress->frame = gtk_frame_new(_("Press Cancel button to stop archiving"));
	gtk_frame_set_shadow_type(GTK_FRAME(progress->frame), GTK_SHADOW_ETCHED_OUT);
	...
	gtk_box_pack_start(GTK_BOX(progress->vbox1), progress->hbox1, FALSE, FALSE, 0);

	progress->add_label = gtk_label_new(_("Archiving:"));
	gtk_box_pack_start(GTK_BOX(progress->hbox1), progress->add_label, FALSE, FALSE, 0);

	progress->file_label = gtk_label_new("");
	gtk_label_set_ellipsize(GTK_LABEL(progress->file_label), PANGO_ELLIPSIZE_START);
	gtk_box_pack_start(GTK_BOX(progress->hbox1), progress->file_label, TRUE, TRUE, 0);
	...
	gtk_box_pack_start(GTK_BOX(progress->hbox1), progress->progress, TRUE, TRUE, 0);
...
static void create_md5sum(const gchar *file, const gchar *md5_file)
{
	...
	if ((fd = open(md5_file, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR)) == -1) {
...
static gchar *descriptive_file_name(struct ArchivePage *page, const gchar *file, MsgInfo *msginfo)
{
	gchar *new_file = NULL;
...
static void walk_folder(struct ArchivePage *page, FolderItem *item, gboolean recursive)
{
	FolderItem *child;
	GSList *msglist;
	GSList *cur;
	...
	if (!page->cancelled) {
		...
		for (cur = msglist; cur && !page->cancelled; cur = cur->next) {
			...
			if (count % 350 == 0) {
				...
				text = g_strdup_printf("Scanning %s: %d files", item->name, count);
				gtk_progress_bar_set_text(GTK_PROGRESS_BAR(progress->progress), text);
				g_free(text);
...
static AFileTest file_is_writeable(struct ArchivePage *page)
{
	...
	if (g_file_test(page->name, G_FILE_TEST_EXISTS) && !page->force_overwrite)
...
static gboolean archiver_save_files(struct ArchivePage *page)
{
	...
	if ((perm = file_is_writeable(page)) != A_FILE_OK) {
		...
		if (perm == A_FILE_EXISTS) {
			...
			aval = alertpanel_full(_("Creating archive"), msg, GTK_STOCK_CANCEL, GTK_STOCK_OK, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
			g_free(msg);
	...
	page->files = 0;
	page->total_size = 0;
	page->rename = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->rename_files));
	recursive = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->recursive));
	page->md5 = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->md5sum));
	page->unlink = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->unlink_files));
	...
	if (orig_file != g_slist_length(list)) {
		...
		msg = g_strdup_printf(_("Adding files in folder failed\n" "Files in folder: %d\n" "Files in list:   %d\n" "\nContinue anyway?"), orig_file, g_slist_length(list));
		aval = alertpanel_full(_("Creating archive"), msg, GTK_STOCK_CANCEL, GTK_STOCK_OK, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
		g_free(msg);
		if (aval != G_ALERTALTERNATE) {
			archive_free_file_list(page->md5, page->rename);
...
static void show_result(struct ArchivePage *page)
{
	...
	dialog = gtk_dialog_new_with_buttons(_("Archive result"), GTK_WINDOW(mainwin->window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_OK, GTK_RESPONSE_NONE, NULL);
	g_signal_connect_swapped(dialog, "response", G_CALLBACK(gtk_widget_destroy), dialog);

	list = gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_STRING);

	view = g_object_new(GTK_TYPE_TREE_VIEW, "model", list, "rules-hint", FALSE, "headers-clickable", FALSE, "reorderable", FALSE, "enable-search", FALSE, NULL);

	renderer = gtk_cell_renderer_text_new();

	header = gtk_tree_view_column_new_with_attributes(_("Attributes"), renderer, "text", STRING1, NULL);
	gtk_tree_view_append_column(view, header);

	header = gtk_tree_view_column_new_with_attributes(_("Values"), renderer, "text", STRING2, NULL);
	gtk_tree_view_append_column(view, header);
	...
	gtk_list_store_set(list, &iter, STRING1, _("Archive"), STRING2, page->name, -1);

	gtk_list_store_append(list, &iter);
	gtk_list_store_set(list, &iter, STRING1, _("Archive format"), STRING2, format, -1);
	g_free(format);

	gtk_list_store_append(list, &iter);
	gtk_list_store_set(list, &iter, STRING1, _("Compression method"), STRING2, method, -1);
	g_free(method);

	gtk_list_store_append(list, &iter);
	msg = g_strdup_printf("%d", (page->md5) ? page->files * 2 : page->files);
	gtk_list_store_set(list, &iter, STRING1, _("Number of files"), STRING2, msg, -1);
	g_free(msg);

	gtk_list_store_append(list, &iter);
	msg = g_strdup_printf("%d byte(s)", (guint)st.st_size);
	gtk_list_store_set(list, &iter, STRING1, _("Archive Size"), STRING2, msg, -1);
	g_free(msg);

	gtk_list_store_append(list, &iter);
	msg = g_strdup_printf("%d byte(s)", page->total_size);
	gtk_list_store_set(list, &iter, STRING1, _("Folder Size"), STRING2, msg, -1);
	g_free(msg);

	gtk_list_store_append(list, &iter);
	msg = g_strdup_printf("%d%%", (guint)((st.st_size * 100) / page->total_size));
	gtk_list_store_set(list, &iter, STRING1, _("Compression level"), STRING2, msg, -1);
	g_free(msg);

	gtk_list_store_append(list, &iter);
	msg = g_strdup_printf("%s", (page->md5) ? _("Yes") : _("No"));
	gtk_list_store_set(list, &iter, STRING1, _("MD5 checksum"), STRING2, msg, -1);
	g_free(msg);

	gtk_list_store_append(list, &iter);
	msg = g_strdup_printf("%s", (page->rename) ? _("Yes") : _("No"));
	gtk_list_store_set(list, &iter, STRING1, _("Descriptive names"), STRING2, msg, -1);
	g_free(msg);

	gtk_list_store_append(list, &iter);
	msg = g_strdup_printf("%s", (page->unlink) ? _("Yes") : _("No"));
	gtk_list_store_set(list, &iter, STRING1, _("Delete selected files"), STRING2, msg, -1);
	g_free(msg);

	msg = g_strdup(gtk_entry_get_text(GTK_ENTRY(page->isoDate)));
	if (msg) {
		gtk_list_store_append(list, &iter);
		gtk_list_store_set(list, &iter, STRING1, _("Select mails before"), STRING2, msg, -1);
	}
	g_free(msg);
...
static void archiver_dialog_cb(GtkWidget *widget, gint action, gpointer data)
{
	...
	if (page->response) {
		...
		if (progress->progress_dialog && GTK_IS_WIDGET(progress->progress_dialog))
...
static void foldersel_cb(GtkWidget *widget, gpointer data)
{
	...
	item = foldersel_folder_sel(NULL, FOLDER_SEL_MOVE, NULL, FALSE, _("Select folder to archive"));
	if (item && (item_id = folder_item_get_identifier(item)) != NULL) {
		gtk_editable_delete_text(GTK_EDITABLE(page->folder), 0, -1);
		gtk_editable_insert_text(GTK_EDITABLE(page->folder), item_id, strlen(item_id), &newpos);
...
static void filesel_cb(GtkWidget *widget, gpointer data)
{
	...
	dialog = gtk_file_chooser_dialog_new(_("Select file name for archive [suffix should reflect archive like .tgz]"), NULL, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_APPLY, GTK_RESPONSE_APPLY, NULL);

	if (archiver_prefs.save_folder)
		gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(dialog), archiver_prefs.save_folder);
	else
		gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(dialog), get_home_dir());
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_APPLY) {
		file = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog));
		if (file) {
			gtk_editable_delete_text(GTK_EDITABLE(page->file), 0, -1);
			gtk_editable_insert_text(GTK_EDITABLE(page->file), file, strlen(file), &newpos);
...
void set_progress_print_all(guint fraction, guint total, guint step)
{
	...
	if (GTK_IS_WIDGET(progress->progress)) {
		if ((fraction - progress->position) % step == 0) {
			...
			gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progress->progress), (total == 0) ? 0 : (gfloat) fraction / (gfloat) total);
			text_count = g_strdup_printf(_("%ld of %ld"), (long)fraction, (long)total);
			gtk_progress_bar_set_text(GTK_PROGRESS_BAR(progress->progress), text_count);
			g_free(text_count);
...
void archiver_gtk_show(void)
{
	...
	dialog = gtk_dialog_new_with_buttons(_("Create Archive"), GTK_WINDOW(mainwin->window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, NULL);

	g_signal_connect(dialog, "response", G_CALLBACK(archiver_dialog_cb), page);
	...
	CLAWS_SET_TIP(page->folder, _("Folder which is the root of the archive"));

	folder_select = gtkut_get_browse_directory_btn(_("_Browse"));
	gtk_box_pack_start(GTK_BOX(hbox1), folder_select, FALSE, FALSE, 0);
	CLAWS_SET_TIP(folder_select, _("Click this button to select a folder which is to be root of the archive"));
	...
	CLAWS_SET_TIP(file_select, _("Click this button to select a name and location for the archive"));
	...
	bzip_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "BZIP_2");
	...
	compress_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "Com_press");
...
#if ARCHIVE_VERSION_NUMBER >= 2006990
	lzma_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "_LZMA");
	...
	xz_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "_XZ");
...
#if ARCHIVE_VERSION_NUMBER >= 3000000
	lzip_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "_LZIP");
...
#if ARCHIVE_VERSION_NUMBER >= 3001000
	lrzip_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "L_RZIP");
	...
	lzop_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "LZ_OP");
	...
	grzip_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "_GRZIP");
...
#if ARCHIVE_VERSION_NUMBER >= 3001900
	lz4_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), "LZ_4");
	...
	no_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(gzip_radio_btn), _("_None"));
	...
	page->compress_methods = gtk_radio_button_get_group(GTK_RADIO_BUTTON(gzip_radio_btn));
	...
	shar_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(tar_radio_btn), "S_HAR");
	...
	cpio_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(tar_radio_btn), "CP_IO");
	...
	pax_radio_btn = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(tar_radio_btn), "PA_X");
	...
	page->archive_formats = gtk_radio_button_get_group(GTK_RADIO_BUTTON(tar_radio_btn));
	...
	CLAWS_SET_TIP(page->recursive, _("Choose this option to include subfolders in the archive"));
	...
	CLAWS_SET_TIP(page->md5sum, _("Choose this option to add MD5 checksums for each file in the archive.\n" "Be aware though, that this dramatically increases the time it\n" "will take to create the archive"));
	...
	CLAWS_SET_TIP(page->rename_files, _("Choose this option to use descriptive names for each file in the archive.\n" "The naming scheme: date_from@to@subject.\n" "Names will be truncated to max 96 characters"));
	...
	CLAWS_SET_TIP(page->unlink_files, _("Choose this option to delete mails after archiving\n" "At this point only handles IMAP4, Local mbox and POP3"));
	...
	CLAWS_SET_TIP(page->isoDate, _("Select emails before a certain date\n" "Date must comply to ISO-8601 [YYYY-MM-DD]"));

	g_signal_connect(G_OBJECT(folder_select), "clicked", G_CALLBACK(foldersel_cb), page);
	g_signal_connect(G_OBJECT(file_select), "clicked", G_CALLBACK(filesel_cb), page);
	g_signal_connect(G_OBJECT(page->folder), "activate", G_CALLBACK(entry_change_cb), page);
	g_signal_connect(G_OBJECT(page->file), "activate", G_CALLBACK(entry_change_cb), page);
