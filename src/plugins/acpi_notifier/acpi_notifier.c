...
static void show_error(struct AcpiNotifierPage *page, const gchar *filepath)
{
	...
	for (i = 0; known_implementations[i].name != NULL; i++) {
		...
		if (known_implementations[i].help) {
			...
			gtk_label_set_text(GTK_LABEL(page->warning_label), _("Control file doesn't exist."));
		}
...
static void type_activated(GtkMenuItem *menuitem, gpointer data)
{
	...
	menu = gtk_cmoption_menu_get_menu(GTK_CMOPTION_MENU(page->default_implementations_optmenu));
	item = gtk_menu_get_active(GTK_MENU(menu));
	selected = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(item), MENU_VAL_ID));

	if (selected != 0) {
		gtk_widget_hide(page->hbox_acpi_file);
		gtk_widget_hide(page->hbox_acpi_values);
		gtk_entry_set_text(GTK_ENTRY(page->file_entry), known_implementations[selected].file_path);
		gtk_entry_set_text(GTK_ENTRY(page->on_value_entry), known_implementations[selected].on_param);
		gtk_entry_set_text(GTK_ENTRY(page->off_value_entry), known_implementations[selected].off_param);
...
static void acpi_prefs_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	no_mail_blink_btn = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(no_mail_off_btn), _("blinking"));
	no_mail_on_btn = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(no_mail_off_btn), _("on"));

	unread_mail_off_btn = gtk_radio_button_new_with_label(NULL, _("off"));
	unread_mail_blink_btn = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(unread_mail_off_btn), _("blinking"));
	unread_mail_on_btn = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(unread_mail_off_btn), _("on"));

	new_mail_off_btn = gtk_radio_button_new_with_label(NULL, _("off"));
	new_mail_blink_btn = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(new_mail_off_btn), _("blinking"));
	new_mail_on_btn = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(new_mail_off_btn), _("on"));
	...
	gtk_cmoption_menu_set_menu(GTK_CMOPTION_MENU(default_implementations_optmenu), default_implementations_menu);
	for (i = 0; known_implementations[i].name != NULL; i++) {
		MENUITEM_ADD(default_implementations_menu, menuitem, known_implementations[i].name, i);
		g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(type_activated), page);
	}
	...
	g_signal_connect(G_OBJECT(file_entry), "changed", G_CALLBACK(file_entry_changed), page);
	...
	warning_label = gtk_label_new(_("Control file doesn't exist."));
	...
	blink_on_err_chkbtn = gtk_check_button_new_with_label(_("Blink when user interaction is required"));
	gtk_box_pack_start(GTK_BOX(vbox), blink_on_err_chkbtn, FALSE, FALSE, 0);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(blink_on_err_chkbtn), acpiprefs.blink_on_err);
	gtk_widget_show(blink_on_err_chkbtn);

	switch (acpiprefs.no_mail_action) {
	case OFF:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(no_mail_off_btn), TRUE);
		break;
	case BLINK:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(no_mail_blink_btn), TRUE);
		break;
	case ON:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(no_mail_on_btn), TRUE);
		break;
	}

	switch (acpiprefs.unread_mail_action) {
	case OFF:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(unread_mail_off_btn), TRUE);
		break;
	case BLINK:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(unread_mail_blink_btn), TRUE);
		break;
	case ON:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(unread_mail_on_btn), TRUE);
		break;
	}

	switch (acpiprefs.new_mail_action) {
	case OFF:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(new_mail_off_btn), TRUE);
		break;
	case BLINK:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(new_mail_blink_btn), TRUE);
		break;
	case ON:
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(new_mail_on_btn), TRUE);
		break;
	}
	...
	if (acpiprefs.file_path != NULL) {
		for (i = 0; known_implementations[i].name != NULL; i++) {
			if (!strcmp(acpiprefs.file_path, known_implementations[i].file_path)) {
				gtk_cmoption_menu_set_history(GTK_CMOPTION_MENU(default_implementations_optmenu), i);
				found = i;
			}
		}
	}
	...
	if (found != 0) {
		...
		gtk_entry_set_text(GTK_ENTRY(file_entry), known_implementations[found].file_path);
		gtk_entry_set_text(GTK_ENTRY(on_value_entry), known_implementations[found].on_param);
		gtk_entry_set_text(GTK_ENTRY(off_value_entry), known_implementations[found].off_param);

		if (!check_impl(known_implementations[found].file_path))
			show_error(page, known_implementations[found].file_path);
	} else {
		gtk_cmoption_menu_set_history(GTK_CMOPTION_MENU(default_implementations_optmenu), 0);
...
static void acpi_prefs_save_func(PrefsPage *_page)
{
	...
	acpiprefs.blink_on_err = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->blink_on_err_chkbtn));

	menu = gtk_cmoption_menu_get_menu(GTK_CMOPTION_MENU(page->default_implementations_optmenu));
	menuitem = gtk_menu_get_active(GTK_MENU(menu));
	selected = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(menuitem), MENU_VAL_ID));
...
void acpi_prefs_init(void)
{
	...
	folder_hook_id = hooks_register_hook(FOLDER_ITEM_UPDATE_HOOKLIST, acpi_update_hook, NULL);
	alertpanel_hook_id = hooks_register_hook(ALERTPANEL_OPENED_HOOKLIST, acpi_alertpanel_hook, NULL);
	should_quit = FALSE;
}
...
void acpi_init(void)
{
	...
	for (i = 0; acpi_help[i] != NULL; i++)
		known_implementations[i].help = *acpi_help[i] ? _(acpi_help[i]) : "";
	acpi_prefs_init();
}
...
{
	if (!check_plugin_version(MAKE_NUMERIC_VERSION(3, 8, 1, 46), VERSION_NUMERIC, PLUGIN_NAME, error))
		return -1;
...
{
	static struct PluginFeature features[] = { {PLUGIN_NOTIFIER, N_("Laptop LED")},
	{PLUGIN_NOTHING, NULL}
	};
	return features;
}
