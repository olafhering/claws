...
static void bsfilter_create_widget_func(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	CLAWS_SET_TIP(learn_from_whitelist_chkbtn, _("If Bsfilter thought an email was spam or unsure, but it was whitelisted, " "learn it as ham."));
	...
	CLAWS_SET_TIP(bspath_entry, _("Path to bsfilter executable"));
...
#ifndef USE_ALT_ADDRBOOK
	g_signal_connect(G_OBJECT(whitelist_ab_select_btn), "clicked", G_CALLBACK(bsfilter_whitelist_ab_select_cb), page);
#else
