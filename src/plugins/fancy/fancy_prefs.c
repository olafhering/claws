...
static void remote_content_set_labels_cb(GtkWidget *button, FancyPrefsPage *prefs_page)
{
	...
	/* Enable images */
	gtk_button_set_label(GTK_BUTTON(prefs_page->enable_images), remote_enabled ? _("Display images")
			     : _("Display embedded images"));

	/* Enable Javascript */
	gtk_button_set_label(GTK_BUTTON(prefs_page->enable_scripts), remote_enabled ? _("Execute javascript")
			     : _("Execute embedded javascript"));

	/* Enable java */
	gtk_button_set_label(GTK_BUTTON(prefs_page->enable_java), remote_enabled ? _("Execute Java applets")
			     : _("Execute embedded Java applets"));

	/* Enable plugins */
	gtk_button_set_label(GTK_BUTTON(prefs_page->enable_plugins), remote_enabled ? _("Render objects using plugins")
	...
	if (gtk_tree_model_get_iter_first(model, &iter)) {
		if (remote_enabled)
			gtk_list_store_set(GTK_LIST_STORE(model), &iter, COMBOBOX_TEXT, _("Open in Viewer (remote content is enabled)"), -1);
		else
			gtk_list_store_set(GTK_LIST_STORE(model), &iter, COMBOBOX_TEXT, _("Do nothing (remote content is disabled)"), -1);
	}
...
static void create_fancy_prefs_page(PrefsPage *page, GtkWindow *window, gpointer data)
{
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(gnome_proxy_checkbox), fancy_prefs.enable_gnome_proxy);
	gtk_box_pack_start(GTK_BOX(vbox_proxy), gnome_proxy_checkbox, FALSE, FALSE, 0);
	gtk_widget_show(gnome_proxy_checkbox);
	g_signal_connect(G_OBJECT(gnome_proxy_checkbox), "toggled", G_CALLBACK(prefs_disable_fancy_proxy), block);
#endif
...
#endif
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(proxy_checkbox), fancy_prefs.enable_proxy);
	prefs_set_proxy_entry_sens(proxy_checkbox, GTK_ENTRY(proxy_str));
	g_signal_connect(G_OBJECT(proxy_checkbox), "toggled", G_CALLBACK(prefs_set_proxy_entry_sens), proxy_str);
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(enable_remote_content), fancy_prefs.enable_remote_content);
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(enable_images), fancy_prefs.enable_images);
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(enable_scripts), fancy_prefs.enable_scripts);
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(enable_java), fancy_prefs.enable_java);
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(enable_plugins), fancy_prefs.enable_plugins);
	...
	GtkListStore *menu = GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(optmenu_open_external)));
	...
	combobox_select_by_data(GTK_COMBO_BOX(optmenu_open_external), fancy_prefs.open_external);
	...
	g_signal_connect(G_OBJECT(stylesheet_browse_button), "clicked", G_CALLBACK(fancy_prefs_stylesheet_browse_cb), stylesheet);
	...
	g_signal_connect(G_OBJECT(stylesheet_edit_button), "clicked", G_CALLBACK(fancy_prefs_stylesheet_edit_cb), stylesheet);
	g_signal_connect(G_OBJECT(stylesheet), "changed", G_CALLBACK(fancy_prefs_stylesheet_changed_cb), stylesheet_edit_button);
	...
	g_signal_connect(G_OBJECT(prefs_page->enable_remote_content), "toggled", G_CALLBACK(remote_content_set_labels_cb), prefs_page);
...
static void prefs_set_proxy_entry_sens(GtkWidget *button, GtkEntry *entry_str)
{
	gtk_widget_set_sensitive(GTK_WIDGET(entry_str), gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(button)));
}
...
static void save_fancy_prefs_page(PrefsPage *page)
{
...
#ifdef HAVE_LIBSOUP_GNOME
	fancy_prefs.enable_gnome_proxy = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->gnome_proxy_checkbox));
#endif
	fancy_prefs.enable_images = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->enable_images));
	fancy_prefs.enable_remote_content = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->enable_remote_content));
	fancy_prefs.enable_scripts = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->enable_scripts));
	fancy_prefs.enable_plugins = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->enable_plugins));
	fancy_prefs.enable_java = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->enable_java));
	fancy_prefs.open_external = combobox_get_active_data(GTK_COMBO_BOX(prefs_page->open_external));
	fancy_prefs.enable_proxy = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->proxy_checkbox));
	...
	fancy_prefs.stylesheet = g_strdup(gtk_entry_get_text(GTK_ENTRY(prefs_page->stylesheet)));
#else
