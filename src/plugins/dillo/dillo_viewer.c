...
static gboolean load_images(DilloViewer *viewer)
{
	...
	if (*dillo_prefs.whitelist_ab_folder != '\0' && strcasecmp(dillo_prefs.whitelist_ab_folder, _("Any")) != 0)
...
static void dillo_show_mimepart(MimeViewer *_viewer, const gchar *infile, MimeInfo *partinfo)
{
	...
	if (!(procmime_get_part(viewer->filename, partinfo) < 0)) {
		...
		g_signal_connect(G_OBJECT(viewer->socket), "destroy", G_CALLBACK(socket_destroy_cb), viewer);

		gdkwin = gtk_widget_get_window(viewer->socket);
		cmd = g_strdup_printf("dillo %s%s-x %d \"%s\"", (!load_images(viewer) ? "-l " : ""), (dillo_prefs.full ? "-f " : ""), (gint)GDK_WINDOW_XWINDOW(gdkwin), viewer->filename);
...
{
	return _("This plugin renders HTML mail using the Dillo " "web browser.\n" "\n" "Options can be found in /Configuration/Preferences/Plugins/Dillo Browser");
}
...
{
	static struct PluginFeature features[] = { {PLUGIN_MIMEVIEWER, "text/html"},
	{PLUGIN_NOTHING, NULL}
	};
	return features;
}
