...
static void create_dillo_prefs_page(PrefsPage *page, GtkWindow *window, gpointer data);
...
static void local_checkbox_toggled(GtkToggleButton *button, gpointer user_data)
{
	gboolean active = gtk_toggle_button_get_active(button);
	DilloBrowserPage *prefs_page = (DilloBrowserPage *) user_data;

	gtk_widget_set_sensitive(prefs_page->whitelist_ab, active);
	gtk_widget_set_sensitive(prefs_page->whitelist_ab_folder_combo, active && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->whitelist_ab)));
#ifndef USE_NEW_ADDRBOOK
	gtk_widget_set_sensitive(prefs_page->whitelist_ab_select_btn, active && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->whitelist_ab)));
#endif
}

static void whitelist_checkbox_toggled(GtkToggleButton *button, gpointer user_data)
{
	gboolean active = gtk_toggle_button_get_active(button);
	DilloBrowserPage *prefs_page = (DilloBrowserPage *) user_data;
...
static void create_dillo_prefs_page(PrefsPage *page, GtkWindow *window, gpointer data)
{
	...
	local_checkbox = gtk_check_button_new_with_label(_("Load remote links in mails"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(local_checkbox), !dillo_prefs.local);
	gtk_box_pack_start(GTK_BOX(vbox), local_checkbox, FALSE, FALSE, 0);
	gtk_widget_show(local_checkbox);
	CLAWS_SET_TIP(local_checkbox, _("Equivalent to Dillo's '--local' option"));

	label = gtk_label_new(_("You can still load remote links " "by reloading the page"));
	...
	if (dillo_prefs.whitelist_ab_folder != NULL) {
		...
		if (strcasecmp(dillo_prefs.whitelist_ab_folder, "Any") == 0)
			gtk_entry_set_text(GTK_ENTRY(gtk_bin_get_child(GTK_BIN((whitelist_ab_folder_combo)))), _("Any"));
		else
			/* backward compatibility (when translated "Any" was stored) */
		if (g_utf8_collate(dillo_prefs.whitelist_ab_folder, _("Any")) == 0)
			gtk_entry_set_text(GTK_ENTRY(gtk_bin_get_child(GTK_BIN((whitelist_ab_folder_combo)))), dillo_prefs.whitelist_ab_folder);
		else
			gtk_entry_set_text(GTK_ENTRY(gtk_bin_get_child(GTK_BIN((whitelist_ab_folder_combo)))), dillo_prefs.whitelist_ab_folder);
	}

	full_checkbox = gtk_check_button_new_with_label(_("Full window mode (hide controls)"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(full_checkbox), dillo_prefs.full);
	gtk_box_pack_start(GTK_BOX(vbox), full_checkbox, FALSE, FALSE, 0);
	gtk_widget_show(full_checkbox);
	CLAWS_SET_TIP(full_checkbox, _("Equivalent to Dillo's '--fullwindow' option"));

	g_signal_connect(G_OBJECT(local_checkbox), "toggled", G_CALLBACK(local_checkbox_toggled), prefs_page);

	g_signal_connect(G_OBJECT(whitelist_ab_checkbtn), "toggled", G_CALLBACK(whitelist_checkbox_toggled), prefs_page);

#ifndef USE_ALT_ADDRBOOK
	g_signal_connect(G_OBJECT(whitelist_ab_select_btn), "clicked", G_CALLBACK(dillo_whitelist_ab_select_cb), prefs_page);
#else
...
static void save_dillo_prefs(PrefsPage *page)
{
	...
	dillo_prefs.local = !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->local));
	dillo_prefs.full = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->full));
	...
	dillo_prefs.whitelist_ab_folder = gtk_editable_get_chars(GTK_EDITABLE(gtk_bin_get_child(GTK_BIN((prefs_page->whitelist_ab_folder_combo)))), 0, -1);
