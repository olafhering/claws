...
static void PrintRrule(FILE *fptr, char *recur_data, int size, TNEFStruct *TNEF)
{
	...
	if (recur_data[0x04] == 0x0A) {
		...
		if (recur_data[0x16] == 0x23 || recur_data[0x16] == 0x22 || recur_data[0x16] == 0x21) {
			if ((filename = MAPIFindUserProp(&(TNEF->MapiProperties), PROP_TAG(PT_I2, 0x0011))) != MAPI_UNDEFINED) {
				fprintf(fptr, ";INTERVAL=%d", *(filename->data));
			}
			if (recur_data[0x16] == 0x22 || recur_data[0x16] == 0x21) {
				fprintf(fptr, ";COUNT=%d", GetRruleCount(recur_data[0x1B], recur_data[0x1A]));
			}
		} else if (recur_data[0x16] == 0x3E) {
			fprintf(fptr, ";BYDAY=MO,TU,WE,TH,FR");
			if (recur_data[0x1A] == 0x22 || recur_data[0x1A] == 0x21) {
				fprintf(fptr, ";COUNT=%d", GetRruleCount(recur_data[0x1F], recur_data[0x1E]));
			}
		}
	} else if (recur_data[0x04] == 0x0B) {
		fprintf(fptr, "WEEKLY;INTERVAL=%d;BYDAY=%s", recur_data[0x0E], GetRruleDayname(recur_data[0x16]));
		if (recur_data[0x1A] == 0x22 || recur_data[0x1A] == 0x21) {
			fprintf(fptr, ";COUNT=%d", GetRruleCount(recur_data[0x1F], recur_data[0x1E]));
		}
		...
		if (recur_data[0x06] == 0x02) {
			fprintf(fptr, ";INTERVAL=%d;BYMONTHDAY=%d", recur_data[0x0E], recur_data[0x16]);
			if (recur_data[0x1A] == 0x22 || recur_data[0x1A] == 0x21) {
				fprintf(fptr, ";COUNT=%d", GetRruleCount(recur_data[0x1F], recur_data[0x1E]));
			}
		} else if (recur_data[0x06] == 0x03) {
			fprintf(fptr, ";BYDAY=%s;BYSETPOS=%d;INTERVAL=%d", GetRruleDayname(recur_data[0x16]), recur_data[0x1A] == 0x05 ? -1 : recur_data[0x1A], recur_data[0x0E]);
			if (recur_data[0x1E] == 0x22 || recur_data[0x1E] == 0x21) {
				fprintf(fptr, ";COUNT=%d", GetRruleCount(recur_data[0x23], recur_data[0x22]));
			}
		}
	} else if (recur_data[0x04] == 0x0D) {
		fprintf(fptr, "YEARLY;BYMONTH=%d", GetRruleMonthNum(recur_data[0x0A], recur_data[0x0B]));
		if (recur_data[0x06] == 0x02) {
			fprintf(fptr, ";BYMONTHDAY=%d", recur_data[0x16]);
		} else if (recur_data[0x06] == 0x03) {
			fprintf(fptr, ";BYDAY=%s;BYSETPOS=%d", GetRruleDayname(recur_data[0x16]), recur_data[0x1A] == 0x05 ? -1 : recur_data[0x1A]);
		}
		if (recur_data[0x1E] == 0x22 || recur_data[0x1E] == 0x21) {
			fprintf(fptr, ";COUNT=%d", GetRruleCount(recur_data[0x23], recur_data[0x22]));
		}
...
gboolean SaveVCalendar(FILE *fptr, TNEFStruct *TNEF)
{
	...
	if ((filename = MAPIFindProperty(&(TNEF->MapiProperties), PROP_TAG(PT_BINARY, PR_SENDER_SEARCH_KEY)))
	    != MAPI_UNDEFINED) {
	...
	// Summary
	filename = NULL;
	if ((filename = MAPIFindProperty(&(TNEF->MapiProperties), PROP_TAG(PT_STRING8, PR_CONVERSATION_TOPIC)))
	    != MAPI_UNDEFINED) {
	...
	// Description
	if ((filename = MAPIFindProperty(&(TNEF->MapiProperties), PROP_TAG(PT_BINARY, PR_RTF_COMPRESSED)))
	    != MAPI_UNDEFINED) {
		variableLength buf;
	...
	if (filename != NULL) {
		...
		fprintf(fptr, "%04i%02i%02iT%02i%02i%02iZ\n", thedate.wYear, thedate.wMonth, thedate.wDay, thedate.wHour, thedate.wMinute, thedate.wSecond);
	}
	// Date End
	filename = NULL;
	...
	// Recurrence
	filename = NULL;
	if ((filename = MAPIFindUserProp(&(TNEF->MapiProperties), PROP_TAG(PT_BINARY, 0x8216))) != MAPI_UNDEFINED) {
...
gboolean SaveVTask(FILE *fptr, TNEFStruct *TNEF)
{
	...
	filename = MAPIFindUserProp(&(TNEF->MapiProperties), PROP_TAG(PT_STRING8, 0x8122));
	...
	filename = MAPIFindProperty(&(TNEF->MapiProperties), PROP_TAG(PT_SYSTIME, PR_CREATION_TIME));
	if (filename != MAPI_UNDEFINED) {
	...
	filename = MAPIFindUserProp(&(TNEF->MapiProperties), PROP_TAG(PT_SYSTIME, 0x8517));
	if (filename != MAPI_UNDEFINED) {
		fprintf(fptr, "DUE:");
		MAPISysTimetoDTR(filename->data, &thedate);
		fprintf(fptr, "%04i%02i%02iT%02i%02i%02iZ\n", thedate.wYear, thedate.wMonth, thedate.wDay, thedate.wHour, thedate.wMinute, thedate.wSecond);
	}
	filename = MAPIFindProperty(&(TNEF->MapiProperties), PROP_TAG(PT_SYSTIME, PR_LAST_MODIFICATION_TIME));
	if (filename != MAPI_UNDEFINED) {
	...
	// Class
	filename = MAPIFindUserProp(&(TNEF->MapiProperties), PROP_TAG(PT_BOOLEAN, 0x8506));
	if (filename != MAPI_UNDEFINED) {
		ddword_ptr = (DDWORD *) filename->data;
#ifdef YTNEF_OLD_SWAPDDWORD
		ddword_val = SwapDDWord((BYTE *) ddword_ptr);
