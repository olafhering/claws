...
static void create_spamreport_prefs_page(PrefsPage *page, GtkWindow *window, gpointer data);
...
void spamreport_prefs_init(void)
{
	...
	for (i = 0; i < INTF_LAST; i++) {
		if (spamreport_prefs.pass[i] != NULL) {
			passwd_store_set(PWS_PLUGIN, "SpamReport", spam_interfaces[i].name, spamreport_prefs.pass[i], TRUE);
...
static void create_spamreport_prefs_page(PrefsPage *page, GtkWindow *window, gpointer data)
{
	...
	for (i = 0; i < INTF_LAST; i++) {
		...
		gtk_entry_set_text(GTK_ENTRY(prefs_page->user_entry[i]), spamreport_prefs.user[i] ? spamreport_prefs.user[i] : "");
		...
		g_free(pass);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(prefs_page->enabled_chkbtn[i]), spamreport_prefs.enabled[i]);
		...
		gtk_table_attach(GTK_TABLE(table), prefs_page->enabled_chkbtn[i], 0, 2, 0, 1, GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
		...
		gtk_table_attach(GTK_TABLE(table), tmp, 0, 1, 1, 2, 0, 0, 0, 0);
		gtk_table_attach(GTK_TABLE(table), prefs_page->user_entry[i], 1, 2, 1, 2, GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
		...
		gtk_table_attach(GTK_TABLE(table), tmp, 0, 1, 2, 3, 0, 0, 0, 0);
		gtk_table_attach(GTK_TABLE(table), prefs_page->pass_entry[i], 1, 2, 2, 3, GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
...
static void save_spamreport_prefs(PrefsPage *page)
{
	...
	for (i = 0; i < INTF_LAST; i++) {
		...
		spamreport_prefs.enabled[i] = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->enabled_chkbtn[i]));
		spamreport_prefs.user[i] = gtk_editable_get_chars(GTK_EDITABLE(prefs_page->user_entry[i]), 0, -1);
