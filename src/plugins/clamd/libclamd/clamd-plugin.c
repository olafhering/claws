...
void clamd_create_config_automatic(const gchar *path)
{
	...
	if (config && config->ConfigType == AUTOMATIC && config->automatic.folder && strcmp(config->automatic.folder, path) == 0) {
...
void clamd_create_config_manual(const gchar *host, int port)
{
	...
	if (config && config->ConfigType == MANUAL && config->manual.host && config->manual.port == port && strcmp(config->manual.host, host) == 0) {
...
static int create_socket()
{
	...
	case UNIX_SOCKET:
		...
		if (strlen(Socket->socket.path) > UNIX_PATH_MAX) {
			g_error("socket path longer than %d-char: %s", UNIX_PATH_MAX, Socket->socket.path);
			new_sock = -2;
		} else {
			memcpy(addr_u.sun_path, Socket->socket.path, strlen(Socket->socket.path));
...
static Clamd_Stat clamd_stream_scan(int sock, const gchar *path, gchar **res, ssize_t size)
{
	int fd;
	ssize_t count;
	gchar buf[BUFSIZ];
