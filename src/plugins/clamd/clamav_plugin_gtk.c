...
static void foldersel_cb(GtkWidget *widget, gpointer data)
{
	...
	item = foldersel_folder_sel(NULL, FOLDER_SEL_MOVE, NULL, FALSE, _("Select folder to store infected messages in"));
...
static void clamd_folder_cb(GtkWidget *widget, gpointer data)
{
	...
	dialog = gtk_file_chooser_dialog_new("Select file with clamd configuration [clamd.conf]", NULL, GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_APPLY, GTK_RESPONSE_APPLY, NULL);
		...
		if (state) {
			...
			if (clamd_find_socket()) {
				...
				if (c->ConfigType == AUTOMATIC) {
					gtk_editable_insert_text(GTK_EDITABLE(page->config_folder), c->automatic.folder, strlen(c->automatic.folder), &newpos);
					clamav_save_config();
				}
			}
...
/*	SET_TOGGLE_SENSITIVITY (enable_arc, hbox1);*/
	...
	gtk_widget_set_tooltip_text(max_size, _("Message attachments larger than this will not be scanned"));
	...
	gtk_widget_set_tooltip_text(recv_infected, _("Save mail that contains viruses"));
	...
	gtk_widget_set_tooltip_text(save_folder, _("Folder for storing infected mail. Leave empty to use the default trash folder"));
	...
	gtk_widget_set_tooltip_text(save_folder_select, _("Click this button to select a folder for storing infected mail"));
	...
	gtk_widget_set_tooltip_text(setting_type, _("Should configuration be done automatic or manual"));
	...
	gtk_widget_set_tooltip_text(config_folder, _("Full path to clamd.conf. If this field is not empty then the plugin has been able to locate the file automatically"));
	...
	gtk_widget_set_tooltip_text(config_folder_select, _("Click this button to select full path to clamd.conf"));
	...
	gtk_widget_set_tooltip_text(permission_select, _("Click this button to check and adjust folder permissions"));
	...
	gtk_widget_set_tooltip_text(config_host, _("Hostname or IP for remote host running clamav daemon"));
	...
	gtk_widget_set_tooltip_text(config_port, _("Port number where clamav daemon is listening"));
...
/*	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(enable_arc), config->clamav_enable_arc);*/
	...
	g_signal_connect(G_OBJECT(config_folder_select), "clicked", G_CALLBACK(clamd_folder_cb), page);
	g_signal_connect(G_OBJECT(permission_select), "clicked", G_CALLBACK(folder_permission_cb), page);
	g_signal_connect(G_OBJECT(setting_type), "clicked", G_CALLBACK(setting_type_cb), page);
	...
	if (!config->clamd_config_type) {
		...
		if (clamd_find_socket()) {
			...
			if (c && c->ConfigType == AUTOMATIC) {
				...
				gtk_editable_insert_text(GTK_EDITABLE(config_folder), config->clamd_config_folder, strlen(config->clamd_config_folder), &newpos);
