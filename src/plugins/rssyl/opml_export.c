...
static void rssyl_opml_export_func(FolderItem *item, gpointer data)
{
	...
	err |= (fprintf(ctx->f, "%s<outline title=\"%s\" text=\"%s\" description=\"%s\" type=\"%s\" %s%s>\n", indent, tmpname, tmpoffn, tmpoffn, (isfolder ? "folder" : "rss"), (xmlurl ? xmlurl : ""), (haschildren ? "" : "/")) < 0);
	...
	if (err) {
		log_warning(LOG_PROTOCOL, _("RSSyl: Error while writing '%s' to feed export list.\n"), item->name);
...
void rssyl_opml_export(void)
{
	...
	opmlfile = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, RSSYL_DIR, G_DIR_SEPARATOR_S, RSSYL_OPML_FILE, NULL);

	if (g_file_test(opmlfile, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_REGULAR)) {
		if (g_remove(opmlfile) != 0) {
			log_warning(LOG_PROTOCOL, _("RSSyl: Couldn't delete old OPML file '%s': %s\n"), opmlfile, g_strerror(errno));
	...
	if ((f = claws_fopen(opmlfile, "w")) == NULL) {
		log_warning(LOG_PROTOCOL, _("RSSyl: Couldn't open file '%s' for feed list exporting: %s\n"), opmlfile, g_strerror(errno));
	...
	/* Write OPML header */
	err |= (fprintf(f, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" "<opml version=\"1.1\">\n" "\t<head>\n" "\t\t<title>RSSyl Feed List Export</title>\n" "\t\t<dateCreated>%s</dateCreated>\n" "\t</head>\n" "\t<body>\n", tmp) < 0);
	g_free(tmp);
	...
	folder_func_to_all_folders((FolderItemFunc) rssyl_opml_export_func, ctx);
