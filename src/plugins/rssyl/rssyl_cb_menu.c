...
void rssyl_new_feed_cb(GtkAction *action, gpointer data)
{
	...
	url = input_dialog(_("Subscribe feed"), _("Input the URL of the news feed you wish to subscribe:"), str ? str : "");
...
void rssyl_new_folder_cb(GtkAction *action, gpointer data)
{
	...
	if (p) {
		alertpanel_error(_("'%c' can't be used in folder name."), G_DIR_SEPARATOR);
		g_free(new_folder);
		return;
	}
...
void rssyl_remove_folder_cb(GtkAction *action, gpointer data)
{
	...
	message = g_strdup_printf(_("All folders and messages under '%s' will be permanently deleted. " "Recovery will not be possible.\n\n" "Do you really want to delete?"), name);
...
void rssyl_remove_mailbox_cb(GtkAction *action, gpointer data)
{
	...
	avalue = alertpanel_full(_("Remove feed tree"), message, GTK_STOCK_CANCEL, _("_Remove"), NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
	g_free(message);
	g_free(n);
...
void rssyl_import_feed_list_cb(GtkAction *action, gpointer data)
{
	...
	path = filesel_select_file_open_with_filter(_("Select an OPML file"), NULL, "*.opml");
