...
static void create_rssyl_prefs_page(PrefsPage *page, GtkWindow *window, gpointer data);
...
static gboolean rssyl_refresh_enabled_toggled_cb(GtkToggleButton *tb, gpointer data)
{
	gtk_widget_set_sensitive(GTK_WIDGET(data), gtk_toggle_button_get_active(tb));
	return FALSE;
}

/* Open a file select dialog and set file path to cookies entry */
static void rssyl_prefs_cookies_browse_cb(GtkWidget *widget, gpointer data)
{
	gchar *filename;
	gchar *utf8_filename;
...
static void create_rssyl_prefs_page(PrefsPage *page, GtkWindow *window, gpointer data)
{
	...
	refresh_enabled = gtk_check_button_new_with_label(_("Default refresh interval"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(refresh_enabled), rssyl_prefs.refresh_enabled);
	gtk_box_pack_start(GTK_BOX(refresh_hbox), refresh_enabled, FALSE, FALSE, 0);

	refresh_adj = gtk_adjustment_new(rssyl_prefs.refresh, 1, 100000, 1, 10, 0);
	...
	g_signal_connect(G_OBJECT(refresh_enabled), "toggled", G_CALLBACK(rssyl_refresh_enabled_toggled_cb), refresh);
	...
	refresh_on_startup = gtk_check_button_new_with_label(_("Refresh all feeds on application start"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(refresh_on_startup), rssyl_prefs.refresh_on_startup);
	gtk_box_pack_start(GTK_BOX(vbox1), refresh_on_startup, FALSE, FALSE, 0);

	vbox2 = gtk_vbox_new(FALSE, 6);

	/* Whether to verify SSL peer certificate */
	ssl_verify_peer = gtk_check_button_new_with_label(_("Verify TLS certificates validity for new feeds"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ssl_verify_peer), rssyl_prefs.ssl_verify_peer);
	...
	gtk_widget_set_tooltip_text(cookies_path, _("Path to Netscape-style cookies.txt file containing your cookies"));

	cookies_btn = gtkut_get_browse_file_btn(_("Bro_wse"));
	gtk_box_pack_start(GTK_BOX(cookies_hbox), cookies_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(cookies_btn), "clicked", G_CALLBACK(rssyl_prefs_cookies_browse_cb), cookies_path);
...
static void save_rssyl_prefs(PrefsPage *page)
{
	...
	rssyl_prefs.refresh_enabled = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->refresh_enabled));
	rssyl_prefs.refresh = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(prefs_page->refresh));
	rssyl_prefs.refresh_on_startup = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->refresh_on_startup));
	g_free(rssyl_prefs.cookies_path);
	rssyl_prefs.cookies_path = g_strdup(gtk_entry_get_text(GTK_ENTRY(prefs_page->cookies_path)));
	rssyl_prefs.ssl_verify_peer = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(prefs_page->ssl_verify_peer));
...
static void rssyl_start_default_refresh_timeouts_func(FolderItem *item, gpointer data)
{
...
static void rssyl_start_default_refresh_timeouts(void)
{
	...
	folder_func_to_all_folders((FolderItemFunc) rssyl_start_default_refresh_timeouts_func, GUINT_TO_POINTER(rsprefs->refresh));
}
