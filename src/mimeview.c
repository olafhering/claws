...
static gint mimeview_key_pressed(GtkWidget *widget, GdkEventKey *event, MimeView *mimeview);

static void mimeview_drag_data_get(GtkWidget *widget, GdkDragContext *drag_context, GtkSelectionData *selection_data, guint info, guint time, MimeView *mimeview);

static gboolean mimeview_scrolled(GtkWidget *widget, GdkEventScroll *event, MimeView *mimeview);
...
static void ctree_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation, MimeView *mimeview);
static gint mime_toggle_button_cb(GtkWidget *button, GdkEventButton *event, MimeView *mimeview);
static gboolean part_button_pressed(MimeView *mimeview, GdkEventButton *event, MimeInfo *partinfo);
static void icon_scroll_size_allocate_cb(GtkWidget *widget, GtkAllocation *layout_size, MimeView *mimeview);
...
static void mimeview_copy_cb(GtkAction *action, gpointer data)
{
	...
	if (mimeinfo->type == MIMETYPE_IMAGE) {
		...
		gtk_clipboard_set_text(gtk_clipboard_get(GDK_SELECTION_CLIPBOARD), data, mimeinfo->length);
		g_free(data);
	}
}
...
static gboolean mimeview_leave_notify(GtkWidget *widget, GdkEventCrossing *event, MimeView *mimeview)
{
	gdk_window_set_cursor(gtk_widget_get_window(widget), NULL);
	return FALSE;
}

static gboolean mimeview_enter_notify(GtkWidget *widget, GdkEventCrossing *event, MimeView *mimeview)
{
	gdk_window_set_cursor(gtk_widget_get_window(widget), hand_cursor);
	return FALSE;
}
...
MimeView *mimeview_create(MainWindow *mainwin)
{
	...
	model = gtk_tree_store_new(N_MIMEVIEW_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER);

	ctree = gtk_tree_view_new_with_model(GTK_TREE_MODEL(model));
	g_object_unref(model);
	gtk_tree_view_set_enable_search(GTK_TREE_VIEW(ctree), FALSE);
	gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(ctree), prefs_common.use_stripes_everywhere);
	gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(ctree), prefs_common.show_col_headers);

	renderer = gtk_cell_renderer_text_new();
	cols = gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(ctree), -1, titles[COL_MIMETYPE], renderer, "text", COL_MIMETYPE, NULL);
	column = gtk_tree_view_get_column(GTK_TREE_VIEW(ctree), cols - 1);

	renderer = gtk_cell_renderer_text_new();
	gtk_cell_renderer_set_alignment(renderer, 1, 0.5);
	cols = gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(ctree), -1, titles[COL_SIZE], renderer, "text", COL_SIZE, NULL);
	column = gtk_tree_view_get_column(GTK_TREE_VIEW(ctree), cols - 1);
	gtk_tree_view_column_set_alignment(GTK_TREE_VIEW_COLUMN(column), 1);

	renderer = gtk_cell_renderer_text_new();
	cols = gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(ctree), -1, titles[COL_NAME], renderer, "text", COL_NAME, NULL);
	column = gtk_tree_view_get_column(GTK_TREE_VIEW(ctree), cols - 1);
	...
	gtk_drag_source_set(ctree, GDK_BUTTON1_MASK | GDK_BUTTON3_MASK, mimeview_mime_types, 1, GDK_ACTION_COPY);

	g_signal_connect(G_OBJECT(selection), "changed", G_CALLBACK(mimeview_selected), mimeview);
	g_signal_connect(G_OBJECT(ctree), "button_release_event", G_CALLBACK(mimeview_button_pressed), mimeview);
	g_signal_connect(G_OBJECT(ctree), "key_press_event", G_CALLBACK(mimeview_key_pressed), mimeview);
	g_signal_connect(G_OBJECT(ctree), "drag_data_get", G_CALLBACK(mimeview_drag_data_get), mimeview);
	...
	g_signal_connect(G_OBJECT(icon_scroll), "scroll_event", G_CALLBACK(mimeview_scrolled), mimeview);

	mime_toggle = gtk_event_box_new();
	gtk_event_box_set_visible_window(GTK_EVENT_BOX(mime_toggle), FALSE);

	g_signal_connect(G_OBJECT(mime_toggle), "motion-notify-event", G_CALLBACK(mimeview_visi_notify), mimeview);
	g_signal_connect(G_OBJECT(mime_toggle), "leave-notify-event", G_CALLBACK(mimeview_leave_notify), mimeview);
	g_signal_connect(G_OBJECT(mime_toggle), "enter-notify-event", G_CALLBACK(mimeview_enter_notify), mimeview);
	...
	g_signal_connect(G_OBJECT(mime_toggle), "button_release_event", G_CALLBACK(mime_toggle_button_cb), mimeview);
	...
	g_signal_connect(G_OBJECT(icon_mainbox), "size_allocate", G_CALLBACK(icon_scroll_size_allocate_cb), mimeview);

	ctree_mainbox = gtk_hbox_new(FALSE, 0);
	gtk_box_pack_start(GTK_BOX(ctree_mainbox), scrolledwin, TRUE, TRUE, 0);
	g_signal_connect(G_OBJECT(ctree_mainbox), "size_allocate", G_CALLBACK(ctree_size_allocate_cb), mimeview);

	mimeview->ui_manager = gtk_ui_manager_new();
	mimeview->action_group = cm_menu_create_action_group_full(mimeview->ui_manager, "MimeView", mimeview_menu_actions, G_N_ELEMENTS(mimeview_menu_actions), (gpointer)mimeview);

	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/", "Menus", "Menus", GTK_UI_MANAGER_MENUBAR)
	    MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/", "MimeView", "MimeView", GTK_UI_MANAGER_MENU);
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "Open", "MimeView/Open", GTK_UI_MANAGER_MENUITEM);
#if (!defined G_OS_WIN32)
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "OpenWith", "MimeView/OpenWith", GTK_UI_MANAGER_MENUITEM);
#endif
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "Copy", "MimeView/Copy", GTK_UI_MANAGER_MENUITEM);
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "SendTo", "MimeView/SendTo", GTK_UI_MANAGER_MENUITEM);
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "DisplayAsText", "MimeView/DisplayAsText", GTK_UI_MANAGER_MENUITEM);
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "SaveAs", "MimeView/SaveAs", GTK_UI_MANAGER_MENUITEM);
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "SaveAll", "MimeView/SaveAll", GTK_UI_MANAGER_MENUITEM);
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "SaveAllAttachments", "MimeView/SaveAllAttachments", GTK_UI_MANAGER_MENUITEM);
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "NextPart", "MimeView/NextPart", GTK_UI_MANAGER_MENUITEM);
	MENUITEM_ADDUI_MANAGER(mimeview->ui_manager, "/Menus/MimeView/", "PrevPart", "MimeView/PrevPart", GTK_UI_MANAGER_MENUITEM);

	popupmenu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(mimeview->ui_manager, "/Menus/MimeView")));
...
void mimeview_init(MimeView *mimeview)
{
	...
	gtk_container_add(GTK_CONTAINER(mimeview->mime_notebook), GTK_WIDGET_PTR(mimeview->textview));
}
...
static MimeViewer *get_viewer_for_content_type(MimeView *mimeview, const gchar *content_type)
{
	...
	gtk_container_add(GTK_CONTAINER(mimeview->mime_notebook), GTK_WIDGET(viewer->get_widget(viewer)));
...
static void mimeview_change_view_type(MimeView *mimeview, MimeViewType type)
{
	...
	if (mainwindow_get_mainwindow())
		focused = gtkut_get_focused_child(GTK_CONTAINER(mainwindow_get_mainwindow()->window));

	if ((mimeview->type != MIMEVIEW_VIEWER) && (mimeview->type == type))
		return;

	switch (type) {
	case MIMEVIEW_TEXT:
		gtk_notebook_set_current_page(GTK_NOTEBOOK(mimeview->mime_notebook), gtk_notebook_page_num(GTK_NOTEBOOK(mimeview->mime_notebook), GTK_WIDGET_PTR(textview)));
		break;
	case MIMEVIEW_VIEWER:
		gtk_notebook_set_current_page(GTK_NOTEBOOK(mimeview->mime_notebook), gtk_notebook_page_num(GTK_NOTEBOOK(mimeview->mime_notebook), GTK_WIDGET(mimeview->mimeviewer->get_widget(mimeview->mimeviewer))));
		break;
	default:
		return;
	}
...
static void check_signature_async_cb(GObject *source_object, GAsyncResult *async_result, gpointer user_data)
{
...
static void check_signature_cb(GtkWidget *widget, gpointer user_data)
{
	...
	ret = privacy_mimeinfo_check_signature(mimeview->siginfo, mimeview->sig_check_cancellable, check_signature_async_cb, mimeview);
	if (ret == 0) {
...
static gboolean part_button_pressed(MimeView *mimeview, GdkEventButton *event, MimeInfo *partinfo)
{
	...
	if (event->button == 2 || (event->button == 1 && (event->time - lasttime) < double_click_time && lastinfo == partinfo)) {
		...
		if (partinfo && (partinfo->type == MIMETYPE_MESSAGE || partinfo->type == MIMETYPE_IMAGE || partinfo->type == MIMETYPE_TEXT))
...
#ifndef G_OS_WIN32
		if (partinfo && partinfo->type == MIMETYPE_APPLICATION && !g_ascii_strcasecmp(partinfo->subtype, "octet-stream"))
		...
		if (mainwin)
			main_window_set_menu_sensitive(mainwin);
		g_object_set_data(G_OBJECT(mimeview->popupmenu), "pop_partinfo", partinfo);

		gtk_menu_popup(GTK_MENU(mimeview->popupmenu), NULL, NULL, NULL, NULL, event->button, event->time);
		return TRUE;
	}
...
void mimeview_select_next_part(MimeView *mimeview)
{
 ...
 skip:
	...
	if (has_next) {
		partinfo = mimeview_get_node_part(mimeview, path);
		if (partinfo->type == MIMETYPE_MULTIPART || (!prefs_common.show_inline_attachments && partinfo->id))
			goto skip;
		selection = gtk_tree_view_get_selection(ctree);
		gtk_tree_selection_select_path(selection, path);
...
static gint mimeview_key_pressed(GtkWidget *widget, GdkEventKey *event, MimeView *mimeview)
{
	...
	case GDK_KEY_Home:
	case GDK_KEY_End:
		textview_scroll_max(mimeview->textview, (event->keyval == GDK_KEY_Home));
		return TRUE;
	case GDK_KEY_Page_Down:
	case GDK_KEY_space:
		if (mimeview_scroll_page(mimeview, (event->state & GDK_SHIFT_MASK) != 0))
			return TRUE;
	...
	case GDK_KEY_KP_Enter:
		mimeview_scroll_one_line(mimeview, (event->state & GDK_MOD1_MASK) != 0);
		return TRUE;
	case GDK_KEY_Up:
	case GDK_KEY_Down:
	...
	if (mimeview->messageview->new_window)
		return FALSE;
...
static void mimeview_drag_data_get(GtkWidget *widget, GdkDragContext *drag_context, GtkSelectionData *selection_data, guint info, guint time, MimeView *mimeview)
{
	...
	} else if (partinfo->type == MIMETYPE_MESSAGE && !g_ascii_strcasecmp(partinfo->subtype, "rfc822")) {
		gchar *name = NULL;
	...
	if (!g_utf8_validate(filename, -1, NULL))
		tmp = conv_codeset_strdup(filename, conv_get_locale_charset_str(), CS_UTF_8);
	...
	g_free(filename);
	filename = g_strconcat(get_mime_tmp_dir(), G_DIR_SEPARATOR_S, tmp, NULL);
 ...
 check_new_file:
	if (is_file_exist(filename)) {
		...
		g_free(ext);
		g_free(filename);
		filename = g_strconcat(get_mime_tmp_dir(), G_DIR_SEPARATOR_S, new_name, NULL);
		g_free(new_name);
		goto check_new_file;
	}
	...
	if ((err = procmime_get_part(filename, partinfo)) < 0)
		alertpanel_error(_("Couldn't save the part of multipart message: %s"), g_strerror(-err));

	tmp = g_filename_to_uri(filename, NULL, NULL);
	uriname = g_strconcat(tmp, "\r\n", NULL);
	g_free(tmp);

	gtk_selection_data_set(selection_data, gtk_selection_data_get_target(selection_data), 8, (guchar *)uriname, strlen(uriname));
...
static gchar *mimeview_get_filename_for_part(MimeInfo *partinfo, const gchar *basedir, gint number)
{
	...
	fullname = g_strconcat(basedir, G_DIR_SEPARATOR_S, (g_path_is_absolute(filename))
...
static gint mimeview_write_part(const gchar *filename, MimeInfo *partinfo, gboolean handle_error)
{
	...
	if (is_file_exist(filename)) {
		...
		res = g_strdup_printf(_("Overwrite existing file '%s'?"), tmp);
		g_free(tmp);
		aval = alertpanel(_("Overwrite"), res, GTK_STOCK_NO, GTK_STOCK_YES, NULL, ALERTFOCUS_FIRST);
		g_free(res);
...
{
	gchar *message = g_strdup_printf(_("An error has occurred while saving message part %d. " "Do you want to cancel saving or ignore error and " "continue?"), n);
...
static void mimeview_save_all_info(gint errors, gint total)
{
	...
	if (!errors && prefs_common.show_save_all_success) {
		gchar *msg = g_strdup_printf(ngettext("%d file saved successfully.",
						      "%d files saved successfully.",
						      total),
					     total);
		aval = alertpanel_full(_("Notice"), msg, GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, TRUE, NULL, ALERT_NOTICE);
		g_free(msg);
		if (aval & G_ALERTDISABLE)
			prefs_common.show_save_all_success = FALSE;
	} else if (prefs_common.show_save_all_failure) {
		gchar *msg1 = g_strdup_printf(ngettext("%d file saved successfully",
						       "%d files saved successfully",
						       total - errors),
					      total - errors);
		gchar *msg2 = g_strdup_printf(ngettext("%s, %d file failed.",
						       "%s, %d files failed.",
						       errors),
					      msg1, errors);
		aval = alertpanel_full(_("Warning"), msg2, GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, TRUE, NULL, ALERT_WARNING);
		g_free(msg2);
		g_free(msg1);
		if (aval & G_ALERTDISABLE)
...
static void mimeview_save_all(MimeView *mimeview, gboolean attachments_only)
{
	...
	while (partinfo != NULL) {
		if ((!attachments_only && (partinfo->type != MIMETYPE_MESSAGE && partinfo->type != MIMETYPE_MULTIPART && (partinfo->disposition != DISPOSITIONTYPE_INLINE || get_real_part_name(partinfo) != NULL))) || (attachments_only && (partinfo->disposition == DISPOSITIONTYPE_ATTACHMENT && get_real_part_name(partinfo) != NULL))) {
			gchar *filename = mimeview_get_filename_for_part(partinfo, dirname, number++);
	...
	g_free(startdir);
	prefs_common.attach_save_dir = g_filename_to_utf8(dirname, -1, NULL, NULL, NULL);
	g_free(dirname);
...
static MimeInfo *mimeview_get_part_to_use(MimeView *mimeview)
{
	...
	if (mimeview->spec_part) {
		...
		partinfo = (MimeInfo *)g_object_get_data(G_OBJECT(mimeview->popupmenu), "pop_partinfo");
		g_object_set_data(G_OBJECT(mimeview->popupmenu), "pop_partinfo", NULL);
		if (!partinfo) {
			partinfo = mimeview_get_selected_part(mimeview);
		}
	}
...
void mimeview_save_as(MimeView *mimeview)
{
	...
	if (!g_utf8_validate(partname, -1, NULL)) {
		...
		if (!tmp) {
			tmp = conv_codeset_strdup(partname, conv_get_locale_charset_str(), CS_UTF_8);
		}
		if (tmp) {
			g_free(partname);
			partname = tmp;
	...
	if (prefs_common.attach_save_dir && *prefs_common.attach_save_dir)
		filepath = g_strconcat(prefs_common.attach_save_dir, G_DIR_SEPARATOR_S, partname, NULL);
	else
...
static void mimeview_open_part_with(MimeView *mimeview, MimeInfo *partinfo, gboolean automatic)
{
	...
	if (!prefs_common.mime_open_cmd_history)
		prefs_common.mime_open_cmd_history = add_history(NULL, prefs_common.mime_open_cmd);
	...
	if (!automatic) {
		...
		if (content_type != NULL)
			cmd = input_dialog_combo_remember(_("Open with"), _("Enter the command-line to open file:\n" "('%s' will be replaced with file name)"), mime_command ? mime_command : prefs_common.mime_open_cmd, prefs_common.mime_open_cmd_history, &remember);
		else
			cmd = input_dialog_combo(_("Open with"), _("Enter the command-line to open file:\n" "('%s' will be replaced with file name)"), mime_command ? mime_command : prefs_common.mime_open_cmd, prefs_common.mime_open_cmd_history);
		if (cmd && remember) {
	...
	if (cmd) {
		...
		prefs_common.mime_open_cmd_history = add_history(prefs_common.mime_open_cmd_history, cmd);
	}
...
static void mimeview_send_to(MimeView *mimeview, MimeInfo *partinfo)
{
	...
	if (!(err = procmime_get_part(filename, partinfo))) {
		...
		ainfo->content_type = procmime_get_content_type_str(partinfo->type, partinfo->subtype);
		ainfo->charset = g_strdup(procmime_mimeinfo_get_parameter(partinfo, "charset"));
...
static void mimeview_view_file(const gchar *filename, MimeInfo *partinfo, const gchar *cmd, MimeView *mimeview)
{
	...
	if (cmd == NULL)
		...
		if ((p = strchr(cmd, '%')) && *(p + 1) == 's' && !strchr(p + 2, '%')) {
	...
	if ((SHGetFileInfo((LPCWSTR) fn16, 0, &file_info, sizeof(SHFILEINFO), SHGFI_EXETYPE)) != 0) {
		AlertValue val = alertpanel_full(_("Execute untrusted binary?"),
						 _("This attachment is an executable file. Executing " "untrusted binaries is dangerous and could compromise " "your computer.\n\n" "Do you want to run this file?"), GTK_STOCK_CANCEL,
						 _("Run binary"),
...
static void icon_selected(MimeView *mimeview, gint num, MimeInfo *partinfo)
{
	...
	do {
		...
		if (curr == partinfo) {
			GtkTreeSelection *sel = gtk_tree_view_get_selection(GTK_TREE_VIEW(mimeview->ctree));
...
static gboolean icon_popup_menu(GtkWidget *widget, gpointer data)
{
	...
	g_object_set_data(G_OBJECT(mimeview->popupmenu), "pop_partinfo", partinfo);
	gtk_menu_popup(GTK_MENU(mimeview->popupmenu), NULL, NULL, NULL, NULL, 0, gtk_get_current_event_time());
	return TRUE;
}
...
static void icon_list_append_icon(MimeView *mimeview, MimeInfo *mimeinfo)
{
	...
	g_signal_connect(G_OBJECT(button), "motion-notify-event", G_CALLBACK(mimeview_visi_notify), mimeview);
	g_signal_connect(G_OBJECT(button), "leave-notify-event", G_CALLBACK(mimeview_leave_notify), mimeview);
	g_signal_connect(G_OBJECT(button), "enter-notify-event", G_CALLBACK(mimeview_enter_notify), mimeview);

	gtk_container_set_border_width(GTK_CONTAINER(button), 2);
	g_object_set_data(G_OBJECT(button), "icon_number", GINT_TO_POINTER(mimeview->icon_count));
	g_object_set_data(G_OBJECT(button), "partinfo", mimeinfo);
	...
	if (siginfo != NULL) {
		...
		case SIGNATURE_CHECK_FAILED:
		case SIGNATURE_CHECK_TIMEOUT:
			pixmap = stock_pixmap_widget_with_overlay(stockp, STOCK_PIXMAP_PRIVACY_EMBLEM_SIGNED, OVERLAY_BOTTOM_RIGHT, 6, 3);
			break;
		case SIGNATURE_OK:
			pixmap = stock_pixmap_widget_with_overlay(stockp, STOCK_PIXMAP_PRIVACY_EMBLEM_PASSED, OVERLAY_BOTTOM_RIGHT, 6, 3);
			break;
		case SIGNATURE_WARN:
		case SIGNATURE_KEY_EXPIRED:
			pixmap = stock_pixmap_widget_with_overlay(stockp, STOCK_PIXMAP_PRIVACY_EMBLEM_WARN, OVERLAY_BOTTOM_RIGHT, 6, 3);
			break;
		case SIGNATURE_INVALID:
			pixmap = stock_pixmap_widget_with_overlay(stockp, STOCK_PIXMAP_PRIVACY_EMBLEM_FAILED, OVERLAY_BOTTOM_RIGHT, 6, 3);
			break;
		}
		sigshort = privacy_mimeinfo_get_sig_info(siginfo, FALSE);
	} else if (encrypted != NULL) {
		pixmap = stock_pixmap_widget_with_overlay(stockp, STOCK_PIXMAP_PRIVACY_EMBLEM_ENCRYPTED, OVERLAY_BOTTOM_RIGHT, 6, 3);
	} else {
	...
	tip = g_strconcat("<b>", _("Type:"), "  </b>", content_type, "\n<b>", _("Size:"), " </b>", to_human_readable((goffset) mimeinfo->length), NULL);
	g_free(content_type);
	if (desc && *desc) {
		...
		tiptmp = g_strconcat(tip, "\n<b>", prefs_common.attach_desc && mimeinfo->description ? _("Description:") : _("Filename:"), " </b>", escaped, NULL);
		g_free(tip);
		tip = tiptmp;
		g_free(tmp);
		g_free(escaped);
	...
	if (sigshort && *sigshort) {
		gchar *sigshort_escaped = g_markup_escape_text(sigshort, -1);
	...
	gtk_drag_source_set(button, GDK_BUTTON1_MASK | GDK_BUTTON3_MASK, mimeview_mime_types, 1, GDK_ACTION_COPY);

	g_signal_connect(G_OBJECT(button), "popup-menu", G_CALLBACK(icon_popup_menu), mimeview);
	g_signal_connect(G_OBJECT(button), "button_release_event", G_CALLBACK(icon_clicked_cb), mimeview);
	g_signal_connect(G_OBJECT(button), "key_press_event", G_CALLBACK(icon_key_pressed), mimeview);
	g_signal_connect(G_OBJECT(button), "drag_data_get", G_CALLBACK(mimeview_drag_data_get), mimeview);
...
static void icon_list_clear(MimeView *mimeview)
{
	...
	for (child = orig; child != NULL; child = g_list_next(child)) {
		gtkut_container_remove(GTK_CONTAINER(mimeview->icon_vbox), GTK_WIDGET(child->data));
	}
	g_list_free(orig);
	mimeview->icon_count = 0;
 ...
 */
static void icon_scroll_size_allocate_cb(GtkWidget *widget, GtkAllocation *size, MimeView *mimeview)
{
	...
	gtk_layout_set_size(GTK_LAYOUT(mimeview->icon_scroll), width, MAX(vbox_size.height, layout_size.height));
...
static void icon_list_create(MimeView *mimeview, MimeInfo *mimeinfo)
{
	...
	if (size.width > requisition.width) {
		gtk_widget_set_size_request(mimeview->icon_mainbox, size.width, -1);
	}
	if (mimeview->opened)
		icon_list_toggle_by_mime_info(mimeview, mimeview_get_node_part(mimeview, mimeview->opened));
}
...
static void icon_list_toggle_by_mime_info(MimeView *mimeview, MimeInfo *mimeinfo)
{
	...
	for (child = children; child != NULL; child = g_list_next(child)) {
		...
		*highlight = (g_object_get_data(G_OBJECT(child->data), "partinfo") == (gpointer)mimeinfo);
...
static gint mime_toggle_button_cb(GtkWidget *button, GdkEventButton *event, MimeView *mimeview)
{
	...
	if (mimeview->ctree_mode) {
		gtk_arrow_set(GTK_ARROW(gtk_bin_get_child(GTK_BIN((button)))), GTK_ARROW_RIGHT, GTK_SHADOW_NONE);
		gtk_widget_hide(mimeview->icon_mainbox);
		gtk_widget_show(mimeview->ctree_mainbox);
		gtk_paned_set_position(GTK_PANED(mimeview->paned), prefs_common.mimeview_tree_height);

		gtkut_container_remove(GTK_CONTAINER(mimeview->icon_mainbox), button);
		gtk_box_pack_end(GTK_BOX(mimeview->ctree_mainbox), button, FALSE, FALSE, 0);
	} else {
		gtk_arrow_set(GTK_ARROW(gtk_bin_get_child(GTK_BIN((button)))), GTK_ARROW_LEFT, GTK_SHADOW_NONE);
		...
		gtkut_container_remove(GTK_CONTAINER(mimeview->ctree_mainbox), button);
		gtk_box_pack_start(GTK_BOX(mimeview->icon_mainbox), button, FALSE, FALSE, 0);
...
void mimeview_handle_cmd(MimeView *mimeview, const gchar *cmd, GdkEventButton *event, gpointer data)
{
	...
	if (!strcmp(cmd, "sc://view_log"))
		...
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(mimeview->messageview->mainwin->summaryview->toggle_search), TRUE);
		quicksearch_set(mimeview->messageview->mainwin->summaryview->quicksearch, ADVANCED_SEARCH_EXTENDED, buf);
		g_free(buf);
	}
}
