...
static gboolean mimeview_write_part(const gchar *filename, MimeInfo *partinfo, gboolean handle_error);
...
static gboolean mimeview_write_part(const gchar *filename, MimeInfo *partinfo, gboolean handle_error)
{
	...
	if (is_file_exist(filename)) {
		...
		g_free(tmp);
		aval = alertpanel(_("Overwrite"), res, GTK_STOCK_CANCEL, GTK_STOCK_OK, NULL, ALERTFOCUS_FIRST);
		g_free(res);
...
{
	gchar *message = g_strdup_printf(_("An error has occurred while saving message part #%d. " "Do you want to cancel operation or skip error and " "continue?"), n);
...
static void mimeview_save_all(MimeView *mimeview)
{
	...
	while (partinfo != NULL) {
		if (partinfo->type != MIMETYPE_MESSAGE && partinfo->type != MIMETYPE_MULTIPART && (partinfo->disposition != DISPOSITIONTYPE_INLINE || get_real_part_name(partinfo) != NULL)) {
