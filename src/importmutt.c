...
static void imp_mutt_create(gboolean *cancelled)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(imp_mutt_delete_event), cancelled);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(imp_mutt_key_pressed), cancelled);
	...
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(imp_mutt_ok), cancelled);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(imp_mutt_cancel), cancelled);
	g_signal_connect(G_OBJECT(file_btn), "clicked", G_CALLBACK(imp_mutt_file_select), NULL);
