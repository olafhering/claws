...
static gboolean prefs_template_search_func_cb(GtkTreeModel *model, gint column, const gchar *key, GtkTreeIter *iter, gpointer search_data);
...
static void prefs_template_list_view_insert_template(GtkWidget *list_view, gint row, const gchar *template, Template *data);
 ...
 */
static void prefs_template_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation)
{
...
static void prefs_template_window_create(void)
{
	...
	gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(scrolled_window), vpaned);
	...
	for (i = 0; widgets_table[i].label; i++) {
		...
		label = gtk_label_new((i != 0) ? prefs_common_translated_header_name(widgets_table[i].label) : _(widgets_table[i].label));
		gtk_widget_show(label);
		gtk_table_attach(GTK_TABLE(table), label, 0, 1, i, (i + 1), (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) 0, 0, 0);
		gtk_misc_set_alignment(GTK_MISC(label), 1, 0.5);

		*(widgets_table[i].entry) = gtk_entry_new();
		gtk_widget_show(*(widgets_table[i].entry));
		gtk_table_attach(GTK_TABLE(table), *(widgets_table[i].entry), 1, 2, i, (i + 1), (GtkAttachOptions) (GTK_EXPAND | GTK_SHRINK | GTK_FILL), (GtkAttachOptions) 0, 0, 0);
		CLAWS_SET_TIP(*(widgets_table[i].entry), _(widgets_table[i].tooltips));
	}
	...
	CLAWS_SET_TIP(reg_btn, _("Append the new template above to the list"));
	...
	CLAWS_SET_TIP(subst_btn, _("Replace the selected template in list with the template above"));
	...
	CLAWS_SET_TIP(del_btn, _("Delete the selected template from the list"));
	...
	g_signal_connect(G_OBJECT(clear_btn), "clicked", G_CALLBACK(prefs_template_clear_cb), NULL);
	...
	g_signal_connect(G_OBJECT(desc_btn), "clicked", G_CALLBACK(quote_fmt_quote_description), window);
	CLAWS_SET_TIP(desc_btn, _("Show information on configuring templates"));
	...
	g_signal_connect(G_OBJECT(top_btn), "clicked", G_CALLBACK(prefs_template_top_cb), NULL);
	CLAWS_SET_TIP(top_btn, _("Move the selected template to the top"));
	...
	CLAWS_SET_TIP(up_btn, _("Move the selected template up"));
	...
	CLAWS_SET_TIP(down_btn, _("Move the selected template down"));
	...
	g_signal_connect(G_OBJECT(bottom_btn), "clicked", G_CALLBACK(prefs_template_bottom_cb), NULL);
	CLAWS_SET_TIP(bottom_btn, _("Move the selected template to the bottom"));
	...
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(prefs_template_size_allocate_cb), NULL);
	...
	g_signal_connect(G_OBJECT(help_btn), "clicked", G_CALLBACK(manual_open_with_anchor_cb), MANUAL_ANCHOR_TEMPLATES);
	...
	gtk_widget_set_size_request(window, prefs_common.templateswin_width, prefs_common.templateswin_height);
...
static void prefs_template_clear_list(void)
{
	...
	store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(templates.list_view)));
	gtk_list_store_clear(store);

	prefs_template_list_view_insert_template(templates.list_view, -1, _("(New)"), NULL);
}
...
static void prefs_template_window_setup(void)
{
	...
	for (i = 0; widgets_table[i].label; i++) {
		if (widgets_table[i].compl)
			address_completion_register_entry(GTK_ENTRY(*(widgets_table[i].entry)), TRUE);
	}

	for (cur = tmpl_list; cur != NULL; cur = cur->next) {
		tmpl = (Template *)cur->data;
		prefs_template_list_view_insert_template(templates.list_view, -1, tmpl->name, tmpl);
	}
...
static void prefs_template_address_completion_end(void)
{
	...
	for (i = 0; widgets_table[i].label; i++) {
		if (widgets_table[i].compl)
			address_completion_unregister_entry(GTK_ENTRY(*(widgets_table[i].entry)));
	}
...
static void prefs_template_cancel_cb(gpointer action, gpointer data)
{
	...
	if (modified && alertpanel(_("Entry not saved"), _("The entry was not saved. Close anyway?"), GTK_STOCK_CLOSE, _("_Continue editing"), NULL, ALERTFOCUS_SECOND) != G_ALERTDEFAULT) {
		return;
	} else if (modified_list && alertpanel(_("Templates list not saved"), _("The templates list has been modified. Close anyway?"), GTK_STOCK_CLOSE, _("_Continue editing"), NULL, ALERTFOCUS_SECOND) != G_ALERTDEFAULT) {
		return;
	}
...
/* return TRUE if the row could be modified */
{
	...
	from = gtk_editable_get_chars(GTK_EDITABLE(templates.entry_from), 0, -1);
	...
	prefs_template_list_view_insert_template(templates.list_view, row, tmpl->name, tmpl);
...
static void prefs_template_delete_cb(gpointer action, gpointer data)
{
	...
	if (alertpanel(_("Delete template"), _("Do you really want to delete this template?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_FIRST) != G_ALERTALTERNATE)
		return;
...
static void prefs_template_delete_all_cb(gpointer action, gpointer data)
{
	if (alertpanel(_("Delete all templates"), _("Do you really want to delete all the templates?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_SECOND) == G_ALERTDEFAULT)
		return;
...
static GtkListStore *prefs_template_create_data_store(void)
{
	return gtk_list_store_new(N_TEMPL_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER, G_TYPE_AUTO_POINTER, -1);
}
...
*/
	...
	if (row == -1) {
		...
		gtk_list_store_set(list_store, &iter, TEMPL_TEXT, template, TEMPL_DATA, data, -1);
	} else if (row < -1) {
		/* duplicate */
		...
		/* change existing */
		GAuto *auto_data = g_auto_pointer_new_with_free(data, (GFreeFunc) template_free);

		/* if replacing data in an existing row, the auto pointer takes care
		 * of destroying the Template data */
		gtk_list_store_set(list_store, &iter, TEMPL_TEXT, template, TEMPL_DATA, data, TEMPL_AUTO_DATA, auto_data, -1);
...
static void prefs_template_row_selected(GtkTreeSelection *selection, GtkTreeView *list_view)
{
	GtkTreePath *path;
	GtkTreeIter iter;
	GtkTreeModel *model;
...
static gint prefs_template_list_btn_pressed(GtkWidget *widget, GdkEventButton *event, GtkTreeView *list_view)
{
	if (event) {
		...
		if (event->button == 3) {
			...
			if (!prefs_template_popup_menu) {
				prefs_template_popup_action = cm_menu_create_action_group("PrefsTemplatePopup", prefs_template_popup_entries, G_N_ELEMENTS(prefs_template_popup_entries), (gpointer)list_view);
				MENUITEM_ADDUI("/Menus", "PrefsTemplatePopup", "PrefsTemplatePopup", GTK_UI_MANAGER_MENU)
				    ...
				    prefs_template_popup_menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(gtkut_ui_manager(), "/Menus/PrefsTemplatePopup")));
			}
			...
			gtk_menu_popup(GTK_MENU(prefs_template_popup_menu), NULL, NULL, NULL, NULL, event->button, event->time);
		}
	}
	return FALSE;
}
...
static GtkWidget *prefs_template_list_view_create(void)
{
	...
	g_signal_connect(G_OBJECT(list_view), "popup-menu", G_CALLBACK(prefs_template_list_popup_menu), list_view);
	g_signal_connect(G_OBJECT(list_view), "button-press-event", G_CALLBACK(prefs_template_list_btn_pressed), list_view);
	...
	g_signal_connect(G_OBJECT(selector), "changed", G_CALLBACK(prefs_template_row_selected), list_view);
...
static void prefs_template_create_list_view_columns(GtkWidget *list_view)
{
	...
	column = gtk_tree_view_column_new_with_attributes(_("Current templates"), renderer, "text", TEMPL_TEXT, NULL);
...
static void prefs_template_select_row(GtkTreeView *list_view, GtkTreePath *path)
{
	...
	gtk_entry_set_text(GTK_ENTRY(templates.entry_from), tmpl->from ? tmpl->from : "");
