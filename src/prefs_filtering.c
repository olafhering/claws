...
static gint prefs_filtering_deleted(GtkWidget *widget, GdkEventAny *event, gpointer data);
static void prefs_filtering_row_selected(GtkTreeSelection *selection, GtkTreeView *list_view);
static gboolean prefs_filtering_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data);
...
static gint prefs_filtering_list_view_insert_rule(GtkListStore *list_store, gint row, gboolean enabled, const gchar *name, gint account_id, const gchar *account_name, const gchar *rule, gboolean prop);
static gchar *prefs_filtering_list_view_get_rule(GtkWidget *list, gint row);
static void prefs_filtering_list_view_get_rule_info(GtkWidget *list, gint row, gboolean *enabled, gchar **name, gint *account_id);
...
void prefs_filtering_open(GSList **p_processing, const gchar *title, const gchar *help_url_anchor, const gchar *header, const gchar *key, gboolean per_account_filtering)
{
	...
	gtk_tree_view_column_set_visible(filtering.account_name_column, per_account_filtering);
	...
	if (title != NULL)
		...
		gtk_window_set_title(GTK_WINDOW(filtering.window), _("Filtering/Processing configuration"));

	if (help_url_anchor != NULL) {
		if (signal_id != 0) {
			g_signal_handler_disconnect(G_OBJECT(filtering.help_btn), signal_id);
		}

		signal_id = g_signal_connect(G_OBJECT(filtering.help_btn), "clicked", G_CALLBACK(manual_open_with_anchor_cb), (gchar *)help_url_anchor);
	} else {
...
static void prefs_filtering_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation)
{
...
static void prefs_filtering_close(void)
{
	...
	store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(filtering.cond_list_view)));
...
static void prefs_filtering_create(void)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(prefs_filtering_deleted), NULL);
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(prefs_filtering_size_allocate_cb), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(prefs_filtering_key_pressed), NULL);
	MANAGE_WINDOW_SIGNALS_CONNECT(window);
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(prefs_filtering_ok), NULL);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(prefs_filtering_cancel), NULL);
	...
	gtk_table_attach(GTK_TABLE(table), name_label, 0, 1, 0, 1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 0);

	name_entry = gtk_entry_new();
	gtk_widget_show(name_entry);
	gtk_table_attach(GTK_TABLE(table), name_entry, 1, 2, 0, 1, (GtkAttachOptions) (GTK_FILL | GTK_EXPAND), (GtkAttachOptions) (0), 0, 0);

	account_label = gtk_label_new(_("Account"));
	gtk_widget_show(account_label);
	gtk_misc_set_alignment(GTK_MISC(account_label), 1, 0.5);
	gtk_table_attach(GTK_TABLE(table), account_label, 0, 1, 1, 2, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 0);

	account_opt_menu = prefs_filtering_account_option_menu(&filtering);
	gtk_widget_show(account_opt_menu);
	gtk_table_attach(GTK_TABLE(table), account_opt_menu, 1, 2, 1, 2, (GtkAttachOptions) (GTK_FILL | GTK_EXPAND), (GtkAttachOptions) (0), 0, 0);
	combobox_select_by_data(GTK_COMBO_BOX(filtering.account_combobox), 0);

	cond_label = gtk_label_new(_("Condition"));
	gtk_widget_show(cond_label);
	gtk_misc_set_alignment(GTK_MISC(cond_label), 1, 0.5);
	gtk_table_attach(GTK_TABLE(table), cond_label, 0, 1, 2, 3, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 0);

	cond_entry = gtk_entry_new();
	gtk_widget_show(cond_entry);
	gtk_table_attach(GTK_TABLE(table), cond_entry, 1, 2, 2, 3, (GtkAttachOptions) (GTK_FILL | GTK_EXPAND), (GtkAttachOptions) (0), 0, 0);

	cond_btn = gtk_button_new_with_mnemonic(_(" Def_ine... "));
	gtk_widget_show(cond_btn);
	gtk_table_attach(GTK_TABLE(table), cond_btn, 2, 3, 2, 3, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 2, 2);
	g_signal_connect(G_OBJECT(cond_btn), "clicked", G_CALLBACK(prefs_filtering_condition_define), NULL);

	action_label = gtk_label_new(_("Action"));
	gtk_widget_show(action_label);
	gtk_misc_set_alignment(GTK_MISC(action_label), 1, 0.5);
	gtk_table_attach(GTK_TABLE(table), action_label, 0, 1, 3, 4, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 0);

	action_entry = gtk_entry_new();
	gtk_widget_show(action_entry);
	gtk_table_attach(GTK_TABLE(table), action_entry, 1, 2, 3, 4, (GtkAttachOptions) (GTK_FILL | GTK_EXPAND), (GtkAttachOptions) (0), 0, 0);

	action_btn = gtk_button_new_with_mnemonic(_(" De_fine... "));
	gtk_widget_show(action_btn);
	gtk_table_attach(GTK_TABLE(table), action_btn, 2, 3, 3, 4, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 2, 2);
	g_signal_connect(G_OBJECT(action_btn), "clicked", G_CALLBACK(prefs_filtering_action_define), NULL);
	...
	g_signal_connect(G_OBJECT(reg_btn), "clicked", G_CALLBACK(prefs_filtering_register_cb), NULL);
	CLAWS_SET_TIP(reg_btn, _("Append the new rule above to the list"));

	subst_btn = gtkut_get_replace_btn(_("_Replace"));
	gtk_widget_show(subst_btn);
	gtk_box_pack_start(GTK_BOX(btn_hbox), subst_btn, FALSE, TRUE, 0);
	g_signal_connect(G_OBJECT(subst_btn), "clicked", G_CALLBACK(prefs_filtering_substitute_cb), NULL);
	CLAWS_SET_TIP(subst_btn, _("Replace the selected rule in list with the rule above"));
	...
	g_signal_connect(G_OBJECT(del_btn), "clicked", G_CALLBACK(prefs_filtering_delete_cb), NULL);
	CLAWS_SET_TIP(del_btn, _("Delete the selected rule from the list"));

	clear_btn = gtk_button_new_with_mnemonic(_("C_lear"));
	gtk_button_set_image(GTK_BUTTON(clear_btn), gtk_image_new_from_stock(GTK_STOCK_CLEAR, GTK_ICON_SIZE_BUTTON));
	gtk_widget_show(clear_btn);
	gtk_box_pack_start(GTK_BOX(btn_hbox), clear_btn, FALSE, TRUE, 0);
	g_signal_connect(G_OBJECT(clear_btn), "clicked", G_CALLBACK(prefs_filtering_clear_cb), NULL);
	CLAWS_SET_TIP(clear_btn, _("Clear all the input fields in the dialog"));
	...
	g_signal_connect(G_OBJECT(top_btn), "clicked", G_CALLBACK(prefs_filtering_top), NULL);
	CLAWS_SET_TIP(top_btn, _("Move the selected rule to the top"));

#ifndef GENERIC_UMPC
	page_up_btn = gtk_button_new_with_mnemonic(_("Page u_p"));
	gtk_button_set_image(GTK_BUTTON(page_up_btn), gtk_image_new_from_stock(GTK_STOCK_GO_UP, GTK_ICON_SIZE_BUTTON));
	gtk_widget_show(page_up_btn);
	gtk_box_pack_start(GTK_BOX(btn_vbox), page_up_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(page_up_btn), "clicked", G_CALLBACK(prefs_filtering_page_up), NULL);
	CLAWS_SET_TIP(page_up_btn, _("Move the selected rule one page up"));
#endif

	up_btn = gtk_button_new_from_stock(GTK_STOCK_GO_UP);
	gtk_widget_show(up_btn);
	gtk_box_pack_start(GTK_BOX(btn_vbox), up_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(up_btn), "clicked", G_CALLBACK(prefs_filtering_up), NULL);
	CLAWS_SET_TIP(up_btn, _("Move the selected rule up"));

	down_btn = gtk_button_new_from_stock(GTK_STOCK_GO_DOWN);
	gtk_widget_show(down_btn);
	gtk_box_pack_start(GTK_BOX(btn_vbox), down_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(down_btn), "clicked", G_CALLBACK(prefs_filtering_down), NULL);
	CLAWS_SET_TIP(down_btn, _("Move the selected rule down"));

#ifndef GENERIC_UMPC
	page_down_btn = gtk_button_new_with_mnemonic(_("Page dow_n"));
	gtk_button_set_image(GTK_BUTTON(page_down_btn), gtk_image_new_from_stock(GTK_STOCK_GO_DOWN, GTK_ICON_SIZE_BUTTON));
	gtk_widget_show(page_down_btn);
	gtk_box_pack_start(GTK_BOX(btn_vbox), page_down_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(page_down_btn), "clicked", G_CALLBACK(prefs_filtering_page_down), NULL);
	CLAWS_SET_TIP(page_down_btn, _("Move the selected rule one page down"));
#endif
	...
	g_signal_connect(G_OBJECT(bottom_btn), "clicked", G_CALLBACK(prefs_filtering_bottom), NULL);
	CLAWS_SET_TIP(bottom_btn, _("Move the selected rule to the bottom"));
	...
	gtk_widget_set_size_request(window, prefs_common.filteringwin_width, prefs_common.filteringwin_height);
...
static void rename_tag(GSList *filters, const gchar *old_tag, const gchar *new_tag);
...
void prefs_filtering_rename_tag(const gchar *old_tag, const gchar *new_tag)
{
	...
	for (cur = folder_get_list(); cur != NULL; cur = g_list_next(cur)) {
		...
		g_node_traverse(folder->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, prefs_filtering_rename_tag_func, tags);
	}
...
static void rename_path(GSList *filters, const gchar *old_path, const gchar *new_path);
...
void prefs_filtering_rename_path(const gchar *old_path, const gchar *new_path)
{
	...
	for (cur = folder_get_list(); cur != NULL; cur = g_list_next(cur)) {
		Folder *folder;
		folder = (Folder *)cur->data;
		g_node_traverse(folder->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, prefs_filtering_rename_path_func, paths);
	}
...
static void rename_path(GSList *filters, const gchar *old_path, const gchar *new_path)
{
	...
	for (cur = filters; cur != NULL; cur = cur->next) {
		...
		filtering_action_list_rename_path(filtering->action_list, old_path, new_path);
	}
}
...
static void rename_tag(GSList *filters, const gchar *old_tag, const gchar *new_tag)
{
	...
	for (cur = filters; cur != NULL; cur = cur->next) {
		...
		for (action_cur = filtering->action_list; action_cur != NULL; action_cur = action_cur->next) {
...
void prefs_filtering_delete_path(const gchar *path)
{
	...
	for (cur = folder_get_list(); cur != NULL; cur = g_list_next(cur)) {
		Folder *folder;
		folder = (Folder *)cur->data;
		g_node_traverse(folder->node, G_PRE_ORDER, G_TRAVERSE_ALL, -1, prefs_filtering_delete_path_func, (gchar *)path);
	}
...
static void prefs_filtering_clear_list(GtkListStore *list_store)
{
	...
	prefs_filtering_list_view_insert_rule(list_store, -1, FALSE, _("(New)"), 0, _("(New)"), _("(New)"), FALSE);
}
...
static void prefs_filtering_set_dialog(const gchar *header, const gchar *key)
{
	...
	for (cur = prefs_filtering; cur != NULL; cur = g_slist_next(cur)) {
		...
		prefs_filtering_list_view_insert_rule(list_store, -1, prop->enabled, prop->name, prop->account_id, account_name, cond_str, TRUE);
	...
	if (header && key) {
		...
		match_str = g_strconcat(header, " ", get_matchparser_tab_str(MATCHTYPE_MATCHCASE), " \"", quoted_key, "\"", NULL);
		g_free(quoted_key);
...
static void prefs_filtering_set_list(void)
{
	...
	while (NULL != (filtering_str = prefs_filtering_list_view_get_rule(filtering.cond_list_view, row))) {
		...
		if (strcmp(filtering_str, _("(New)")) != 0) {
			...
			prefs_filtering_list_view_get_rule_info(filtering.cond_list_view, row, &enabled, &name, &account_id);
			...
			if (prop) {
				...
				prefs_filtering = g_slist_append(prefs_filtering, prop);
			}
		}
...
static gint prefs_filtering_list_view_set_row(gint row, FilteringProp *prop)
{
	...
	row = prefs_filtering_list_view_insert_rule(list_store, row, enabled, name, account_id, account_name, str, prop != NULL);
...
static void prefs_filtering_action_define(gpointer action, gpointer data)
{
	...
	prefs_filtering_action_open(action_list, prefs_filtering_action_define_done);
...
static void prefs_filtering_substitute_cb(gpointer action, gpointer data)
{
	gint selected_row = gtkut_list_view_get_selected_row(filtering.cond_list_view);
	FilteringProp *prop;
	gboolean enabled;
	gchar *name;
	gint account_id;
	...
	prefs_filtering_list_view_get_rule_info(filtering.cond_list_view, selected_row, &enabled, &name, &account_id);
	...
	prefs_filtering_row_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(filtering.cond_list_view)), GTK_TREE_VIEW(filtering.cond_list_view));
	modified = TRUE;
}
...
static void prefs_filtering_delete_cb(gpointer action, gpointer data)
{
	...
	if (alertpanel(_("Delete rule"), _("Do you really want to delete this rule?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_SECOND) == G_ALERTDEFAULT)
		return;
...
static void prefs_filtering_delete_all_cb(gpointer action, gpointer data)
{
	...
	if (alertpanel(_("Delete all rules"), _("Do you really want to delete all the rules?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_SECOND) == G_ALERTDEFAULT)
		return;
...
static gboolean prefs_filtering_check_mod(gboolean check_changed_list)
{
	...
	if (check_changed_list) {
		if (modified && alertpanel(_("Filtering rules not saved"), _("The list of filtering rules have been modified. Close anyway?"), GTK_STOCK_CLOSE, _("_Continue editing"), NULL, ALERTFOCUS_SECOND) != G_ALERTDEFAULT) {
			if (prop != NULL)
				filteringprop_free(prop);
			return TRUE;
		}
	...
	if (prop != NULL) {
		...
		if (strlen(name) || strlen(condition) || strlen(action)) {
			val = alertpanel(_("Entry not saved"), _("The entry was not saved. Close anyway?"), GTK_STOCK_CLOSE, _("_Continue editing"), NULL, ALERTFOCUS_SECOND);
			if (G_ALERTDEFAULT != val) {
				g_free(name);
				g_free(condition);
...
static GtkListStore *prefs_filtering_create_data_store(void)
{
	return gtk_list_store_new(N_PREFS_FILTERING_COLUMNS, G_TYPE_BOOLEAN, G_TYPE_STRING, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_BOOLEAN, -1);
}
...
static gint prefs_filtering_list_view_insert_rule(GtkListStore *list_store, gint row, gboolean enabled, const gchar *name, gint account_id, const gchar *account_name, const gchar *rule, gboolean prop)
{
	...
	if (row >= 0) {
		...
		if (!gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(list_store), &sibling, NULL, -row - 2))
			row = -1;
	}
	...
	if (row == -1) {
		...
		gtk_list_store_set(list_store, &iter, PREFS_FILTERING_ENABLED, enabled, PREFS_FILTERING_NAME, name, PREFS_FILTERING_ACCOUNT_ID, account_id, PREFS_FILTERING_ACCOUNT_NAME, account_name, PREFS_FILTERING_RULE, rule, PREFS_FILTERING_PROP, prop, -1);
		...
		/* change existing */
		gtk_list_store_set(list_store, &iter, PREFS_FILTERING_ENABLED, enabled, PREFS_FILTERING_NAME, name, PREFS_FILTERING_ACCOUNT_ID, account_id, PREFS_FILTERING_ACCOUNT_NAME, account_name, PREFS_FILTERING_RULE, rule, -1);
		return row;
	}
}
...
static gchar *prefs_filtering_list_view_get_rule(GtkWidget *list, gint row)
{
	...
	gtk_tree_model_get(model, &iter, PREFS_FILTERING_RULE, &result, -1);
...
static void prefs_filtering_list_view_get_rule_info(GtkWidget *list, gint row, gboolean *enabled, gchar **name, gint *account_id)
{
	...
	*name = NULL;
	...
	if (gtk_tree_model_iter_nth_child(model, &iter, NULL, row)) {
		gtk_tree_model_get(model, &iter, PREFS_FILTERING_ENABLED, enabled, PREFS_FILTERING_NAME, name, PREFS_FILTERING_ACCOUNT_ID, account_id, -1);
	}
}
...
static gint prefs_filtering_list_btn_pressed(GtkWidget *widget, GdkEventButton *event, GtkTreeView *list_view)
{
	if (event) {
		...
		if (event->button == 1 || event->button == 3) {
			...
			if (gtk_tree_view_get_path_at_pos(list_view, event->x, event->y, &path, NULL, NULL, NULL)) {
				prefs_filtering_select_row(list_view, path);
			}
			if (path)
				gtk_tree_path_free(path);
		...
		if (event->button == 3) {
			...
			if (!prefs_filtering_popup_menu) {
				prefs_filtering_popup_action = cm_menu_create_action_group("PrefsFilteringPopup", prefs_filtering_popup_entries, G_N_ELEMENTS(prefs_filtering_popup_entries), (gpointer)list_view);
			...
			gtk_menu_popup(GTK_MENU(prefs_filtering_popup_menu), NULL, NULL, NULL, NULL, event->button, event->time);
		}
	}
	return FALSE;
}
...
static GtkWidget *prefs_filtering_list_view_create(void)
{
	...
	g_signal_connect(G_OBJECT(list_view), "popup-menu", G_CALLBACK(prefs_filtering_list_popup_menu), list_view);
	g_signal_connect(G_OBJECT(list_view), "button-press-event", G_CALLBACK(prefs_filtering_list_btn_pressed), list_view);
	...
	g_signal_connect(G_OBJECT(selector), "changed", G_CALLBACK(prefs_filtering_row_selected), list_view);
...
static void prefs_filtering_enable_toggled(GtkCellRendererToggle *widget, gchar *path, GtkWidget *list_view)
{
	GtkTreeIter iter;
	...
	gtk_tree_model_get(model, &iter, PREFS_FILTERING_ENABLED, &enabled, -1);

	gtk_list_store_set(GTK_LIST_STORE(model), &iter, PREFS_FILTERING_ENABLED, !enabled, -1);
}
...
static void prefs_filtering_create_list_view_columns(GtkWidget *list_view)
{
	...
	g_object_set(renderer, "radio", FALSE, "activatable", TRUE, NULL);
	column = gtk_tree_view_column_new_with_attributes(_("Enable"), /* FIXME : Enable, Enabled, or 'E' ? */
							  renderer, "active", PREFS_FILTERING_ENABLED, NULL);
	gtk_tree_view_column_set_alignment(column, 0.5);
	gtk_tree_view_append_column(GTK_TREE_VIEW(list_view), column);
	g_signal_connect(G_OBJECT(renderer), "toggled", G_CALLBACK(prefs_filtering_enable_toggled), list_view);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(_("Name"), renderer, "text", PREFS_FILTERING_NAME, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(list_view), column);
	gtk_tree_view_column_set_resizable(column, TRUE);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(_("Account"), renderer, "text", PREFS_FILTERING_ACCOUNT_NAME, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(list_view), column);
	gtk_tree_view_column_set_resizable(column, TRUE);
	...
	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(_("Rule"), renderer, "text", PREFS_FILTERING_RULE, NULL);
...
static void prefs_filtering_select_row(GtkTreeView *list_view, GtkTreePath *path)
{
	...
	if (path && model) {
		...
		gtk_tree_model_get(model, &iter, PREFS_FILTERING_PROP, &has_prop, -1);
		...
		if (has_prop) {
			...
			gtk_tree_model_get(model, &iter, PREFS_FILTERING_RULE, &filtering_str, PREFS_FILTERING_NAME, &name, PREFS_FILTERING_ACCOUNT_ID, &account_id, -1);
