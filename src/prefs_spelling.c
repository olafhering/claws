...
static void prefs_spelling_colorsel(GtkWidget *widget, gpointer data)
{
	SpellingPage *spelling = (SpellingPage *) data;
	gint rgbcolor;

	rgbcolor = colorsel_select_color_rgb(_("Pick color for misspelled word"), spelling->misspell_col);
...
static void prefs_spelling_create_widget(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	enable_aspell_checkbtn = gtk_check_button_new_with_label(_("Enable spell checker"));
	gtk_widget_show(enable_aspell_checkbtn);
	gtk_box_pack_start(GTK_BOX(vbox2), enable_aspell_checkbtn, TRUE, TRUE, 0);

	use_alternate_checkbtn = gtk_check_button_new_with_label(_("Enable alternate dictionary"));
	gtk_widget_show(use_alternate_checkbtn);
	gtk_box_pack_start(GTK_BOX(vbox2), use_alternate_checkbtn, TRUE, TRUE, 0);

	CLAWS_SET_TIP(use_alternate_checkbtn, _("Faster switching with last used dictionary"));

	vbox2 = gtkut_get_options_frame(vbox1, &automatic_frame, _("Automatic spell checking"));

	check_while_typing_checkbtn = gtk_check_button_new_with_label(_("Check while typing"));
	gtk_widget_show(check_while_typing_checkbtn);
	gtk_box_pack_start(GTK_BOX(vbox2), check_while_typing_checkbtn, TRUE, TRUE, 0);

	recheck_when_changing_dict_checkbtn = gtk_check_button_new_with_label(_("Re-check message when changing dictionary"));
	...
	gtk_table_attach(GTK_TABLE(table), default_dict_label, 0, 1, 0, 1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 2);
	...
	gtk_table_attach(GTK_TABLE(table), default_dict_combo, 1, 2, 0, 1, GTK_EXPAND | GTK_FILL, 0, 0, 0);

	default_alt_dict_label = gtk_label_new(_("Default alternate dictionary"));
	gtk_widget_show(default_alt_dict_label);
	gtk_table_attach(GTK_TABLE(table), default_alt_dict_label, 0, 1, 1, 2, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (0), 0, 2);
	...
	gtk_table_attach(GTK_TABLE(table), default_alt_dict_combo, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL, 0, 0, 0);

	both_dict_check = gtk_check_button_new_with_label(_("Check with both dictionaries"));
	gtk_widget_show(both_dict_check);
	gtk_table_attach(GTK_TABLE(table), both_dict_check, 1, 2, 2, 3, GTK_EXPAND | GTK_FILL, 0, 0, 0);

#ifdef WIN32
	get_dictionaries_btn = gtkut_get_link_btn(GTK_WIDGET(window), DICTS_URI, _("Get more dictionaries..."));

	gtk_widget_show(get_dictionaries_btn);
	gtk_table_attach(GTK_TABLE(table), get_dictionaries_btn, 1, 2, 3, 4, GTK_SHRINK, 0, 0, 0);
#endif
	...
	gtk_box_pack_start(GTK_BOX(misspelled_hbox), misspelled_label, FALSE, FALSE, 0);
	...
	CLAWS_SET_TIP(misspelled_colorbtn, _("Pick color for misspelled word. " "Use black to underline"));
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(enable_aspell_checkbtn), prefs_common.enable_aspell);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(both_dict_check), prefs_common.use_both_dicts);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(check_while_typing_checkbtn), prefs_common.check_while_typing);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(recheck_when_changing_dict_checkbtn), prefs_common.recheck_when_changing_dict);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(use_alternate_checkbtn), prefs_common.use_alternate);
	if (prefs_common.dictionary && strrchr(prefs_common.dictionary, '/')) {
		gchar *tmp = g_strdup(strrchr(prefs_common.dictionary, '/') + 1);
		g_free(prefs_common.dictionary);
		prefs_common.dictionary = tmp;
	}
	if (prefs_common.alt_dictionary && strrchr(prefs_common.alt_dictionary, '/')) {
	...
	if (prefs_common.dictionary && strchr(prefs_common.dictionary, '-')) {
		*(strchr(prefs_common.dictionary, '-')) = '\0';
	}
	if (prefs_common.alt_dictionary && strchr(prefs_common.alt_dictionary, '-')) {
		*(strchr(prefs_common.alt_dictionary, '-')) = '\0';
	}
	gtkaspell_set_dictionary_menu_active_item(GTK_COMBO_BOX(default_dict_combo), prefs_common.dictionary);

	gtkaspell_set_dictionary_menu_active_item(GTK_COMBO_BOX(default_alt_dict_combo), prefs_common.alt_dictionary);

	g_signal_connect(G_OBJECT(misspelled_colorbtn), "clicked", G_CALLBACK(prefs_spelling_colorsel), prefs_spelling);
	...
	prefs_spelling->check_while_typing_checkbtn = check_while_typing_checkbtn;
	prefs_spelling->recheck_when_changing_dict_checkbtn = recheck_when_changing_dict_checkbtn;
...
static void prefs_spelling_save(PrefsPage *_page)
{
	...
	prefs_common.enable_aspell = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(spelling->enable_aspell_checkbtn));
	prefs_common.check_while_typing = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(spelling->check_while_typing_checkbtn));
	prefs_common.recheck_when_changing_dict = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(spelling->recheck_when_changing_dict_checkbtn));
	prefs_common.use_alternate = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(spelling->use_alternate_checkbtn));
	prefs_common.use_both_dicts = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(spelling->both_dict_check));

	g_free(prefs_common.dictionary);
	prefs_common.dictionary = gtkaspell_get_dictionary_menu_active_item(GTK_COMBO_BOX(spelling->default_dict_combo));

	g_free(prefs_common.alt_dictionary);
	prefs_common.alt_dictionary = gtkaspell_get_dictionary_menu_active_item(GTK_COMBO_BOX(spelling->default_alt_dict_combo));
