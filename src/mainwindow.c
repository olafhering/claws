...
static void toolbar_child_attached(GtkWidget *widget, GtkWidget *child, gpointer data);
static void toolbar_child_detached(GtkWidget *widget, GtkWidget *child, gpointer data);
#ifndef GENERIC_UMPC
static gboolean ac_label_button_pressed(GtkWidget *widget, GdkEventButton *event, gpointer data);
#endif
static gint main_window_close_cb(GtkWidget *widget, GdkEventAny *event, gpointer data);

static void main_window_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation, gpointer data);
static void folder_window_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation, gpointer data);
static void message_window_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation, gpointer data);
...
static void toggle_col_headers_cb(GtkAction *action, gpointer data);
#ifndef GENERIC_UMPC
static void toggle_statusbar_cb(GtkAction *action, gpointer data);
...
static void delete_duplicated_all_cb(GtkAction *action, gpointer data);
...
static void create_filter_cb(GtkAction *action, gpointer data);
static void create_processing_cb(GtkAction *action, gpointer data);
...
#endif
static gboolean mainwindow_focus_in_event(GtkWidget *widget, GdkEventFocus *focus, gpointer data);
static gboolean mainwindow_visibility_event_cb(GtkWidget *widget, GdkEventVisibility *state, gpointer data);
static gboolean mainwindow_state_event_cb(GtkWidget *widget, GdkEventWindowState *state, gpointer data);
static void main_window_reply_cb(GtkAction *action, gpointer data);
static gboolean mainwindow_progressindicator_hook(gpointer source, gpointer userdata);

static gint mailing_list_create_submenu(MainWindow *mainwindow, MsgInfo *msginfo);
...
static void mainwindow_colorlabel_menu_item_activate_item_cb(GtkMenuItem *menu_item, gpointer data)
{
	MainWindow *mainwin;
	GtkMenuShell *menu;
...
static void mainwindow_colorlabel_menu_item_activate_cb(GtkWidget *widget, gpointer data)
{
	...
	if (g_object_get_data(G_OBJECT(mainwin->colorlabel_menu), "dont_toggle"))
		return;

	summary_set_colorlabel(mainwin->summaryview, color, NULL);
}

static void mainwindow_tags_menu_item_activate_item_cb(GtkMenuItem *menu_item, gpointer data)
{
	MainWindow *mainwin;
	GtkMenuShell *menu;
	GList *children, *cur;
...
static void mainwindow_tags_menu_item_activate_cb(GtkWidget *widget, gpointer data)
{
	...
	/* "dont_toggle" state set? */
	if (g_object_get_data(G_OBJECT(mainwin->tags_menu), "dont_toggle"))
		return;
...
static void mainwindow_colorlabel_menu_create(MainWindow *mainwin, gboolean refresh)
{
	...
	g_signal_connect(G_OBJECT(label_menuitem), "activate", G_CALLBACK(mainwindow_colorlabel_menu_item_activate_item_cb), mainwin);
	gtk_widget_show(label_menuitem);

	menu = gtk_menu_new();
	gtk_menu_set_accel_group(GTK_MENU(menu), gtk_ui_manager_get_accel_group(mainwin->ui_manager));
	...
	g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(mainwindow_colorlabel_menu_item_activate_cb), GUINT_TO_POINTER(0));
	...
	for (i = 0; i < N_COLOR_LABELS; i++) {
		item = colorlabel_create_check_color_menu_item(i, refresh, MAINWIN_COLORMENU);
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
		g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(mainwindow_colorlabel_menu_item_activate_cb), GUINT_TO_POINTER(i + 1));
		g_object_set_data(G_OBJECT(item), "mainwin", mainwin);
		gtk_widget_show(item);
		...
		g_free(accel_path);
		g_signal_connect(gtk_ui_manager_get_accel_group(mainwin->ui_manager), "accel-changed", G_CALLBACK(mainwin_accel_changed_cb), item);
...
static void mainwindow_tags_menu_item_apply_tags_activate_cb(GtkWidget *widget, gpointer data)
{
	MainWindow *mainwin;
...
static void mainwindow_tags_menu_create(MainWindow *mainwin, gboolean refresh)
{
	...
	g_signal_connect(G_OBJECT(label_menuitem), "activate", G_CALLBACK(mainwindow_tags_menu_item_activate_item_cb), mainwin);
	...
	for (; cur; cur = cur->next) {
		...
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
		g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(mainwindow_tags_menu_item_activate_cb), GINT_TO_POINTER(id));
		g_object_set_data(G_OBJECT(item), "mainwin", mainwin);
	...
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
	g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(mainwindow_tags_menu_item_apply_tags_activate_cb), NULL);
	g_object_set_data(G_OBJECT(item), "mainwin", mainwin);
	gtk_widget_show(item);
...
#ifndef GENERIC_UMPC
static gboolean warning_icon_pressed(GtkWidget *widget, GdkEventButton *evt, MainWindow *mainwindow)
{
	if (evt && evt->button == 1) {
...
static gboolean warning_visi_notify(GtkWidget *widget, GdkEventVisibility *event, MainWindow *mainwindow)
{
	gdk_window_set_cursor(gtk_widget_get_window(mainwindow->warning_btn), hand_cursor);
	return FALSE;
}

static gboolean warning_leave_notify(GtkWidget *widget, GdkEventCrossing *event, MainWindow *mainwindow)
{
	gdk_window_set_cursor(gtk_widget_get_window(mainwindow->warning_btn), NULL);
	return FALSE;
}

static gboolean warning_enter_notify(GtkWidget *widget, GdkEventCrossing *event, MainWindow *mainwindow)
{
	gdk_window_set_cursor(gtk_widget_get_window(mainwindow->warning_btn), hand_cursor);
	return FALSE;
}
...
static gboolean mainwindow_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
	...
	if (quicksearch_has_focus(mainwin->summaryview->quicksearch)) {
		GtkWidget *entry = quicksearch_get_entry(mainwin->summaryview->quicksearch);
		gboolean handled;
	...
	case GDK_KEY_space:
		BREAK_ON_MODIFIER_KEY();
		if (gtk_window_is_active(GTK_WINDOW(mainwin->window))) {
			if (mainwin->folderview != NULL && mainwin->summaryview != NULL && ((!mainwin->summaryview->displayed && !mainwin->summaryview->selected)
											    || (mainwin->summaryview->folder_item && mainwin->summaryview->folder_item->total_msgs == 0))) {
...
MainWindow *main_window_create()
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(main_window_close_cb), mainwin);
	MANAGE_WINDOW_SIGNALS_CONNECT(window);
	g_signal_connect(G_OBJECT(window), "focus_in_event", G_CALLBACK(mainwindow_focus_in_event), mainwin);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(mainwindow_key_pressed), mainwin);
	...
	mainwin->action_group = cm_menu_create_action_group_full(mainwin->ui_manager, "Menu", mainwin_entries, G_N_ELEMENTS(mainwin_entries), (gpointer)mainwin);
	gtk_action_group_add_toggle_actions(mainwin->action_group, mainwin_toggle_entries, G_N_ELEMENTS(mainwin_toggle_entries), (gpointer)mainwin);
	gtk_action_group_add_radio_actions(mainwin->action_group, mainwin_showhide_radio_entries, G_N_ELEMENTS(mainwin_showhide_radio_entries), C_AUTO, G_CALLBACK(toggle_toolbar_cb), (gpointer)mainwin);
#ifndef GENERIC_UMPC
	gtk_action_group_add_radio_actions(mainwin->action_group, mainwin_layout_radio_entries, G_N_ELEMENTS(mainwin_layout_radio_entries), C_AUTO, G_CALLBACK(set_layout_cb), (gpointer)mainwin);
#endif
	gtk_action_group_add_radio_actions(mainwin->action_group, mainwin_sort_radio_entries, G_N_ELEMENTS(mainwin_sort_radio_entries), C_AUTO, G_CALLBACK(sort_summary_cb), (gpointer)mainwin);
	gtk_action_group_add_radio_actions(mainwin->action_group, mainwin_sorttype_radio_entries, G_N_ELEMENTS(mainwin_sorttype_radio_entries), C_AUTO, G_CALLBACK(sort_summary_type_cb), (gpointer)mainwin);
	gtk_action_group_add_radio_actions(mainwin->action_group, mainwin_radio_enc_entries, G_N_ELEMENTS(mainwin_radio_enc_entries), C_AUTO, G_CALLBACK(set_charset_cb), (gpointer)mainwin);
	gtk_action_group_add_radio_actions(mainwin->action_group, mainwin_radio_dec_entries, G_N_ELEMENTS(mainwin_radio_dec_entries), C_AUTO, G_CALLBACK(set_decode_cb), (gpointer)mainwin);
...
/* Help menu */
	...
	if (prefs_common.toolbar_detachable) {
		...
		g_signal_connect(G_OBJECT(handlebox), "child_attached", G_CALLBACK(toolbar_child_attached), mainwin);
		g_signal_connect(G_OBJECT(handlebox), "child_detached", G_CALLBACK(toolbar_child_detached), mainwin);
	} else {
	...
	mainwin->toolbar = toolbar_create(TOOLBAR_MAIN, handlebox, (gpointer)mainwin);
	toolbar_set_learn_button(mainwin->toolbar, LEARN_SPAM);
	...
	warning_icon = gtk_image_new_from_stock(GTK_STOCK_DIALOG_WARNING, GTK_ICON_SIZE_SMALL_TOOLBAR);
	...
	g_signal_connect(G_OBJECT(warning_btn), "button-press-event", G_CALLBACK(warning_icon_pressed), (gpointer)mainwin);
	g_signal_connect(G_OBJECT(warning_btn), "motion-notify-event", G_CALLBACK(warning_visi_notify), mainwin);
	g_signal_connect(G_OBJECT(warning_btn), "leave-notify-event", G_CALLBACK(warning_leave_notify), mainwin);
	g_signal_connect(G_OBJECT(warning_btn), "enter-notify-event", G_CALLBACK(warning_enter_notify), mainwin);

	gtk_container_add(GTK_CONTAINER(warning_btn), warning_icon);

	CLAWS_SET_TIP(warning_btn, _("Some error(s) happened. Click here to view log."));
	...
	CLAWS_SET_TIP(online_switch, _("You are online. Click the icon to go offline"));
	offline_switch = gtk_button_new();
	CLAWS_SET_TIP(offline_switch, _("You are offline. Click the icon to go online"));
	...
	g_signal_connect(G_OBJECT(ac_button), "button_press_event", G_CALLBACK(ac_label_button_pressed), mainwin);
	...
	set_log_prefs(LOG_PROTOCOL, &prefs_common.logwin_width, &prefs_common.logwin_height);
	set_log_title(LOG_DEBUG_FILTERING, _("Filtering/Processing debug log"));
	set_log_prefs(LOG_DEBUG_FILTERING, &prefs_common.filtering_debugwin_width, &prefs_common.filtering_debugwin_height);
	...
	log_window_set_clipping(mainwin->logwin, prefs_common.cliplog, prefs_common.loglength);

	log_window_init(mainwin->filtering_debugwin);
	log_window_set_clipping(mainwin->filtering_debugwin, prefs_common.filtering_debug_cliplog, prefs_common.filtering_debug_loglength);
	...
	mainwin->mainwin_cid = gtk_statusbar_get_context_id(GTK_STATUSBAR(statusbar), "Main Window");
	mainwin->folderview_cid = gtk_statusbar_get_context_id(GTK_STATUSBAR(statusbar), "Folder View");
	mainwin->summaryview_cid = gtk_statusbar_get_context_id(GTK_STATUSBAR(statusbar), "Summary View");
	mainwin->messageview_cid = gtk_statusbar_get_context_id(GTK_STATUSBAR(statusbar), "Message View");
	...
	summaryview->color_dim.red = summaryview->color_dim.green = summaryview->color_dim.blue = COLOR_DIM;

	gtkut_convert_int_to_gdk_color(prefs_common.color[COL_NEW], &folderview->color_new);

	gtkut_convert_int_to_gdk_color(prefs_common.color[COL_TGT_FOLDER], &folderview->color_op);
	...
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(main_window_size_allocate_cb), mainwin);
	...
	toolbar_set_style(mainwin->toolbar->toolbar, mainwin->handlebox, prefs_common.toolbar_style);
#ifndef GENERIC_UMPC
	gtk_widget_hide(mainwin->hbox_stat);
	menuitem = gtk_ui_manager_get_widget(mainwin->ui_manager, "/Menu/View/ShowHide/StatusBar");
	gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menuitem), prefs_common.show_statusbar);
#endif
	menuitem = gtk_ui_manager_get_widget(mainwin->ui_manager, "/Menu/View/ShowHide/ColumnHeaders");
	gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menuitem), prefs_common.show_col_headers);
...
#define	ADD_MENU_ACCEL_GROUP_TO_WINDOW(menu,win)			\
	...
	g_signal_connect(G_OBJECT(window), "window_state_event", G_CALLBACK(mainwindow_state_event_cb), mainwin);
	g_signal_connect(G_OBJECT(window), "visibility_notify_event", G_CALLBACK(mainwindow_visibility_event_cb), mainwin);
	gtk_widget_add_events(GTK_WIDGET(window), GDK_VISIBILITY_NOTIFY_MASK);

	if (prefs_common.layout_mode == VERTICAL_LAYOUT || prefs_common.layout_mode == SMALL_LAYOUT) {
	...
	mainwin->progressindicator_hook = hooks_register_hook(PROGRESSINDICATOR_HOOKLIST, mainwindow_progressindicator_hook, mainwin);
	...
	if (prefs_common.mainwin_fullscreen) {
		cm_toggle_menu_set_active_full(mainwin->ui_manager, "Menu/View/FullScreen", TRUE);
	}
	if (prefs_common.mainwin_menubar)
...
static gboolean reflect_prefs_timeout_cb(gpointer data)
{
	...
	for (cur = mainwin_list; cur != NULL; cur = cur->next) {
		...
		if (pixmap_theme_changed) {
			...
			gtk_container_remove(GTK_CONTAINER(mainwin->online_switch), mainwin->online_pixmap);
			...
			gtk_container_remove(GTK_CONTAINER(mainwin->offline_switch), mainwin->offline_pixmap);
		...
		headerview_set_visibility(mainwin->messageview->headerview, prefs_common.display_header_pane);
...
void main_window_reflect_prefs_custom_colors(MainWindow *mainwin)
{
	...
	children = gtk_container_get_children(GTK_CONTAINER(menu));
	for (cur = children; cur != NULL && cur->data != NULL; cur = cur->next) {
		g_signal_handlers_disconnect_matched(gtk_ui_manager_get_accel_group(mainwin->ui_manager), G_SIGNAL_MATCH_DATA | G_SIGNAL_MATCH_FUNC, 0, 0, NULL, mainwin_accel_changed_cb, cur->data);
...
{
	if (tags_tag == 0) {
		tags_tag = g_timeout_add(100, main_window_reflect_tags_changes_real, mainwin);
	}
}
...
{
	if (prefs_tag == 0) {
		prefs_tag = g_timeout_add(100, reflect_prefs_timeout_cb, GINT_TO_POINTER(pixmap_theme_changed));
	}
}
...
static void main_window_set_account_selector_menu(MainWindow *mainwin, GList *account_list)
{
	GList *cur_ac;
	GtkWidget *menuitem;
	...
	for (cur_ac = account_list; cur_ac != NULL; cur_ac = cur_ac->next) {
		...
		menuitem = gtk_menu_item_new_with_label(ac_prefs->account_name ? ac_prefs->account_name : _("Untitled"));
		gtk_widget_show(menuitem);
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
		g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(account_selector_menu_cb), ac_prefs);
		accel_path = g_strconcat("<Actions>/Menu/Configuration/ChangeAccount/", (ac_prefs->account_name ? ac_prefs->account_name : _("Untitled")), NULL);
...
static void main_window_set_account_receive_menu(MainWindow *mainwin, GList *account_list)
{
	...
	menu = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(mainwin->ui_manager, "/Menu/Message/Receive")));
	...
	for (child = account_list; child != NULL; child = child->next) {
		...
		gtk_widget_show(menuitem);
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
		g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(account_receive_menu_cb), ac_prefs);
	}
}

static void main_window_set_toolbar_combo_receive_menu(MainWindow *mainwin, GList *account_list)
{
	GList *cur_ac;
	GtkWidget *menuitem;
	PrefsAccount *ac_prefs;
...
static void main_window_set_toolbar_combo_compose_menu(MainWindow *mainwin, GList *account_list)
{
#ifndef GENERIC_UMPC
	GList *cur_ac;
	GtkWidget *menuitem;
	...
	for (cur_ac = account_list; cur_ac != NULL; cur_ac = cur_ac->next) {
		...
		gtk_widget_show(menuitem);
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
		g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(account_compose_menu_cb), ac_prefs);
	}
...
static void main_window_separation_change(MainWindow *mainwin, LayoutType layout_mode)
{
	...
	gtkut_container_remove(GTK_CONTAINER(gtk_widget_get_parent(folder_wid)), folder_wid);
	gtkut_container_remove(GTK_CONTAINER(gtk_widget_get_parent(summary_wid)), summary_wid);
	gtkut_container_remove(GTK_CONTAINER(gtk_widget_get_parent(message_wid)), message_wid);
...
void mainwindow_reset_paned(GtkPaned *paned)
{
	...
	GTK_EVENTS_FLUSH();
	g_object_get(G_OBJECT(paned), "min-position", &min, NULL);
	g_object_get(G_OBJECT(paned), "max-position", &max, NULL);
	mid = (min + max) / 2;
...
void main_window_toggle_message_view(MainWindow *mainwin)
{
	...
	if (messageview_is_visible(mainwin->messageview))
		gtk_arrow_set(GTK_ARROW(mainwin->summaryview->toggle_arrow), GTK_ARROW_DOWN, GTK_SHADOW_OUT);
	else
		gtk_arrow_set(GTK_ARROW(mainwin->summaryview->toggle_arrow), GTK_ARROW_UP, GTK_SHADOW_OUT);
...
void main_window_get_size(MainWindow *mainwin)
{
	...
	if (mainwin_list == NULL || mainwin->messageview == NULL) {
		debug_print("called after messageview " "has been deallocated!\n");
		return;
	}

	if (prefs_common.mainwin_fullscreen) {
		debug_print("mainwin in full screen state. " "Keeping original settings\n");
	}
	...
	if (allocation.width > 1 && allocation.height > 1 && !prefs_common.mainwin_maximised && !prefs_common.mainwin_fullscreen) {
...
gboolean main_window_empty_trash(MainWindow *mainwin, gboolean confirm, gboolean for_quit)
{
	if (confirm && procmsg_have_trashed_mails_fast()) {
		...
		if (for_quit)
			val = alertpanel(_("Empty trash"), _("Delete all messages in trash folders?"), GTK_STOCK_NO, GTK_STOCK_YES, _("Don't quit"), ALERTFOCUS_SECOND);
		else
			val = alertpanel(_("Empty trash"), _("Delete all messages in trash folders?"), GTK_STOCK_NO, GTK_STOCK_YES, NULL, ALERTFOCUS_SECOND);
	...
	if (mainwin->summaryview->folder_item && mainwin->summaryview->folder_item->stype == F_TRASH)
...
static void main_window_add_mailbox(MainWindow *mainwin)
{
	...
	path = input_dialog(_("Add mailbox"), _("Input the location of the mailbox.\n" "The location can be either the full path or relative to the \n" "home directory.\n" "If the location of an existing mailbox is specified, it will be\n" "scanned automatically."), "Mail");
	if (!path)
		return;
	...
	folder = folder_new(folder_get_class_from_string("mh"), !strcmp(path, "Mail") ? _("Mailbox") : g_path_get_basename(path), path);
	g_free(path);
...
#define UPDATE_STATE(...) \
	...
	if (item && item->path && folder_item_parent(item) && !item->no_select) {
		...
		if (prefs_common.immediate_exec == 0 && mainwin->lock_count == 0)
	...
	if (selection == SUMMARY_SELECTED_SINGLE || selection == SUMMARY_SELECTED_MULTIPLE)
		UPDATE_STATE(M_TARGET_EXIST);
	if (selection == SUMMARY_SELECTED_SINGLE)
		UPDATE_STATE(M_SINGLE_TARGET_EXIST);
	if (mainwin->summaryview->folder_item && mainwin->summaryview->folder_item->folder->klass->type == F_NEWS)
		UPDATE_STATE(M_NEWS);
	else
		UPDATE_STATE(M_NOT_NEWS);
	if (mainwin->summaryview->folder_item && (mainwin->summaryview->folder_item->stype != F_TRASH || !folder_has_parent_of_type(mainwin->summaryview->folder_item, F_TRASH)))
		UPDATE_STATE(M_NOT_TRASH);

	if (mainwin->summaryview->folder_item && mainwin->summaryview->folder_item->stype != F_DRAFT)
	...
	if (selection == SUMMARY_SELECTED_SINGLE && (item && (folder_has_parent_of_type(item, F_DRAFT) || folder_has_parent_of_type(item, F_OUTBOX) || folder_has_parent_of_type(item, F_QUEUE))))
	...
	if (procmsg_spam_can_learn() && (mainwin->summaryview->folder_item && mainwin->summaryview->folder_item->folder->klass->type != F_UNKNOWN && mainwin->summaryview->folder_item->folder->klass->type != F_NEWS)) {
	...
	if (mainwin->summaryview->deleted > 0 || mainwin->summaryview->moved > 0 || mainwin->summaryview->copied > 0)
		UPDATE_STATE(M_DELAY_EXEC);
...
#undef SET_SENSITIVE
	...
	for (; cur_item != NULL; cur_item = cur_item->next) {
		gtk_widget_set_sensitive(GTK_WIDGET(cur_item->data), (main_window_get_mask(M_UNLOCKED, -1) & state) != 0);
	}
	...
	cm_toggle_menu_set_active_full(mainwin->ui_manager, "Menu/View/ShowHide/MessageView", messageview_is_visible(mainwin->messageview));

	summaryview = mainwin->summaryview;

	switch (summaryview->sort_key) {
	case SORT_BY_NUMBER:
		menu_path = "Menu/View/Sort/Number";
		break;
	case SORT_BY_SIZE:
		menu_path = "Menu/View/Sort/Size";
		break;
	case SORT_BY_DATE:
		menu_path = "Menu/View/Sort/Date";
		break;
	case SORT_BY_THREAD_DATE:
		menu_path = "Menu/View/Sort/ThreadDate";
		break;
	case SORT_BY_FROM:
		menu_path = "Menu/View/Sort/From";
		break;
	case SORT_BY_TO:
		menu_path = "Menu/View/Sort/To";
		break;
	case SORT_BY_SUBJECT:
		menu_path = "Menu/View/Sort/Subject";
		break;
	case SORT_BY_LABEL:
		menu_path = "Menu/View/Sort/Color";
		break;
	case SORT_BY_MARK:
		menu_path = "Menu/View/Sort/Mark";
		break;
	case SORT_BY_STATUS:
		menu_path = "Menu/View/Sort/Status";
		break;
	case SORT_BY_MIME:
		menu_path = "Menu/View/Sort/Attachment";
		break;
	case SORT_BY_SCORE:
		menu_path = "Menu/View/Sort/Score";
		break;
	case SORT_BY_LOCKED:
		menu_path = "Menu/View/Sort/Locked";
		break;
	case SORT_BY_TAGS:
		menu_path = "Menu/View/Sort/Tag";
		break;
	case SORT_BY_NONE:
	default:
	...
	if (mainwin->messageview && mainwin->messageview->mimeview && mainwin->messageview->mimeview->textview)
		cm_toggle_menu_set_active_full(mainwin->ui_manager, "Menu/View/AllHeaders", prefs_common.show_all_headers);
	...
	if ((mainwin->summaryview->folder_item && mainwin->summaryview->folder_item->hide_read_msgs) || quicksearch_has_sat_predicate(mainwin->summaryview->quicksearch))
		cm_menu_set_sensitive_full(mainwin->ui_manager, "Menu/View/HideReadThreads", FALSE);
	if ((mainwin->summaryview->folder_item && mainwin->summaryview->folder_item->hide_read_threads) || quicksearch_has_sat_predicate(mainwin->summaryview->quicksearch))
	...
	cm_menu_set_sensitive_full(mainwin->ui_manager, "Menu/View/Goto/PrevHistory", messageview_nav_has_prev(mainwin->messageview));
	cm_menu_set_sensitive_full(mainwin->ui_manager, "Menu/View/Goto/NextHistory", messageview_nav_has_next(mainwin->messageview));
	...
	if (mimepart_selected) {
		...
		if (partinfo && (partinfo->type == MIMETYPE_MESSAGE || partinfo->type == MIMETYPE_IMAGE || partinfo->type == MIMETYPE_MULTIPART)) {
			sensitive = FALSE;
		}
	}
	...
	if (list_header) {
		for (url_pt = list_header; url_pt && *url_pt;) {
			...
			if (!g_ascii_strncasecmp(url_decoded, "mailto:", 7)) {
				...
				g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(mailing_list_compose), NULL);
			} else if (!g_ascii_strncasecmp(url_decoded, "http:", 5) || !g_ascii_strncasecmp(url_decoded, "https:", 6)) {

				item = cm_menu_item_new_label_from_url((url_decoded));
				g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(mailing_list_open_uri), NULL);
			}
			if (item) {
				gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
				gtk_widget_show(item);
...
static void get_url_part(const gchar **buffer, gchar *url_decoded)
{
	...
	if (*buf == '<') {
		...
		if (!strncmp(buf, "mailto:", strlen("mailto:")))
			with_plus = FALSE;
		for (i = 0; *buf != '>' && *buf != 0x00 && i < BUFFSIZE; tmp[i++] = *(buf++)) ;
		if (*buf == 0) {
			*buffer = NULL;
			*url_decoded = '\0';
			return;
...
#ifndef GENERIC_UMPC
	gtk_window_move(GTK_WINDOW(mainwin->window), prefs_common.mainwin_x, prefs_common.mainwin_y);

	gtk_widget_set_size_request(GTK_WIDGET_PTR(mainwin->folderview), prefs_common.folderview_width, prefs_common.folderview_height);
	gtk_widget_set_size_request(GTK_WIDGET_PTR(mainwin->summaryview), prefs_common.summaryview_width, prefs_common.summaryview_height);
	gtk_widget_set_size_request(GTK_WIDGET_PTR(mainwin->messageview), prefs_common.msgview_width, prefs_common.msgview_height);
#endif
}
...
static void main_window_set_widgets(MainWindow *mainwin, LayoutType layout_mode)
{
	...
	cm_menu_set_sensitive_full(mainwin->ui_manager, "Menu/View/ShowHide/MessageView", (layout_mode != WIDE_MSGLIST_LAYOUT && layout_mode != SMALL_LAYOUT));
	switch (layout_mode) {
	case VERTICAL_LAYOUT:
	case NORMAL_LAYOUT:
	case SMALL_LAYOUT:
		...
		gtk_paned_add1(GTK_PANED(hpaned), GTK_WIDGET_PTR(mainwin->folderview));
		gtk_widget_show(hpaned);
		gtk_widget_queue_resize(hpaned);

		if (messageview_is_visible(mainwin->messageview)) {
			gtk_paned_add2(GTK_PANED(hpaned), vpaned);
			gtk_paned_pack1(GTK_PANED(vpaned), GTK_WIDGET_PTR(mainwin->summaryview), TRUE, TRUE);
		} else {
			gtk_paned_add2(GTK_PANED(hpaned), GTK_WIDGET_PTR(mainwin->summaryview));
			g_object_ref(vpaned);
		}
		gtk_paned_add2(GTK_PANED(vpaned), GTK_WIDGET_PTR(mainwin->messageview));
	...
	case WIDE_MSGLIST_LAYOUT:
		...
		gtk_paned_add1(GTK_PANED(vpaned), GTK_WIDGET_PTR(mainwin->summaryview));
		gtk_paned_add1(GTK_PANED(hpaned), GTK_WIDGET_PTR(mainwin->folderview));
		...
		if (messageview_is_visible(mainwin->messageview)) {
			gtk_paned_add2(GTK_PANED(hpaned), GTK_WIDGET_PTR(mainwin->messageview));
		} else {
			g_object_ref(GTK_WIDGET_PTR(mainwin->messageview));
		}
	...
	if (layout_mode == SMALL_LAYOUT && first_set) {
		...
		gtk_widget_set_size_request(GTK_WIDGET_PTR(mainwin->folderview), prefs_common.folderview_width, prefs_common.folderview_height);
		gtk_widget_set_size_request(GTK_WIDGET_PTR(mainwin->summaryview), 0, 0);
		gtk_widget_set_size_request(GTK_WIDGET_PTR(mainwin->messageview), 0, 0);
		gtk_widget_set_size_request(GTK_WIDGET(mainwin->window), prefs_common.mainwin_width, prefs_common.mainwin_height);
	...
	if (folderwin) {
		g_signal_connect(G_OBJECT(folderwin), "size_allocate", G_CALLBACK(folder_window_size_allocate_cb), mainwin);
	}
	if (messagewin) {
		g_signal_connect(G_OBJECT(messagewin), "size_allocate", G_CALLBACK(message_window_size_allocate_cb), mainwin);
	}
...
static gboolean ac_label_button_pressed(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
	...
	gtk_menu_popup(GTK_MENU(menu), NULL, NULL, menu_button_position, widget, event->button, event->time);
...
static void app_exit_cb(GtkAction *action, gpointer data)
{
	...
	if (prefs_common.confirm_on_exit) {
		if (alertpanel(_("Exit"), _("Exit Claws Mail?"), GTK_STOCK_CANCEL, GTK_STOCK_QUIT, NULL, ALERTFOCUS_FIRST)
		    != G_ALERTALTERNATE)
			return;
		manage_window_focus_in(mainwin->window, NULL, NULL);
	}
...
static void mainwindow_check_synchronise(MainWindow *mainwin, gboolean ask)
{
	...
	if (offline_ask_sync && ask && alertpanel(_("Folder synchronisation"), _("Do you want to synchronise your folders now?"), GTK_STOCK_CANCEL, _("_Synchronise"), NULL, ALERTFOCUS_SECOND) != G_ALERTALTERNATE)
		return;
...
static void debug_log_show_cb(GtkAction *action, gpointer data)
{
	...
	if (error != NULL) {
		g_warning("couldn't convert debug log path '%s' to UTF-16: %s", logpath8, error->message);
		g_error_free(error);
		g_free(logpath16);
		return;
	}
...
static void show_all_header_cb(GtkAction *action, gpointer data)
{
	...
	if (mainwin->menu_lock_count)
		return;
	prefs_common.show_all_headers = gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action));
	summary_display_msg_selected(mainwin->summaryview, gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action)));
}
...
static void hide_quotes_cb(GtkAction *action, gpointer data)
{
	...
	if (gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action))) {
		...
		if (!strcmp(a_name, "View/Quotes/CollapseAll"))
			prefs_common.hide_quotes = 1;
		else if (!strcmp(a_name, "View/Quotes/Collapse2"))
			prefs_common.hide_quotes = 2;
		else if (!strcmp(a_name, "View/Quotes/Collapse3"))
			prefs_common.hide_quotes = 3;
	} else
...
static void open_urls_cb(GtkAction *action, gpointer data)
{
	...
	if (!mainwin->summaryview->displayed && mainwin->summaryview->selected) {
		summary_display_msg_selected(mainwin->summaryview, prefs_common.show_all_headers);
	}
...
static void hide_read_messages(GtkAction *action, gpointer data)
{
	...
	if (!mainwin->summaryview->folder_item || g_object_get_data(G_OBJECT(menuitem), "dont_toggle"))
		return;
...
static void thread_cb(GtkAction *action, gpointer data)
{
	...
	summary_show(mainwin->summaryview, mainwin->summaryview->folder_item, FALSE);
...
static void sort_summary_cb(GtkAction *action, GtkRadioAction *current, gpointer data)
{
	...
	if (gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(current))) {
		summary_sort(mainwin->summaryview, (FolderSortKey)value, mainwin->summaryview->sort_type);
	}
}
...
static void sort_summary_type_cb(GtkAction *gaction, GtkRadioAction *current, gpointer data)
{
	...
	if (gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(current)))
		summary_sort(mainwin->summaryview, mainwin->summaryview->sort_key, (FolderSortType)value);
}
...
void mainwindow_delete_duplicated(MainWindow *mainwin)
{
	...
	if (item) {
		...
		result = folderutils_delete_duplicates(item, prefs_common.immediate_exec ? DELETE_DUPLICATES_REMOVE : DELETE_DUPLICATES_SETFLAG);
...
static void update_summary_cb(GtkAction *action, gpointer data)
{
	...
	if (!mainwin->summaryview->folder_item)
		return;
	if ((fitem = folderview_get_opened_item(folderview)) == NULL)
		return;
...
static void prev_history_cb(GtkAction *action, gpointer data)
{
	...
	if (info) {
		...
		summary_select_by_msgnum(mainwin->summaryview, info->msgnum, OPEN_SELECTED_ON_PREVNEXT);
...
static void goto_folder_cb(GtkAction *action, gpointer data)
{
	...
	to_folder = foldersel_folder_sel(NULL, FOLDER_SEL_ALL, NULL, FALSE, _("Select folder to go to"));
...
static void allsel_cb(GtkAction *action, gpointer data)
{
	...
	if (messageview_is_visible(msgview) && (gtk_widget_has_focus(msgview->mimeview->textview->text)))
...
static void prefs_pre_processing_open_cb(GtkAction *action, gpointer data)
{
	prefs_filtering_open(&pre_global_processing, _("Processing rules to apply before folder rules"), MANUAL_ANCHOR_PROCESSING, NULL, NULL, FALSE);
}

static void prefs_post_processing_open_cb(GtkAction *action, gpointer data)
{
	prefs_filtering_open(&post_global_processing, _("Processing rules to apply after folder rules"), MANUAL_ANCHOR_PROCESSING, NULL, NULL, FALSE);
}

static void prefs_filtering_open_cb(GtkAction *action, gpointer data)
{
	prefs_filtering_open(&filtering_rules, _("Filtering configuration"), MANUAL_ANCHOR_FILTERING, NULL, NULL, TRUE);
}
...
static void account_selector_menu_cb(GtkMenuItem *menuitem, gpointer data)
{
	...
	main_window_set_menu_sensitive(mainwindow_get_mainwindow());
	toolbar_main_set_sensitive(mainwindow_get_mainwindow());
	item = folderview_get_selected_item(mainwindow_get_mainwindow()->folderview);
	if (item) {
		toolbar_set_compose_button(mainwindow_get_mainwindow()->toolbar, FOLDER_TYPE(item->folder) == F_NEWS ? COMPOSEBUTTON_NEWS : COMPOSEBUTTON_MAIL);
	}
}
...
static void set_default_client_cb(GtkAction *action, gpointer data)
{
	...
	r = write_w32_registry_string(HKEY_LOCAL_MACHINE, "Software\\Clients\\Mail", "", "Claws Mail");

	r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail", "", "Claws Mail");
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail", "DLLPath", "");
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail\\Protocols\\mailto", "", "URL:MailTo-Protocol");
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail\\Protocols\\mailto", "URL Protocol", "");
	if (r)
		r = write_w32_registry_dword(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail\\Protocols\\mailto", "EditFlags", 2);
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail\\Protocols\\mailto", "FriendlyTypeName", "Claws-Mail URL");
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail\\Protocols\\mailto\\DefaultIcon", "", binary_icon);
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail\\Protocols\\mailto\\shell\\open\\command", "", binary_compose);
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Clients\\Mail\\Claws Mail\\shell\\open\\command", "", binary_run);

	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Classes\\mailto", "", "URL:MailTo-Protocol");
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Classes\\mailto", "URL Protocol", "");
	if (r)
		r = write_w32_registry_dword(HKEY_CURRENT_USER, "Software\\Classes\\mailto", "EditFlags", 2);
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Classes\\mailto", "FriendlyTypeName", "Claws-Mail URL");
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Classes\\mailto\\DefaultIcon", "", binary_icon);
	if (r)
		r = write_w32_registry_string(HKEY_CURRENT_USER, "Software\\Classes\\mailto\\shell\\open\\command", "", binary_compose);
...
static gboolean mainwindow_focus_in_event(GtkWidget *widget, GdkEventFocus *focus, gpointer data)
{
	...
	if (GTK_CMCLIST(summary->ctree)->selection && g_list_length(GTK_CMCLIST(summary->ctree)->selection) > 1)
		return FALSE;

	return FALSE;
}

static gboolean mainwindow_visibility_event_cb(GtkWidget *widget, GdkEventVisibility *event, gpointer data)
{
...
static gboolean mainwindow_state_event_cb(GtkWidget *widget, GdkEventWindowState *state, gpointer data)
{
	if (!claws_is_starting()
	    && state->changed_mask & GDK_WINDOW_STATE_ICONIFIED && state->new_window_state & GDK_WINDOW_STATE_ICONIFIED) {

		if (iconified_count > 0)
			hooks_invoke(MAIN_WINDOW_GOT_ICONIFIED, NULL);
		iconified_count++;
	} else if (!claws_is_starting()) {
		prefs_common.mainwin_maximised = ((state->new_window_state & GDK_WINDOW_STATE_MAXIMIZED) != 0);
	}
...
static gboolean mainwindow_progressindicator_hook(gpointer source, gpointer userdata)
{
	...
	case PROGRESS_COMMAND_SET_PERCENTAGE:
		gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(mainwin->progressbar), data->value);
		break;
	}
