...
GdkPixbuf *pixbuf_from_cairo(cairo_t *cr, gboolean alpha, int width, int height)
{
	...
	*s_pixels; /* Surface's pixel data */
	...
	/* Inspect pixbuf */
	g_object_get(G_OBJECT(pixbuf), "rowstride", &p_stride, "n-channels", &p_n_channels, "pixels", &p_pixels, NULL);
	/* and surface */
...
GdkPixbuf *pixbuf_from_svg_like_icon(char *filename, GError **error, StockPixmapData *icondata, gboolean alpha)
{
	...
	if (handle == NULL) {
		g_warning("failed loading SVG '%s': %s (%d)", filename, (*error)->message, (*error)->code);
		return NULL;
	}
	...
	surface = cairo_image_surface_create(alpha ? CAIRO_FORMAT_ARGB32 : CAIRO_FORMAT_RGB24, width, height);
	if (cairo_surface_status(surface) != CAIRO_STATUS_SUCCESS) {
		g_warning("failed to create a cairo surface: %s", cairo_status_to_string(cairo_surface_status(surface)));
	...
	if (cairo_status(context) != CAIRO_STATUS_SUCCESS) {
		g_warning("failed to create a cairo context: %s", cairo_status_to_string(cairo_status(context)));
	...
	if (!pix_d->pixbuf || theme_changed) {
		...
		if (strcmp(prefs_common.pixmap_theme_path, DEFAULT_PIXMAP_THEME) != 0) {
			if (is_dir_exist(prefs_common.pixmap_theme_path)) {
				char *icon_file_name;
 try_next_extension:
				icon_file_name = g_strconcat(prefs_common.pixmap_theme_path, G_DIR_SEPARATOR_S, pix_d->file, extension[i], NULL);
				if (is_file_exist(icon_file_name)) {
					GError *err = NULL;
#ifdef HAVE_SVG
					if (!strncmp(extension[i], ".svg", 4)) {
						pix = pixbuf_from_svg_like_icon(icon_file_name, &err, pix_d, prefs_common.enable_alpha_svg);
					} else {
...
gchar *stock_pixmap_get_system_theme_dir_for_theme(const gchar *theme)
{
	if (theme && *theme) {
#ifndef G_OS_WIN32
		return g_strconcat(PACKAGE_DATA_DIR, G_DIR_SEPARATOR_S, PIXMAP_THEME_DIR, G_DIR_SEPARATOR_S, theme, NULL);
#else
		return g_strconcat(w32_get_themes_dir(), G_DIR_SEPARATOR_S, theme, NULL);
#endif
	} else {
#ifndef G_OS_WIN32
		return g_strconcat(PACKAGE_DATA_DIR, G_DIR_SEPARATOR_S, PIXMAP_THEME_DIR, NULL);
#else
...
GList *stock_pixmap_themes_list_new(void)
{
	...
	userthemes = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, PIXMAP_THEME_DIR, NULL);
...
static gboolean do_pix_draw(GtkWidget *widget, cairo_t *cr, OverlayData *data)
{
	...
	if (data->highlight) {
		...
		if (mw != NULL && mw->menubar != NULL) {
			...
			cairo_rectangle(cr, data->border_x - 2, data->border_y - 2, data->base_width + 3, data->base_height + 3);
			cairo_stroke(cr);
			cairo_destroy(cr);
		}
	}
...
static gboolean pixmap_with_overlay_expose_event_cb(GtkWidget *widget, GdkEventExpose *expose, OverlayData *data)
{
	cairo_t *cr;
	GdkWindow *drawable = gtk_widget_get_window(widget);
	gboolean result;
	...
	cr = gdk_cairo_create(drawable);
	gdk_window_clear_area(drawable, expose->area.x, expose->area.y, expose->area.width, expose->area.height);
...
GtkWidget *stock_pixmap_widget_with_overlay(StockPixmap icon, StockPixmap overlay, OverlayPosition pos, gint border_x, gint border_y)
{
	...
	gtk_widget_set_size_request(widget, data->base_width + border_x * 2, data->base_height + border_y * 2);
	g_signal_connect(G_OBJECT(widget), "expose_event", G_CALLBACK(pixmap_with_overlay_expose_event_cb), data);
	g_signal_connect(G_OBJECT(widget), "destroy", G_CALLBACK(pixmap_with_overlay_destroy_cb), data);
