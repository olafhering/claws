...
/* callback functions */
static void prefs_summary_open_register_cb(GtkButton *btn, gpointer hidden_data);
...
static gboolean prefs_summary_open_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data);
static void prefs_summary_open_ok(void);
static void prefs_summary_open_cancel(void);
static gint prefs_summary_open_deleted(GtkWidget *widget, GdkEventAny *event, gpointer data);

static GtkListStore *prefs_summary_open_create_store(void);
static void prefs_summary_open_insert_action(GtkListStore *store, gchar *name, gint act);
static GtkWidget *prefs_summary_open_list_view_create(const gchar *name);
static void prefs_summary_open_create_list_view_columns(GtkWidget *list_view, const gchar *name);
static void actions_list_model_row_changed(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, GtkTreeView *list_view);
...
static gchar *action_name[SUMMARY_OPEN_ACTIONS] = { ("UNSET (!)"),
...
static void prefs_summary_open_create(void)
{
	...
	gtk_window_set_title(GTK_WINDOW(window), _("Message selection when entering a folder"));
	MANAGE_WINDOW_SIGNALS_CONNECT(window);
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(prefs_summary_open_deleted), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(prefs_summary_open_key_pressed), NULL);
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(prefs_summary_open_ok), NULL);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(prefs_summary_open_cancel), NULL);
	...
	gtk_box_pack_start(GTK_BOX(list_view_hbox1), list_view_scrolledwin, TRUE, TRUE, 0);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(list_view_scrolledwin), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(list_view_scrolledwin), GTK_SHADOW_IN);

	possible_actions_list_view = prefs_summary_open_list_view_create(_("Available selections"));
	...
	g_signal_connect(G_OBJECT(reg_btn), "clicked", G_CALLBACK(prefs_summary_open_register_cb), possible_actions_list_view);
	...
	list_view_scrolledwin = gtk_scrolled_window_new(NULL, NULL);
	gtk_widget_show(list_view_scrolledwin);
	gtk_box_pack_start(GTK_BOX(list_view_hbox2), list_view_scrolledwin, TRUE, TRUE, 0);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(list_view_scrolledwin), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(list_view_scrolledwin), GTK_SHADOW_IN);

	actions_list_view = prefs_summary_open_list_view_create(_("Current selections"));
	g_signal_connect(G_OBJECT(del_btn), "clicked", G_CALLBACK(prefs_summary_open_delete_cb), actions_list_view);
	gtk_widget_show(actions_list_view);
	gtk_container_add(GTK_CONTAINER(list_view_scrolledwin), actions_list_view);

	gtk_tree_view_set_reorderable(GTK_TREE_VIEW(actions_list_view), TRUE);
	g_signal_connect(G_OBJECT(actions_list_view), "drag_begin", G_CALLBACK(drag_begin), actions_list_view);

	g_signal_connect(G_OBJECT(actions_list_view), "drag_end", G_CALLBACK(drag_end), actions_list_view);
	...
	g_signal_connect(G_OBJECT(up_btn), "clicked", G_CALLBACK(prefs_summary_open_up), NULL);

	down_btn = gtk_button_new_from_stock(GTK_STOCK_GO_DOWN);
	gtk_widget_show(down_btn);
	gtk_box_pack_start(GTK_BOX(btn_vbox), down_btn, FALSE, FALSE, 0);
	g_signal_connect(G_OBJECT(down_btn), "clicked", G_CALLBACK(prefs_summary_open_down), NULL);
...
void prefs_summary_open_set_defaults(void)
{
	...
	case SELECTONENTRY_MNU:
		SET_PRIO(ACTION_OLDEST_MARKED, ACTION_OLDEST_NEW, ACTION_OLDEST_UNREAD, ACTION_NEWEST_LIST);
		break;
	case SELECTONENTRY_MUN:
		SET_PRIO(ACTION_OLDEST_MARKED, ACTION_OLDEST_UNREAD, ACTION_OLDEST_NEW, ACTION_NEWEST_LIST);
		break;
	case SELECTONENTRY_NMU:
		SET_PRIO(ACTION_OLDEST_NEW, ACTION_OLDEST_MARKED, ACTION_OLDEST_UNREAD, ACTION_NEWEST_LIST);
		break;
	case SELECTONENTRY_NUM:
		SET_PRIO(ACTION_OLDEST_NEW, ACTION_OLDEST_UNREAD, ACTION_OLDEST_MARKED, ACTION_NEWEST_LIST);
		break;
	case SELECTONENTRY_UNM:
		SET_PRIO(ACTION_OLDEST_UNREAD, ACTION_OLDEST_NEW, ACTION_OLDEST_MARKED, ACTION_NEWEST_LIST);
		break;
	case SELECTONENTRY_UMN:
		SET_PRIO(ACTION_OLDEST_UNREAD, ACTION_OLDEST_MARKED, ACTION_OLDEST_NEW, ACTION_NEWEST_LIST);
		break;
	case SELECTONENTRY_LAST:
		SET_PRIO(ACTION_LAST_OPENED, ACTION_NEWEST_LIST, ACTION_UNSET, ACTION_UNSET);
		break;
	case SELECTONENTRY_NOTHING:
		SET_PRIO(ACTION_NEWEST_LIST, ACTION_UNSET, ACTION_UNSET, ACTION_UNSET);
		break;
	default:
		break;
	}
...
static void prefs_summary_open_set_dialog(void)
{
 ...
 fill:
	for (i = 0; i < SUMMARY_OPEN_ACTIONS - 1; i++) {
		...
		set = TRUE;
		used[act - 1] = TRUE;
		prefs_summary_open_insert_action(GTK_LIST_STORE(model_act), action_name[act], act);
	}
	if (!set) {
		/* backward compat */
	...
	for (i = 1; i < SUMMARY_OPEN_ACTIONS; i++) {
		if (!used[i - 1]) {
			prefs_summary_open_insert_action(GTK_LIST_STORE(model_poss), action_name[i], i);
		}
	}
}
...
static void prefs_summary_open_register_cb(GtkButton *btn, gpointer list_view_data)
{
	...
	i = GPOINTER_TO_INT(data);
	prefs_summary_open_insert_action(GTK_LIST_STORE(model), action_name[i], i);
...
static void prefs_summary_open_up(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(summaryopen.actions_list_view)), &model, &isel))
		return;
...
static void prefs_summary_open_down(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(summaryopen.actions_list_view)), &model, &sel))
		return;
	store = (GtkListStore *)model;
...
static GtkListStore *prefs_summary_open_create_store(void)
{
	return gtk_list_store_new(N_PREFS_SUMMARY_OPEN_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER, -1);
}

static void prefs_summary_open_insert_action(GtkListStore *store, gchar *name, gint dp)
{
	GtkTreeIter iter;

	/* add new */
	gtk_list_store_append(store, &iter);
	gtk_list_store_set(store, &iter, PREFS_SUMMARY_OPEN_HEADER, gettext(name), PREFS_SUMMARY_OPEN_DATA, GINT_TO_POINTER(dp), -1);
}
...
static void prefs_summary_open_create_list_view_columns(GtkWidget *list_view, const gchar *name)
{
	...
	column = gtk_tree_view_column_new_with_attributes(name, renderer, "text", PREFS_SUMMARY_OPEN_HEADER, NULL);
...
static void drag_begin(GtkTreeView *list_view, GdkDragContext *context, gpointer data)
{
	...
	g_signal_connect(G_OBJECT(model), "row_changed", G_CALLBACK(actions_list_model_row_changed), list_view);
}

static void drag_end(GtkTreeView *list_view, GdkDragContext *context, gpointer data)
{
	GtkTreeModel *model = gtk_tree_view_get_model(list_view);
	g_signal_handlers_disconnect_by_func(G_OBJECT(model), G_CALLBACK(actions_list_model_row_changed), list_view);
	prefs_summary_open_set_list();
}
