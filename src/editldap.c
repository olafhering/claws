...
static gboolean edit_ldap_validate(void)
{
	...
	errorFlag = FALSE;
	str = gtk_editable_get_chars(GTK_EDITABLE(ldapedit.entry_name), 0, -1);
	if (*str == '\0') {
		page = PAGE_BASIC;
	...
	if (!errorFlag) {
		str = gtk_editable_get_chars(GTK_EDITABLE(ldapedit.entry_server), 0, -1);
		if (*str == '\0') {
			page = PAGE_BASIC;
			gtk_widget_grab_focus(ldapedit.entry_server);
			edit_ldap_status_show(_("A Hostname must be supplied for the server."));
			errorFlag = TRUE;
		}
		g_free(str);
	}
	...
	if (!errorFlag) {
		str = gtk_editable_get_chars(GTK_EDITABLE(ldapedit.entry_criteria), 0, -1);
		if (editldap_validate_criteria(str)) {
			page = PAGE_SEARCH;
			gtk_widget_grab_focus(ldapedit.entry_criteria);
			edit_ldap_status_show(_("At least one LDAP search attribute should be supplied."));
			errorFlag = TRUE;
		}
		g_free(str);
	}
...
static void editldap_update_port(GtkToggleButton *ssl_btn, gpointer data)
{
	...
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_port), val ? LDAPCTL_DFL_SSL_PORT : LDAPCTL_DFL_PORT);
...
static void addressbook_edit_ldap_page_basic(gint pageNum, gchar *pageLbl)
{
	...
	gtk_notebook_set_tab_label(GTK_NOTEBOOK(ldapedit.notebook), gtk_notebook_get_nth_page(GTK_NOTEBOOK(ldapedit.notebook), pageNum), label);
	...
	CLAWS_SET_TIP(entry_name, _("A name that you wish to call the server."));
	...
	CLAWS_SET_TIP(entry_server, _("This is the hostname of the server. For example, " "\"ldap.example.org\" may be appropriate for the " "\"example.org\" organization. An IP address may also be " "used. You may specify \"localhost\" if running an LDAP " "server on the same computer as Claws Mail."));
	...
	CLAWS_SET_TIP(enable_tls_checkbtn, _("Enable secure connection to the LDAP server via STARTTLS. " "Connection starts unencrypted and is secured by STARTTLS command. " "If connection fails, be sure to check the correct " "configuration in ldap.conf (TLS_CACERTDIR and TLS_REQCERT fields)."));
	CLAWS_SET_TIP(enable_ssl_checkbtn, _("Enable secure connection to the LDAP server via TLS. " "If connection fails, be sure to check the correct " "configuration in ldap.conf (TLS_CACERTDIR and TLS_REQCERT fields)."));
	...
	CLAWS_SET_TIP(spinbtn_port, _("The port number that the server listens on. Port 389 is " "the default."));
	...
	CLAWS_SET_TIP(check_btn, _("Press this button to test the connection to the server."));
	...
	CLAWS_SET_TIP(entry_baseDN, _("This specifies the name of the directory to be searched " "on the server. Examples include:\n" "  dc=claws-mail,dc=org\n" "  ou=people,dc=domainname,dc=com\n" "  o=Organization Name,c=Country\n"));

	lookdn_btn = gtkut_get_browse_file_btn(_("_Browse"));
	gtk_table_attach(GTK_TABLE(table), lookdn_btn, 2, 3, top, (top + 1), GTK_FILL, 0, 3, 0);

	CLAWS_SET_TIP(lookdn_btn, _("Press this button to lookup the name of available " "directory names on the server."));
	...
	g_signal_connect(G_OBJECT(enable_ssl_checkbtn), "toggled", G_CALLBACK(editldap_update_port), NULL);
#endif
}
...
static void addressbook_edit_ldap_page_search(gint pageNum, gchar *pageLbl)
{
	...
	CLAWS_SET_TIP(entry_criteria, _("A list of LDAP attribute names that should be searched " "when attempting to find a name or address."));

	reset_btn = gtk_button_new_with_label(_(" Defaults "));
	gtk_table_attach(GTK_TABLE(table), reset_btn, 2, 3, top, (top + 1), GTK_FILL, 0, 3, 0);

	CLAWS_SET_TIP(reset_btn, _("This resets the attribute names to a default value " "that should find most names and addresses during a " "name or address search process."));
	...
	hbox_spin = gtk_hbox_new(FALSE, 8);
	spinbtn_queryage_adj = GTK_ADJUSTMENT(gtk_adjustment_new(LDAPCTL_DFL_QUERY_AGE, 1, LDAPCTL_MAX_QUERY_AGE, 10, 1000, 0));
	...
	CLAWS_SET_TIP(spinbtn_queryage, _("This defines the maximum period of time (in seconds) that " "an address search result is valid for address completion " "purposes. Search results are stored in a cache until this " "period of time has passed and then retired. This will " "improve the response time when attempting to search for " "the same name or address on subsequent address completion " "requests. The cache will be searched in preference to " "performing a new server search request. The default value " "of 600 seconds (10 minutes), should be sufficient for most " "servers. A larger value will reduce the search time for " "subsequent searches. This is useful for servers that have " "slow response times at the expense of more memory to cache " "results."));

	/* Next row */
	++top;
	check_dynsearch = gtk_check_button_new_with_label(_("Include server in dynamic search"));
	gtk_table_attach(GTK_TABLE(table), check_dynsearch, 1, 3, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(check_dynsearch, _("Check this option to include this server for dynamic " "searches when using address completion."));

	/* Next row */
	++top;
	check_matchoption = gtk_check_button_new_with_label(_("Match names 'containing' search term"));
	gtk_table_attach(GTK_TABLE(table), check_matchoption, 1, 3, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(check_matchoption, _("Searches for names and addresses can be performed either " "using \"begins-with\" or \"contains\" search term. Check " "this option to perform a \"contains\" search; this type of " "search usually takes longer to complete. Note that for " "performance reasons, address completion uses " "\"begins-with\" for all searches against other address " "interfaces."));
...
static void showpwd_checkbtn_toggled(GtkToggleButton *button, gpointer user_data)
{
...
static void addressbook_edit_ldap_page_extended(gint pageNum, gchar *pageLbl)
{
	...
	gtk_table_attach(GTK_TABLE(table), entry_bindDN, 1, 3, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(entry_bindDN, _("The LDAP user account name to be used to connect to the server. " "This is usually only used for protected servers. This name " "is typically formatted as: \"cn=user,dc=claws-mail,dc=org\". " "This is usually left empty when performing a search."));
	...
	CLAWS_SET_TIP(entry_bindPW, _("The password to be used when connecting as the \"Bind DN\" " "user."));
	...
	g_signal_connect(G_OBJECT(showpwd_checkbtn), "toggled", G_CALLBACK(showpwd_checkbtn_toggled), entry_bindPW);
	gtk_table_attach(GTK_TABLE(table), showpwd_checkbtn, 2, 3, top, (top + 1), 0, 0, 0, 0);
	...
	CLAWS_SET_TIP(spinbtn_timeout, _("The timeout period in seconds."));
	...
	CLAWS_SET_TIP(spinbtn_maxentry, _("The maximum number of entries that should be returned " "in the search result."));
...
static void edit_ldap_clear_fields(void)
{
	...
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_port), LDAPCTL_DFL_PORT);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_timeout), LDAPCTL_DFL_TIMEOUT);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_maxentry), LDAPCTL_DFL_TIMEOUT);
	gtk_entry_set_text(GTK_ENTRY(ldapedit.entry_criteria), LDAPCTL_DFL_ATTR_LIST);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_queryage), LDAPCTL_DFL_QUERY_AGE);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ldapedit.check_dynsearch), TRUE);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ldapedit.check_matchoption), FALSE);
#if (defined USE_LDAP_TLS || defined G_OS_WIN32)
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ldapedit.enable_ssl), FALSE);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ldapedit.enable_tls), FALSE);
#endif
}
...
static void edit_ldap_set_fields(LdapServer *server)
{
	...
	if (ldapsvr_get_name(server))
		gtk_entry_set_text(GTK_ENTRY(ldapedit.entry_name), ldapsvr_get_name(server));

	ctl = server->control;
	if (ctl->hostName)
		gtk_entry_set_text(GTK_ENTRY(ldapedit.entry_server), ctl->hostName);
	if (ctl->baseDN)
		gtk_entry_set_text(GTK_ENTRY(ldapedit.entry_baseDN), ctl->baseDN);
	if (ctl->bindDN)
		gtk_entry_set_text(GTK_ENTRY(ldapedit.entry_bindDN), ctl->bindDN);

	pwd = passwd_store_get(PWS_CORE, "LDAP", ctl->hostName);
	gtk_entry_set_text(GTK_ENTRY(ldapedit.entry_bindPW), (pwd ? pwd : ""));
	if (pwd != NULL) {
		memset(pwd, 0, strlen(pwd));
		g_free(pwd);
	}

	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_timeout), ctl->timeOut);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_maxentry), ctl->maxEntries);
#if (defined USE_LDAP_TLS || defined G_OS_WIN32)
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ldapedit.enable_tls), ctl->enableTLS);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ldapedit.enable_ssl), ctl->enableSSL);
#endif
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_port), ctl->port);
	/* Format criteria */
	...
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(ldapedit.spinbtn_queryage), ctl->maxQueryAge);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ldapedit.check_dynsearch), server->searchFlag);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ldapedit.check_matchoption), (ctl->matchingOption == LDAPCTL_MATCH_CONTAINS));
}
...
AdapterDSource *addressbook_edit_ldap(AddressIndex *addrIndex, AdapterDSource *ads)
{
	...
	bMatch = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ldapedit.check_matchoption));
#if (defined USE_LDAP_TLS || defined G_OS_WIN32)
	ssl = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ldapedit.enable_ssl));
	tls = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ldapedit.enable_tls));
#endif
	...
	if (!fin) {
		...
		ldapctl_set_matching_option(ctl, bMatch ? LDAPCTL_MATCH_CONTAINS : LDAPCTL_MATCH_BEGINWITH);
