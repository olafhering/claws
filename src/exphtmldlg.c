...
static void export_html_status_show(gchar *msg)
{
	if (exphtml_dlg.statusbar != NULL) {
		gtk_statusbar_pop(GTK_STATUSBAR(exphtml_dlg.statusbar), exphtml_dlg.status_cid);
		if (msg) {
			gtk_statusbar_push(GTK_STATUSBAR(exphtml_dlg.statusbar), exphtml_dlg.status_cid, msg);
		}
	}
}
...
static gboolean exp_html_move_file(void)
{
	...
	/* Prompt to create */
	msg = g_strdup_printf(_("The HTML output directory '%s'\n" "does not exist. Do you want to create it?"), _exportCtl_->dirOutput);
...
static gboolean exp_html_move_format(void)
{
	...
	exporthtml_set_banding(_exportCtl_, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(exphtml_dlg.checkBanding)));
	exporthtml_set_link_email(_exportCtl_, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(exphtml_dlg.checkLinkEMail)));
	exporthtml_set_attributes(_exportCtl_, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(exphtml_dlg.checkAttributes)));
...
static void export_html_prev(GtkWidget *widget)
{
	...
	if (pageNum == PAGE_FORMAT) {
		...
		/* Goto format page */
		gtk_notebook_set_current_page(GTK_NOTEBOOK(exphtml_dlg.notebook), PAGE_FORMAT);
...
static void export_html_next(GtkWidget *widget)
{
	...
	if (pageNum == PAGE_FILE_INFO) {
		...
		if (exp_html_move_format()) {
			gtk_notebook_set_current_page(GTK_NOTEBOOK(exphtml_dlg.notebook), PAGE_FINISH);
			gtk_button_set_label(GTK_BUTTON(exphtml_dlg.btnCancel), GTK_STOCK_CLOSE);
...
static void export_html_page_file(gint pageNum, gchar *pageLbl)
{
	...
	gtk_notebook_set_tab_label(GTK_NOTEBOOK(exphtml_dlg.notebook), gtk_notebook_get_nth_page(GTK_NOTEBOOK(exphtml_dlg.notebook), pageNum), label);
	...
	gtk_table_attach(GTK_TABLE(table), entryHtml, 1, 2, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);
	...
	/* Button handler */
	g_signal_connect(G_OBJECT(btnFile), "clicked", G_CALLBACK(exp_html_file_select), NULL);
...
static void export_html_page_format(gint pageNum, gchar *pageLbl)
{
	...
	gtk_table_attach(GTK_TABLE(table), optmenuCSS, 1, 2, top, (top + 1), GTK_FILL, 0, 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table), optmenuName, 1, 2, top, (top + 1), GTK_FILL, 0, 0, 0);

	/* Third row */
	top++;
	checkBanding = gtk_check_button_new_with_label(_("Color Banding"));
	gtk_table_attach(GTK_TABLE(table), checkBanding, 1, 2, top, (top + 1), GTK_FILL, 0, 0, 0);

	/* Fourth row */
	top++;
	checkLinkEMail = gtk_check_button_new_with_label(_("Format Email Links"));
	gtk_table_attach(GTK_TABLE(table), checkLinkEMail, 1, 2, top, (top + 1), GTK_FILL, 0, 0, 0);

	/* Fifth row */
	top++;
	checkAttributes = gtk_check_button_new_with_label(_("Format User Attributes"));
	gtk_table_attach(GTK_TABLE(table), checkAttributes, 1, 2, top, (top + 1), GTK_FILL, 0, 0, 0);
...
static void export_html_page_finish(gint pageNum, gchar *pageLbl)
{
	...
	/* Button handlers */
	g_signal_connect(G_OBJECT(btnBrowse), "clicked", G_CALLBACK(export_html_browse), NULL);
...
static void export_html_dialog_create(void)
{
	...
	gtk_window_set_title(GTK_WINDOW(window), _("Export Address Book to HTML File"));
	gtk_window_set_position(GTK_WINDOW(window), GTK_WIN_POS_CENTER);
	gtk_window_set_type_hint(GTK_WINDOW(window), GDK_WINDOW_TYPE_HINT_DIALOG);
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(export_html_delete_event), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(export_html_key_pressed), NULL);
	...
	/* Button handlers */
	g_signal_connect(G_OBJECT(btnPrev), "clicked", G_CALLBACK(export_html_prev), NULL);
	g_signal_connect(G_OBJECT(btnNext), "clicked", G_CALLBACK(export_html_next), NULL);
	g_signal_connect(G_OBJECT(btnCancel), "clicked", G_CALLBACK(export_html_cancel), NULL);
	...
	exphtml_dlg.status_cid = gtk_statusbar_get_context_id(GTK_STATUSBAR(statusbar), "Export HTML Dialog");
}
...
static void export_html_fill_fields(ExportHtmlCtl *ctl)
{
	...
	if (ctl->path) {
		gtk_entry_set_text(GTK_ENTRY(exphtml_dlg.entryHtml), ctl->path);
	}

	combobox_select_by_data(GTK_COMBO_BOX(exphtml_dlg.optmenuCSS), ctl->stylesheet);
	combobox_select_by_data(GTK_COMBO_BOX(exphtml_dlg.optmenuName), ctl->nameFormat);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(exphtml_dlg.checkBanding), ctl->banding);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(exphtml_dlg.checkLinkEMail), ctl->linkEMail);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(exphtml_dlg.checkAttributes), ctl->showAttribs);
}
...
void addressbook_exp_html(AddressCache *cache)
{
	...
	gtk_button_set_label(GTK_BUTTON(exphtml_dlg.btnCancel), GTK_STOCK_CANCEL);
