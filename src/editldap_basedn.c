...
static void set_selected()
{
	...
	text = gtkut_tree_view_get_selected_pointer(GTK_TREE_VIEW(view), 0, NULL, NULL, NULL);
...
static void basedn_list_cursor_changed(GtkTreeView *view, gpointer user_data)
{
	set_selected();
}

static void basedn_list_row_activated(GtkTreeView *view, GtkTreePath *path, GtkTreeViewColumn *column, gpointer user_data)
{
	set_selected();
...
static void edit_ldap_bdn_create(void)
{
	...
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(lwindow), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	...
	col = gtk_tree_view_column_new_with_attributes(_("Available Search Base(s)"), rdr, "markup", 0, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(basedn_list), col);

	gtk_container_add(GTK_CONTAINER(lwindow), basedn_list);

	g_signal_connect(G_OBJECT(basedn_list), "cursor-changed", G_CALLBACK(basedn_list_cursor_changed), NULL);
	g_signal_connect(G_OBJECT(basedn_list), "row-activated", G_CALLBACK(basedn_list_row_activated), NULL);
	...
	/* Button panel */
	gtkut_stock_button_set_create(&hbbox, &ok_btn, GTK_STOCK_OK, &cancel_btn, GTK_STOCK_CANCEL, NULL, NULL);
...
static void edit_ldap_bdn_load_data(const gchar *hostName, const gint iPort, const gint tov, const gchar *bindDN, const gchar *bindPW, int ssl, int tls)
{
	gchar *sHost;
	gchar *sMsg = NULL;
	gchar sPort[20];
		...
		if (ldaputil_test_connect(sHost, iPort, ssl, tls, tov)) {
			...
			if (baseDN) {
				...
				while (node) {
					...
					gtk_list_store_set(GTK_LIST_STORE(model), &iter, 0, (gchar *)node->data, -1);
