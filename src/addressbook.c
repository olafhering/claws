...
static void addressbook_tree_selected(GtkCMCTree *ctree, GtkCMCTreeNode *node, gint column, gpointer data);
static void addressbook_select_row_tree(GtkCMCTree *ctree, GtkCMCTreeNode *node, gint column, gpointer data);
static void addressbook_list_row_selected(GtkCMCTree *clist, GtkCMCTreeNode *node, gint column, gpointer data);
static void addressbook_list_row_unselected(GtkCMCTree *clist, GtkCMCTreeNode *node, gint column, gpointer data);
...
static gboolean addressbook_list_button_pressed(GtkWidget *widget, GdkEventButton *event, gpointer data);
static gboolean addressbook_list_button_released(GtkWidget *widget, GdkEventButton *event, gpointer data);
static gboolean addressbook_tree_button_pressed(GtkWidget *ctree, GdkEventButton *event, gpointer data);
static gboolean addressbook_tree_button_released(GtkWidget *ctree, GdkEventButton *event, gpointer data);
...
static GtkCMCTreeNode *addressbook_add_object(GtkCMCTreeNode *node, AddressObject *obj);
...
static GtkCMCTreeNode *addressbook_node_add_folder(GtkCMCTreeNode *node, AddressDataSource *ds, ItemFolder *itemFolder, AddressObjectType otype);
static GtkCMCTreeNode *addressbook_node_add_group(GtkCMCTreeNode *node, AddressDataSource *ds, ItemGroup *itemGroup);
...
static gint addressbook_treenode_compare_func(GtkCMCList *clist, gconstpointer ptr1, gconstpointer ptr2);
static void addressbook_folder_load_one_person(GtkCMCTree *clist, ItemPerson *person, AddressTypeControlItem *atci, AddressTypeControlItem *atciMail);
static void addressbook_folder_remove_node(GtkCMCTree *clist, GtkCMCTreeNode *node);

static void addressbook_edit_address(gpointer data, guint action, GtkWidget *widget, gboolean force_focus);
...
static void addressbook_start_drag(GtkWidget *widget, gint button, GdkEvent *event, void *data);
static void addressbook_drag_data_get(GtkWidget *widget, GdkDragContext *drag_context, GtkSelectionData *selection_data, guint info, guint time, void *data);
static gboolean addressbook_drag_motion_cb(GtkWidget *widget, GdkDragContext *context, gint x, gint y, guint time, void *data);
static void addressbook_drag_leave_cb(GtkWidget *widget, GdkDragContext *context, guint time, void *data);
static void addressbook_drag_received_cb(GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, GtkSelectionData *data, guint info, guint time, void *pdata);
...
void addressbook_open(Compose *target)
{
	...
	if (!addrbook.window) {
		...
		gtk_sctree_select(GTK_SCTREE(addrbook.ctree), GTK_CMCTREE_NODE(GTK_CMCLIST(addrbook.ctree)->row_list));
	} else {
...
void addressbook_refresh(void)
{
	if (addrbook.window) {
		if (addrbook.treeSelected) {
			gtk_sctree_select(GTK_SCTREE(addrbook.ctree), addrbook.treeSelected);
			addressbook_set_clist(gtk_cmctree_node_get_row_data(GTK_CMCTREE(addrbook.ctree), addrbook.treeSelected), TRUE);
 ...
 */
static void addressbook_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation)
{
...
static void addressbook_sort_list(GtkCMCList *clist, const gint col, const GtkSortType sort_type)
{
	gint pos;
	...
	for (pos = 0; pos < N_LIST_COLS; pos++) {
		...
		if (pos == col) {
			arrow = gtk_arrow_new(sort_type == GTK_SORT_ASCENDING ? GTK_ARROW_DOWN : GTK_ARROW_UP, GTK_SHADOW_IN);
...
static gboolean addressbook_address_index_focus_evt_in(GtkWidget *widget, GdkEventFocus *event, gpointer data)
{
	address_index_has_focus = TRUE;
	return FALSE;
}

static gboolean addressbook_address_index_focus_evt_out(GtkWidget *widget, GdkEventFocus *event, gpointer data)
{
	address_index_has_focus = FALSE;
	if (!prefs_common.addressbook_use_editaddress_dialog && !address_list_has_focus)
		addressbook_address_list_disable_some_actions();
	return FALSE;
}

static gboolean addressbook_address_list_focus_evt_in(GtkWidget *widget, GdkEventFocus *event, gpointer data)
{
	address_list_has_focus = TRUE;
	return FALSE;
}

static gboolean addressbook_address_list_focus_evt_out(GtkWidget *widget, GdkEventFocus *event, gpointer data)
{
	address_list_has_focus = FALSE;
	if (!prefs_common.addressbook_use_editaddress_dialog && !address_index_has_focus)
		addressbook_address_list_disable_some_actions();
	return FALSE;
}
...
static void addressbook_create(void)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(addressbook_close), NULL);
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(addressbook_size_allocate_cb), NULL);
	...
	action_group = cm_menu_create_action_group_full(ui_manager, "Menu", addressbook_entries, G_N_ELEMENTS(addressbook_entries), NULL);
	gtk_action_group_add_actions(action_group, addressbook_tree_popup_entries, G_N_ELEMENTS(addressbook_tree_popup_entries), NULL);
	gtk_action_group_add_actions(action_group, addressbook_list_popup_entries, G_N_ELEMENTS(addressbook_list_popup_entries), NULL);
...
/* Tools menu */
	...
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(ctree_swin), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	...
	gtk_cmclist_set_compare_func(GTK_CMCLIST(ctree), addressbook_treenode_compare_func);

	g_signal_connect(G_OBJECT(ctree), "tree_select_row", G_CALLBACK(addressbook_tree_selected), NULL);
	g_signal_connect(G_OBJECT(ctree), "button_press_event", G_CALLBACK(addressbook_tree_button_pressed), NULL);
	g_signal_connect(G_OBJECT(ctree), "button_release_event", G_CALLBACK(addressbook_tree_button_released), NULL);
	/* TEMPORARY */
	g_signal_connect(G_OBJECT(ctree), "select_row", G_CALLBACK(addressbook_select_row_tree), NULL);

	gtk_drag_dest_set(ctree, GTK_DEST_DEFAULT_ALL & ~GTK_DEST_DEFAULT_HIGHLIGHT, addressbook_drag_types, 1, GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_DEFAULT);
	g_signal_connect(G_OBJECT(ctree), "drag_motion", G_CALLBACK(addressbook_drag_motion_cb), ctree);
	g_signal_connect(G_OBJECT(ctree), "drag_leave", G_CALLBACK(addressbook_drag_leave_cb), ctree);
	g_signal_connect(G_OBJECT(ctree), "drag_data_received", G_CALLBACK(addressbook_drag_received_cb), ctree);
	g_signal_connect(G_OBJECT(ctree), "focus_in_event", G_CALLBACK(addressbook_address_index_focus_evt_in), NULL);
	g_signal_connect(G_OBJECT(ctree), "focus_out_event", G_CALLBACK(addressbook_address_index_focus_evt_out), NULL);

	clist_vbox = gtk_vbox_new(FALSE, 4);

	clist_swin = gtk_scrolled_window_new(NULL, NULL);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(clist_swin), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	...
	gtk_cmctree_set_expander_style(GTK_CMCTREE(clist), GTK_CMCTREE_EXPANDER_TRIANGLE);
	gtk_sctree_set_stripes(GTK_SCTREE(ctree), prefs_common.use_stripes_in_summaries);
	gtk_cmctree_set_indent(GTK_CMCTREE(clist), CTREE_INDENT);
	gtk_cmclist_set_column_width(GTK_CMCLIST(clist), COL_NAME, COL_NAME_WIDTH);
	gtk_cmclist_set_column_width(GTK_CMCLIST(clist), COL_ADDRESS, COL_ADDRESS_WIDTH);
	gtk_widget_set_size_request(clist, -1, 80);

	addressbook_sort_list(GTK_CMCLIST(clist), COL_NAME, GTK_SORT_ASCENDING);
	g_signal_connect(G_OBJECT(GTK_CMCLIST(clist)->column[COL_NAME].button), "clicked", G_CALLBACK(addressbook_name_clicked), clist);
	g_signal_connect(G_OBJECT(GTK_CMCLIST(clist)->column[COL_ADDRESS].button), "clicked", G_CALLBACK(addressbook_address_clicked), clist);
	g_signal_connect(G_OBJECT(GTK_CMCLIST(clist)->column[COL_REMARKS].button), "clicked", G_CALLBACK(addressbook_remarks_clicked), clist);
	g_signal_connect(G_OBJECT(clist), "focus_in_event", G_CALLBACK(addressbook_address_list_focus_evt_in), NULL);
	g_signal_connect(G_OBJECT(clist), "focus_out_event", G_CALLBACK(addressbook_address_list_focus_evt_out), NULL);

	for (i = 0; i < N_LIST_COLS; i++)
		gtk_widget_set_can_focus(GTK_CMCLIST(clist)->column[i].button, FALSE);

	g_signal_connect(G_OBJECT(clist), "tree_select_row", G_CALLBACK(addressbook_list_row_selected), NULL);
	g_signal_connect(G_OBJECT(clist), "tree_unselect_row", G_CALLBACK(addressbook_list_row_unselected), NULL);
	g_signal_connect(G_OBJECT(clist), "button_press_event", G_CALLBACK(addressbook_list_button_pressed), NULL);
	g_signal_connect(G_OBJECT(clist), "button_release_event", G_CALLBACK(addressbook_list_button_released), NULL);
	g_signal_connect(G_OBJECT(clist), "tree_expand", G_CALLBACK(addressbook_person_expand_node), NULL);
	g_signal_connect(G_OBJECT(clist), "tree_collapse", G_CALLBACK(addressbook_person_collapse_node), NULL);
	g_signal_connect(G_OBJECT(clist), "start_drag", G_CALLBACK(addressbook_start_drag), NULL);
	g_signal_connect(G_OBJECT(clist), "drag_data_get", G_CALLBACK(addressbook_drag_data_get), NULL);
	hbox = gtk_hbox_new(FALSE, 4);
	...
	g_signal_connect(G_OBJECT(entry), "key_press_event", G_CALLBACK(addressbook_entry_key_pressed), NULL);
	g_signal_connect(G_OBJECT(entry), "activate", G_CALLBACK(addressbook_entry_activated), NULL);
	...
	g_signal_connect(G_OBJECT(help_btn), "clicked", G_CALLBACK(manual_open_with_anchor_cb), MANUAL_ANCHOR_ADDRBOOK);

	g_signal_connect(G_OBJECT(edit_btn), "clicked", G_CALLBACK(addressbook_edit_clicked), NULL);
	g_signal_connect(G_OBJECT(del_btn), "clicked", G_CALLBACK(addressbook_del_clicked), NULL);
	g_signal_connect(G_OBJECT(reg_btn), "clicked", G_CALLBACK(addressbook_reg_clicked), NULL);
	g_signal_connect(G_OBJECT(lup_btn), "clicked", G_CALLBACK(addressbook_lup_clicked), NULL);

	to_btn = gtk_button_new_with_label(prefs_common_translated_header_name("To:"));
	gtk_widget_set_can_default(to_btn, TRUE);
	gtk_box_pack_start(GTK_BOX(hbbox), to_btn, TRUE, TRUE, 0);
	cc_btn = gtk_button_new_with_label(prefs_common_translated_header_name("Cc:"));
	gtk_widget_set_can_default(cc_btn, TRUE);
	gtk_box_pack_start(GTK_BOX(hbbox), cc_btn, TRUE, TRUE, 0);
	bcc_btn = gtk_button_new_with_label(prefs_common_translated_header_name("Bcc:"));
	...
	g_signal_connect(G_OBJECT(to_btn), "clicked", G_CALLBACK(addressbook_to_clicked), GINT_TO_POINTER(COMPOSE_TO));
	g_signal_connect(G_OBJECT(cc_btn), "clicked", G_CALLBACK(addressbook_to_clicked), GINT_TO_POINTER(COMPOSE_CC));
	g_signal_connect(G_OBJECT(bcc_btn), "clicked", G_CALLBACK(addressbook_to_clicked), GINT_TO_POINTER(COMPOSE_BCC));
	g_signal_connect(G_OBJECT(close_btn), "clicked", G_CALLBACK(addressbook_close_clicked), NULL);
	...
	while (nodeIf) {
		...
		if (iface->useInterface) {
			...
			adapter->treeNode = gtk_sctree_insert_node(GTK_CMCTREE(ctree), NULL, NULL, &text, FOLDER_SPACING, interfacexpm, interfacexpm, FALSE, FALSE);
			cm_menu_set_sensitive_full(ui_manager, atci->menuCommand, adapter->haveLibrary);
			gtk_cmctree_node_set_row_data_full(GTK_CMCTREE(ctree), adapter->treeNode, adapter, addressbook_free_treenode);
		}
	}
	...
	MENUITEM_ADDUI_MANAGER(ui_manager, "/Popups", "ABTreePopup", "ABTreePopup", GTK_UI_MANAGER_MENU)
	    ...
	    tree_popup = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(ui_manager, "/Popups/ABTreePopup")));
	...
	MENUITEM_ADDUI_MANAGER(ui_manager, "/Popups", "ABListPopup", "ABListPopup", GTK_UI_MANAGER_MENU)
	    ...
	    list_popup = gtk_menu_item_get_submenu(GTK_MENU_ITEM(gtk_ui_manager_get_widget(ui_manager, "/Popups/ABListPopup")));
	...
	addrbook.ctree_swin = ctree_swin;
	...
	gtk_widget_set_size_request(window, prefs_common.addressbookwin_width, prefs_common.addressbookwin_height);
#ifdef G_OS_WIN32
	...
	*addressbook_msgbuf = '\0';
	if (ds) {
		...
		if (retVal == MGU_SUCCESS) {
			...
			g_snprintf(addressbook_msgbuf, sizeof(addressbook_msgbuf), "%s: %s", name, desc);
		}
	}
...
static void addressbook_del_clicked(GtkButton *button, gpointer data)
{
	...
	iface = ds->interface;
	if (iface->readOnly) {
		alertpanel(_("Delete address(es)"), _("This address data is read-only and cannot be deleted."), GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST);
		return;
	}
	...
	g_signal_handlers_block_by_func(G_OBJECT(addrbook.clist), G_CALLBACK(addressbook_list_row_unselected), NULL);
	...
	if (pobj->type == ADDR_DATASOURCE || pobj->type == ADDR_ITEM_FOLDER) {
		...
		if (group_delete) {
			aval = alertpanel(_("Delete group"), _("Really delete the group(s)?\n" "The addresses it contains will not be lost."), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_SECOND);
		...
		if (refreshList) {
			...
			addressbook_set_clist(gtk_cmctree_node_get_row_data(GTK_CMCTREE(addrbook.ctree), addrbook.opened), TRUE);
		}
 ...
 thaw_ret:
	...
	g_signal_handlers_unblock_by_func(G_OBJECT(addrbook.clist), G_CALLBACK(addressbook_list_row_unselected), NULL);
}
...
static void addressbook_tree_selected(GtkCMCTree *ctree, GtkCMCTreeNode *node, gint column, gpointer data)
{
	...
	/* Update address list */
	g_signal_handlers_block_by_func(G_OBJECT(ctree), G_CALLBACK(addressbook_tree_selected), NULL);
	addressbook_set_clist(obj, FALSE);
	g_signal_handlers_unblock_by_func(G_OBJECT(ctree), G_CALLBACK(addressbook_tree_selected), NULL);
	if (!prefs_common.addressbook_use_editaddress_dialog)
		addressbook_edit_person_invalidate(NULL, NULL, NULL);
...
static void addressbook_list_menu_setup(void)
{
	...
	if (list && list->next) {
		item = list->data;
		aio = (AddrItemObject *)item->addressItem;
		if (aio->type == ITEMTYPE_EMAIL || aio->type == ITEMTYPE_PERSON) {
			canMerge = TRUE;
		}
	}
 ...
 */
static void addressbook_treenode_add_list(GtkCMCTreeNode *treeNode, AddressDataSource *ds, GList *listItems)
{
	GList *node;
	...
	while (node) {
		...
		if (ADDRESS_OBJECT_TYPE(aio) == ADDR_ITEM_GROUP) {
			...
			nn = addressbook_node_add_folder(treeNode, ds, folder, ADDR_ITEM_FOLDER);
			if (nn == NULL) {
...
static void addressbook_clip_paste_cb(GtkAction *action, gpointer data)
{
	...
	if (folderGroup) {
		...
		addressbook_treenode_add_list(addrbook.treeSelected, ds, folderGroup);
...
static void addressbook_list_row_unselected(GtkCMCTree *ctree, GtkCMCTreeNode *node, gint column, gpointer data)
{
	AddrItemObject *aio;
...
static gboolean addressbook_list_button_pressed(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
	if (!event)
		return FALSE;
	if (event->window != GTK_CMCLIST(widget)->clist_window)
		return FALSE;
	...
	if (event->button == 3) {
		...
		if (event->type == GDK_2BUTTON_PRESS) {
			if (prefs_common.add_address_by_click && addrbook.target_compose)
				addressbook_to_clicked(NULL, GINT_TO_POINTER(COMPOSE_TO));
			else if (prefs_common.addressbook_use_editaddress_dialog)
				addressbook_edit_address_cb(NULL, NULL);
			else {
				GtkCMCTree *clist = GTK_CMCTREE(addrbook.clist);
				AddressObject *obj = gtk_cmctree_node_get_row_data(clist, addrbook.listSelected);
...
/*	if( ! event || event->type != GDK_BUTTON_PRESS) return FALSE;*/
	...
	if (event->button == 1) {
		if (event->type == GDK_2BUTTON_PRESS) {
			...
			if (obj->type == ADDR_ITEM_GROUP || obj->type == ADDR_DATASOURCE) {
				/* edit group */
 ...
 just_set_sens:
	...
	if (event->button == 3)
		gtk_menu_popup(GTK_MENU(addrbook.tree_popup), NULL, NULL, NULL, NULL, event->button, event->time);
...
static void addressbook_new_folder_cb(GtkAction *action, gpointer data)
{
	...
	if (obj->type == ADDR_DATASOURCE) {
		if (ADAPTER_DSOURCE(obj)->subType != ADDR_BOOK)
			return;
	} else if (obj->type == ADDR_ITEM_FOLDER) {
...
static void addressbook_change_node_name(GtkCMCTreeNode *node, const gchar *name)
{
	...
	gtk_cmctree_get_node_info(ctree, node, text, &spacing, &pix_cl, &pix_op, &is_leaf, &expanded);
	gtk_cmctree_set_node_info(ctree, node, name, spacing, pix_cl, pix_op, is_leaf, expanded);
}
...
static void addressbook_treenode_edit_cb(GtkAction *action, gpointer data)
{
	...
	if (!addrbook.treeSelected)
		return;
	node = addrbook.treeSelected;
	if (GTK_CMCTREE_ROW(node)->level == 1)
		return;
	obj = gtk_cmctree_node_get_row_data(ctree, node);
	if (obj == NULL)
		return;
...
static void addressbook_treenode_delete_cb(GtkAction *action, gpointer data)
{
	...
	if (obj->type == ADDR_ITEM_FOLDER) {
		if (iface && iface->externalQuery) {
			message = g_strdup_printf(_("Do you want to delete the query " "results and addresses in '%s'?"), obj->name);
			aval = alertpanel(_("Delete"), message, GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_SECOND);
			g_free(message);
			if (aval == G_ALERTALTERNATE) {
				delType = ADDRTREE_DEL_FOLDER_ADDR;
			}
		} else {
			message = g_strdup_printf(_("Do you want to delete '%s'? " "If you delete the folder only, the addresses it contains will be moved into the parent folder."), obj->name);
			aval = alertpanel(_("Delete folder"), message, GTK_STOCK_CANCEL, _("Delete _folder only"), _("Delete folder and _addresses"), ALERTFOCUS_SECOND);
			g_free(message);
			if (aval == G_ALERTALTERNATE) {
		...
		g_free(message);
		if (aval == G_ALERTALTERNATE)
			delType = ADDRTREE_DEL_DATA;
	}
	if (delType == ADDRTREE_DEL_NONE)
		return;
	...
	if (obj->type == ADDR_DATASOURCE) {
		...
		if (delType == ADDRTREE_DEL_DATA && ds->interface && ds->interface->type == ADDR_IF_BOOK)
...
static int addressbook_treenode_find_folder_cb(gconstpointer pA, gconstpointer pB)
{
	...
	if (aoA->type == ADDR_ITEM_FOLDER) {
		...
		if (fld == folder)
			return 0; /* Found folder */
	}
	return 1;
}

static ItemFolder *addressbook_setup_subf(AddressDataSource *ds, gchar *title, GtkCMCTreeNode *pNode)
{
	AddrBookBase *adbase;
	AddressCache *cache;
	ItemFolder *folder;
	...
	if ((children = addrcache_get_list_folder(cache)) != NULL) {
		...
		for (; cur; cur = cur->next) {
			...
			if (!g_strcmp0(ADDRITEM_NAME(child), title)) {
				nNode = gtk_cmctree_find_by_row_data_custom(ctree, NULL, child, addressbook_treenode_find_folder_cb);
				if (nNode) {
...
static void addressbook_new_address_cb(GtkAction *action, gpointer data)
{
	...
	if (pobj->type == ADDR_DATASOURCE) {
		if (ADAPTER_DSOURCE(pobj)->subType == ADDR_BOOK || ADAPTER_DSOURCE(pobj)->subType == ADDR_LDAP) {
			ItemPerson *person;
			ItemFolder *folder = NULL;
#ifdef USE_LDAP
			if (abf && abf->type == ADBOOKTYPE_LDAP) {
				...
				if (!ds->interface->haveLibrary)
					return;
				if (!ds->interface->externalQuery)
					return;
...
#endif
			person = addressbook_edit_person(abf, folder, NULL, FALSE, addrbook.editaddress_vbox, addressbook_new_address_from_book_post_cb, TRUE);
#ifdef USE_LDAP
			if (ds && abf && abf->type == ADBOOKTYPE_LDAP) {
				...
				if (server->retVal != LDAPRC_SUCCESS) {
					alertpanel(_("Add address(es)"), addressbook_err2string(_lutErrorsLDAP_, server->retVal), GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST);
...
#endif
		person = addressbook_edit_person(abf, folder, NULL, FALSE, addrbook.editaddress_vbox, addressbook_new_address_from_folder_post_cb, TRUE);
#ifdef USE_LDAP
		if (ds && abf && abf->type == ADBOOKTYPE_LDAP) {
			LdapServer *server = ds->rawDataSource;
			ldapsvr_set_modified(server, TRUE);
...
static void addressbook_edit_address(gpointer data, guint action, GtkWidget *widget, gboolean force_focus)
{
	...
	if (obj->type == ADDR_ITEM_EMAIL) {
		...
		if (pobj && pobj->type == ADDR_ITEM_GROUP) {
			...
			ItemPerson *person;
			person = (ItemPerson *)ADDRITEM_PARENT(email);
			if (addressbook_edit_person(abf, NULL, person, TRUE, addrbook.editaddress_vbox, addressbook_edit_address_post_cb, (prefs_common.addressbook_use_editaddress_dialog || force_focus))
			    != NULL) {
#ifdef USE_LDAP
				if (abf && abf->type == ADBOOKTYPE_LDAP) {
		...
		if (addressbook_edit_person(abf, NULL, person, FALSE, addrbook.editaddress_vbox, addressbook_edit_address_post_cb, (prefs_common.addressbook_use_editaddress_dialog || force_focus))
		    != NULL) {
#ifdef USE_LDAP
			if (abf && abf->type == ADBOOKTYPE_LDAP) {
				ldapsvr_set_modified((LdapServer *)abf, TRUE);
...
static gboolean addressbook_match_item(const gchar *name, const gchar *email_alias, const gchar *addr, const gchar *remarks, const gchar *str)
{
	if (!name)
		return FALSE;
	if (!str || str[0] == '\0')
...
static void addressbook_load_group(GtkCMCTree *clist, ItemGroup *itemGroup)
{
	...
	for (; items != NULL; items = g_list_next(items)) {
		...
		nodeEMail = gtk_sctree_insert_node(clist, NULL, NULL, text, FOLDER_SPACING, atci->iconXpm, atci->iconXpmOpen, FALSE, FALSE);
...
static void addressbook_folder_load_one_person(GtkCMCTree *clist, ItemPerson *person, AddressTypeControlItem *atci, AddressTypeControlItem *atciMail)
{
	...
	while (node) {
		...
		if (flgFirst) {
...
#ifdef USE_LDAP
			else if (abf && abf->type == ADBOOKTYPE_LDAP && person && person->nickName) {
				if (person->nickName) {
			...
			nodePerson = gtk_sctree_insert_node(clist, NULL, NULL, text, FOLDER_SPACING, atci->iconXpm, atci->iconXpmOpen, FALSE, person->isOpened);
			g_free(str);
			str = NULL;
			...
			nodeEMail = gtk_sctree_insert_node(clist, nodePerson, NULL, text, FOLDER_SPACING, atciMail->iconXpm, atciMail->iconXpmOpen, FALSE, TRUE);
			gtk_cmctree_node_set_row_data(clist, nodeEMail, email);
		}
		flgFirst = FALSE;
		haveAddr = TRUE;
...
static void addressbook_folder_remove_node(GtkCMCTree *clist, GtkCMCTreeNode *node)
{
	...
	addressbook_menubar_set_sensitive(FALSE);
	addressbook_menuitem_set_sensitive(gtk_cmctree_node_get_row_data(GTK_CMCTREE(clist), addrbook.treeSelected), addrbook.treeSelected);
}
...
static void addressbook_folder_load_group(GtkCMCTree *clist, ItemFolder *itemFolder)
{
	...
	for (; items != NULL; items = g_list_next(items)) {
		...
		if (group == NULL)
			continue;
		if (!addressbook_match_item(ADDRITEM_NAME(group), NULL, NULL, NULL, search_str))
			continue;

		text[COL_NAME] = ADDRITEM_NAME(group);
		text[COL_ADDRESS] = "";
		text[COL_REMARKS] = "";
		nodeGroup = gtk_sctree_insert_node(clist, NULL, NULL, text, FOLDER_SPACING, atci->iconXpm, atci->iconXpmOpen, FALSE, FALSE);
...
static void addressbook_treenode_remove_item(void)
{
	...
	while (node) {
		cutItem = node->data;
		node = g_list_next(node);
		cache = addrindex_get_cache(_clipBoard_->addressIndex, cutItem->cacheID);
		if (cache == NULL)
			continue;
		...
		if (aio) {
			...
			if (ADDRITEM_TYPE(aio) == ITEMTYPE_FOLDER) {
				...
				folder = (ItemFolder *)aio;
				tn = gtk_cmctree_find_by_row_data_custom(ctree, NULL, folder, addressbook_treenode_find_folder_cb);
			} else if (ADDRITEM_TYPE(aio) == ITEMTYPE_GROUP) {
				ItemGroup *group;

				group = (ItemGroup *)aio;
				tn = gtk_cmctree_find_by_row_data_custom(ctree, NULL, group, addressbook_treenode_find_group_cb);
			}
...
static AddressDataSource *addressbook_find_datasource(GtkCMCTreeNode *node)
{
	...
	while (node) {
		if (GTK_CMCTREE_ROW(node)->level < 2)
			return NULL;
...
static void addressbook_set_clist(AddressObject *obj, gboolean refresh)
{
	...
	if (obj->type == ADDR_DATASOURCE) {
		...
		if (ds) {
			...
			ItemFolder *rootFolder = NULL;
			rootFolder = addrindex_ds_get_root_folder(ds);
			addressbook_folder_load_person(ctreelist, rootFolder);
			addressbook_folder_load_group(ctreelist, rootFolder);
		}
	} else {
...
static gboolean addressbook_convert(AddressIndex *addrIndex)
{
	...
	if (errFlag) {
		debug_print("Error\n%s\n", msg);
		alertpanel_full(_("Addressbook conversion error"), msg, GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_ERROR);
	} else if (msg) {
		debug_print("Warning\n%s\n", msg);
		alertpanel_full(_("Addressbook conversion error"), msg, GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_WARNING);
	}
...
static gboolean migrate_addrbook(const gchar *origdir, const gchar *destdir)
{
	...
	if ((dp = g_dir_open(origdir, 0, &error)) == NULL) {
		debug_print("opening '%s' failed: %d (%s)\n", origdir, error->code, error->message);
		g_error_free(error);
		return FALSE;
	}
...
void addressbook_read_file(void)
{
	...
	if (addrIndex->retVal == MGU_NO_FILE) {
		...
		alertpanel_full(_("Addressbook Error"), _("Could not read address index"), GTK_STOCK_CLOSE, NULL, NULL, ALERTFOCUS_FIRST, FALSE, NULL, ALERT_ERROR);
	}
	debug_print("done.\n");
}
...
static GtkCMCTreeNode *addressbook_add_object(GtkCMCTreeNode *node, AddressObject *obj)
{
	...
	if (atci) {
		if (atci->showInTree) {
			...
			gchar **name;
			name = &obj->name;
			added = gtk_sctree_insert_node(ctree, node, NULL, name, FOLDER_SPACING, atci->iconXpm, atci->iconXpmOpen, atci->treeLeaf, atci->treeExpand);
			gtk_cmctree_node_set_row_data_full(ctree, added, obj, addressbook_free_treenode);
		}
	}
...
static GtkCMCTreeNode *addressbook_node_add_group(GtkCMCTreeNode *node, AddressDataSource *ds, ItemGroup *itemGroup)
{
	...
	newNode = gtk_sctree_insert_node(ctree, node, NULL, name, FOLDER_SPACING, atci->iconXpm, atci->iconXpm, atci->treeLeaf, atci->treeExpand);
	gtk_cmctree_node_set_row_data_full(ctree, newNode, adapter, addressbook_free_treenode);
	gtk_sctree_sort_node(ctree, node);
	return newNode;
}
...
static void addressbook_search_message(gint queryType, gint sts)
{
	...
	*addressbook_msgbuf = '\0';
	...
	if (desc) {
		g_snprintf(addressbook_msgbuf, sizeof(addressbook_msgbuf), "%s", desc);
		addressbook_status_show(addressbook_msgbuf);
	} else {
		addressbook_status_show("");
	}
 ...
 */
static void addressbook_search_callback_end(gpointer sender, gint queryID, gint status, gpointer data)
{
	gpointer ptrQID;
	QueryRequest *req;
	AddrQueryObject *aqo;
 ...
 */
static void addressbook_perform_search(AddressDataSource *ds, gchar *searchTerm, GtkCMCTreeNode *pNode)
{
	ItemFolder *folder;
	gchar *name;
	gint queryID;
	...
	/* Setup a query */
	if (*searchTerm == '\0' || strlen(searchTerm) < 1)
		return;
	...
	/* Setup the search */
	queryID = addrindex_setup_explicit_search(ds, searchTerm, folder, addressbook_search_callback_end, NULL);
	if (queryID == 0)
		return;

	/* Set up idler function */
	idleID = g_idle_add((GSourceFunc) addressbook_search_idle, GINT_TO_POINTER(queryID));
	if (idleID == 0) {
...
static void addressbook_lup_clicked(GtkButton *button, gpointer data)
{
	...
	if (obj->type != ADDR_DATASOURCE || ADAPTER_DSOURCE(obj)->subType != ADDR_LDAP) {
		addressbook_set_clist(gtk_cmctree_node_get_row_data(GTK_CMCTREE(addrbook.ctree), addrbook.treeSelected), TRUE);
	}
	...
	if (iface->type == ADDR_IF_LDAP) {
		...
		if (ldap_ctl != NULL && ldap_ctl->bindDN != NULL && strlen(ldap_ctl->bindDN) > 0) {
	...
	searchTerm = gtk_editable_get_chars(GTK_EDITABLE(addrbook.entry), 0, -1);
...
ItemObjectType addressbook_type2item(AddressObjectType abType)
{
	...
	switch (abType) {
	case ADDR_ITEM_PERSON:
		ioType = ITEMTYPE_PERSON;
		break;
	case ADDR_ITEM_EMAIL:
		ioType = ITEMTYPE_EMAIL;
		break;
	case ADDR_ITEM_FOLDER:
		ioType = ITEMTYPE_FOLDER;
		break;
	case ADDR_ITEM_GROUP:
		ioType = ITEMTYPE_GROUP;
		break;
	case ADDR_DATASOURCE:
		ioType = ITEMTYPE_DATASOURCE;
		break;
	default:
...
static AdapterInterface *addrbookctl_find_interface(AddressIfType ifType)
{
	...
	while (node) {
		...
		if (adapter->interfaceType == ifType)
			return adapter;
		node = g_list_next(node);
	}
	return NULL;
 ...
 */
gboolean addressbook_add_contact(const gchar *name, const gchar *address, const gchar *remarks, GdkPixbuf *picture)
{
...
static void addressbook_peek_subfolder_exists_load_folder(ItemFolder *parentFolder, FolderInfo *fiParent, FolderPathMatch *match)
{
	GList *list;
	ItemFolder *folder;
	gchar *fName;
...
gboolean addressbook_peek_folder_exists(gchar *folderpath, AddressDataSource **book, ItemFolder **folder)
{
	AddressDataSource *ds;
	GList *list, *nodeDS;
	ItemFolder *rootFolder;
	...
	while (list && !folder_path_match.matched) {
		...
		if (interface && interface->type == ADDR_IF_BOOK) {
			...
			while (nodeDS && !folder_path_match.matched) {
				...
				/* match book name */
				if (abf && abf->fileName && strcmp(folder_path_match.folder_path[0], abf->fileName) == 0) {
...
void addressbook_harvest(FolderItem *folderItem, gboolean sourceInd, GList *msgList)
{
	...
	abf = addrgather_dlg_execute(folderItem, _addressIndex_, sourceInd, msgList);
	if (abf) {
		ds = addrindex_index_add_datasource(_addressIndex_, ADDR_IF_BOOK, abf);
		...
		if (adapter) {
			if (adapter->treeNode) {
				ads = addressbook_create_ds_adapter(ds, ADDR_BOOK, addrbook_get_name(abf));
				newNode = addressbook_add_object(adapter->treeNode, ADDRESS_OBJECT(ads));
				if (newNode == NULL) {
					g_message("error adding addressbook object\n");
				}
...
static void addressbook_start_drag(GtkWidget *widget, gint button, GdkEvent *event, void *data)
{
	...
	if (addressbook_target_list == NULL)
		addressbook_target_list = gtk_target_list_new(addressbook_drag_types, 1);
	context = gtk_drag_begin(widget, addressbook_target_list, GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_DEFAULT, button, event);
...
static void addressbook_drag_data_get(GtkWidget *widget, GdkDragContext *drag_context, GtkSelectionData *selection_data, guint info, guint time, void *data)
{
	...
	for (cur = GTK_CMCLIST(addrbook.clist)->selection; cur; cur = cur->next) {
		aio = (AddrItemObject *)gtk_cmctree_node_get_row_data(GTK_CMCTREE(addrbook.clist), GTK_CMCTREE_NODE(cur->data));
	...
	if (aio && aio->type == ITEMTYPE_PERSON) {
		if (ds && ds->interface && ds->interface->readOnly)
			gtk_selection_data_set(selection_data, gtk_selection_data_get_target(selection_data), 8, (const guchar *)"Dummy_addr_copy", 15);
		else
			gtk_selection_data_set(selection_data, gtk_selection_data_get_target(selection_data), 8, (const guchar *)"Dummy_addr_move", 15);
	}
}
...
static gboolean addressbook_drag_motion_cb(GtkWidget *widget, GdkDragContext *context, gint x, gint y, guint time, void *data)
{
	...
	GtkAdjustment *pos = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(addrbook.ctree_swin));
	gfloat vpos = gtk_adjustment_get_value(pos);

	if (gtk_cmclist_get_selection_info(GTK_CMCLIST(widget), x - 24, y - 24, &row, &column)) {
		...
		if (node != NULL) {
			...
			if (obj == NULL)
				return FALSE;
			if (obj->type == ADDR_ITEM_FOLDER || obj->type == ADDR_ITEM_GROUP)
				acceptable = TRUE;
			else {
				AdapterDSource *ads = NULL;
				AddressDataSource *ds = NULL;
	...
	if (acceptable) {
		g_signal_handlers_block_by_func(G_OBJECT(widget), G_CALLBACK(addressbook_tree_selected), NULL);
		gtk_sctree_select(GTK_SCTREE(widget), node);
		g_signal_handlers_unblock_by_func(G_OBJECT(widget), G_CALLBACK(addressbook_tree_selected), NULL);
...
static void addressbook_drag_received_cb(GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y, GtkSelectionData *data, guint info, guint time, void *pdata)
{
	...
	if (!strncmp(gtk_selection_data_get_data(data), "Dummy_addr", 10)) {
		if (gtk_cmclist_get_selection_info(GTK_CMCLIST(widget), x - 24, y - 24, &row, &column) == 0) {
			return;
		}
		...
		gtk_cmclist_freeze(GTK_CMCLIST(addrbook.clist));
		if (gdk_drag_context_get_selected_action(drag_context) == GDK_ACTION_COPY || !strcmp(gtk_selection_data_get_data(data), "Dummy_addr_copy"))
