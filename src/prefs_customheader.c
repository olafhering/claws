...
static void prefs_custom_header_list_view_insert_header(GtkWidget *list_view, GtkTreeIter *row_iter, gchar *header, gpointer data);
...
static void prefs_custom_header_create(void)
{
	...
	gtk_table_attach(GTK_TABLE(table1), hdr_label, 0, 1, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);
	gtk_misc_set_alignment(GTK_MISC(hdr_label), 0, 0.5);

	hdr_combo = combobox_text_new(TRUE, "User-Agent", "Face", "X-Face", "X-Operating-System", NULL);
	gtk_table_attach(GTK_TABLE(table1), hdr_combo, 0, 1, 1, 2, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	val_label = gtk_label_new(_("Value"));
	gtk_widget_show(val_label);
	gtk_table_attach(GTK_TABLE(table1), val_label, 1, 2, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);
	gtk_misc_set_alignment(GTK_MISC(val_label), 0, 0.5);

	val_entry = gtk_entry_new();
	gtk_widget_show(val_entry);
	gtk_table_attach(GTK_TABLE(table1), val_entry, 1, 2, 1, 2, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	val_btn = gtkut_get_browse_file_btn(_("Bro_wse"));
	gtk_widget_show(val_btn);
	gtk_table_attach(GTK_TABLE(table1), val_btn, 2, 3, 1, 2, GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);
	g_signal_connect(G_OBJECT(val_btn), "clicked", G_CALLBACK(prefs_custom_header_val_from_file_cb), NULL);
...
void prefs_custom_header_read_config(PrefsAccount *ac)
{
	...
	rcpath = g_strconcat(get_rc_dir(), G_DIR_SEPARATOR_S, CUSTOM_HEADER_RC, NULL);
...
static void prefs_custom_header_write_config(PrefsAccount *ac)
{
	...
	if ((fp = claws_fopen(rcpath, "rb")) == NULL) {
		...
		while (claws_fgets(buf, sizeof(buf), fp) != NULL) {
			...
			if (ch) {
				if (ch->account_id != ac->account_id)
					all_hdrs = g_slist_append(all_hdrs, ch);
				else
					custom_header_free(ch);
			}
		}
	...
	for (cur = all_hdrs; cur != NULL; cur = cur->next) {
		...
		if (claws_fputs(chstr, pfile->fp) == EOF || claws_fputc('\n', pfile->fp) == EOF) {
...
static void prefs_custom_header_set_dialog(PrefsAccount *ac)
{
	...
	store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(customhdr.list_view)));
	...
	for (cur = ac->customhdr_list; cur != NULL; cur = cur->next) {
		...
		prefs_custom_header_list_view_insert_header(customhdr.list_view, NULL, ch_str, ch);
...
static void prefs_custom_header_set_list(PrefsAccount *ac)
{
	...
	if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(store), &iter)) {
		do {
			gtk_tree_model_get(GTK_TREE_MODEL(store), &iter, CUSTHDR_DATA, &ch, -1);
			ac->customhdr_list = g_slist_append(ac->customhdr_list, ch);
...
static void prefs_custom_header_val_from_file_cb(void)
{
	...
	if (!strcmp(hdr, "Face") || !strcmp(hdr, "X-Face")) {
		if (filename && is_file_exist(filename)) {
			...
			gchar *tmp = NULL;
			gint w, h;
			GdkPixbufFormat *format = gdk_pixbuf_get_file_info(filename, &w, &h);
			...
			while ((len = claws_fread(inbuf, sizeof(gchar), B64_LINE_SIZE, fp))
			       == B64_LINE_SIZE) {
...
static void prefs_custom_header_delete_cb(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(customhdr.list_view)), &model, &sel))
		return;

	if (alertpanel(_("Delete header"), _("Do you really want to delete this header?"), GTK_STOCK_CANCEL, GTK_STOCK_DELETE, NULL, ALERTFOCUS_FIRST) != G_ALERTALTERNATE)
		return;

	gtk_tree_model_get(model, &sel, CUSTHDR_DATA, &ch, -1);
...
static void prefs_custom_header_up(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(customhdr.list_view)), &model, &isel))
		return;
...
static GtkListStore *prefs_custom_header_create_data_store(void)
{
	return gtk_list_store_new(N_CUSTHDR_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER, -1);
}
...
static void prefs_custom_header_list_view_insert_header(GtkWidget *list_view, GtkTreeIter *row_iter, gchar *header, gpointer data)
{
	...
	if (row_iter == NULL) {
		...
		gtk_list_store_set(list_store, &iter, CUSTHDR_STRING, header, CUSTHDR_DATA, data, -1);
	} else {
		/* change existing */
		CustomHeader *old_data;

		gtk_tree_model_get(GTK_TREE_MODEL(list_store), row_iter, CUSTHDR_DATA, &old_data, -1);

		custom_header_free(old_data);

		gtk_list_store_set(list_store, row_iter, CUSTHDR_STRING, header, CUSTHDR_DATA, data, -1);
	}
}
...
static GtkWidget *prefs_custom_header_list_view_create(void)
{
	...
	gtk_tree_selection_set_select_function(selector, prefs_custom_header_selected, NULL, NULL);
...
static void prefs_custom_header_create_list_view_columns(GtkWidget *list_view)
{
	...
	column = gtk_tree_view_column_new_with_attributes(_("Current custom headers"), renderer, "text", CUSTHDR_STRING, NULL);
...
static gboolean prefs_custom_header_selected(GtkTreeSelection *selector, GtkTreeModel *model, GtkTreePath *path, gboolean currently_selected, gpointer data)
{
	...
	gtk_tree_model_get(model, &iter, CUSTHDR_DATA, &ch, -1);
