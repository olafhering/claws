...
static void set_selected_ptr()
{
	...
	addressbook_foldersel_dlg.fiSelected = gtkut_tree_view_get_selected_pointer(GTK_TREE_VIEW(view), COL_PTR, NULL, NULL, NULL);
}
...
static void addressbook_foldersel_row_activated(GtkTreeView *view, GtkTreePath *path, GtkTreeViewColumn *col, gpointer user_data)
{
	addressbook_foldersel_ok(NULL, NULL);
}

static void addressbook_foldersel_size_allocate_cb(GtkWidget *widget, GtkAllocation *allocation)
{
...
static void addressbook_foldersel_create(void)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(addressbook_foldersel_delete_event), NULL);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(addressbook_foldersel_key_pressed), NULL);
	g_signal_connect(G_OBJECT(window), "size_allocate", G_CALLBACK(addressbook_foldersel_size_allocate_cb), NULL);
	...
	store = gtk_tree_store_new(N_COLS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_POINTER);
	...
	gtk_tree_view_column_set_attributes(col, rdr, "pixbuf", COL_ICON, NULL);
	rdr = gtk_cell_renderer_text_new();
	gtk_tree_view_column_pack_start(col, rdr, TRUE);
	gtk_tree_view_column_set_attributes(col, rdr, "markup", COL_NAME, NULL);
	...
	g_signal_connect(G_OBJECT(view), "row-activated", G_CALLBACK(addressbook_foldersel_row_activated), NULL);
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(addressbook_foldersel_ok), NULL);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(addressbook_foldersel_cancel), NULL);
	...
	gtk_widget_set_size_request(window, prefs_common.addressbook_folderselwin_width, prefs_common.addressbook_folderselwin_height);
...
static gboolean tree_clear_foreach_func(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer user_data)
{
	FolderInfo *fi;
...
static void addressbook_foldersel_load_folder(GtkTreeIter *parent_iter, ItemFolder *parentFolder, FolderInfo *fiParent)
{
	GtkWidget *view = addressbook_foldersel_dlg.view_folder;
	GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(view));
	GtkTreeIter iter;
	...
	while (list) {
		...
		gtk_tree_store_set(GTK_TREE_STORE(model), &iter, COL_ICON, folderXpm, COL_NAME, name, COL_PTR, fi, -1);
		g_free(name);
	...
	while (list) {
		...
		if (interface->type == ADDR_IF_BOOK) {
			...
			while (nodeDS) {
				...
				gtk_tree_store_set(GTK_TREE_STORE(model), &iter, COL_ICON, bookXpm, COL_NAME, name, COL_PTR, fi, -1);
				g_free(name);
