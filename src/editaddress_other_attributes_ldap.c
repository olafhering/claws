...
static void edit_person_combo_box_changed(GtkComboBox *opt_menu, gpointer data)
{
	...
	UserAttribute *attrib = gtkut_tree_view_get_selected_pointer(GTK_TREE_VIEW(view), ATTRIB_COL_PTR, NULL, NULL, NULL);
...
static void edit_person_attrib_add(gpointer data)
{
	...
	GtkTreeSelection *sel = gtk_tree_view_get_selection(GTK_TREE_VIEW(personEditDlg->view_attrib));
...
void addressbook_edit_person_page_attrib_ldap(PersonEditDlg *dialog, gint pageNum, gchar *pageLbl)
{
	...
	g_signal_connect(G_OBJECT(combo_box), "changed", G_CALLBACK(edit_person_entry_att_changed), NULL);
	g_signal_connect(G_OBJECT(entry_value), "key_press_event", G_CALLBACK(edit_person_entry_att_pressed), NULL);
	g_signal_connect(G_OBJECT(combo_box), "changed", G_CALLBACK(edit_person_combo_box_changed), view);
