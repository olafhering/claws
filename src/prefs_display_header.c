...
/* callback functions */
static void prefs_display_header_register_cb(GtkButton *btn, gpointer hidden_data);
...
static void prefs_display_header_insert_header(GtkListStore *store, gchar *name, DisplayHeaderProp *dp);
...
static void headers_list_model_rows_reordered(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer arg, GtkTreeView *list_view);
...
static void prefs_display_header_create(void)
{
	...
	gtk_window_set_title(GTK_WINDOW(window), _("Displayed header configuration"));
	...
	for (i = 0; i < 9; i++)
		gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(hdr_combo), (*defaults[i] == '-') ? defaults[i] + 1 : defaults[i]);
	...
	hidden_headers_list_view = prefs_display_header_list_view_create(_("Hidden headers"));
	gtk_widget_show(hidden_headers_list_view);
	gtk_container_add(GTK_CONTAINER(list_view_scrolledwin), hidden_headers_list_view);
	...
	g_signal_connect(G_OBJECT(del_btn), "clicked", G_CALLBACK(prefs_display_header_delete_cb), hidden_headers_list_view);
	...
	headers_list_view = prefs_display_header_list_view_create(_("Displayed Headers"));
	...
	g_signal_connect(G_OBJECT(headers_list_view), "drag_end", G_CALLBACK(drag_end), headers_list_view);

	/* connect rows change for this list view's model */
	g_signal_connect(G_OBJECT(gtk_tree_view_get_model(GTK_TREE_VIEW(headers_list_view))), "rows-reordered", G_CALLBACK(headers_list_model_rows_reordered), headers_list_view);
	...
	gtk_widget_show(reg_btn);
	gtk_box_pack_start(GTK_BOX(btn_vbox), reg_btn, FALSE, TRUE, 0);
	g_signal_connect(G_OBJECT(reg_btn), "clicked", G_CALLBACK(prefs_display_header_register_cb), GINT_TO_POINTER(FALSE));
	del_btn = gtk_button_new_from_stock(GTK_STOCK_REMOVE);
	gtk_widget_show(del_btn);
	gtk_box_pack_start(GTK_BOX(btn_vbox), del_btn, FALSE, TRUE, 0);
	g_signal_connect(G_OBJECT(del_btn), "clicked", G_CALLBACK(prefs_display_header_delete_cb), headers_list_view);
	...
	PACK_CHECK_BUTTON(vbox, checkbtn_other_headers, _("Show all unspecified headers"));
...
static void prefs_display_header_write_config(void)
{
	...
	for (cur = prefs_common.disphdr_list; cur != NULL; cur = cur->next) {
		...
		if (claws_fputs(dpstr, pfile->fp) == EOF || claws_fputc('\n', pfile->fp) == EOF) {
...
static void prefs_display_header_set_dialog(void)
{
	...
	for (cur = prefs_common.disphdr_list; cur != NULL; cur = cur->next) {
		...
		if (dp->hidden)
			prefs_display_header_insert_header(GTK_LIST_STORE(model_hidden), dp->name, dp);
		else
			prefs_display_header_insert_header(GTK_LIST_STORE(model_list), dp->name, dp);
	}
...
static void prefs_display_header_set_list(void)
{
	...
	model = gtk_tree_view_get_model(GTK_TREE_VIEW(dispheader.hidden_headers_list_view));
	row = 0;
	while (gtk_tree_model_iter_nth_child(model, &iter, NULL, row)) {
		gtk_tree_model_get(model, &iter, PREFS_HDR_DATA, &dp, -1);
		if (dp)
...
static gint prefs_display_header_find_header(GtkTreeView *list_view, const gchar *header)
{
	gint row = 0;
	DisplayHeaderProp *dp;
	GtkTreeModel *model;
...
static void prefs_display_header_list_view_set_row(gboolean hidden)
{
	...
	if (!entry_text)
		entry_text = gtk_editable_get_chars(GTK_EDITABLE(gtk_bin_get_child(GTK_BIN(dispheader.hdr_combo))), 0, -1);
	...
	model = gtk_tree_view_get_model(list_view);
	prefs_display_header_insert_header(GTK_LIST_STORE(model), dp->name, dp);
...
static void prefs_display_header_up(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(dispheader.headers_list_view)), &model, &isel))
		return;
...
static void prefs_display_header_down(void)
{
	...
	if (!gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(dispheader.headers_list_view)), &model, &sel))
		return;
	store = (GtkListStore *)model;
...
static GtkListStore *prefs_display_header_create_store(void)
{
	return gtk_list_store_new(N_PREFS_HDR_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER, -1);
}
...
static void prefs_display_header_insert_header(GtkListStore *store, gchar *name, DisplayHeaderProp *dp)
{
	...
	gtk_list_store_set(store, &iter, PREFS_HDR_HEADER, name, PREFS_HDR_DATA, dp, -1);
}
...
static void prefs_filtering_create_list_view_columns(GtkWidget *list_view, const gchar *name)
{
	...
	column = gtk_tree_view_column_new_with_attributes(name, renderer, "text", PREFS_HDR_HEADER, NULL);
