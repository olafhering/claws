 ...
 */
static void _populate_iter(GtkListStore *store, GtkTreeIter *iter, Ldif_FieldRec *rec)
{
	gtk_list_store_set(store, iter, FIELD_COL_FIELD, rec->tagName, FIELD_COL_ATTRIB, rec->userName, FIELD_COL_PTR, rec, -1);
	gtk_list_store_set(store, iter, FIELD_COL_SELECT, rec->selected ? markxpm : NULL, -1);
	gtk_list_store_set(store, iter, FIELD_COL_RESERVED, rec->reserved ? markxpm : NULL, -1);
}
...
static void _update_selected_row()
{
	...
	rec = gtkut_tree_view_get_selected_pointer(GTK_TREE_VIEW(impldif_dlg.view_fields), FIELD_COL_PTR, &model, NULL, &iter);
	if (rec == NULL)
		return;

	ldif_field_set_name(rec, gtk_entry_get_text(GTK_ENTRY(impldif_dlg.entryAttrib)));
	ldif_field_set_selected(rec, gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(impldif_dlg.checkSelect)));
...
static void imp_ldif_page_file(gint pageNum, gchar *pageLbl)
{
	...
	gtk_notebook_set_tab_label(GTK_NOTEBOOK(impldif_dlg.notebook), gtk_notebook_get_nth_page(GTK_NOTEBOOK(impldif_dlg.notebook), pageNum), label);
	...
	gtk_table_attach(GTK_TABLE(table), entryName, 1, 2, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(entryName, _("Specify the name for the address book that will " "be created from the LDIF file data."));
	...
	gtk_table_attach(GTK_TABLE(table), entryFile, 1, 2, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(entryFile, _("The full file specification of the LDIF file to import."));

	btnFile = gtkut_get_browse_file_btn(_("B_rowse"));
	gtk_table_attach(GTK_TABLE(table), btnFile, 2, 3, top, (top + 1), GTK_FILL, 0, 3, 0);

	CLAWS_SET_TIP(btnFile, _("Select the LDIF file to import."));

	gtk_widget_show_all(vbox);

	/* Button handler */
	g_signal_connect(G_OBJECT(btnFile), "clicked", G_CALLBACK(imp_ldif_file_select), NULL);
...
static void imp_ldif_field_list_cursor_changed(GtkTreeView *view, gpointer user_data)
{
	Ldif_FieldRec *rec;

	gtk_entry_set_text(GTK_ENTRY(impldif_dlg.entryAttrib), "");

	rec = gtkut_tree_view_get_selected_pointer(view, FIELD_COL_PTR, NULL, NULL, NULL);

	if (rec != NULL) {
		/* Update widget contents */
		gtk_label_set_text(GTK_LABEL(impldif_dlg.entryField), rec->tagName);
		if (rec->userName)
			gtk_entry_set_text(GTK_ENTRY(impldif_dlg.entryAttrib), rec->userName);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(impldif_dlg.checkSelect), rec->selected);

		/* Disable widgets for reserved fields */
		gtk_widget_set_sensitive(impldif_dlg.entryAttrib, !rec->reserved);
		gtk_widget_set_sensitive(impldif_dlg.checkSelect, !rec->reserved);
		gtk_widget_set_sensitive(impldif_dlg.btnModify, !rec->reserved);
	}
	gtk_widget_grab_focus(impldif_dlg.entryAttrib);
}

static void imp_ldif_field_list_row_activated(GtkTreeView *view, GtkTreePath *path, GtkTreeViewColumn *col, gpointer user_data)
{
	...
	gtk_list_store_set(GTK_LIST_STORE(model), &iter, FIELD_COL_SELECT, rec->selected ? markxpm : NULL, -1);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(impldif_dlg.checkSelect), rec->selected);
}
...
static void imp_ldif_page_fields(gint pageNum, gchar *pageLbl)
{
	...
	store = gtk_list_store_new(FIELDS_N_COLS, GDK_TYPE_PIXBUF, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER, -1);
	...
	col = gtk_tree_view_column_new_with_attributes(_("R"), rdr, "pixbuf", FIELD_COL_RESERVED, NULL);
	gtk_tree_view_column_set_min_width(col, FIELD_COL_RESERVED);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_fields), col);

	col = gtk_tree_view_column_new_with_attributes(_("S"), rdr, "pixbuf", FIELD_COL_SELECT, NULL);
	gtk_tree_view_column_set_min_width(col, FIELD_COL_SELECT);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_fields), col);

	rdr = gtk_cell_renderer_text_new();
	col = gtk_tree_view_column_new_with_attributes(_("LDIF Field Name"), rdr, "markup", FIELD_COL_FIELD, NULL);
	gtk_tree_view_column_set_min_width(col, FIELD_COL_FIELD);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view_fields), col);

	col = gtk_tree_view_column_new_with_attributes(_("Attribute Name"), rdr, "markup", FIELD_COL_ATTRIB, NULL);
	...
	gtk_table_attach(GTK_TABLE(table), entryField, 1, 3, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);
	...
	gtk_misc_set_alignment(GTK_MISC(label), 0, 0.5);
	gtk_table_attach(GTK_TABLE(table), eventBox, 0, 1, top, (top + 1), GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(eventBox, _("Choose the LDIF field that will be renamed or selected " "for import in the list above. Reserved fields (marked " "with a tick in the \"R\" column), are automatically " "imported and cannot be renamed. A single click in the " "Select (\"S\") column will select the field for import " "with a tick. A single click anywhere in the row will " "select that field for rename in the input area below " "the list. A double click anywhere in the row will also " "select the field for import."));

	entryAttrib = gtk_entry_new();
	gtk_table_attach(GTK_TABLE(table), entryAttrib, 1, 3, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(entryAttrib, _("The LDIF field can be renamed to the User Attribute name."));

	/* Next row */
	++top;

	checkSelect = gtk_check_button_new_with_label(_("Select for Import"));
	gtk_table_attach(GTK_TABLE(table), checkSelect, 1, 2, top, (top + 1), GTK_EXPAND | GTK_SHRINK | GTK_FILL, 0, 0, 0);

	CLAWS_SET_TIP(checkSelect, _("Select the LDIF field for import into the address book."));

	btnModify = gtk_button_new_with_label(_(" Modify "));
	gtk_table_attach(GTK_TABLE(table), btnModify, 2, 3, top, (top + 1), GTK_FILL, 0, 3, 0);

	CLAWS_SET_TIP(btnModify, _("This button will update the list above with the data supplied."));

	gtk_widget_show_all(vbox);

	/* Event handlers */
	g_signal_connect(G_OBJECT(view_fields), "cursor-changed", G_CALLBACK(imp_ldif_field_list_cursor_changed), NULL);
	g_signal_connect(G_OBJECT(view_fields), "row-activated", G_CALLBACK(imp_ldif_field_list_row_activated), NULL);
	g_signal_connect(G_OBJECT(btnModify), "clicked", G_CALLBACK(imp_ldif_modify_pressed), NULL);
	g_signal_connect(G_OBJECT(entryAttrib), "activate", G_CALLBACK(imp_ldif_entryattrib_activate), NULL);
...
static void imp_ldif_dialog_create()
{
	...
	/* Button panel */
	gtkut_stock_button_set_create(&hbbox, &btnCancel, GTK_STOCK_CANCEL, &btnPrev, GTK_STOCK_GO_BACK, &btnNext, GTK_STOCK_GO_FORWARD);

	btnProceed = gtk_button_new_with_mnemonic(_("Proceed"));
	gtk_button_set_image(GTK_BUTTON(btnProceed), gtk_image_new_from_stock(GTK_STOCK_OK, GTK_ICON_SIZE_BUTTON));
	...
	g_signal_connect(G_OBJECT(btnProceed), "clicked", G_CALLBACK(imp_ldif_next), NULL);
