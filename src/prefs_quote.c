...
static void prefs_quote_set_default_new_msg_fmt(void)
{
	...
	pref_set_textview_from_pref(GTK_TEXT_VIEW(prefs_quote->text_format), _("Hello,\\n"));
}
...
static void prefs_quote_set_default_reply_fmt(void)
{
	...
	pref_set_textview_from_pref(GTK_TEXT_VIEW(prefs_quote->text_quotefmt), _("On %d\\n%f wrote:\\n\\n%q"));
}
...
static void prefs_quote_set_default_forward_fmt(void)
{
	...
	pref_set_textview_from_pref(GTK_TEXT_VIEW(prefs_quote->text_fw_quotefmt), _("\\n\\nBegin forwarded message:\\n\\n" "?d{Date: %d\\n}?f{From: %f\\n}?t{To: %t\\n}?c{Cc: %c\\n}" "?n{Newsgroups: %n\\n}?s{Subject: %s\\n}\\n\\n%M"));
}

static void prefs_quote_create_widget(PrefsPage *_page, GtkWindow *window, gpointer data)
{
	...
	quotefmt_create_new_msg_fmt_widgets(window, vbox2, &prefs_quote->checkbtn_compose_with_format, NULL, &prefs_quote->entry_subject, &prefs_quote->text_format, TRUE, prefs_quote_set_default_new_msg_fmt);
	...
	quotefmt_create_reply_fmt_widgets(window, vbox2, NULL, NULL, &prefs_quote->entry_quotemark, &prefs_quote->text_quotefmt, TRUE, prefs_quote_set_default_reply_fmt);
	...
	quotefmt_create_forward_fmt_widgets(window, vbox2, NULL, NULL, &prefs_quote->entry_fw_quotemark, &prefs_quote->text_fw_quotefmt, TRUE, prefs_quote_set_default_forward_fmt);
	gtk_notebook_append_page(GTK_NOTEBOOK(notebook), vbox2, gtk_label_new(_("Forward")));

	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(prefs_quote->checkbtn_compose_with_format), prefs_common.compose_with_format);
	...
	gtk_entry_set_text(GTK_ENTRY(prefs_quote->entry_quotemark), prefs_common.quotemark ? prefs_common.quotemark : "");
	if (prefs_common.quotefmt)
		pref_set_textview_from_pref(GTK_TEXT_VIEW(prefs_quote->text_quotefmt), prefs_common.quotefmt);
	else
		prefs_quote_set_default_reply_fmt();

	gtk_entry_set_text(GTK_ENTRY(prefs_quote->entry_fw_quotemark), prefs_common.fw_quotemark ? prefs_common.fw_quotemark : "");
	if (prefs_common.fw_quotefmt)
		pref_set_textview_from_pref(GTK_TEXT_VIEW(prefs_quote->text_fw_quotefmt), prefs_common.fw_quotefmt);
	else
...
static void prefs_quote_save(PrefsPage *_page)
{
	...
	prefs_common.compose_with_format = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(page->checkbtn_compose_with_format));
	prefs_common.compose_subject_format = pref_get_pref_from_entry(GTK_ENTRY(page->entry_subject));
	prefs_common.compose_body_format = pref_get_pref_from_textview(GTK_TEXT_VIEW(page->text_format));
	quotefmt_check_new_msg_formats(prefs_common.compose_with_format, NULL, prefs_common.compose_subject_format, prefs_common.compose_body_format);

	prefs_common.quotemark = gtk_editable_get_chars(GTK_EDITABLE(page->entry_quotemark), 0, -1);
	prefs_common.quotefmt = pref_get_pref_from_textview(GTK_TEXT_VIEW(page->text_quotefmt));
	quotefmt_check_reply_formats(TRUE, NULL, prefs_common.quotemark, prefs_common.quotefmt);

	prefs_common.fw_quotemark = gtk_editable_get_chars(GTK_EDITABLE(page->entry_fw_quotemark), 0, -1);
	prefs_common.fw_quotefmt = pref_get_pref_from_textview(GTK_TEXT_VIEW(page->text_fw_quotefmt));
	quotefmt_check_forward_formats(TRUE, NULL, prefs_common.fw_quotemark, prefs_common.fw_quotefmt);
}
