...
static void addressbook_edit_jpilot_create(gboolean *cancelled)
{
	...
	g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(edit_jpilot_delete_event), cancelled);
	g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(edit_jpilot_key_pressed), cancelled);
	...
	g_signal_connect(G_OBJECT(ok_btn), "clicked", G_CALLBACK(edit_jpilot_ok), cancelled);
	g_signal_connect(G_OBJECT(cancel_btn), "clicked", G_CALLBACK(edit_jpilot_cancel), cancelled);
	g_signal_connect(G_OBJECT(file_btn), "clicked", G_CALLBACK(edit_jpilot_file_select), NULL);
	g_signal_connect(G_OBJECT(check_btn), "clicked", G_CALLBACK(edit_jpilot_file_check), NULL);
...
AdapterDSource *addressbook_edit_jpilot(AddressIndex *addrIndex, AdapterDSource *ads)
{
	...
	if (ads) {
		...
		if (jpilot_get_name(jpf))
			gtk_entry_set_text(GTK_ENTRY(jpilotedit.name_entry), jpilot_get_name(jpf));
		if (jpf->path)
